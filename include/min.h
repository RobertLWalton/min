...
namespace min {
    ...
    min::unsptr strlen
        ( const min::unprotected::str_pointer & sp );
    min::uns32 strhash
        ( const min::unprotected::str_pointer & sp );
    char * strcpy ( char * p,
                    const min::unprotected
	...
	public:
		...
		( const unprotected::str_pointer & sp );
    ...
    // Function needed in min::str_pointer definition.
    //
    ...
    class str_pointer : public unprotected::str_pointer
    {
    public:
	str_pointer ( min::gen v )
	{
	    MIN_ASSERT ( min::is_str ( v ) );
	    new ( this ) unprotected::str_pointer ( v );
	}
    };
...
namespace min {
    ...
    class lab_pointer : public unprotected::lab_pointer
    {
    public:
    ...
    inline void initialize
	    ( min::unprotected::lab_pointer & labp,
    	      min::gen v )
    {
        labp.~lab_pointer();
...
namespace min {
    ...
    class packed_struct_pointer
	  : public
	    internal::packed_struct_base_pointer<S>
    {
	...
	typedef typename
		min::packed_struct_pointer<S> pointer;
	typedef typename
		min::packed_struct_updatable_pointer<S>
		updatable_pointer;

	static bool id;
	    // & id acts as a unique identifier of the
	    // type min::packed_struct<S>.  Only the
	    // address is used; the value of id has
	    // no use.
    };
...
namespace min {
	...
	struct packed_vec_descriptor
	{
	    ...
	    const min::uns32 header_size;
	        // computed_header_size: see below.
	...
	: public internal::packed_vec_base_pointer<H,E>
    {
	...
	//
	typedef typename
	        min::packed_vec_pointer<H,E>
	        pointer;
	typedef typename
	        min::packed_vec_updatable_pointer<H,E>
	        updatable_pointer;
	typedef typename
	        min::packed_vec_insertable_pointer<H,E>
	        insertable_pointer;
	...
	* (min::uns32 *) & pvip->length -= n;
    ...
    template < typename H, typename E >
    void packed_vec_insertable_pointer<H,E>
...
#	    endif
    ...
    // Declarations for reference in friend
    // declarations.
    //
    template < typename T,
               const raw_vec_type_info & type_info >
    ...
    class updatable_raw_vec_pointer :
        public raw_vec_pointer<T,type_info>
    {
    public:
    ...
    template < typename T,
               const raw_vec_type_info & type_info >
    class insertable_raw_vec_pointer :
        public updatable_raw_vec_pointer<T,type_info>
    {
    public:
...
template < typename T,
	   const min::raw_vec_type_info & type_info >
inline min::unsptr min::max_length_of
	( insertable_raw_vec_pointer<T,type_info>
	      & rvp )
{
...
{
    internal::resize
        ( rvp.stub, new_max_length, type_info );
}
