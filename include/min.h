...
// Date:	Fri Sep 19 04:23:08 AM EDT 2025
//
...
namespace min {
    ...
    // WARNING: These support set constants must match
    // those in the ../unicode directory.
    //
    const min::uns32 IS_ASCII		= ( 1 << 16 );
    const min::uns32 IS_LATIN1		= ( 1 << 17 );

    const min::uns32 IS_LETTER		= ( 1 << 20 );
    const min::uns32 IS_DIGIT		= ( 1 << 21 );
    const min::uns32 IS_MARK		= ( 1 << 22 );
    const min::uns32 IS_NATURAL		= ( 1 << 23 );

    const min::uns32 IS_BREAKABLE	= ( 1 << 27 );
...
namespace min {
	...
	const min::gen input_hash_table;
	...
	const min::packed_vec_insptr
	    ...
	    //	      max_offset = offset;
	    //
	    // The output_hash_table is created by
	    // `find' if it does not exist.  If
	    //
	    //  output_hash_table->length < 2 * occupied
	    //
	    // the output_hash_table is reset to
	    // NULL_STUB so it will be recreated with a
	    // longer length by the next `find'.
	    //
    ...
    uns32 find_or_add
            ( min::id_map map, min::gen g );
    inline min::gen map_get
	    ( min::id_map map, min::uns32 id )
    {
        if ( id >= map->length ) return min::NONE();
	else			 return map[id];
    }
    void map_set ( min::id_map map, min::uns32 id,
                   min::gen g );
    void map_clear ( min::id_map map, min::uns32 id );
    min::gen get ( min::gen obj, min::gen attr );
    inline min::gen map_get
	    ( min::id_map map, min::gen name )
    {
	return min::get ( map->input_hash_table, name );
    }
    void map_set ( min::id_map map, min::gen name,
                   min::gen g );
    void map_clear ( min::id_map map, min::gen name );
    inline void map_clear_input ( min::id_map map )
    {
...
namespace min { namespace unprotected {
    ...
    // Private Data:
	    ...
	    // last locate_reverse function call;
...
#	endif
	...
	unsigned state;  // One of:
		...
		LOCATE_ANY		= 3,
		    ...
		    // locate_reverse set the reverse
		    // attribute to ANY().
...
#   endif
	...
	case ap_type::INIT:
	    MIN_ABORT
		( "min::attr_info_of called before"
		  " min::locate" );
	...
	info.name = ap.reverse_attr_name;
	info.value_count = 0;
	info.value = min::NONE();
	...
	case ap_type::INIT:
	    MIN_ABORT
		( "min::reverse_attr_info_of called"
		  " before min::locate_reverse" );
	case ap_type::LOCATE_FAIL:
...
namespace min {
    ...
    extern const line_format * marked_line_format;
    ...
    extern const op disable_str_breaks;
    extern const op nodisable_str_breaks;
    ...
    inline min::printer print_space
...
namespace min {
	...
	const min::gen_format *   text_element_format;
	...
	const min::stub *	  text_type_table;
