...
The following macros describe these decisions:
...
Similarly the parameters that control stub address packing
(\pagref{STUB-ADDRESS-PACKING}) must be defined before C++ code is
compiled.  The following macros determine these decisions:
...
TBD: MIN\_USES\_OBJECT\_AUX\_STUBS

All the above macros must be defined identically
for all separately compiled parts of a single program.
...
\verb|min::indirect_aux_of|) and \verb|min::index_of| functions
...
auxiliary pointer, index, and control code
...
\begin{TABULAR}{0.6in}
\verb|min::| & \MINKEY{MISSING}
\LABEL{MIN::MISSING} \\
\verb|min::| & \MINKEY{ANY}
\LABEL{MIN::ANY} \\
\verb|min::| & \MINKEY{MULTI\_VALUED}
\LABEL{MIN::MULTI_VALUED} \\
\verb|min::| & \MINKEY{NULL}
\LABEL{MIN::NULL} \\
\verb|min::| & \MINKEY{SUCCESS}
\LABEL{MIN::SUCCESS}
\end{TABULAR}
