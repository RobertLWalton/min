...
The actual direct value, stub pointer, auxilary pointer, and control code
...
to determine this type.  See
\secref{AUXILARY-STUBS} for functions associated with auxilary stubs.
...
\verb|const char * | & \MUPKEY{str\_of}\verb| ( MUP::str_pointer & sp )|
\LABEL{MUP::STR_OF_STR_POINTER} \\
\verb|void | & \MUPKEY{relocate}\verb| ( MUP::str_pointer & sp )|
\LABEL{MUP::RELOCATE_OF_STR_POINTER} \\
\end{tabular}\end{center}
...
\subsubsubsection{Object Auxilary Stubs}
\label{OBJECT-AUXILARY-STUBS}

As an optimization, \skey{object auxilary stub}s\index{auxilary stub!object}
can be used instead of object
auxilary area elements.  Object auxilary stubs are a way of adding elements to
...
eliminate the use of object auxilary stubs extending the object
by moving information from these into a new larger object auxilary area.

The value of an object
auxilary stub is treated as an auxilary area element value.
The chain pointer of an object auxilary stub is treated as the next auxilary
...
An object auxilary stub has one of the following two uncollectible stub type
codes:
...
\skey{list auxilary stub}s,\index{auxilary stub!list}
and those with the second type code are called
\skey{sublist auxilary stub}s.\index{auxilary stub!sublist}
...
If an object auxilary stub is pointed at by a chain pointer in another
object auxilar stub, the first stub must be a list auxilary stub.

An object auxilary stub can be pointed at by only one pointer stored in an
object or in some object auxilary stub extending an object.
Thus object auxilary
stubs can be freed as soon as the pointer to them is deleted.  This
mechanism, and not garbage collection, frees object auxilary stubs.
