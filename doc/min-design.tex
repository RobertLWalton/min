...
The {\tt min::insert\_reserve} function may be called for a pointer
before any {\tt start} function is called to set the pointer.  However,
only one pointer at a time on an object may have an effective reservation.
...
\\[1ex]
\emkey{vector-list} ::= {\em attribute-descriptor-elements}
\\[1ex]
...
\\[1ex]
\emkey{attribute-sublist} ::= {\em attribute-descriptor-elements}
\\[1ex]
\emkey{attribute-descriptor-elements} ::=
...
A {\em vector-list} is a list that has the same format as an
{\em attribute-sublist}.  This is associated with the
...
attribute vector index then a {\em vector-list} with the same format as an
{\em attribute-sublist}
...
\\[1ex]
\emkey{vector-list} ::= {\em node-descriptor-elements}
\\[1ex]
...
\\[1ex]
\emkey{node-sublist} ::= {\em node-descriptor-elements}
\\[1ex]
\emkey{node-descriptor-elements}
    \begin{tabular}[t]{@{}r@{~}l@{}}
    ::= & {\em flag-set}
	  {\em value}$\,^\star$ \\
    $|$ & {\em child-sublist}
