...
\verb|min::op min::|
    & \MINKEY{clear\_\EOL gen\_\EOL flags}
      \verb|( min::uns32 gen_flags )|
...
{\tt min::\EOL set\_\EOL gen\_\EOL flags} logically OR's
its argument into {\tt print\_\EOL format.gen\_\EOL flags},
...
and {\tt min::\EOL clear\_\EOL gen\_\EOL flags} logically AND's
the bitwise complement of its argument into
{\tt print\_\EOL format.gen\_\EOL flags}.
...
instead for names and values of object attributes.  This is mainly of
use in printing objects \verb|v| that are being flushed from the
{\tt printer->\EOL id\_\EOL map}, when, to take an example, it is
...
If a single {\tt gen\_\EOL flags}
argument is given to {\tt min::\EOL pgen}, it is used
in place of 
{\tt printer->\EOL print\_\EOL format.gen\_\EOL flags}.
If both {\tt gen\_\EOL flags} and {\tt attr\_\EOL gen\_\EOL flags}
arguments are given, {\tt gen\_\EOL flags} are used if \verb|v|
is not an object, and are also used to control the top level
printing of an object \verb|v|, while {\tt attr\_\EOL gen\_\EOL flags}
are used instead to print the attribute names and values of \verb|v|
if \verb|v| is an object.
...
\verb|    //     [B][.] for B == '(', '[', '{', 0xA8 (|\LAQUOTE\verb|)| \\
\verb|    //     [.][C] for C == ')', ']', '}', 0xB8 (|\RAQUOTE
                                                       \verb|), ',', ';'| \\
\verb|    //     ['.'][L] for L a letter (i.e., with unicode_class U or L)| \\
\end{tabular}\end{indpar}
...
If \underline{not} set,
and if an object is not being printed as an expression as
per the {\tt OBJ\_\EOL EXP\_\EOL FLAG},
then the object will be printed in the general form:
\begin{center}
\begin{tabular}{ll}
...
\end{tabular}
\end{center}
where further details are provided on \pagref{GEN-OBJ-FORM}.
...
If the datum being printed by these operations is not an object,
or if it is an object that is being printed as per the
{\tt OBJ\_\EOL EXP\_\EOL FLAG}, a line will be printed with the format:
\begin{center}
\verb|@|i \verb|=| {\em value}
\end{center}
where {\tt value} is the normal printed representation of the datum value.
If the line is too long, it may be broken into multiple lines
as specified by the printer parameters.


If the datum is an object not being printed as per
{\tt OBJ\_\EOL EXP\_\EOL FLAG}, then the object will be printed on
...
This form is similar to the usual object representation
(see {\tt OBJ\_\EOL ID\_\EOL FLAG} above),
except that line ends, indentation, and the indentation mark
\verb/:|/ are used in place of semi-colons, and there
are no surrounding \verb/{|/ \verb/|}/ marks.

In the lines above:\label{GEN-OBJ-FORM}
