...
\newcommand{\ttmokey}[2]{{\tt \bf #1}%
                         \index{#1@{\tt operator #1}!{#2}}}
...
i.e., {\tt min::\EOL packed\_\EOL struct\_\EOL xxxptr\SARG} or
{\tt min::\EOL packed\_\EOL vec\_\EOL xxxptr\EHLARG}:
...
\begin{center}
\begin{tabular}{ll}
packed structures & see {\tt MIN\_REF}
...
packed vector elements	& see {\tt operator []}
    ...
    {\tt push} function \pagref{PACKED_VEC_PUSH_REF} \\
...
that encapsulate {\tt const min::\EOL stub~*} values are
...
A {\tt min::ref\TARG} value is implicitly convertible to the type {\tt T \&}.
...
{\tt T} is convertible to {\tt const min::\EOL stub~*} and the
...
However, unlike {\tt MUP::\EOL new\_\EOL ref},
...
a non-relocatable pointer to a value of type {\tt T} that is located
in static or stack memory.

Given a {\tt min::\EOL ptr<T>} value {\tt p} and an index {\tt i} of some
...
`{\tt T \&}' reference
without calling a {\tt min::\EOL acc\_\EOL write\_\EOL update} function
explicitly.
However, it can be written using a
{\tt min::\EOL ref<T>} reference,
which will call the {\tt min::\EOL acc\_\EOL write\_\EOL update} function
implicitly,
so a {\tt min::\EOL ref<T>} reference is returned by {\tt p[i]} if
{\tt T} is locatable.
...
we can.  (Ideally C++ would have an `{\tt operator .}' which better
...
{\tt min::\EOL gen} or is convertible to `{\tt const min::\EOL stub~*}',
...
actual value of {\tt p}, just as {(T *)(q)} is a coercion of {\tt q}
if {\tt q} has type `{\tt S *}'.  Such coercions are inherently
...
and {\tt min::\EOL begin/end\_\EOL ptr\_\EOL of} on
...
\verb|(constructor) min::| & \MINKEY{locatable\_gen}
    \verb|var ( min::gen value )|
...
\verb|min::ptr<min::gen> |
    & \TTOMKEY{\&}{\&}%
              {of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( const min::locatable_gen & var )|
\LABEL{MIN::AMPERSAND_LOCATABLE_GEN} \\
\end{tabular}\end{indpar}
...
value is implicitly convertible to a {\tt const min::\EOL stub~*} value.
...
{\tt var->\ldots}, {\tt var[\ldots]}, etc will work for a
...
due to a technical difficulty with C++ it is not possible to use
{\tt =} to pass an initializer
to a {\tt min::\EOL locatable\_\EOL var\TARG} constructor
...
implicitly convertible to a {\tt const min::\EOL stub~*} value nor
...
members `{\tt const}', so accidentally updating locatable members
...
Because \verb|min::ptr<char>| is not implicitly cast-able to
...
the `\key{white frowning face}%
\index{frowning face}%
\index{face, white frowning}',
\ILLEGALCHAR.
...
ASCII and LATIN-1\footnote{
Note that some possibly useful LATIN-1 non-letter characters
and their compose key sequences for keyboard entry are:
\textquestiondown(??), \textexclamdown(!!), \textsection(so),
% \textparagraph(PP),
\textregistered(OR), \textcopyright(OC),
\textcent(/c), \textsterling(-L), \textcurrency(xo), \textyen(=Y),
\guillemotleft({\tt <{}<})
\guillemotright({\tt >{}>}),
$\pm$(+-), $\times$(xx), $\div$(-:),
$\neg$(-,).  At the moment (2012), other non-ASCII, non-letter
characters are not well supported by Linux.  LATIN-1 letters
are supported via the compose key, but other non-ASCII letters are not.
Note that \ss(ss), \th(th), \TH(TH), \ae(ae)
are considered to be lower case letters and \AE(AE)
is considered to be an upper case letter, other LATIN-1
letters may be made by composing
{\tt `},
{\tt '},
{\tt \textasciicircum},
{\tt \textasciitilde},
...
{\tt ,},
{\tt -}, or
{\tt /} with a letter, as in
\`A({\tt `A}),
\'A({\tt 'A}),
\^A({\tt \textasciicircum A}),
\~A({\tt \textasciitilde A}),
\"A({\tt "A}),
\c{C}({\tt ,C}),
\DH({\tt -D}), and
\O({\tt /O}), and the remaining two cases
\AA({\tt AA}), and
\aa({\tt aa}) use a compose sequence consisting of just the letter doubled.
Note that LATIN-1 control codes cannot currently be input and have
no special MIN support for output.}
characters (characters with codes {\tt < 256})
are the characters themselves, e.g., the UNICODE class of
`{\tt *}' is \verb|'*'|, \underline{except}:
\begin{center}
\begin{tabular}{l@{~}l}
\verb|'U'| & is the class of upper case letters; \\
           & {\tt A~\ldots~Z~\`A~\'A~\^A~\~A~\"A~\AA~\AE~\c{C}
	      \ldots~\DH~\ldots~\O~\ldots~\'Y}. \\
\verb|'L'| & is the class of lower case letters; \\
           & {\tt a~\dots~z \TH~\ss~\`a~\'a~\^a~\~a~\aa~\ae~\c{c}
	      \ldots~\dh~\~n \ldots~\o~\ldots~\th~\"y}. \\
\end{tabular}
\end{center}

The UNICODE class of a character with code {\tt >= 256} is determined
by the UNICODE character `\mkey{general category}{UNICODE}' and the
UNICODE character `\mkey{numeric value}{UNICODE}'.
If the numeric value is a single
decimal digit, this becomes the UNICODE class of the character.
Otherwise the general category of the character is encoded
in a UNICODE class that is a single ASCII letter as indicated
on Figure~\ref{UNICODE-CLASSES}.

\begin{boxedfigure}
\small
\begin{center}
\begin{tabular}{l@{\hspace*{0.3in}}l}
\begin{tabular}[t]{l@{~denotes~}l}
\tt U & upper case letter \\
\tt L & lower case letter \\
\tt T & title case letter \\
\tt M & modifier letter \\
\tt O & other letter
\\[2ex]
\tt N & nonspacing mark \\
...
\tt H & other number
\\[2ex]
\tt n & connector punctuation \\
\tt d & dash punctuation \\
\tt o & open punctuation \\
\tt c & close punctuation \\
\tt i & initial punctuation \\
\tt f & final punctuation \\
\tt h & other punctuation
\end{tabular}
&
\begin{tabular}[t]{l@{~denotes~}l}
\tt m & math symbol \\
...
\end{tabular}
\end{center}
\caption{{\tt min::unicode\_class} values}
\label{UNICODE-CLASSES}
\end{boxedfigure}
...
in the example.  Of course, legal short string values should have more
characters that can be stored in a direct string
...
in the length.  To be legal the length should be larger than 8.
...
The label value may be read by using {\tt min::\EOL lab\_\EOL ptr}'s:
...
{\tt min::\EOL end\_\EOL ptr\_\EOL of}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::uns32 min::| & \MINKEY{lab\_of}\ARGBREAK
	  \verb|( min::gen * p, min::uns32 n,|\ARGBREAK
	  \verb|  const min::stub * s )|
\LABEL{MIN::LAB_OF} \\
\verb|min::uns32 min::| & \MINKEY{lab\_of}\ARGBREAK
	  \verb|( min::gen * p, min::uns32 n,|\ARGBREAK
...
The class type must be similar to a C-language {\tt struct} in that:
\begin{enumerate}
\item Construction\label{PACKED-STRUCTURE-TYPE-REQUIREMENTS}
of a datum of the type
by a no-argument constructor does not
...
header member described below, the {\tt control} member,
is given a value.
...
is equivalent to a {\tt memcpy} operation.

When a packed structure is moved by the copying part of
the ACC, it is moved by {\tt memcpy}.
\end{enumerate}

Numeric, \verb|min::gen|, and \verb|min::stub *| values are permitted as
packed structure class members.

The type of a packed structure
must have as its first member
(that is, the member at displacement {\tt 0});
\begin{indpar}
...
`{\tt control}' member must be the first member of the base structure.
This rule can be applied recursively to get, for example, a `{\tt struct}'
based on a public `{\tt struct}'
based on a public `{\tt struct}' whose first member is `{\tt control}'.
...
\end{tabular}\end{indpar}
where \verb|S| is the class type of the structure.
...
\verb|S| is the type of the structure.
...
Ditto but for \verb|const min::stub *| members of \verb|S|
...
{\tt min::\EOL packed\_\EOL struct\_\EOL xxxptr\SARG} ~and~
{\tt min::\EOL packed\_\EOL vec\_\EOL xxxptr\EHLARG} members
are also treated as {\tt const min::\EOL stub~*} members.

\end{itemlist}

The \verb|pstype.new_gen| and \verb|pstype.new_stub|
functions create a new packed structure
datum of the type described by \verb|pstype| and return a
...
The types of both the vector header and the vector elements must be
similar to C-language {\tt struct}, number, or pointer types in that:
\begin{enumerate}
\item Construction\label{PACKED-VECTOR-TYPE-REQUIREMENTS}
of a datum of the type
by a no-argument constructor does not
set any datum byte to a non-zero value.  Note that bytes need not be
set to any value, but zero must be an acceptable value for all bytes
...
When a packed vector is created it is zeroed and then a few special
header members described below, the {\tt control}, {\tt length},
and {\tt max\_\EOL length} members, are given values.
No constructor is called.
\item
Destruction of a datum of the type does nothing.

When a packed vector is destroyed, destructors are not called.
\item
Assignment of a datum of the type from another datum of the same type
is equivalent to a {\tt memcpy} operation.

When a packed vector is expanded, or moved by the copying part of
the ACC, it is moved by {\tt memcpy}.
\end{enumerate}

Numeric, \verb|min::gen|, and \verb|min::stub *| values are permitted as
vector element types by themselves or as members of a class that
is a packed vector element type or a packed vector header type.

The type of a packed vector header, like that of a packed structure,
must be a class whose first member
(that is, the member at displacement {\tt 0}) is
\begin{indpar}
\verb|const min::uns32 control;|
\end{indpar}
...
\end{indpar}
where {\tt L} is an unsigned integer type, and defaults to
{\tt min::\EOL uns32}.

There are also three pointer-to-packed-vector types:
\begin{indpar}\begin{tabular}{l@{~~~~~a.k.a~~~~~}l}
\verb|min::packed_vec_ptr<E,H,L>| & \verb|min::packed_vec<E,H,L>::ptr| \\
\verb|min::packed_vec_updptr<E,H,L>| & \verb|min::packed_vec<E,H,L>::updptr| \\
\verb|min::packed_vec_insptr<E,H,L>| & \verb|min::packed_vec<E,H,L>::insptr|
\end{tabular}\end{indpar}
where \verb|H| is the type of the packed vector header struct,
\verb|E| is the type of the packed vector element, and
\verb|L| is the unsigned integer type of the {\tt length}
and {\tt max\_\EOL length} members of the header, and also of the
...
{\tt min::packed\_\EOL vec\EHLARG} C++ static object, where
\verb|E|, \verb|H|, and \verb|L| are as above.
The {\tt new\_gen} and {\tt new\_stub}
member functions of this C++ static object can be
...
{\tt pvtype.header\_\EOL stub\_\EOL disp},
~~{\tt pvtype.ele\-\EOL ment\_\EOL gen\_\EOL disp},
and {\tt pvtype.ele\-\EOL ment\_\EOL stub\_\EOL disp}
...
The \verb|pvtype.new_gen| and \verb|pvtype.new_stub|
functions create a new packed vector
datum of the type described by \verb|pvtype| and return a
...
main ( ... )
{
    ...
    s2 = e4.s;
        ...
        // are assuming they are NOT related to name1 above.
...
A main reason why the {\tt min::file} type exists is to allow lines
...
{\tt line\_index} must exist (i.e., not be {\tt NULL\_\EOL STUB}),
and
...
the setting of {\tt spool\_\EOL lines}
and possibly use of the {\tt min::\EOL flush\_\EOL spool} described below.

If {\tt spool\_\EOL lines} equals {\tt 0},
{\tt line\_\EOL index} does not exist, and {\tt min::\EOL line}
always returns {\tt min::\EOL NO\_\EOL LINE}.
 If {\tt spool\_\EOL lines} equals
\verb|min::|\MINNBKEY{ALL\_\EOL LINES}, the default argument setting
for all {\tt min::\EOL init\_\EOL input\ldots} functions,
then all lines
previously returned by {\tt min::\EOL next\_\EOL line} are always available,
{\tt line\_\EOL index->\EOL length} always equals
...
For other values of {\tt spool\_\EOL lines},
{\tt line\_\EOL index} exists and
{\tt line\_\EOL index->\EOL length} is a function of the history
...
{min::\EOL flush\_\EOL line} except that, instead of copying
to the file's {\tt ostream}, {\tt ofile}, and/or
{\tt printer}, they copy to their lefthand operand:
...
and {\tt min::\EOL skip\_\EOL remaining} functions in the same way as
...
\verb|    min::uns32        | & \TTMKEY{gen\_\EOL flags}%
                                       {in {\tt min::print\_format}}
			          & \verb|// Default:| \\
			      &   & \verb|//    min::EXPRESSION_FLAG|
\LABEL{MIN::GEN_FLAGS_IN_PRINT_FORMAT} \\
\verb|    min::gen_format * | & \TTMKEY{gen\_format}%
                                           {in {\tt min::print\_format}}
			          & \verb|// Default:| \\
...
\verb|min::file printer->|
    & \TTMKEY{obj\_\EOL id\_\EOL map}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_OBJ_ID_MAP} \\
\end{tabular}\end{indpar}
...
The {\tt min::}\MINKEY{init\_\EOL ostream}
...
\item[\ttnbmkey{ASCII}{character category}] ASCII Characters: characters
with codes {\tt <= 0x7F}.
\end{itemlist}
...
\item[\ttnbmkey{GRAPHIC\_HSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]
\item[\ttnbmkey{GRAPHIC\_VSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
\item[\ttnbmkey{GRAPHIC\_NSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
~\\
...
\item[\ttnbmkey{GRAPHIC\_FLAGS}{in printer {\tt print\_\EOL format.flags}}]~\\
...
\item[\ttnbmkey{ASCII\_FLAG}{in printer {\tt print\_\EOL format.flags}}]~\\
Used in graphics mode to control the printing of ASCII control characters:
see {\tt GRAPHIC\_XXX\_FLAG}'s above.

Also controls the printing of non-ASCII UNICODE characters as follows:

If the {\tt ASCII\_FLAG} is on (this is known as being in \key{ASCII mode}),
...
{\tt XXXX} begins with
one of the digits {\tt 0}, {\tt 1}, \dots, {\tt 9}, to avoid
...
\item[\ttnbmkey{ALLOW\_HSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]
\item[\ttnbmkey{ALLOW\_VSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
\item[\ttnbmkey{ALLOW\_NSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
~\\
If the {\tt GRAPHIC\_\EOL XXX\_\EOL FLAG} is off,
printing of characters of class {\tt XXX} is controlled by the
{\tt ALLOW\_\EOL XXX\_\EOL FLAG}, which
...
\item[\ttnbmkey{DISPLAY\_EOL\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]~\\
...
\item[\ttnbmkey{HBREAK\_FLAG}{in printer {\tt print\_\EOL format.flags}}]~\\
...
\item[\ttnbmkey{GBREAK\_FLAG}{in printer {\tt print\_\EOL format.flags}}]~\\
If this flag is on, a break point is automatically set
...
\item[\ttnbmkey{EOL\_FLUSH\_FLAG}{in printer {\tt print\_\EOL format.flags}}]~\\
...
\begin{itemlist}[0.6in]
\item[\ttmkey{print\_format.gen\_flags}{in {\tt min::printer}}]
...
\item[\ttmkey{obj\_\EOL id\_\EOL map}{in {\tt min::printer}}]~\\
Used to find or create object IDs when printing
{\tt min::gen} object values.  See \pagref{OBJ-ID-MAP}.
...
in non-graphic mode with {\tt min::\EOL ALLOW\_\EOL NSPACE\_\EOL FLAG} on,
it is encoded as the overlong UTF-8 encoding {\tt 0xC0},{\tt 0x80}.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::printer |
    & \TTOMKEY{<<}{\LT\LT\RESIZE}{of {\tt min::printer}}\ARGBREAK
      \verb|( min::printer printer,|\ARGBREAK
...
flush operation if the {\tt min::\EOL EOL\_\EOL FLUSH\_\EOL FLAG} is set.
...
column is greater than {\tt line\_\EOL break.indent}, and then inserts
...
{\tt min::save\_print\_format}
pushes into the format stack a new element containing
the current {\tt print\_\EOL format}, while
{\tt min::\EOL restore\_\EOL print\_\EOL format}
resets {\tt print\_\EOL format} from the top format stack element and then
pops this element from the stack.

Similarly the line break parameters of a printer
may be saved in the
...
{\tt min::save\_line\_break}
pushes into the line break stack a new element containing
the current value of {\tt line\_\EOL break}, while
{\tt min::\EOL restore\_\EOL line\_\EOL break}
resets {\tt line\_\EOL break} from the top line break stack element and then
pops this element from the stack.
...
the printer {\tt line\_\EOL break.line\_\EOL length} parameter,
then if any break stack entry is enabled, the first such entry's
{\tt offset}, {\tt column}, and {\tt indent},
are used in place of the printer
{\tt line\_\EOL break.offset},
{\tt line\_\EOL break.column}, and
{\tt line\_\EOL break.indent} parameters.
If this happens, then all later line break stack entries
and the printer {\tt line\_\EOL break}
...
\begin{center}
\tt \BRA~aaa,~bbb,~[~ccc,~ddd,~eee,~(~fff,~ggg~),~hhh~],~iii,~jjj~\KET
\end{center}
...
{\tt min::\EOL restore\_\EOL line\_\EOL break} (i.e., a
{\tt min::\EOL restore\_\EOL indent} without any implied
{\tt min::\EOL indent}), a {\tt min::\EOL eol},
and a {\tt min::\EOL restore\_\EOL print\_\EOL format}.
Note that the print format flags in effect just before {\tt min::\EOL eom}
is executed govern the {\tt min::\EOL eol} operation executed by
{\tt min::\EOL eom}.
...
(\pagref{MIN::SET_INDENT}) or with the operation:
...
This operation stores the sum of `{\tt offset}' and the printer `{\tt column}'
in {\tt line\_\EOL break.indent}.  If the
value stored would be negative, {\tt 0} is stored instead.
...
\verb|min::op min::|
    & \MINKEY{set\_\EOL gen\_\EOL flags}
      \verb|( min::uns32 gen_flags )|
\LABEL{MIN::SET_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{set\_\EOL gen\_\EOL format}
      \verb|( min::gen_format * gen_format )|
\LABEL{MIN::SET_GEN_FORMAT} \\
\end{tabular}\end{indpar}

For example, `{\tt printer<<min::set\_print\_flags(min::ASCII\_FLAG)}'
...
Note that {\tt min::\EOL set\_\EOL print\_\EOL flags} logically OR's
its argument into {\tt print\_\EOL format.flags},
and {\tt min::\EOL clear\_\EOL print\_\EOL flags} logically AND's
the bitwise complement of its argument into {\tt print\_\EOL format.flags},
whereas {\tt min::\EOL set\_\EOL gen\_\EOL flags} stores its argument
into {\tt print\_\EOL for\-mat.gen\_\EOL flags}.
...
the value of {\tt min::\EOL ostream\_\EOL print\_\EOL format}
instead, where this is a global variable initialized by:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\multicolumn{2}{l}{\tt min::print\_format
                       min::\MINKEY{ostream\_print\_format} =}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  min::ASCII + min::GRAPHIC_FLAGS,    // flags|\ARGBREAK
    \verb|  min::EXPRESSION_FLAG,               // gen_flags|\ARGBREAK
    \verb|  & min::default_gen_format           // gen_format|\ARGBREAK
    \verb|};|
...
{\tt min::\EOL gen\_\EOL format} enabled by a set of {\tt gen\_\EOL flags}.
The {\tt min::\EOL pgen} printer operation can specify the flags
explicitly or use defaults equal to
{\tt printer->\EOL print\_\EOL format.gen\_flags}:
...
{\tt printer} using the given {\tt gen\_\EOL flags}.
{\tt printer->\EOL print\_\EOL format.gen\_\EOL format}
is used, unless that is {\tt NULL}, in which case
{\tt \&\,min::default\_\EOL gen\_\EOL format} is used.
If the {\tt gen\_flags} argument is omitted,
{\tt printer->\EOL print\_\EOL format.gen\_\EOL flags}
is used in its place.

Note that {\tt print\_\EOL format.gen\_\EOL flags} differs
from {\tt print\_\EOL format.flags} in that {\tt gen\_\EOL flags}
acts as a unified operation code for printing general values
whereas {\tt flags} operates as a collection of disconnected
parameters for printing characters.  Thus one can set and clear
individual {\tt flags}, but {\tt gen\_flags} can only be
set all together as a unit.
...
{\tt min::gen\_format} \underline{cannot} be stored in a
...
The detailed structure of {\tt min::gen\_format} and the
flags in {\tt min::gen\_\EOL flags} are as follows:
...
\verb|struct min::| & \MINKEY{gen\_format}\ARGBREAK
    ...
    \verb|  const char *           str_quote;|\ARGBREAK
    ...
    \verb|  const char * const *   special_names;|\ARGBREAK
    \verb|  min::uns32             special_names_size;|\ARGBREAK
    \verb|  const min::uns32       space_prefix_mask[256];|\ARGBREAK
    \verb|  const min::uns32       space_postfix_mask[256];|\ARGBREAK
    \verb|};|
\ttmindex{gen}{in {\tt min::gen\_format}}
...
\ttmindex{special\_names\_size}{in {\tt min::gen\_format}}
\ttmindex{space\_prefix\_mask}{in {\tt min::gen\_format}}
\ttmindex{space\_postfix\_mask}{in {\tt min::gen\_format}}
\LABEL{MIN::GEN_FORMAT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const char * const * min::|
    & \TTKEY{standard\_\EOL special\_\EOL names}
\LABEL{MIN::STANDARD_SPECIAL_NAMES} \\
\verb|const min::uns32 ::|
    & \TTKEY{standard\_\EOL special\_\EOL names\_\EOL size}
\LABEL{MIN::STANDARD_SPECIAL_NAMES_SIZE} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::uns32 min::|
    & \MINKEY{default\_\EOL space\_\EOL prefix\_\EOL mask}{\tt [256]}
\LABEL{MIN::DEFAULT_SPACE_PREFIX_MASK} \\
\verb|min::uns32 min::|
    & \MINKEY{default\_\EOL space\_\EOL postfix\_\EOL mask}{\tt [256]}
\LABEL{MIN::DEFAULT_SPACE_POSTFIX_MASK} \\
\end{tabular}\end{indpar}

{\tt space\_prefix/postfix\_mask} flags:
...
\multicolumn{2}{l}{\tt const min::gen\_format
    ...
    \verb|{|\ARGBREAK
    \verb|  min::default_pgen,   // pgen|\ARGBREAK
    ...
    \verb|  min::default_space_prefix_mask,     // space_prefix_mask|\ARGBREAK
    \verb|  min::default_space_postfix_mask,    // space_postfix_mask|\ARGBREAK
    \verb|};|
\LABEL{MIN::DEFAULT_GEN_FORMAT} \\
\end{tabular}\end{indpar}

{\tt print\_format.gen\_flags}:
...
\verb|const min::uns32 min::| & \MINNBKEY{SUPPRESS\_OBJ\_SPACE\_FLAG}
\LABEL{MIN::SUPPRESS_OBJ_SPACE_FLAG} \\
...
the given {\tt gen\_\EOL flags} and {\tt gen\_\EOL format}.
The function must return the {\tt printer}.  The description of
{\tt gen\_\EOL flags} and the {\tt gen\_\EOL format} members in
...
{\tt BRACKET\_\EOL IMPLICIT\_\EOL FLAG} and {\tt EXPRES\-SION\_\EOL FLAG}
...
\item[\ttmkey{special\_names\_size}{in {\tt min::gen\_format}}]\vspace{-2ex}~\\
These members are only enabled if
{\tt SUPPRESS\_\EOL SPECIAL\_\EOL NAMES\_\EOL FLAG} is \underline{off}
and {\tt spec\-ial\_\EOL names} is not {\tt NULL}.
...
\item[\ttmkey{space\_prefix\_mask}{in {\tt min::gen\_format}}]
\item[\ttmkey{space\_postfix\_mask}{in {\tt min::gen\_format}}]\vspace{-2ex}~\\
These members are only enabled if both are not {\tt NULL} and
{\tt SUP\-PRESS\_\EOL LAB\_\EOL SPACE\_\EOL FLAG} is on for labels or
{\tt SUPPRESS\_\EOL OBJ\_\EOL SPACE\_\EOL FLAG} is on for objects.
In addition, {\tt lab\_\EOL separator} must not be being used for labels
(which will be the case if {\tt BRACKET\_\EOL LAB\_\EOL FLAG}
is \underline{off}).

Then the space normally output between label elements
or between object vector elements is suppressed if
both elements are names (as per {\tt min::\EOL is\_\EOL name},
\pagref{MIN::IS_NAME}) and
\begin{center}
...
b = {\rm the last UNICODE character of the previous element} \\
a = {\rm the first UNICODE character of the next element} \\
\end{tabular}
\end{center}

Here the last UNICODE character of a number is assumed to be a digit,
and the first UNICODE character of a non-negative number is assumed
to be a digit, while the first UNICODE character of a negative number
is assumed to be `{\tt -}'.


{\tt space\_prefix\_mask} and {\tt space\_postfix\_mask} must both be
addresses of
C++ vectors each with exactly {\tt 256} {\tt min::\EOL uns32} elements.

The following flags may be used in these masks:

\end{itemlist}

\begin{indpar}\begin{itemlist}[0.2in]

\item[\ttnbmkey{PREFIX\_SEPARATOR\_FLAG}%
    {{\tt min::gen\_format} flag}]{\tt = 1}\\
...
a character may be adjacent to an immediately preceding prefix separator
character.
Set in {\tt min::\EOL default\_\EOL space\_\EOL prefix\_\EOL mask}
for the classes `{\tt (}', `{\tt [}', `{\tt \BRA}', and
`{\tt \guillemotleft}'
and in {\tt min::\EOL default\_\EOL space\_\EOL postfix\_\EOL mask}
for all classes.

\item[\ttnbmkey{POSTFIX\_SEPARATOR\_FLAG}%
    {{\tt min::gen\_format} flag}]{\tt = 2}\\
In the {\tt space\_\EOL postfix\_\EOL mask} this flag means that a character
is a postfix separator character that may be adjacent to a preceding character,
...
character.
Set in {\tt min::\EOL default\_\EOL space\_\EOL postfix\_\EOL mask}
for the classes `{\tt )}', `{\tt ]}', `{\tt \KET}', `{\tt \guillemotright}',
`{\tt ,}', and `{\tt ;}'
and in {\tt min::\EOL default\_\EOL space\_\EOL prefix\_\EOL mask}
for all classes.


\end{itemlist}\end{indpar}

Printer {\tt print\_format.gen\_flags}:

\begin{indpar}[0.1in]\begin{itemlist}[0.4in]

\item[\ttnbmkey{GRAPHIC\_STR\_FLAG}{{\tt min::gen\_format} flag}]~\\
...
\item[\ttnbkey{BRACKET\_STR\_FLAG}]
\item[\ttnbkey{BRACKET\_LAB\_FLAG}]\vspace{-1ex}
\item[\ttnbkey{BRACKET\_SPECIAL\_FLAG}]\vspace{-1ex}
\item[\ttnbkey{BRACKET\_IMPLICIT\_FLAG}]\vspace{-1ex}~\\
These flags enable use of the following members which are described above:
\begin{center}
\begin{tabular}{l@{~~~~~~~~~~}l}
...
\item[\ttnbkey{SUPPRESS\_SPECIAL\_NAME\_FLAG}]~\\
This flag controls the printing of a
{\tt min::\EOL new\_\EOL special\_\EOL gen(i)} value.

If the flag is \underline{off}, if {\tt special\_\EOL names}
is not {\tt NULL}, and if
{\tt 0 <= i < special\_\EOL names\_\EOL size}, then
the value prints as {\tt special\_\EOL names[i]}.

If the flag is \underline{off} and
{\tt 0 <= 0xFFFFFF\,-\,i\,<\,min::\EOL standard\_\EOL special\_\EOL names\_\EOL size}, then
the value prints as {\tt min::\EOL standard\_\EOL special\_\EOL names[i]}.

Otherwise the value prints as `{\tt SPECIAL(i)}'.

\item[\ttnbkey{SUPPRESS\_LAB\_SPACE\_FLAG}]
\item[\ttnbkey{SUPPRESS\_OBJ\_SPACE\_FLAG}]\vspace{-1ex}~\\
See {\tt space\_prefix/postfix\_mask} above.

\item[\ttnbkey{EXPRESSION\_FLAG}]
\item[\ttnbkey{DATA\_FLAG}]\vspace{-1ex}
\item[\ttnbkey{RAW\_FLAG}]\vspace{-1ex}~\\
...
being printed.
\end{itemlist}\end{indpar}
\begin{indpar}[0.3in]\begin{itemlist}[0.4in]

\item[\ttnbmkey{EXPRESSION\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set, only the vector elements of
an object and the \ttdkey{initiator}, \ttdkey{separator}, \ttdkey{terminator},
...
\item[\ttnbmkey{DATA\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set, objects are printed on
one or more lines using the format:
\begin{indpar}
\verb|@|\verb|<|{\em id}\verb|> =| \ldots
\end{indpar}

where {\em id} is an object ID.
These lines are all indented by the current printer column.
Complex subobjects and non-pub\-lish\-ed subobjects are
referenc\-ed by \verb|@|\verb|<|{\em id}\verb|>|'s and
printed separately from their containing objects.
Object IDs are created or found by
using the printer {\tt obj\_\EOL id\_\EOL map}:
see below (\pagref{OBJ-ID-MAP}).

\item[\ttnbmkey{RAW\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set,
...
When objects are printed using lines of the form
\begin{indpar}
\verb|@|\verb|<|{\em id}\verb|> =| \ldots
\end{indpar}
the {\em ids} are choosen using an \key{object ID map}:\label{OBJ-ID-MAP}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|typedef min::|
	& \verb|packed_vec_insptr|\ARGBREAK
	  \verb|    < const min::stub *,|\ARGBREAK
	  \verb|      min::obj_id_map_header<min::uns32> >|\ARGBREAK
	  \verb|min::|\MINKEY{obj\_\EOL id\_\EOL map}
\LABEL{MIN::OBJ_ID_MAP} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::obj_id_map min::| & \MINKEY{init\REL}
    \verb|( min::ref<min::obj_id_map> map )|
\LABEL{MIN::INIT_OF_OBJ_ID_MAP} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::stub * map| & \TTBMKEY{id}{of {\tt min::obj\_id\_map}}
\LABEL{MIN::OBJ_ID_MAP_[]} \\
\verb|min::uns32 min::| & \MINKEY{find\REL}\ARGBREAK
    \verb|( min::ref<min::obj_id_map> map,|\ARGBREAK
    \verb|  const min::stub * s )|
\LABEL{MIN::FIND_OF_OBJ_ID_MAP} \\
\verb|min::uns32 min::| & \MINKEY{find\_\EOL or\_\EOL add\REL}\ARGBREAK
    \verb|( min::ref<min::obj_id_map> map,|\ARGBREAK
    \verb|  const min::stub * s )|
\LABEL{MIN::FIND_OR_ADD_OF_OBJ_ID_MAP} \\
\verb|void min::| & \MINKEY{insert\REL}\ARGBREAK
    \verb|( min::ref<min::obj_id_map> map,|\ARGBREAK
    \verb|  const min::stub * s,|\ARGBREAK
    \verb|  min::uns32 id )|
\LABEL{MIN::INSERT_OF_OBJ_ID_MAP} \\
\end{tabular}\end{indpar}

An object ID map maintains a 1-1 correspondence between pointers to
{\tt min::\EOL stub}'s
and IDs that are small strictly positive integers.  
If {\tt map} is a {\tt min::\EOL obj\_\EOL id\_\EOL map},
and {\tt (s,id)} is a correspondence stored in {\tt map} between
a `{\tt const min::stub *}' pointer {\tt s} and a {\tt min::\EOL uns32}
ID {\tt id}, then
\begin{center}
\tt
s == map[id] \\
...
The ID value {\tt 0} denotes a missing ID, and cannot be part of the
correspondence stored in an object ID map.
...
If stubs are being output, the {\tt min::find\_\EOL or\_\EOL add}
function can be used to find the ID of a stub.  If {\tt s} is already
in the {\tt map}, its associated ID is returned.  Otherwise the {\tt s}
is pushed to the end of the {\tt map} vector and assigned an ID which
is returned.  However, as a special case,
{\tt min::\EOL find\_\EOL or\_\EOL add} returns
zero if its stub pointer argument is {\tt min::\EOL NULL\_\EOL stub}.

If stubs IDs are being input, {\tt map[id]} can be used to find the stub
associated with {\tt id}, unless {\tt id >= map->length}, in which case
there is no such stub.  Also, there is no stub if
{\tt map[id] == min::\EOL NULL\_\EOL STUB}.  If there is no stub associated
with {\tt id}, a stub {\tt s}
...
It merely returns the ID associated with a stub {\tt s} if one exists,
or returns {\tt 0} if no such ID exists.  {\tt min::\EOL find} also returns
zero if its stub pointer argument is {\tt min::\EOL NULL\_\EOL stub}.

Part of an object ID map is a hash table that permits
...
{\tt min::\EOL insert} discards it so a larger hash table will be created
by the next call to a {\tt min::\EOL find\ldots} function.

A printer's {\tt obj\_id\_map} member is initialized automatically when
needed.  However, it can also be set by:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::obj_id_map min::|
    & \MINKEY{set\_\EOL obj\_\EOL id\_\EOL map\RESIZE}\ARGBREAK
    \verb|( <min::printer printer,|\ARGBREAK
    \verb|  min::obj_id_map map = min::NULL_STUB )|
\LABEL{MIN::SET_OBJ_ID_MAP_OF_PRINTER} \\
\end{tabular}\end{indpar}

which permits maps to be shared among printers.  If no second argument
is given, an object ID map is created for the printer if none previously
exists.  This can then be shared with other printers.  The final
{\tt obj\_\EOL id\_\EOL map} member of the printer is returned, and this
...
\begin{center}
\begin{tabular}{l}
\ttnbkey{TINY\_OBJ}\LABEL{MIN::TINY_OBJ} \\
\ttnbkey{SHORT\_OBJ}\LABEL{MIN::SHORT_OBJ} \\
\ttnbkey{LONG\_OBJ}\LABEL{MIN::LONG_OBJ} \\
\ttnbkey{HUGE\_OBJ}\LABEL{MIN::HUGE_OBJ} \\
\end{tabular}
\end{center}
...
\LABEL{MIN::ATTR_INFO} \\
\verb|typedef | & \verb|min::packed_vec<attr_info>|
...
\verb|typedef | & \verb|min::attr_info_vec::ptr|
                       \verb|min::|\MINKEY{attr\_info\_ptr}
\LABEL{MIN::ATTR_INFO_PTR} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{get\_attrs}
     \verb|( min::attr_ptr ap )|
\LABEL{MIN::GET_ATTRS} \\
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{get\_reverse\_attrs}
     \verb|( min::attr_ptr ap )|
...
\begin{TABULAR}{1.8in}
\verb|T & |
    & \TTOMKEY{T \&}{{\tt T \&}}{of {\tt min::ref\TARG}}\ARGBREAK
      \verb|( const min::ref<T> & r )|
\LABEL{MIN::REF_TO_T} \\
\verb|T |
	& \TTOMKEY{-\GT}{-\GT}%
	          {of {\tt min::ref\TARG}}\ARGBREAK
	  \verb|( const min::ref<T> & r )|
\LABEL{MIN::REF_->} \\
\end{TABULAR}
...
\LABEL{MIN::=AMPERSAND_OF_REF_OF_T} \\
\verb|min::ref<T> |
    & \TTOMKEY{*}{*}{of {\tt min::ptr\TARG}}\ARGBREAK
      \verb|( const min::ptr<T> & p )|
\LABEL{MIN::=*_OF_PTR_OF_T} \\
\end{TABULAR}
...
\begin{TABULAR}{1.8in}
\verb|T & p| & \TTBMKEY{i}{of {\tt min::ptr}}
    \hspace*{0.2in} [if \verb|T| {\tt const}
                     or \verb|T| is \underline{not} locatable]
\LABEL{MIN::PTR_[]} \\
\verb|min::ref<T> p| & \TTBMKEY{i}{of {\tt min::ptr}}
    \hspace*{0.2in} [if \verb|T| \underline{not} {\tt const}
                     and \verb|T| is locatable]
\LABEL{MIN::PTR_TO_STUB_[]} \\
\verb|min::ptr<T> p|
    & \TTMOKEY{+}{of {\tt min::ptr}}\verb|i|
...
\LABEL{MIN::PACKED_VEC_TYPE} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\_with\_base}\BRACKETED{E,H,B,L}
		\verb|pvtype|\ARGBREAK
	  \verb|( const char * name,|\ARGBREAK
	  \verb|  const min::uns32 * element_gen_disp = NULL,|\ARGBREAK
	  \verb|  const min::uns32 * element_stub_disp|\ARGBREAK
