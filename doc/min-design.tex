...
indirectly using a logrithmic or pseudo-floating-point code.
...
The exact unused area size is given.
...
the actual variables vector size,
the actual hash table size,
the unused area size given,
and the header size.  Note that if the given sizes are powers of two
not greater than the maximums allowed, the corresponding actual sizes will
be equal to the given sizes.

The returned object is short if the total size is less than
$2^{16}$ and the actual variables vector and hash table sizes
are permitted for short objects.  Otherwise the object is long.
...
the body vector.
An auxiliary pointer is a body vector index.
The 0 body vector index points into the header, and is
therefore not usable as the index of a \verb|min::gen| value, so it
can be used as a form of null pointer.
...
or the {\tt min::\EOL OBJ\_\EOL PUBLIC} flag is set on an object
