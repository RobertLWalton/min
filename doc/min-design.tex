...
{\tt min::\EOL packed\_\EOL vec\_\EOL xxxptr\EHARG}:
...
initializable by setting all bytes to zero and copyable
by \verb|memcpy|.
...
There is one exception to the rule that a packed structure type must be
a C type: it may have a public base structure, as long as that is an
admissible packed structure type.  In this case the
...
However, {\tt E} must \underline{not} be a {\tt const} type.
Also, if it has a zero argument constructor, that may not
be used to initialize the element, but instead the element
bytes may be zeroed.  Similarly an assignment or copy operator
may not be used, but {\tt memcpy} may be used instead,
and a destructor may not be used.  Lastly, the element may
be relocated in memory using {\tt memcpy}.
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|E & min::|
	& \MINKEY{push\RESIZE} \verb| ( packed_vec_insptr<E,H> pvip )|\\&
...
off (see printer format flags at \pagref{FORMAT_FLAGS}).
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{init\_\EOL input\RESIZE}%
    \verb| ( min::ref<min::file> file )|
\LABEL{MIN::INIT_INPUT_OF_FILE} \\
\verb|void min::| & \MINKEY{init\_\EOL output\RESIZE}%
    \verb| ( min::ref<min::file> file )|
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::|
    & \MINKEY{init\_output\_ostream\REL}\ARGBREAK
          \verb| ( min::ref<min::file> file,|\ARGBREAK
	  \verb|   std::ostream & ostream )|
\LABEL{MIN::INIT_OUTPUT_OSTREAM_OF_FILE} \\
\verb|void min::|
    & \MINKEY{init\_output\_file\REL}\ARGBREAK
          \verb| ( min::ref<min::file> file,|\ARGBREAK
	  \verb|   min::file ofile )|
\LABEL{MIN::INIT_OUTPUT_FILE_OF_FILE} \\
\verb|void min::|
    & \MINKEY{init\_output\_printer\REL}\ARGBREAK
          \verb| ( min::ref<min::file> file,|\ARGBREAK
...
\end{center}
For other non-{\tt 0} values of {\tt spool\_\EOL lines},
...
by using the packed vector {\tt min::push\RESIZE} function
...
This function sets {\tt file->buffer[offset]} to NUL, overwriting
...
\verb|min::printer min::| & \MINKEY{init\_output\_stream\RESIZE}\ARGBREAK
    \verb| ( min::ref<min::printer> printer,|\ARGBREAK
    \verb|   std::ostream & ostream )|
...
\verb|const min::uns32 printer|
    & \TTARMKEY{line\_length}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_LINE_LENGTH} \\
\verb|const min::uns32 printer|
    & \TTARMKEY{indent}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_INDENT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::uns32 printer|
    & \TTARMKEY{format\_flags}{in {\tt min::printer}}
\LABEL{MIN::FORMAT_FLAGS} \\
\verb|min::pgen_format printer_format|
    & \TTARMKEY{pgen\_format}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_PGEN_FORMAT} \\
\end{tabular}\end{indpar}
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::packed_vec_insptr<min::break> printer|
    & \TTARMKEY{break\_stack}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_LINE_BREAK_STACK} \\
\verb|const min::packed_vec_insptr<min::format> printer|
    & \TTARMKEY{format\_stack}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_FORMAT_STACK} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|struct min::| & \MINKEY{break}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  min::uns32 break_offset;|\ARGBREAK
    \verb|  min::uns32 break_column;|\ARGBREAK
    \verb|  min::uns32 line_length;|\ARGBREAK
    \verb|  min::uns32 indent;|\ARGBREAK
    \verb|};|
\ttmindex{line\_length}{in {\tt min::break}}
\ttmindex{indent}{in {\tt min::break}}
\ttmindex{break\_offset}{in {\tt min::break}}
\ttmindex{break\_column}{in {\tt min::break}}
\LABEL{MIN::BREAK_STRUCT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|struct min::| & \MINKEY{format}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  min::uns32 flags;|\ARGBREAK
    \verb|  const min::pgen_format * pgen_format;|\ARGBREAK
    \verb|};|
\ttmindex{flags}{in {\tt min::format}}
\ttmindex{pgen\_format}{in {\tt min::format}}
\LABEL{MIN::FORMAT_STRUCT} \\
\end{tabular}\end{indpar}
...
\item[\ttmkey{line\_length}{in {\tt min::printer}}]%
\label{LINE-LENGTH}
...
of columns in the line to exceed the {\tt line\_length}, then if
\begin{center}
\tt break\_column > indent
\end{center}
a break, consisting of a line end followed by {\tt indent}
spaces,
is inserted into the printer file buffer just after the break point
(i.e., at {\tt break\_\EOL offset}).

Horizontal space
characters immediately before the inserted line end are deleted.
After the break is inserted, {\tt break\_\EOL column} is reset to
the number of indentation spaces
and {\tt break\_\EOL offset} to the first position after
...
If a non-empty {\tt break\_stack} exists
because a {\tt min::\EOL save\_\EOL break} was executed,
then elements of the break stack may override
{\tt break\_\EOL column}, {\tt break\_\EOL offset}, and
{\tt indent} (but not {\tt line\_length}).
See \pagref{BREAK-STACK} for details.

\item[\ttmkey{ indent}{in {\tt min::printer}}]
...
\item[\ttmkey{break\_\EOL stack}{in {\tt min::printer}}]
Used with {\tt min::save\_break} and {\tt min::restore\_break} to set
multiple break points when printing nested lists.
See \pagref{BREAK-STACK} for details.
Initialized to {\tt min::\EOL NULL\_\EOL STUB}.

\end{itemlist}

\begin{itemlist}

\item[\ttmkey{format\_flags}{in {\tt min::printer}}]%
\label{FORMAT_FLAGS}
...
\item[\ttnbmkey{GRAPHIC\_HSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]
\item[\ttnbmkey{GRAPHIC\_VSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]%
\vspace{-1ex}
\item[\ttnbmkey{GRAPHIC\_NSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]%
\vspace{-1ex}
{\raggedright~\\}
...
\item[\ttnbmkey{GRAPHIC\_FLAGS}{in printer {\tt format\_\EOL flags}}]
The sum of the {\tt min::GRAPHIC\_XXX} format flags:
\begin{indpar}
\tt
min::GRAPHIC\_HSPACE \\
min::GRAPHIC\_VSPACE \\
...
\item[\ttnbmkey{ASCII\_FLAG}{in printer {\tt format\_\EOL flags}}]
...
\item[\ttnbmkey{ALLOW\_HSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]
\item[\ttnbmkey{ALLOW\_VSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]%
\vspace{-1ex}
\item[\ttnbmkey{ALLOW\_NSPACE\_FLAG}{in printer {\tt format\_\EOL flags}}]%
\vspace{-1ex}
{\raggedright~\\}
...
\item[\ttnbmkey{DISPLAY\_EOL\_FLAG}{in printer {\tt format\_\EOL flags}}]
...
\item[\ttnbmkey{HBREAK\_FLAG}{in printer {\tt format\_\EOL flags}}]
...
\item[\ttnbmkey{GBREAK\_FLAG}{in printer {\tt format\_\EOL flags}}]
If this flag is on, a line break is automatically set
...
and horizontal tab) in non-graphic
...
\item[\ttnbmkey{EOL\_FLUSH\_FLAG}{in printer {\tt format\_\EOL flags}}]
...
\begin{itemlist}
\item[\ttmkey{pgen\_format}{in printer {\tt format\_\EOL flags}}]
Controls the printing of
{\tt min::gen} values as described in Section \itemref{PRINTING-GENERAL-VALUES}.

\item[\ttmkey{format\_\EOL stack}{in {\tt min::printer}}]
Used with {\tt min::save\_format} and {\tt min::restore\_format} to save and
restore format parameters.
See \pagref{FORMAT-STACK} for details.
Initialized to {\tt min::\EOL NULL\_\EOL STUB}.
...
The {\tt min::setbreak} printer operation:
...
Recall from the description of {\tt line\_\EOL length} on
\pagref{LINE-LENGTH}
...
tab is to be inserted into the line, and this would cause the number
of columns in the line to exceed the {\tt line\_length}, a line end
will be inserted at the break point.  See {\tt line\_\EOL length}
on \pagref{LINE-LENGTH} for details.

For example, if {\tt line\_length} is 72 one might
print
\begin{center}
\tt [ aaa, bbb, ccc, ddd, eee, fff, ggg ]
\end{center}
in which {\tt min::setbreak} was called after every single space
character (which could be done automatically via the {\tt HBREAK\_\EOL FLAG}).
If instead {\tt line\_\EOL length} was just 14 and {\tt indent} was 2,
...
(so {\tt break\_\EOL column} is {\tt 0}) or setting it to
just after the {\tt indent} spaces if these are inserted after the
line end.
Thus {\tt min::\EOL eol} and the {\tt min::\EOL indent} operation
(see \pagref{MIN::INDENT}) both end by executing
an implied {\tt min::\EOL setbreak}, and an implied {\tt min::\EOL setbreak}
is also executed just after inserting a line end and {\tt indent} spaces
at a break point.
...
equal to {\tt break\_\EOL column + width}.
...
by {\tt indent} single spaces followed by setting a break
as per {\tt min::\EOL setbreak}.

When you set a break point for a {\tt min::left} or {\tt min::\EOL right}
operation you may want to do a {\tt min::\EOL reserve} first.
An adjustment that extends
beyond the printer {\tt line\_\EOL length} will format badly if
any breaks are enabled.
...
column is greater than {\tt indent}, and then inserts
single spaces until the current printer column is equal to
{\tt indent}.  The operation
ends by setting a break as per {\tt min::setbreak}.

The format parameters of a printer
may be saved in the
\ttmkey{format\_\EOL stack}{in {\tt min::printer}}\label{FORMAT-STACK}
...
\verb|const min::op min::|
    & \MINKEY{save\_format}
\LABEL{MIN::SAVE_FORMAT} \\
\verb|const min::op min::|
    & \MINKEY{restore\_format}
\LABEL{MIN::RESTORE_FORMAT} \\
\end{tabular}\end{indpar}

{\tt min::save\_format} pushes into the format stack a new element containing
the current values of
{\tt format\_\EOL flags} and {\tt pgen\_\EOL format}.
{\tt min::\EOL restore\_\EOL format}
resets these values from the top format stack element and then
pops this element from the stack.

Similarly the break parameters of a printer
may be saved in the
\ttmkey{break\_\EOL stack}{in {\tt min::printer}}\label{BREAK-STACK}
of the printer, and restored from that stack.
This may be done explicitly by the following
operations:
...
\verb|const min::op min::|
    & \MINKEY{save\_break}
\LABEL{MIN::SAVE_BREAK} \\
\verb|const min::op min::|
    & \MINKEY{restore\_break}
\LABEL{MIN::RESTORE_BREAK} \\
\end{tabular}\end{indpar}

{\tt min::save\_break} pushes into the format stack a new element containing
the current values of
{\tt break\_\EOL offset}, {\tt break\_\EOL column},
{\tt indent}, and {\tt line\_\EOL length}.
{\tt min::\EOL restore\_\EOL break}
resets these values from the top break stack element and then
pops this element from the stack.

However, {\tt min::\EOL save\_\EOL break}, after it
pushes the new break stack element, resets {\tt indent} to equal the printer
{\tt column}.  In addition, a non-empty break stack modifies break
behavior in the following way.
When a character representative other that single space or horizontal
tab is to be inserted into the line, and this would cause the number
of columns in the line to exceed the printer {\tt line\_length} parameter,
then if any break stack entry is enabled, the first such entry's
{\tt break\_\EOL offset}, {\tt break\_\EOL column}, and {\tt indent},
are used in place of the similarly named printer parameters.
If this happens, then all later entries
and the {\tt break\_\EOL offset}, {\tt break\_\EOL column},
and {\tt indent} printer parameters are adjusted by adding to offsets
...
To be a bit more specific, we say that a break stack
entry {\tt break\_\EOL stack[i]} is `\key{enabled}' if
\begin{center}
\tt break\_stack[i].break\_column > break\_stack[i].indent
\end{center}
...
which is the bottommost (not topmost) enabled element of the break stack.
The `later entries' are entries with larger {\tt i}.  Note that
after using an entry to insert a line end, that entry's
{\tt break\_\EOL offset}
and {\tt break\_\EOL column} are reset to make its break point
be just after the indentation spaces that are inserted after the line end,
just as would happen if a break stack entry were not used.  This sets
{\tt break\_stack[i].break\_column} equal to {\tt break\_stack[i].indent}.
Note also
that if \underline{no} break stack entry is enabled, break operation proceeds
using printer parameters as if the break stack did not exist.

Also the {\tt min::restore\_break} operation ends by executing an
implied {\tt min::\EOL indent} operation if the break stack element
it popped was disabled and was not the first element of the stack.

The {\tt min::save\_break} operation is intended to be use just
after printing the header of a list, and
the {\tt min::restore\_break} operation is intended to be use just
after printing the trailer of a list.
Suppose we print the following
...
\begin{indpar}\begin{verbatim}
    printer << min::setbreak << "{ " << min::save_break
            << min::setbreak << "aaa, "
            << min::setbreak << "bbb, "
            << min::setbreak << "[ " << min::save_break 
            << min::setbreak << "ccc, "
            << min::setbreak << "ddd, "
            << min::setbreak << "eee, "
            << min::setbreak << "( " << min::save_break 
...
\end{center}
a break is triggered just before printing the `{\tt ,}' following
`{\tt fff)}'.  The first element in the break stack was pushed
just after printing `{\tt \BRA~}'% }
and is disabled as it is at the beginning of line.
...
enabled break points left in the break stack, so the non-stack
...
The \key{end of message} operation executes {\tt min::\EOL restore\_\EOL break}
operations until the break stack is empty, then executes a {\tt min::\EOL eol}
operation, then
flushes {\tt printer->\EOL file}
if {\tt min::\EOL EOL\_\EOL FLUSH\_\EOL FLAG} is set,
and lastly executes a {\tt min::\EOL restore\_\EOL format} operation.
The \key{beginning of message} operation executes both a
a {\tt min::\EOL save\_\EOL break}
and a {\tt min::\EOL save\_\EOL format} operation.
...
can be bracketed by {\tt min::bom} and {\tt min::eom}
to restore formats at the end of the message.
Furthermore if {\tt min::\EOL bom} is executed after a message
header such as `{\tt ERROR: }', the message body will be indented
by the size of the header.

{\tt min::bom} will set {\tt indent} to the current value of {\tt column},
but if a smaller indent is desired, it can be set by following
{\tt min::\EOL bom} with {\tt min::\EOL set\_\EOL indent}
(\pagref{MIN::SET_INDENT}).

{\tt min::gen} values may be printed under the control of a
{\tt min::\EOL pgen\_\EOL format}.  As printing objects
...
{\tt format\_\EOL flags}.  For the horizontal tab character,
...
after the character is printed.  Here {\tt printer->\EOL format\_\EOL flags}
...
\verb|min::op min::|
    & \MINKEY{set\_format\_flags}\verb| ( min::uns32 format_flags )|
\LABEL{MIN::SET_FORMAT_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_format\_flags}\verb| ( min::uns32 format_flags )|
\LABEL{MIN::CLEAR_FORMAT_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{set\_\EOL pgen\_\EOL format}%
      \verb| ( min::printer_format * format )|
\LABEL{MIN::SET_PGEN_FORMAT} \\
\end{tabular}\end{indpar}

For example, `{\tt printer<<min::set\_flags(min::ASCII\_FLAG)}'
sets the {\tt ASCII\_FLAG} of {\tt for\-mat\_\EOL flags}.

Particular {\tt format\_\EOL flags} may be set and cleared by the
following
...
{\tt printer->\EOL format\_\EOL flags} {\tt min::\EOL ASCII\_\EOL FLAG}
...
If {\tt f} is {\tt NULL}, {\tt printer->\EOL format\_EOL flags} is used
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{resize\RESIZE}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr variable_size,| \\
    \verb|   min::unsptr unused_size,| \\
    \verb|   bool expand = true )| \\
    \end{tabular}
\LABEL{MIN::RESIZE} \\
...
\verb|void min::| & \MINKEY{expand\RESIZE}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr unused_size )| \\
    \end{tabular}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{reorganize\REORG}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr hash_size,| \\
    \verb|   min::unsptr variable_size,| \\
    \verb|   min::unsptr unused_size,| \\
    \verb|   bool expand = true )| \\
    \end{tabular}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{private\_flag\_of}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
\LABEL{MIN::PRIVATE_FLAG_OF} \\
\verb|void min::| & \MINKEY{public\_flag\_of}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{hash\_count\_of}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::obj_vec_ptr & vp )| \\
    \end{tabular}
...
\verb|bool min::| & \MINKEY{resize\RESIZE}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::list_insptr & lp,| \\
      \verb|   min::unsptr unused_size )| \\
      \end{tabular}
...
\verb|void min::| & \MINKEY{init\_\EOL input\RESIZE}\ARGBREAK
    \verb| ( min::ref<min::file> file )|
\LABEL{MIN::INIT_INPUT_OF_FILE} \\
\verb|void min::| & \MINKEY{init\_\EOL output\RESIZE}\ARGBREAK
    \verb| ( min::ref<min::file> file )|
\LABEL{MIN::INIT_OUTPUT_OF_FILE} \\
\end{TABULAR}
