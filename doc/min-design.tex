...
multi-sets of values.  Here names are sequences of name components, which are
...
{\em segment-sublists} and {\em child-sublists} are sublists.
...
assigns a {\em name-component} and a {\em segment-entry} to its
node.  The {\em segment-entry} assigns to the node a list of {\em values},
...
named by the successive elements of the attribute name.
An element of the object's hash table is a {\em hash-table-entry}
that is a {\em map-list} that lists the children of the root whose
{\em name-components} hash values equal the vector index of the hash
table element in the hash table element,
modulo the length of the hash table.
An element of the attribute vector
is the {\em segment-value} for a child of the root whose {\em name-component}
is the vector index of the element in the attribute vector.
If the {\em name-component} of a child of the object root is an integer
within range to be a vector index of an attribute vector element,
that child is defined by the attribute vector element, and not by
a hash table element {\em map-pair}.
...
So a {\em hash-table-entry} is a {\em map-list} which is a
list of elements that alternate between {\em name-components} and
{\em segment-entries}.  A {\em segment-entry} may be a {\em segment-sublist}
that specifies {\em flag-sets} and {\em values}, and may also specify
{\em child-sublists} that are like {\em map-lists} and give more
{\em name-component}/{\em value} pairs.  There are various abbreviations.
Although the general {\em segment-entry} is a {\em segment-sublist},
if this sublist just has a single {\em value}, that {\em value}
can used by itself as the {\em segment-entry}.
Either both the {\em child-sublist} and {\em reverse-child-sublist}
may be omitted from a {\em segment-sublist},
or just the {\em reverse-child-sublist} may be omitted.
If only one {\em child-sublist} is a omitted,
it must be the {\em reverse-child-sublist} that is omitted, to avoid ambiguity.
...
\item $E$ must be a {\em segment-sublist}.
