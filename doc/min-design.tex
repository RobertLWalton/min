...
\TT{min::IS\_GRAPHIC}	& Unless the \TT{NEEDS\_QUOTES} flag is present,
			  ...
			  For \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
			  this is returned for strings \underline{all}
			  of whose characters
...
\begin{indpar}\begin{tabular}{p{1.5in}@{~~~}p{4.0in}}
\TT{min::IS\_DIGIT}	& The string is a number.

			  ~~

			  For \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
			  this is returned for strings
			  that have a character with the \TT{IS\_\EOL DIGIT}
			  flag before any character with the
			  \TT{IS\_\EOL LETTER} flag.
\label{IS_DIGIT_CLASSIFIER} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{p{1.5in}@{~~~}p{4.0in}}
\TT{min::IS\_NATURAL}	& The string is a natural number.

			  ~~

			  For \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
			  this is returned for strings \underline{all}
			  of whose characters are have the
			  \TT{IS\_\EOL NATURAL} flag (indicating they
			  are ASCII digits) and which
			  either do not begin with `\TT{0}' or consist
			  of the single digit `\TT{0}'.
\label{IS_NATURAL_CLASSIFIER} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{p{1.5in}@{~~~}p{4.0in}}
\TT{min::IS\_LETTER}	& The string is a word.

			  ~~

			  For \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
			  this is returned for strings
			  that have a character with the \TT{IS\_\EOL LETTER}
			  flag before any character with the
			  \TT{IS\_\EOL DIGIT} flag.
...
if the string class has one of the
\TT{min::\EOL IS\_\EOL DIGIT},
\TT{min::\EOL IS\_\EOL LETTER},
or \TT{min::\EOL IS\_\EOL MARK} flag, has the
\TT{min::\EOL IS\_\EOL GRAPHIC} flag, and does \underline{not} either the 
\TT{min::\EOL IS\_\EOL LEADING} or
\TT{min::\EOL IS\_\EOL TRAILING} flag, the string is printed by
the \TT{min::\EOL print\_\EOL lexeme\_\EOL unicode} function
(\pagref{MIN::PRINT_LEXEME_UNICODE}) that may break the string
into parts.
