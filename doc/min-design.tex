...
Of these flags, only the first eight:
...
The specific assignment of these flags is generally done
according to their
...
some visible mark.  It is given to all characters except spacing
characters, control characters, and unsupported characters, all
...
the non-spacing combining mark characters
and all control characters except single space, horizontal tab,
and non-break space.  Note that the
...
The other flags, besides the first eight,
are used by the printer context data and can
be adapted according to the application.

TBD

With one exception a character gets exactly one of these flags.  The
exception is that a character can get both the
\TT{min::\EOL IS\_\EOL LEADING} and \TT{min::\EOL IS\_\EOL TRAILING}
flags.

Graphic characters
\label{LEADING/MIDDLING/TRAILING-CHAR-FLAGS}
which make a mark on the paper get at least one of the
