...
off (see print format flags at \pagref{PRINT_FORMAT_FLAGS}).
...
because a {\tt min::\EOL save\_\EOL line\_\EOL break} was executed,
then an element of the break stack may provide overrides for the
...
Used with {\tt min::save\_\EOL line\_\EOL break}
and {\tt min::\EOL restore\_\EOL line\_\EOL break} to set
multiple break points when printing nested lists.
See \pagref{LINE-BREAK-STACK} for details.
Initialized to and empty stack.
...
restore the print format parameters.
...
(so {\tt line\_break.column} is {\tt 0}) or setting it to
...
may be saved in the
\ttmkey{format\_\EOL stack}{in {\tt min::printer}}\label{PRINT-FORMAT-STACK}
...
the current {\tt print\_\EOL format}.
{\tt min::\EOL restore\_\EOL print\_\EOL format}
resets the {\tt print\_\EOL format} from the top format stack element and then
pops this element from the stack.

Similarly the break parameters of a printer
may be saved in the
\ttmkey{break\_\EOL stack}{in {\tt min::printer}}\label{LINE-BREAK-STACK}
of the printer, and restored from that stack.
This may be done explicitly by the following
operations:
...
{\tt line\_\EOL break.offset}, {\tt line\_\EOL break.column},
...
However, {\tt min::\EOL save\_\EOL line\_\EOL break}, after it
...
{\tt line\_\EOL break.offset}, {\tt line\_\EOL break.column}, and {\tt indent},
are used in place of the similarly named printer parameters.
If this happens, then all later entries
and the {\tt line\_\EOL break.offset}, {\tt line\_\EOL break.column},
...
{\tt break\_stack[i].column} equal to {\tt break\_stack[i].indent}.
Note also
...
Also the {\tt min::restore\_line\_\EOL break} operation ends by executing an
...
The {\tt min::save\_line\_break} operation is intended to be use just
after printing the header of a list, and
the {\tt min::\EOL restore\_\EOL line\_\EOL break}
operation is intended to be use just
...
\begin{indpar}\begin{verbatim}
    printer << min::setbreak << "{ " << min::save_line_break
            << min::setbreak << "aaa, "
            << min::setbreak << "bbb, "
            << min::setbreak << "[ " << min::save_line_break 
            << min::setbreak << "ccc, "
            << min::setbreak << "ddd, "
            << min::setbreak << "eee, "
            << min::setbreak << "( " << min::save_line_break 
...
a {\tt min::\EOL save\_\EOL line\_\EOL break}
and a {\tt min::\EOL save\_\EOL print\_\EOL format} operation.
{\tt min::\EOL eom} executes in order a
{\tt min::\EOL restore\_\EOL line\_\EOL break}, {\tt min::\EOL eol},
and {\tt min::\EOL restore\_\EOL print\_\EOL format}.
...
If {\tt f} is {\tt NULL}, {\tt printer->\EOL format\_\EOL flags} is used
...
\ttmindex{str\_ascii\_quote}{in {\tt min::gen\_format}}
\ttmindex{str\_utf8\_quote}{in {\tt min::gen\_format}}
...
\item[\ttmkey{str\_ascii\_quote}{in {\tt min::gen\_format}}]
\item[\ttmkey{str\_utf8\_quote}{in {\tt min::gen\_format}}]\vspace{-2ex}
