...
\LABEL{MIN::PRINTER} \\
\verb|void min::| & \MINKEY{init}\ARGBREAK
    \verb| ( min::printer & printer,|\ARGBREAK
    \verb|   min::file & file = min::NULL_STUB )|
\LABEL{MIN::INIT_OF_PRINTER} \\
\verb|void min::| & \MINKEY{init\_output\_stream}\ARGBREAK
    \verb| ( min::printer & printer,|\ARGBREAK
...
\verb|min::printer_parameters printer|
    & \TTARMKEY{saved\_\EOL parameters[4]}{in {\tt min::printer}}
...
{\tt min::\EOL NULL\_\EOL STUB} then if the printer already exists,
its current file is not changed, and if the printer is being
created, a suitable {\tt min::file} for it is created.
...
which is the most common way of setting up a printer.
...
when a printer is created.

\item[\ttmkey{saved\_\EOL parameters[4]}{in {\tt min::printer}}]
...
\verb|const min::op min::| & \MINKEY{eol} & (end of line)
\LABEL{MIN::EOL} \\
\verb|const min::op min::| & \MINKEY{eom} & (end of message)
\LABEL{MIN::EOM} \\
\end{tabular}\end{indpar}
...
\verb|const min::op min::| & \MINKEY{autobreak}
\LABEL{MIN::AUTOBREAK} \\
\verb|const min::op min::| & \MINKEY{noautobreak}
...
\verb|const min::op min::| & \MINKEY{eom\_flush}
\LABEL{MIN::EOM_FLUSH} \\
\verb|const min::op min::| & \MINKEY{noeom\_flush}
\LABEL{MIN::NOEOM_FLUSH} \\
\verb|const min::op min::| & \MINKEY{keep}
\LABEL{MIN::KEEP} \\
\verb|const min::op min::| & \MINKEY{nokeep}
\LABEL{MIN::NOKEEP} \\
\end{tabular}\end{indpar}

For example, `{\tt printer<<min::autobreak}' sets the
{\tt printer->\EOL parameters.flags} {\tt AUTO\-BREAK\_FLAG}
and `{\tt printer<<min::noautobreak}' clears this flag.
