...
(including \TT{min::\EOL OBJ\_\EOL PRIVATE}, \TT{min::\EOL OBJ\_\EOL PUBLIC},
\TT{min::\EOL OBJ\_\EOL GTYPED}, and \TT{min::\EOL OBJ\_\EOL IS\_\EOL GTYPE})
...
called the `graph typed object'.

Graph typed objects may be introduced in two different ways.  In the first
method the context is given the \minkey{OBJ\_\EOL GTYPED} flag
and its first variable points at the graph type.  In the second method
the graph typed object is represented by
a stub of \minkey{GTYPED\_\EOL OBJ}\label{MIN::GTYPED_OBJ} type
that points at an auxilary stub
of \minkey{GTYPED\_\EOL OBJ\_\EOL AUX}\label{MIN::GTYPED_OBJ_AUX} type
...
This second method of introducing graph typed objects allows graph typed
objects to be viewed with several different graph types.

Typed objects make use of two kinds of indirect pointer.  The first is
just a single index \TT{min::gen} value in the graph type
that holds the index of a
variable vector element.  This always refers to a variable in the context.
The second is a stub of \minkey{VAR\_\EOL PTR}\label{MIN::VAR_PTR} type
that holds a pointer to an auxilary stub of
\minkey{VAR\_\EOL PTR\_\EOL AUX}\label{MIN::VAR_PTR_AUX} type
that contains a pointer to
a context object and an index of a variable in the context.
An auxilary stub is necessary in order to allow garbage collection
of the stubs (the control of the primary stub is reserved for GC use).
This second kind of pointer is only supported when it is stored in
context variables.
...
so it is not possible to add attributes to a graph type or
graph type object.  It is also not allowed for a graph type
object to have attributes
with more than one value or double arrow values.

Graph types are objects with the
\minkey{OBJ\_\EOL IS\_\EOL GTYPE} flag.  The first variable of a graph
type is an integer that bounds the indices stored in the graph type.
That is, if this integer is \TT{B} then any index \TT{i} stored in the graph
type is in the range $0\leq$\TT{i}$<$\TT{B}.
Strictly speaking, a graph type is an object that is the root of a tree
of objects, and the non-root members of the tree do \underline{not}
have the \minkey{OBJ\_\EOL IS\_\EOL GTYPE} flag, unless they are
roots of graph subtypes.
...
and sets the object's \TT{min::\EOL OBJ\_\EOL IS\_\EOL GTYPE} flag.
It is permissible for some subgraphs to already
be graph types and some objects in the graph to already be public
before this function is called.
The function returns the \TT{gtype} argument if the new graph type
is acyclic and does not have any attributes with more than one
value or double arrow values,
...
The object \TT{obj} must be public, even if it does not
have the graph type.

Pointers of \TT{min::list\_xxxptr} or \TT{min::\EOL attr\_xxxptr} type
cannot point at contexts (i.e., objects with
the \TT{OBJ\_\EOL GTYPED} flag set).
Pointers of \TT{min::\EOL list\_\EOL updptr} or
\TT{min::\EOL attr\_\EOL updptr} type
cannot point at graph types (i.e., objects with
the \TT{OBJ\_\EOL IS\_\EOL GTYPE} flag set).
Pointers of \TT{min::xxxx\_insptr} type
cannot point at contexts or graph types (i.e., objects with
the \TT{OBJ\_\EOL PUBLIC} flag set).

When used with attribute pointers created with the above functions,
\TT{min::get} follows the indirect pointers that are index \TT{min::gen}
values, treating these as indices of variables in the graph typed object's
contest,
and also follows indirect pointers that are 
\TT{min::gen} values pointing at \TT{min::\EOL VAR\_\EOL PTR}
...
\TT{Min::gen} values pointing at \TT{min::\EOL VAR\_\EOL PTR}
stubs may be stored using \TT{min::\EOL update}.
...
\LABEL{MIN::SUBLIST_AUX} \\
\verb|const int min::| & \MINKEY{GTYPED\_OBJ}
\LABEL{MIN::GTYPED_OBJ} \\
\verb|const int min::| & \MINKEY{GTYPED\_OBJ\_AUX}
\LABEL{MIN::GTYPED_OBJ_AUX} \\
\verb|const int min::| & \MINKEY{VAR\_PTR}
\LABEL{MIN::VAR_PTR} \\
\verb|const int min::| & \MINKEY{VAR\_PTR\_AUX}
\LABEL{MIN::VAR_PTR_AUX} \\
\end{TABULAR}
\end{indpar}
