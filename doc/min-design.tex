...
\multicolumn{2}{l}{\TTKEY{MIN\_MAX\_ABSOLUTE\_STUB\_ADDRESS}} \\
	...
	& ~~~~ if \TTX{MIN\_STUB\_BASE} defined: \\
	& ~~~~~~~~ \TTX{  MIN\_STUB\_BASE} \\
	& ~~~~~~~~ \TTX{+ MIN\_MAX\_RELATIVE\_STUB\_ADDRESS} \\
	& ~~~~ else if \begin{tabular}[t]{@{}l@{}}
	               \TTX{MIN\_IS\_COMPACT} = 1 and \\
...
You can also find out the character flags of \TT{c} as decribed
...
to mark graphic characters which when preceeded by a few graphic
...
(horizonal or vertical) space characters and these are made into
...
if \TT{id\_\EOL map} is given (not \TT{NULL\_\EOL STUB}),
that is used in place of \TT{printer->\EOL id\_\EOL map}, and
if \TT{gen\_\EOL format} is given (not \TT{NULL}),
that is used in place of
\TT{printer->\EOL printer\_\EOL format.gen\_\EOL format->\EOL
                  id\_\EOL map\_\EOL format}.
...
\begin{indpar}\begin{verbatim}
    min::find_and_add ( id_map, min::stub_of ( v ) );
    return min::print_id_map ( printer, id_map, gen_format );
\end{verbatim}\end{indpar}

if \TT{v} has a stub and is not already in \TT{id\_\EOL map}.
If \TT{id\_\EOL map} is \underline{not} given (i.e., equals
\TT{min::\EOL NULL\_\EOL STUB}), \TT{printer->\EOL id\_\EOL map}
is used instead, and is initialized before use if it has not previously
been initialized.

If \TT{v} has a stub and is already in \TT{id\_\EOL map},
this function prints a line for \TT{v} with the same format
as \TT{min::\EOL print\_\EOL one\_\EOL id}.
If instead \TT{v} does not have a stub, this function prints a line that
is the same except `\TT{@(NONE)}' is printed in place of the \TT{id\_\EOL map}
derived ID.
...
(including \TT{min::OBJ\_\EOL PRIVATE}, \TT{min::OBJ\_\EOL PUBLIC},
and \TT{min::OBJ\_\EOL TYPED})
...
object \TT{OBJ\_\EOL PUBLIC} flag which prevents changes to the
...
The hash table of the returned object is filled with \TTX{min::LIST\_END()}
values (\pagref{MIN::LIST_END}), and is therefore an empty hash table.
The variable vector is filled with \TTX{min::UNDEFINED()} values.
...
A \TTX{min::gen} value may be tested to see if it points at an
...
element is $H$, then the pair is put into the list of the $H$\TTX{\%}$S+1$'st
...
header, which does not contain \TTX{min::gen} values.
0 can be used as a form of null pointer.

An auxiliary pointer index \TTX{i} corresponds to the
body vector index total size {\tt - i}, so \TTX{i=1} corresponds
...
to the object may be created, and this will set the \TTX{min::OBJ\_PRIVATE}
...
If an object has neither flag, its \TTX{min::OBJ\_PUBLIC} flag may be set
...
The \TTX{min::OBJ\_PUBLIC} flag may not be cleared by
protected functions.  The idea here is that setting the \TTX{min::OBJ\_PUBLIC}
...
\TTX{min::OBJ\_PRIVATE} nor the \TTX{min::OBJ\_PUBLIC} flags are set.
...
Here if \TTX{v} or \TTX{s} do not point at the stub of an object
...
The case where \TTX{s} equals {\tt min::\EOL NULL\_\EOL STUB}
...
Here the sizes are in \TTX{min::gen} units.  When an object
...
Given a vector pointer \TTX{vp}, \TTX{min::var(vp,i)} can be used
to read the \TTX{i+1}'st variable in the object pointed at
by \TTX{vp}, for $0\leq i<\mbox{\tt min::var\_size\_of(vp)}$.
Similarly, \TTX{min::hash(vp,i)} can be used
to read the \TTX{i+1}'st hash table entry
for $0\leq i<\mbox{\tt min::hash\_size\_of(vp)}$,
\TTX{min::attr(vp,i)} can be used
to read the \TTX{i+1}'st attribute vector entry
for $0\leq i<\mbox{\tt min::attr\_size\_of(vp)}$.
\TTX{min::aux(vp,p)} differs slightly in that it reads
the \TTX{i}'th auxiliary area element ordering the
...
returns either a {\tt min::\EOL ref<min::gen>} reference to
...
so \TTX{p[0]} is pushed into or popped from a body vector location immediately
before the location into which \TTX{p[1]} is pushed into or popped from, etc.
Thus the attribute vector push function pushes
in the order \TTX{p[0]}, \TTX{p[1]}, \TTX{p[2]}, \ldots; while the
auxiliary vector push function pushes
in the order \TTX{p[n-1]}, \TTX{p[n-2]}, \TTX{p[n-3]}, \ldots.
...
and auxiliary area({\tt aux}).  These offsets are in \TTX{min::gen}
...
{\tt min::\EOL obj\_\EOL vec\_\EOL insptr} which are copied
back into the object when the 
{\tt min::\EOL obj\_\EOL vec\_\EOL insptr} is destructed.

The following are equivalences except for
the omission of {\tt MIN\_\EOL ASSERT} checks that the index \TTX{i}
...
actually exist,
conversions to \TTX{min::ref<min::gen>}
...
\underline{not} \TTX{min::gen} values.  Even so it is
sized in \TTX{min::gen} units; e.g., a short object header size
...
\TTX{min::gen} values other than the list or sublist auxiliary
...
If it has the value \TTX{min::LIST\_END()}, the final segment is empty.
...
pointer or the special value \TTX{min::LIST\_END()}.  But it can be
a sublist auxiliary pointer, or the special value \TTX{min::EMPTY\_SUBLIST()},
...
A list element
that is a sublist auxiliary pointer or the value \TTX{min::EMPTY\_SUBLIST()}
...
value \TTX{min::EMPTY\_SUBLIST()}, it represents an empty sublist.
...
area element containing a \TTX{min::LIST\_END()} value.
...
A \TTX{min::list\_ptr} read-only list pointer permits read-only access to list
elements.
...
{\tt min::\EOL start\_\EOL sublist} with \TTX{lp2}$\equiv$\TTX{lp}).
...
pointer to the same object and that function returned \TTX{true} indicating
...
\TTX{lp2} is a read-only list pointer, but {\tt min::start\_\EOL copy}
does not.
...
\TTX{lp2} is a read-only or updatable
...
It returns \TTX{true} if it expands the
object's body, and \TTX{false} if it does not.

Importantly {\tt min::\EOL insert\_reserve\RESIZE} is the only list pointer
function that may cause a resizing of
the object pointed at, and if a resizing occurs (i.e., if \TTX{true}
is returned),
...
\TTX{p[0]} is inserted into the list before \TTX{p[1]}, etc.
If \TTX{n=0} elements are to be inserted, both insert
...
or a \TTX{min::LIST\_END()} value.  However, as a list element,
the value may be a sublist auxiliary pointer (or equivalent, see below)
or a \TTX{min::EMPTY\_SUBLIST()} value.
...
or a \TTX{min::LIST\_END()} value, and is never a list element.
...
auxiliary area \TTX{min::gen} values or in auxiliary stub controls.
...
auxiliary area \TTX{min::gen} values or in auxiliary stub \TTX{min::gen}
values.
...
A \TTX{min::gen} value that points at a stub $S$
...
must point at an object auxiliary stub $S$ of type \TTX{min::LIST\_AUX} and the
...
A \TTX{min::gen} value that points at
...
be a list auxiliary pointer value, a \TTX{min::gen} value pointing at
a stub of type \TTX{min::LIST\_AUX}, or a \TTX{min::LIST\_END()} value.

However, it can be a sublist auxiliary pointer value, a \TTX{min::gen}
value pointing at a stub of type \TTX{min::SUBLIST\_AUX}, or a
\TTX{min::EMPTY\_SUBLIST()} value.
...
Every object auxiliary stub is pointed at by a \TTX{min::gen}
...
A \TTX{min::gen} value pointing at an object auxiliary stub $S$
must be one of the following:
\begin{center}
\begin{tabular}{l@{~~~}l}
Location of \TTX{min::gen} & Type of auxiliary stub
\\[2ex]
value of auxiliary area element & \TTX{LIST\_AUX} or \TTX{SUBLIST\_AUX}
\\[1ex]
value of auxiliary stub & \TTX{SUBLIST\_AUX}
\\[1ex]
value of hash table element & \TTX{LIST\_AUX}
\\[1ex]
value of attribute vector element & \TTX{LIST\_AUX}
\end{tabular}
\end{center}
...
at an auxiliary area element that holds a \TTX{min::gen} value
pointing at an object auxiliary stub of type \TTX{min::LIST\_AUX},
...
\item All the lists headed by the two object hashs are equal.
...
Here the syntactic categories represent \TTX{min::gen} values
or lists or sublists
of \TTX{min::gen} values in the sense of the object list level.
...
An {\em atom} is a \TTX{min::gen} number or string.
An {\em label} is a \TTX{min::gen} label.
An {\em object} is a \TTX{min::gen} value pointing at an object stub.
A {\em control-code} is a \TTX{min::gen} control code value.
An {\em index} is a \TTX{min::gen} index value.
An {\em indirect-auxiliary} is a \TTX{min::gen} indirect auxiliary value.
...
{\em node-sublist}.
The {\em flag-set} is a sequence of \TTX{min::gen}
control codes.
...
is the value of the \TTX{min::VSIZE} constant (\pagref{MIN::VSIZE}),
which is 24 if \TTX{min::gen}
values are 32-bits, and 40 if \TTX{min::gen} values are 64-bits.
...
length of this initial segment is returned in the `\TTX{length}'
argument.  If \TTX{0} is returned in `\TTX{length}',
...
unless the latter are \TTX{min::NONE()} or \TTX{min::ANY()}.
...
at if the reverse attribute name is \TTX{min::NONE()}, or is that of
the reverse attribute pointed at if the reverse attribute name is not
\TTX{min::NONE()} or \TTX{min::ANY()}.
If the reverse attribute name is \TTX{min::ANY()} the value multiset is
the set of all values associated with any reverse attribute name, but
this \TTX{min::ANY()}
value set can only be read by the 3-argument \TTX{min::get}
function, and made empty by the {\tt min::\EOL set\RESIZE}
function with 0 new values (see \pagref{MIN::SET_DELETE_ANY}).
It is an error to attempt any other access to this \TTX{min::ANY()} value set.
...
\TTX{min::NONE()} (\pagref{MIN::NONE}) if the value multiset is empty,
or returns the special value
\TTX{min::MULTI\_VALUED} (\pagref{MIN::MULTI_VALUED}) if the value multiset
has more than one value.

The {\tt min::\EOL get\_\EOL flags}
function returns the control codes in the flag set pointed at and stores
these in the \TTX{min::gen} vector `{\tt out}'.
...
The implementation dependent order of the \TTX{attr\_info}
...
reverse attribute name is not \TTX{min::NONE()}.
...
argument, unless {\tt v} is \TTX{min::NONE()},
...
the pointer's reverse attribute name can be \TTX{min::NONE()}
but cannot be \TTX{min::ANY()}, with the exception of the case
where all values are being deleted.  If the reverse attribute name
is \TTX{min:ANY()} and the {\tt min::\EOL set\RESIZE} function is called to
...
if {\tt v} is \TTX{min::NONE()} there is no new value and the function
is a no-operation.
For these functions the pointer's reverse attribute name
can be \TTX{min::NONE()}
but cannot be \TTX{min::ANY()}.
...
if {\tt v} is \TTX{min::NONE()} there is no value to remove and the function
is a no-operation.  These functions return the number of values actually
removed from the multiset of values.
For these functions the pointer's reverse attribute name
can be \TTX{min::NONE()} but cannot be \TTX{min::ANY()}.
...
\subsubsection{Object Attribute Short-Cuts}
...
The only difference is that when possible a \TT{min::attr\_ptr}
or \TT{min::attr\_updptr} is used instead of a
\TT{min::attr\_insptr} which improves efficiency slightly,
and more importantly permits the function to be used on
a public object (\pagref{MIN::PUBLISH}).
...
just a single index \TTX{min::gen} value that holds the index of a
variable vector element.  This always refers to a variable in the context.
The second is an indirect \TTX{min::gen} value that points at a pair
of values in an auxiliary area: the first being an index \TTX{min::gen}
value and the second a pointer-to-object-stub \TTX{min::gen} value.
...
\verb|// All other members are NULL or 0 or NONE().|
\end{tabular}
...
if the \ttkey{ENABLE\_\EOL LOGICAL\_\EOL LINE} object format operation
flag is on.
\item A \TT{.terminator} value may be \TT{min::\EOL INDENTED\_\EOL PARAGRAPH()}
if the \ttkey{ENABLE\_\EOL IN\-DENTED\_\EOL PARAGRAPH} object format operation
flag is on.
\end{enumerate}
...
and the \ttkey{ENABLE\_\EOL LOGICAL\_\EOL LINE} object format operation
flag is on.
...
the \ttkey{ENABLE\_\EOL INDENTED\_\EOL PARAGRAPH} object format operation
flag is on,
...
\item Otherise the `\key{bracketed compact format}'
...
\TT{min::set\_break} just before printing every other element.
...
at least one line element preceeding
...
\verb|min::ENABLE_COMPACT                      // obj_op_flags|
\end{TABULAR}
...
\verb|min::PRINT_ID                            // obj_op_flags|\ARGBREAK[0in]
\verb|// All other members are NULL or 0 or NONE().|
\end{TABULAR}
