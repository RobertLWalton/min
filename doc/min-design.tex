...
multiple break points when printing nested lists.  See \pagref{BREAK-STACK}
for details.
...
is inserted into the printer file buffer just after
{\tt printer->break\_offset}.
...
{\tt break\_\EOL column} and {\tt break\_\EOL offset} and
cause {\tt indent} (see below) to be reset.
...
One typically sets a break just before printing text that one wants
...
When you set a break for a {\tt min::left} or {\tt min::\EOL right}
