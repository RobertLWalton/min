...
and \TT{quote\_\EOL control.in\_\EOL class\_\EOL skip}
...
\ttmindex{in\_class\_if\_first}{in {\tt min::str\_classifier}}\ARGBREAK
    \verb|  min::uns32    in_class_skip;|%
\ttmindex{in\_class\_skip}{in {\tt min::str\_classifier}}\ARGBREAK
    \verb|  min::uns32    in_class_if_none_of;|%
\ttmindex{in\_class\_if\_none\_of}{in {\tt min::str\_classifier}}\ARGBREAK
    \verb|};|
...
\verb|0                          // in_class_skip|\ARGBREAK
\verb|0                          // in_class_if_none_of|
...
\verb|min::QUOTE_SKIP            // in_class_skip|\ARGBREAK
\verb|min::IS_NON_GRAPHIC        // in_class_if_none_of|
...
if any character in the string has a character
flag that is in \TT{qc.in\_\EOL class\_\EOL if\_\EOL none\_\EOL of}, the
string is quoted.  Otherwise if string begins with zero or more characters
each with a flag in \TT{qc.in\_\EOL class\_\EOL skip}, followed by a single
character with a flag in \TT{qc.in\_\EOL class\_\EOL if\_\EOL first}, the string
is \underline{not} quoted; otherwise the string is quoted.%
\label{IN_CLASS_IF_FIRST}  See the \TT{min::\EOL QUOTE\_\EOL SUPPRESS}
...
\begin{indpar}
It has \TT{.initiator} and \TT{.terminator} attributes,
but no other attributes except possibly \TT{.separator} and/or
\TT{.position}.
\end{indpar}
or
\begin{indpar}
It a \TT{.type} attribute,
but no other attributes except possibly \TT{.separator} and/or
\TT{.position}.
\end{indpar}
...
if the \TT{obj\_\EOL attreol} member of the object format is not \TT{NULL}.
...
objects are being printed in line format.
...
object elements and attributes in the absence of object
\TT{.initiator} and \TT{.terminator} attributes.
...
followed optionally by the object \TT{.type} and attributes
followed by \TT{obj\_\EOL ket} serves as the closing bracket
for the object.

If \TT{obj\_bra} is \TT{NULL}, the object is printed
in `\key{line format}'.  None of these members are used,
and the object is printed as a list
of element values, with the last line ending with \TT{obj\_\EOL attreol}
if there are attributes, and each attribute is printed on its own indented
line.  Special attributes
such as \TT{.type}, \TT{.initiator}, and \TT{.separator}
are not given any special
treatment.  This format is usually used by \TT{flush\_\EOL id\_\EOL map}
to print objects.

Otherwise, if any of these members other than
\TT{obj\_\EOL ket} are \TT{NULL}, they just print as the
