...
Or {\tt end\_\EOL offset} if there is no next complete line.
...
stored in {\tt buffer} and {\tt line\_\EOL index}.  This last
guarentee is equivalent to
\begin{center}
{\tt line\_index->length => \\
min ( next\_line\_number, spool\_lines )}
\end{center}
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{init\_\EOL input}%
    \verb| ( min::ref<min::file> file )|
\LABEL{MIN::INIT_INPUT_OF_FILE} \\
\verb|void min::| & \MINKEY{init\_\EOL output}%
    \verb| ( min::ref<min::file> file )|
...
and {\tt next\_\EOL line\_\EOL offset} to {\tt 0} and emptying
...
The {\tt min::skip\_partial} function can have an undesireable
interaction with the use of {\tt min::\EOL end\_\EOL line} to
make an initial segment of a partial line into a complete line:
see \pagref{SKIP_PARTIAL_END_LINE}.  It also affects the next
call to {\tt min::\EOL next\_\EOL line}; specifically, if a partial
line is skipped, then added to and made into a complete line,
...
after the call to {\tt min::\EOL skip\_\EOL partial}.
...
to work, the line to be returned
must have been left in {\tt file->\EOL buffer},
and whether or not
...
is required.
In the absence of calls to {\tt min::rewind}(\pagref{MIN::REWIND_FILE})
for a file,
\begin{center}\begin{tabular}{rl}
       & \tt file->line\_index->length => file->spool\_lines \\
or \\
       & \tt file->line\_index->length >= file->next\_line\_number \\
\end{tabular}\end{center}
and in this case
\begin{center}\begin{tabular}{rl}
       & \tt file->next\_line\_number - file->spool\_lines \\
\tt <= & \tt line\_number \\
\tt < & \tt file->next\_line\_number
\end{tabular}\end{center}
is a sufficient condition for {\tt min::\EOL line} to be able to
find the line with the given {\tt line\_\EOL number}.

For input files it is common to set {\tt spool\_\EOL lines}
...
{\tt min::\EOL DISPLAY\_\EOL EOL} flag, is returned.  If a line is
unavailable (i.e., {\tt min::\EOL line} returns {\tt min::\EOL NO\_\EOL LINE},
see above), then {\tt unavailable\_\EOL line} is printed instead and
{\tt 0} is returned.  If the line would print as blank, i.e., consisting of
nothing but horizontal space characters with no end of line display, then
{\tt blank\_\EOL line} is printed instead and {\tt 0} is returned.
If there is no line
because {\tt line\_\EOL number == file->\EOL file\_\EOL lines},
then {\tt end\_\EOL of\_\EOL file} is printed instead and
{\tt 0} is returned.  In these last three special cases,
{\tt file->\EOL print\_\EOL flags} is not used, and it is best if the
special string printed in place of the line consist of nothing but
graphic ASCII characters and single spaces.
In all cases exactly one complete print line is
produced.
...
which eliminates the possibility of failure.  Note that
if {\tt spool\_\EOL length != min::\EOL ALL\_\EOL LINES},
a rewind can make {\tt line\_\EOL index} shorter than
{\tt spool\_\EOL length}, causing {\tt min::\EOL line}
to fail when it otherwise would not.
...
Note that a partial line \underline{cannot}\label{SKIP_PARTIAL_END_LINE}
be split using the
last function if {\tt min::\EOL skip\_\EOL partial}
(\pagref{MIN::SKIP_PARTIAL}) has been used to skip past the
...
file, and calls {\tt min::\EOL skip\_\EOL partial}
(\pagref{MIN::SKIP_PARTIAL}) to skip over this partial line
...
while {\tt min::\EOL flush\_\EOL partial} copies any partial line
...
or {\tt min::\EOL skip\_\EOL partial}.
...
a {\tt min::\EOL printer\_\EOL format}.
...
\verb|struct min::| & \MINKEY{printer\_format}\ARGBREAK
    ...
    \verb|  const char *       special_names;|\ARGBREAK
...
\item[\ttnbmkey{GRAPHIC\_STR\_FLAG}{{\tt min::printer\_parameters} flag}]
...
\item[\ttnbmkey{EXPRESSION\_FLAG}{{\tt min::printer\_parameters} flag}]
\item[\ttnbmkey{IMPLICIT\_PARENTHESES\_FLAG}%
    {{\tt min::printer\_parameters} flag}]\vspace{-1ex}
\item[\ttnbmkey{DATA\_FLAG}{{\tt min::printer\_parameters} flag}]\vspace{-1ex}
\item[\ttnbmkey{RAW\_FLAG}{{\tt min::printer\_parameters} flag}]\vspace{-1ex}
\item[\ttnbmkey{OBJ\_PSTUB\_FLAG}{{\tt min::printer\_parameters} flag}]%
\vspace{-1ex}
These flags control the printing of objects.
...
Also, the parsing special {\tt .initiator}'s {\tt \#} and {\tt "} are
unwrapped (i.e., \verb/"foo" {| .initiator "<Q>" |}/ prints as
\verb|"foo"|).
Otherwise all attributes are printed, along with the vector elements,
and parsing special {\tt .initiator}'s are not recognized.

If the {\tt IMPLICIT\_\EOL PARENTHESIS\_\EOL FLAG} is set,
implicit parentheses are printed surrounding objects that have no
attributes (and only vector elements).  Otherwise no parentheses are
printed surrounding such objects.

If the {\tt DATA\_\EOL FLAG} is set the object is printed on
...
If the {\tt RAW\_\EOL FLAG} is set, output of an object vector element that is
a string is surrounded by {\tt str\_\EOL pre/postfix}, and
output of an object vector element that is an object has the form
\verb|@<|{\em number}\verb|>| and references an object output separately
(if the object has not been otherwise output it is output as per the
{\tt DATA\_\EOL FLAG}).
...
also called for object stubs.  In this case the other object output control
flags (e.g., the {\tt EXPRESSION\_\EOL FLAG}) may or may not be
be ignored by the {\tt pstub} function.
