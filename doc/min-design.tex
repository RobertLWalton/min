...
\verb|unsigned min::|
    & \MINKEY{auxiliary\_area\_size\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::AUXILIARY_AREA_SIZE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{auxiliary\_area\_size\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::AUXILIARY_AREA_SIZE_OF_LONG_OBJ} \\
\verb|unsigned min::|
...
\verb|void min::|
	& \MINKEY{insert\_reserve\REL}%
	  ...
	  \verb|   bool use_auxiliaries =| \\
	  \verb|       min::use_list_auxiliary_stubs )| \\
	  \end{tabular}
...
The {\tt insert\_reserve} function reserves space for the given number
...
The {\tt insert\_reserve} function can increase the size of the object
...
runs out of space, in which case {\tt insert\_reserve} must be sure
there are sufficient free stubs to satisfy the insertion calls.  The
last argument to {\tt insert\_reserve} determines which strategy is
used.  It defaults to the value of \minkey{use\_list\_auxiliary\_stubs}.

The {\tt insert\_before} function inserts list elements
just before the current position of a list pointer.
The {\tt insert\_after} function inserts list elements just after the
current position.  If the list pointer is at the end of list,
{\tt insert\_before} inserts the elements at the end of the list, and
...
\verb|void min::|
	& \MINKEY{insert\_reserve\REL}\ARGBREAK
	  ...
	  \verb|   bool use_auxiliaries =|\ARGBREAK
	  \verb|       min::use_list_auxiliary_stubs )|
\LABEL{MIN::INSERT_RESERVE} \\
\verb|void min::|
