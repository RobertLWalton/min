...
There are some macros that can be defined to control compilation.
These have names beginning with \ttkey{MIN\_}.  One of these
is:

\begin{center}
\begin{tabular}{lp{4.0in}}
\TTNBKEY{MIN\_DEBUG}	& 1 if extra debugging checks are to be compiled;
			  0 (the default) if not
\LABEL{MIN_DEBUG} \\
\end{tabular}
\end{center}

Parts of MIN provide protection against bugs by using the {\tt MIN\_ASSERT}
macro:
...
By setting {\tt MIN\_PROTECT} to {\tt 0}, a compilation can optimized
debugged code.  Alternatively,
...
The above macros ({\tt MIN\_DEBUG}, {\tt MIN\_ASSERT},
{\tt MIN\_PROTECT})
...
\verb|-DMIN_DEBUG=1|	& add debugging checks
					& \pagref{MIN_DEBUG} \\
...
The other compilation parameters involve technical details of the
implementation.  For details see the file
\ttkey{include/min\_parameters.h}.
...
Some, but not all, of the protection provided by the protected interface
...
a 24-bit sublist auxiliary pointer \\
a 24-bit indirect pair auxiliary pointer \\
a 12-bit+12-bit indexed auxiliary pointer\COMPACT \\
a 20-bit+20-bit indexed auxiliary pointer\LOOSE \\
a 24-bit index \\
a 24-bit control code \\
a 40-bit control code\LOOSE \\
a 24-bit special value \\
\end{tabular}
\end{center}
...
\TTNBKEY{MIN\_\EOL MAXIMUM\_\EOL ABSOLUTE\_\EOL STUB\_\EOL ADDRESS}
	& The maximum absolute address of any stub as an unsigned integer
	  constant.
\LABEL{MIN_MAXIMUM_ABSOLUTE_STUB_ADDRESS} \\[1ex]
\TTNBKEY{MIN\_\EOL MAXIMUM\_\EOL RELATIVE\_\EOL STUB\_\EOL ADDRESS}
	& The maximum relative address of any stub as an unsigned integer
	  constant.
\LABEL{MIN_MAXIMUM_RELATIVE_STUB_ADDRESS} \\[1ex]
\TTNBKEY{MIN\_STUB\_BASE}
	& The stub base address for relative addressing.  This can be
          a non-negative integer constant or the name of a global
...
{\tt MIN\_\EOL MAXIMUM\_\EOL ABSOLUTE\_\EOL STUB\_\EOL ADDRESS}
can be stored in 44 bits for 64 bit general values, or is at most
$2^{32}-2^{29}-1$ for 32 bit general values.

The second fastest scheme is the \key{relative stub address} scheme, where the
stub address relative to a constant \key{stub base}
address is stored.  The stub base is stored in {\tt MIN\_STUB\_BASE}, and
the relation between the absolute and relative stub addresses is:

\begin{center}
stub absolute address = {\tt MIN\_STUB\_BASE} + stub relative address
\end{center}

This scheme can be used if
{\tt MIN\_\EOL MAXIMUM\_\EOL RELATIVE\_\EOL STUB\_\EOL ADDRESS}
can be stored in 44 bits for 64 bit general values, or is at most
$2^{32}-2^{29}-1$ for 32 bit general values.
...
scheme will be efficient.  Again, this scheme can be used if
...
{\tt MIN\_\EOL MAXIMUM\_\EOL RELATIVE\_\EOL STUB\_\EOL ADDRESS/16}
can be stored in 44 bits for 64 bit general values, or is at most
$2^{32}-2^{29}-1$ for 32 bit general values.

Note that on machines that have 32 bit addresses (e.g.,
the IA32\footnote{Intel Architecture 32-bit, a.k.a, i386 machines.}
machines), there is little to gain by the stub index packing scheme,
so that scheme should not be used on such machines.
...
for a definition of auxiliary pointers).  For indexed
...
a 24-bit auxiliary pointer; larger auxiliary pointers are not allowed.  For
indexed auxiliary pointer general values
the datum holds a 20-bit auxiliary pointer
and a 20-bit index.  For index and special values the datum holds a
24-bit index; again larger indices are not allowed.
...
compact or loose (\pagref{COMPACT-OR-LOOSE}) and
...
\begin{center}
\begin{tabular}{ll}
\TTNBKEY{MIN\_MAXIMUM\_ABSOLUTE\_STUB\_ADDRESS}
			& See \pagref{STUB-ADDRESS-PACKING} \\
\TTNBKEY{MIN\_STUB\_BASE} & See \pagref{STUB-ADDRESS-PACKING} \\
\TTNBKEY{MIN\_MAXIMUM\_RELATIVE\_STUB\_ADDRESS}
			& See \pagref{STUB-ADDRESS-PACKING} \\
\end{tabular}
\end{center}
...
\verb|bool min::| & \MINKEY{is\_indexed\_aux}\verb| ( min::gen v )|
\LABEL{MIN::IS_INDEXED_AUX} \\
...
subtype, after checking the subtype with an {\tt MIN\_ASSERT} statement:
...
\verb|unsigned min::|
    & \MINKEY{indexed\_aux\_of}\verb| ( min::gen v )|
\LABEL{MIN::INDEXED_AUX_OF} \\
\verb|unsigned min::|
    & \MINKEY{indexed\_index\_of}\verb| ( min::gen v )|
\LABEL{MIN::INDEXED_INDEX_OF} \\
...
\verb|unsigned min::| & \MINKEY{special\_index\_of}\verb| ( min::gen v )|
...
\verb|min::indexed_aux_of|, the \verb|min::index_of| function, and
the {\tt min::\EOL spec\-ial\_\EOL index\_\EOL of} function all
...
The \verb|min::indexed_aux_of| and \verb|min::indexed_index_of| functions
...
The \verb|min::control_code_of| function returns
...
\verb|unsigned MUP::|
    & \MUPKEY{indexed\_aux\_of}\verb| ( min::gen v )|
\LABEL{MUP::INDEXED_AUX_OF} \\
\verb|unsigned MUP::|
    & \MUPKEY{indexed\_index\_of}\verb| ( min::gen v )|
\LABEL{MUP::INDEXED_INDEX_OF} \\
...
\verb|unsigned MUP::| & \MUPKEY{special\_index\_of}\verb| ( min::gen v )|
...
\verb|min::gen min::|
    & \MINKEY{new\_indexed\_aux\_gen}%
      \verb| ( unsigned p, unsigned i )|
\LABEL{MIN::NEW_INDEXED_AUX_GEN} \\
\verb|min::gen min::| & \MINKEY{new\_index\_gen}\verb| ( unsigned i )|
...
\verb|min::gen min::| & \MINKEY{new\_special\_gen}\verb| ( unsigned i )|
...
\verb|strncmp|).
The {\tt unsigned} arguments used to make auxiliary pointers, indices,
and special values
must fit within 24 bits, except for {\tt min::new\_indexed\_aux\_gen}
for which they must fit within 12 bits when \verb|min::gen| values are
32 bits, and 20 bits when \verb|min::gen| values are 64 bits.
The {\tt unsigned} argument used with {\tt min::new\_control\_code\_gen}
must fit within 24 bits, while the {\tt min::uns64} argument used with
{\tt long\_control\_code\_gen\LOOSE} must fit within 40 bits.
...
are chosen so they are not normally generated by the compiler,
...
\verb|min::gen MUP::|
    & \MUPKEY{new\_direct\_str\_gen}\verb| ( const char * p, unsigned n )|
...
\verb|min::gen MUP::|
    & \MUPKEY{new\_indexed\_aux\_gen}%
      \verb| ( unsigned p, unsigned i )|
\LABEL{MUP::NEW_INDEXED_AUX_GEN} \\
\verb|min::gen MUP::| & \MUPKEY{new\_index\_gen}\verb| ( unsigned i )|
...
\verb|min::gen MUP::| & \MUPKEY{new\_special\_gen}\verb| ( unsigned i )|
\LABEL{MUP::NEW_SPECIAL_GEN} \\
\end{tabular}\end{center}

The following unprotected functions can be used to replace the data
(non-subtype)
part of a \verb|min::gen| value that is \underline{not}
...
in a general value that has one of several subtype codes.

\begin{center}\begin{tabular}{r@{}l}
\verb|min::gen MUP::|
    & \MUPKEY{renew\_gen\COMPACT}\verb| ( min::gen v, min::uns32 p )|
\LABEL{MUP::RENEW_GEN_COMPACT} \\
\verb|min::gen MUP::|
    & \MUPKEY{renew\_gen\LOOSE}\verb| ( min::gen v, min::uns64 p )|
\LABEL{MUP::RENEW_GEN_LOOSE} \\
\end{tabular}\end{center}
...
\verb|const unsigned min::| & \MINKEY{GEN\_INDEXED\_AUX}
\LABEL{MIN::GEN_INDEXED_AUX} \\
...
\verb|min::GEN_ILLEGAL| is actually illegal as a subtype code that may
...
There are a few general value functions that concern atoms.
...
or a label (\secref{LABELS}).
The \verb|min::hash| function returns a non-zero hash value if its argument
...
(\pagref{MIN::DEALLOCATE}) can also happen inside relocating functions,
...
\begin{center}
\begin{tabular}{l}
check that object bodies pointed at have not been deallocated \\
recompute all local variables containing body pointers \\
...
The relocating functions update body pointers that are not stored
in local variables, in particular those stored in stubs.
The local variables are then recomputed from the pointers stored in
the stubs.
...
This check is most efficiently done by the {\tt min::assert\_allocated}
function (\pagref{MIN::ASSERT_ALLOCATED}).
...
code when reloading a possibly relocated body pointer.  Any object
whose size is not greater than the value of the following
macro is guaranteed to be pointed at unimplemented memory when it is
deallocated:
...
This macro is used to optimize the inline function:

\begin{center}\begin{tabular}{r@{}l}
\verb|void min::|
    & \MINKEY{assert\_allocated}\verb| ( min::stub * s, unsigned size )|
\LABEL{MIN::ASSERT_ALLOCATED}
\end{tabular}\end{center}
...
However if {\tt MIN\_DEBUG} is set {\tt min::assert\_allocated} always
executes
...
A stub contains an 8 byte stub value and an 8 byte
...
for additional numbers.  Note that the auxiliary stub itself may not
contain enough information to tell the type of the value it stores;
one may have to trace the reference from the object pointing at the
auxiliary stub to determine this type.  See \secref{STUB-FUNCTIONS}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|int min::| & \MINKEY{type\_of}\verb| ( min::stub * s )|
\LABEL{MIN::TYPE_OF}
\end{tabular}\end{center}
...
A stub contains a 64-bit \key{stub value} whose type depends upon the stub
...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::float64 min::| & \MINKEY{float\_of\COMPACT}\verb| ( min::stub * s )|
\LABEL{MIN::FLOAT_OF} \\
\end{tabular}\end{center}
...
The \verb|min::float_of| function for a 64-bit \verb|min::gen|
...
also applies to number stubs created by {\tt min::new\_num\_gen}.
\end{quote}
...
\verb|char * MUP::| & \MUPKEY{writable\_str\_of}\verb| ( MUP::long_str * str )|
\LABEL{MUP::WRITABLE_STR_OF_LONG_STR} \\
\verb|unsigned min::| & \MINKEY{length\_of}\verb| ( MUP::long_str * str )|
\LABEL{MIN::LENGTH_OF_LONG_STR} \\
\verb|unsigned min::| & \MINKEY{hash\_of}\verb| ( MUP::long_str * str )|
\LABEL{MIN::HASH_OF_LONG_STR} \\
\end{tabular}\end{center}
...
the length of the string, and a 32-bit non-zero \mkey{hash value}{of string}
\label{HASH-VALUE}
...
constant that could be used is $33$.}  A hash value is never zero
(so zero can be used to denote a missing hash value).
...
The following unprotected functions set the length and hash value as stored
in the body:

\begin{center}\begin{tabular}{r@{}l}
\verb|void MUP::| &
   \MUPKEY{set\_length\_of}\verb| ( MUP::long_str * str, unsigned length )|
\LABEL{MUP::SET_LENGTH_OF_LONG_STR} \\
\verb|void MUP::| &
   \MUPKEY{set\_hash\_of}\verb| ( MUP::long_str * str, unsigned hash )|
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{strlen}\verb| ( min::stub * s )|
\LABEL{MIN::STRLEN} \\
\verb|unsigned min::| & \MINKEY{strhash}\verb| ( min::stub * s )|
\LABEL{MIN::STRHASH_OF_STUB} \\
\end{tabular}\end{center}

return the length of the string (excluding the terminating NUL) and
the hash value of the string.

The functions

\begin{center}\begin{tabular}{r@{}l}
\verb|char * min::| & \MINKEY{strcpy}\verb| ( char * p, min::stub * s )|
\LABEL{MIN::STRCPY} \\
\verb|char * min::|
    & \MINKEY{strncpy}\verb| ( char * p, min::stub * s, unsigned n )|
\LABEL{MIN::STRNCPY} \\
\end{tabular}\end{center}
...
\verb|min::strncpy| function copies the \verb|n|'th \verb|char|.  The value
...
be obtained using the following functions to create and manage
a \key{string pointer}:

\begin{center}\begin{tabular}{r@{}l}
\verb|(constructor) MUP::| & \MUPKEY{str\_pointer}\verb| ( min::gen v )|
\LABEL{MUP::STR_POINTER_OF_GEN} \\
\verb|const char * min::| & \MINKEY{str\_of}\verb| ( MUP::str_pointer & sp )|
\LABEL{MIN::STR_OF_STR_POINTER} \\
\verb|void min::|
    & \MINKEY{initialize}\verb| ( MUP::str_pointer & sp, min::gen v )|
\LABEL{MIN::INIT_OF_STR_POINTER} \\
\verb|void min::| & \MINKEY{relocate}\verb| ( MUP::str_pointer & sp )|
\LABEL{MIN::RELOCATE_OF_STR_POINTER} \\
\end{tabular}\end{center}
...
The {\tt min::str\_of} function returns a standard C++ pointer to the string,
taking care to ensure the vector of {\tt char}'s is NUL-terminated.
The {\tt min::initialize} function resets the pointer to point at another
\verb|min::gen| value.
The {\tt min::relocate}
function resets the pointer after the relocated flag has been set,
in case this is necessary.
The value of {\tt min::str\_of} does not change and may become obsolete when
the relocated flag is been set, and the {\tt min::relocate} function
makes the value current.
...
a more efficient way of reading a string than {\tt min::strcpy}.

A typical use of string pointers that indicates how the {\tt min::relocate}
function works is:
...
... some_function ( ... )
{
    ...
    const char * p = min::str_of ( sp );
    . . . use and increment p . . .

    some_relocating_function ( ... );
    if ( relocated )
    {
        // The relocating function does NOT change sp
        // which remains pointing at its former now
        // possibly incorrect place.
        //
        int displacement = p - min::str_of ( sp );

        // Make sp current.
        min::relocate ( sp );

        // Revise p.
        p = min::str_of ( sp ) + displacement;
    }
    . . . . . use p . . . . .
    return ...;
    // relocated deconstructor restores relocated flag.
...
have the property that no two distinct label stubs may have equal vectors of
name components.  
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{lablen}\verb| ( min::stub * s )|
\LABEL{MIN::LABLEN} \\
...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::uns32 min::| & \MINKEY{labhash}\verb| ( min::stub * s )|
\LABEL{MIN::LABHASH} \\
...
The \mkey{header}{of object} contains the sizes of the other 4 parts.
...
\verb|unsigned min::|
    & \MINKEY{hash\_table\_size\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::HASH_TABLE_SIZE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{hash\_table\_size\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::HASH_TABLE_SIZE_OF_LONG_OBJ} \\
\verb|unsigned min::| &
    \MINKEY{attribute\_vector\_size\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::ATTRIBUTE_VECTOR_SIZE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{attribute\_vector\_size\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::ATTRIBUTE_VECTOR_SIZE_OF_LONG_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{aux\_area\_size\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::AUX_AREA_SIZE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{aux\_area\_size\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::AUX_AREA_SIZE_OF_LONG_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{unused\_area\_size\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::UNUSED_AREA_SIZE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{unused\_area\_size\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::UNUSED_AREA_SIZE_OF_LONG_OBJ} \\
...
\verb|min::gen min::| & \MINKEY{new\_obj\_gen\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( unsigned hash_table_size,| \\
    \verb|   unsigned unused_area_size )| \\
    \end{tabular}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::|
	& \MINKEY{hash\_table\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::HASH_TABLE_OF_SHORT_OBJ} \\
\verb|unsigned min::|
	& \MINKEY{hash\_table\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::HASH_TABLE_OF_LONG_OBJ} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::ATTRIBUTE_VECTOR_OF_SHORT_OBJ} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::ATTRIBUTE_VECTOR_OF_LONG_OBJ} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_of}\verb| ( MUP::short_obj * so )|
\LABEL{MIN::AUX_AREA_OF_SHORT_OBJ} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_of}\verb| ( MUP::long_obj * lo )|
\LABEL{MIN::AUX_AREA_OF_LONG_OBJ} \\
\end{tabular}\end{center}
...
\verb|aux_area_of| returns the index of the last value pushed into
the auxiliary area.
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MIN::SHORT_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::SHORT_AUX_AREA_PUSH_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MIN::LONG_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MIN::LONG_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MIN::LONG_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MIN::LONG_AUX_AREA_PUSH_OF_GENS} \\
\end{tabular}\end{center}\label{OBJECT_PUSH_FUNCTIONS}
...
\verb|min::unused_area_size_of| function
(\pagref{MIN::UNUSED_AREA_SIZE_OF_SHORT_OBJ}).
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::SHORT_AUX_AREA_PUSH_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MUP::LONG_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MUP::LONG_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MUP::LONG_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MUP::LONG_AUX_AREA_PUSH_OF_GENS} \\
\end{tabular}\end{center}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MIN::SHORT_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::SHORT_AUX_AREA_POP_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen & value )|
\LABEL{MIN::LONG_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MIN::LONG_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen & value )|
\LABEL{MIN::LONG_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MIN::LONG_AUX_AREA_POP_OF_GENS} \\
\end{tabular}\end{center}\label{OBJECT_POP_FUNCTIONS}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::SHORT_AUX_AREA_POP_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MUP::LONG_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MUP::LONG_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen value )|
\LABEL{MUP::LONG_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop} \\
	& \verb|    ( MUP::long_obj * lo, min::gen * p, unsigned n )|
\LABEL{MUP::LONG_AUX_AREA_POP_OF_GENS} \\
\end{tabular}\end{center}
...
use a {\tt MUP::writ\-able\_\EOL list\_\EOL pointer}.
All the functions
defined for {\tt MUP::list\_\EOL pointer}'s are also defined for
{\tt MUP::writ\-able\_\EOL list\_\EOL pointer}'s:

\begin{center}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{writable\_list\_pointer}\verb| ( min::stub * s )|
\LABEL{MUP::WRITABLE_LIST_POINTER_OF_STUB} \\
(constructor)~\verb|MUP::|
	& \MUPKEY{writable\_list\_pointer}\verb| ( min::gen obj )|
\LABEL{MUP::WRITABLE_LIST_POINTER_OF_GEN} \\
\verb|min::gen min::|
	& \MINKEY{start\_hash}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::writable_list_pointer & lp,| \\
	  \verb|   unsigned index )|
	  \end{tabular}
\LABEL{MIN::START_HASH_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{start\_vector}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::writable_list_pointer & lp,| \\
	  \verb|   unsigned index )|
	  \end{tabular}
\LABEL{MIN::START_VECTOR_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{start\_copy}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::writable_list_pointer & lp,| \\
	  \verb|   const min::writable_list_pointer & lp2 )|
	  \end{tabular}
\LABEL{MIN::START_COPY_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{next}\verb| ( min::writable_list_pointer & lp )|
\LABEL{MIN::NEXT_OF_LIST_POINTER_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{current}\verb| ( min::writable_list_pointer & lp )|
\LABEL{MIN::CURRENT_OF_LIST_POINTER_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{start\_sublist}\verb| ( min::writable_list_pointer & lp )|
\LABEL{MIN::START_SUBLIST_OF_WRITABLE} \\
\end{tabular}\end{center}
...
\verb|min:gen min::|
	& \MINKEY{refresh}%
	  ...
	  \verb| ( min::writable_list_pointer & lp )| \\
	  \end{tabular}
\LABEL{MIN::REFRESH_OF_LIST_POINTER_OF_WRITABLE} \\
\end{tabular}\end{center}
...
All functions
defined for {\tt MUP::writ\-able\_list\_\EOL point\-er}'s are also defined for
...
\verb|void min::|
	& \MINKEY{insert\_reserve\REL}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::insertable_list_pointer & lp,| \\
	  \verb|   unsigned insertions, unsigned elements = 0,| \\
...
then $P_2$ will be invalidated if it points into the same list as $P_1$
or into a descendant sublist of that list.  The effects can be subtle
...
The invalidation of $P_2$ is \underline{not} a detectable error
...
At the \key{attribute level}, the object is a map attribute name/reverse-name
pairs to flags and multi-sets of values.
...
a read function may be too long, and the read function will use only an
initial segment of that name, returning the length of that segment
along with the values and flags associated with that segment.

If partial attribute names are \underline{not} supported
the object lists have the syntax:

\begin{indpar}
\emkey{hash-table-entry} ::= {\em map-list}
\\[1ex]
\emkey{attribute-vector-entry} ::= {\em vector-list}
\\[1ex]
\emkey{map-list} ::= {\em map-pair}$\,^\star$
\\[1ex]
\emkey{vector-list} ::= {\em attribute-descriptor} {\em map-pair}$\,^\star$
\\[1ex]
\emkey{map-pair} ::= {\em attribute-name } {\em attribute-descriptor}
\\[1ex]
...
\emkey{attribute-sublist} ::=
    {\em double-arrow-map-sublist-option} {\em flag-set} {\em value}$\,^\star$
\\[1ex]
\emkey{double-arrow-map-sublist} ::= {\em double-arrow-map-pair}$\,^\star$
\\[1ex]
\emkey{double-arrow-map-pair} ::=
    {\em reverse-attribute-name } {\em value-set-descriptor}
\\[1ex]
\emkey{reverse-attribute-name} ::= {\em atom } $|$ {\em label}
\\[1ex]
\emkey{value-set-descriptor} ::= {\em value} $|$ {\em value-sublist}
\\[1ex]
...
\emkey{map-list} ::= {\em map-pair}$\,^\star$
\\[1ex]
\emkey{map-pair} ::= {\em name-component } {\em node-descriptor}
\\[1ex]
\emkey{name-component} ::= {\em number } $|$ {\em string} $|$ {\em label}
\\[1ex]
...
\emkey{node-sublist}
    ...
    ::= & {\em flag-set}
	  {\em value}$\,^\star$ \\
    $|$ & {\em child-sublist}
	  {\em reverse-child-sublist-option}
	  {\em flag-set}
	  {\em value}$\,^\star$ \\
    \end{tabular}
\\[1ex]
\emkey{child-sublist} ::= {\em map-pair}$\,^\star$
\\[1ex]
\emkey{reverse-child-sublist} ::= {\em map-pair}$\,^\star$
\\[1ex]
\emkey{flag-set} ::= {\em control-code}$\,^\star$
\\[1ex]
\emkey{value} ::= {\em number} $|$ {\em string} $|$ {\em label}
              $|$ {\em object} $|$ {\em index}
\end{indpar}
...
a {\em hash-table-entry} is a {\em map-list} which is a list.
...
{\em node-sublists} and {\em child-sublists} are sublists.

A {\em value} is a list element that is not a sublist,
and is an {\em number}, {\em string},
{\em label}, {\em object} (i.e., a pointer to an object stub), or {\em index}.

Everything else is a sequence of elements in some list or sublist.
...
The object map represents a tree whose root is the object and whose
other nodes are represented by {\em map-pairs}.  Each {\em map-pair}
assigns a {\em name-component} and a {\em node-descriptor} to its
node.  The {\em node-descriptor} assigns to the node a list of {\em values},
a {\em flag-set}, and optionally a {\em child-sublist} that represents
the children of the node, and a {\em reverse-child-sublist}
that also represents a different kind of children of the node.
Both the {\em child-sublist} and {\em reverse-child-sublist},
or just the latter, may be omitted to optimize storage.  If a
{\em node-descriptor} consists of a single {\em value} and nothing else,
it can be replaced by that {\em value}, to optimize storage.

An attribute name is a sequence of {\em name-components} and corresponds
to the node reached from the root by going successively to the children
named by the successive elements of the attribute name.  An element of
the object's hash table is a
{\em hash-table-entry} which is a {\em map-list} whose
elements alternate between {\em name-components} and
{\em node-descriptors}.  Each {\em name-component} and following
{\em node-descriptor} constitute a {\em map-pair} which describes a node
that is a child of the object root.
A {\em node-descriptor} may be a {\em node-sublist}
that specifies {\em flag-sets} and {\em values}, and may also specify
{\em child-sublists} and {\em reverse-child-lists}
that are like {\em map-lists} and describe the children of the node.

If a child of the object root has a {\em name-component} that
is a natural number that is a legal index in the object's attribute
vector, the child is described by giving just its {\em node-descriptor}
in the attribute vector element indexed by the child's {\em name-component}.
Otherwise a {\em map-pair} describing the child is placed in the
{\em hash-table-entry} whose index in the object's hash table equals the
{\em name-component's} hash value modulo the length of the hash table.

An attribute
name/reverse attribute name pair is handled as if the attribute name
were followed by the reverse attribute name, but at the junction of the
two names a switch is made between normal node children represented by
the {\em child-sublist} and reverse attribute name children represented by
the {\em reverse-child-sublist}.  The nodes that are only accessible
using a reverse attribute name have {\em value} lists, {\em flag-sets},
and {\em child-sublists}, but may not have {\em reverse-child-sublists}.

Given the above, the following search algorithm may be used to find the
multi-set of values and set of flags associated with an attribute name or
with an attribute name/reverse name pair.
...
($E$ is a {\em node-descriptor}) and go to step 6.

\item Otherwise let $H$ be the hash value of $C$ modulo the size of
the hash table.  Let $M$ be the hash table entry with index $H$.

\item $M$ is a {\em child-sublist}.  Look through the {\em map-pairs} in this
list for one whose {\em name-component} equals $C$.  If none is found,
the search fails.
If one is found, let $E$ be the {\em node-descriptor} part of the {\em map-pair}.

\item $E$ is a {\em node-descriptor}. If $E$ is a {\em value}, and not a sublist,
...
\item $E$ must be a {\em node-sublist}.
...
Any sublists beginning $E$ are skipped over (these will be {\em child-sublists}
and {\em reverse-child-sublists}).  Any
...
last component of $N$ (which must be the attribute name and there must
be a reverse attribute name) and $E$ does not begin with two sublists,
the search fails.
...
name and there must be a reverse attribute name),
replace $N$ by the reverse attribute name, $C$ by the first component
...
are represented by a {\em flag-set} in an {\em node-sublist}.
...
Note that programming languages may choose to not use {\em flag-sets}
or not use {\em flag-sets} for nodes where both an attribute name
and a reverse attribute name are given, but \verb|min.h|
allows any node to have a {\em flag-set}.
...
and reverse attribute names can be atoms or labels.  A label specifies
the sequence of {\em nam-components} that is the attribute name or reverse
attribute name.  An atom is
treated as if it were a label with the atom as its only element.  The
reverse attribute name can also be the special value {\tt min::NONE},
in which case the count is returned of values with the attribute name
with no reverse attribute name.  Or the reverse attribute name
can be the special value {\tt min::ANY},
in which case the count is returned of values with the attribute name
...
The reverse attribute name can be \verb|min::NONE|
but cannot be \verb|min::ANY|.

The {\tt get\_flagsg} function returns flags in the \verb|min::uns32|
vector vector {\tt out} for a given object, attribute name,
and reverse attribute name.
The argument {\tt n} is the maximum number of vector elements that
...
If the reverse name argument to {\tt setg} is \verb|min::NONE|,
the values deleted or set represent single arrows.

If the reverse name is not
\verb|min::NONE|, the values deleted or set may have reverse attribute
...
The reverse attribute name can be \verb|min::NONE| or \verb|min::ANY|.
...
\begin{center}\begin{tabular}{r@{}l}
\verb|bool min::| & \MINKEY{test\_flagg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( unsigned n, min::gen obj,| \\
    \verb|   min::gen name, min::gen reverse_name )|
    \end{tabular}
...
\minkey{add\_\EOL to\_\EOL multiseti},
\minkey{set\_\EOL some\_\EOL flagsi},
\minkey{clear\_\EOL some\_\EOL flagsi},
\minkey{flip\_\EOL some\_\EOL flagsi},
\minkey{test\_\EOL flagi},
\minkey{set\_\EOL flagi},
\minkey{clear\_\EOL flagi},
and
\minkey{flip\_\EOL flagi}.\label{G_TO_I_VARIANTS}
\end{indpar}
...
if the value \verb|min::NONE| were given to this argument.
...
\verb|unsigned |
    & \TTKEY{aux\_area\_size\_of} \\
    & \verb|    ( min::variable_vector * vv )|
\LABEL{AUX_AREA_SIZE_OF_VARIABLE_VECTOR} \\
\verb|unsigned |
...
\LABEL{MUP::FLOAT_OF} \\
\verb|min::gen MUP::| & \MUPKEY{gen\_of}\verb| ( min::stub * s )|
\LABEL{MUP::GEN_OF} \\
\verb|void * MUP::| & \MUPKEY{pointer\_of}\verb| ( min::stub * s )|
...
\verb|int min::| & \MINKEY{type\_of}\verb| ( min::stub * s )| \\
\verb|bool MUP::|
    & \MUPKEY{test\_flags\_of}\verb| ( min::stub * s, min::uns64 flags )|
...
control values: (ordinary) control values and GC control values.
Ordinary control values
have 48 bits for the address/value field, 8 bits for the type,
and 8 bits for flags.
GC control values have 56 - G bits for a stub address, 8 bits
for the type, and G bits for the GC flags, where G is some value from 8
through 24.  The larger G, the more GC flags, which may permit the
...
GC control values are used with
...
Indicates that the address/value field of an ordinary (non-gc)
...
or inside the type code field if that is used.
...
Ordinary (non-gc) control values can be manipulated by the following functions:

\begin{center}\begin{tabular}{@{}r@{}l@{}}
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_control} \\
    & \verb|    ( int type_code, min::uns64 v, min::uns64 flags = 0 )|
\LABEL{MUP::NEW_CONTROL_OF_VALUE} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_control} \\
    & \verb|    ( int type_code, min::stub * s, min::uns64 flags = 0 )|
\LABEL{MUP::NEW_CONTROL_OF_STUB} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_control\_value}\verb| ( min::uns64 c, min::uns64 v )|
\LABEL{MUP::RENEW_CONTROL_VALUE} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_control\_stub}\verb| ( min::uns64 c, min::stub * s )|
...
GC control values can be manipulated by the following functions:

\begin{center}\begin{tabular}{@{}r@{}l@{}}
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_gc\_control} \\
    & \verb|    ( int type_code, min::stub * s, min::uns64 flags = 0 )|
\LABEL{MUP::NEW_GC_CONTROL_OF_STUB} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_gc\_control\_stub}\verb| ( min::uns64 c, min::stub * s )|
\LABEL{MUP::RENEW_GC_CONTROL_STUB} \\
\verb|min::stub * MUP::|
    & \MUPKEY{stub\_of\_gc\_control}\verb| ( min::uns64 c )|
\LABEL{MUP::STUB_OF_GC_CONTROL} \\
\end{tabular}\end{center}

Either ordinary or GC control values can be
...
\LABEL{MIN_IS_LOOSE}	 \\
\TTNBKEY{MIN\_MAXIMUM\_ABSOLUTE\_STUB\_ADDRESS}
\LABEL{MIN_MAXIMUM_ABSOLUTE_STUB_ADDRESS} \\
\TTNBKEY{MIN\_STUB\_BASE}
\LABEL{MIN_STUB_BASE} \\
\TTNBKEY{MIN\_MAXIMUM\_RELATIVE\_STUB\_ADDRESS}
\LABEL{MIN_MAXIMUM_RELATIVE_STUB_ADDRESS} \\
...
\verb|void min::| & \MINKEY{assert\_allocated}\ARGBREAK
...
\LABEL{MIN::NEW_INDIRECT_PAIR_AUX_GEN} \\
\verb|min::gen min::|
    & \MINKEY{new\_indexed\_aux\_gen}\ARGBREAK
      \verb| ( unsigned p, unsigned i )|
\LABEL{MIN::NEW_INDEXED_AUX_GEN} \\
\verb|min::gen min::| & \MINKEY{new\_index\_gen}\verb| ( unsigned i )|
\LABEL{MIN::NEW_INDEX_GEN} \\
...
\LABEL{MUP::NEW_INDIRECT_PAIR_AUX_GEN} \\
\verb|min::gen MUP::|
    & \MUPKEY{new\_indexed\_aux\_gen}\ARGBREAK
      \verb| ( unsigned p, unsigned i )|
\LABEL{MUP::NEW_INDEXED_AUX_GEN} \\
\verb|min::gen MUP::| & \MUPKEY{new\_index\_gen}\verb| ( unsigned i )|
\LABEL{MUP::NEW_INDEX_GEN} \\
...
\verb|void min::|
    & \MINKEY{initialize}\ARGBREAK
      \verb| ( MUP::str_pointer & sp, min::gen v )|
\LABEL{MIN::INIT_OF_STR_POINTER} \\
\verb|void min::| & \MINKEY{relocate}\verb| ( MUP::str_pointer & sp )|
\LABEL{MIN::RELOCATE_OF_STR_POINTER} \\
\end{TABULAR}
...
\verb|void MUP::| &
   ...
   \verb| ( MUP::long_str * str,|\ARGBREAK\verb|   unsigned length )|
\LABEL{MUP::SET_LENGTH_OF_LONG_STR} \\
\verb|void MUP::| &
   \MUPKEY{set\_hash\_of}\ARGBREAK
    \verb| ( MUP::long_str * str,|\ARGBREAK\verb|   unsigned hash )|
...
\LABEL{MIN::HASH_TABLE_SIZE_OF_LONG_OBJ} \\
\verb|unsigned min::|
    & \MINKEY{attribute\_vector\_size\_of}\ARGBREAK
...
\begin{TABULAR}{1.4in}
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_push} \\
...
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MIN::SHORT_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_push}\ARGBREAK
...
\begin{TABULAR}{1.4in}
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
...
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::SHORT_AUX_AREA_PUSH_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_push}\ARGBREAK
...
\begin{TABULAR}{1.4in}
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{attribute\_vector\_pop} \\
...
\LABEL{MIN::SHORT_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MIN::SHORT_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned min::|
	& \MINKEY{aux\_area\_pop}\ARGBREAK
...
\begin{TABULAR}{1.4in}
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{attribute\_vector\_pop} \\
...
\LABEL{MUP::SHORT_ATTRIBUTE_VECTOR_POP_OF_GENS} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop}\ARGBREAK
	  \verb|    ( MUP::short_obj * so, min::gen & value )|
\LABEL{MUP::SHORT_AUX_AREA_POP_OF_GEN} \\
\verb|unsigned MUP::|
	& \MUPKEY{aux\_area\_pop}\ARGBREAK
...
\verb|min::gen min::|
	& \MINKEY{start\_hash}\ARGBREAK
	  \verb| ( min::writable_list_pointer & lp,|\ARGBREAK
	  \verb|   unsigned index )|
\LABEL{MIN::START_HASH_OF_WRITABLE} \\
\verb|min::gen min::|
	& \MINKEY{start\_vector}\ARGBREAK
...
\begin{TABULAR}{1.2in}
\verb|bool min::| & \MINKEY{test\_flagg}\ARGBREAK
    \verb| ( unsigned n, min::gen obj,|\ARGBREAK
    \verb|   min::gen name, min::gen reverse_name )|
\LABEL{MIN::TEST_FLAGG} \\
...
\begin{tabular}{@{\hspace*{1.0in}}p{5.2in}@{}}
...
\verb|min::set_some_flagsi| & \verb|min::clear_some_flagsi| \\
\verb|min::flip_some_flagsi| \\
\verb|min::test_flagi| & \verb|min::set_flagi| \\
\verb|min::clear_flagi| & \verb|min::flip_flagi| \\
\end{tabular}
...
(implicitly equal to \verb|min::NONE|)
...
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_control} \\
    & \verb|    ( int type_code, min::stub * s,|\ARGBREAK
      \verb|      min::uns64 flags = 0 )|
\LABEL{MUP::NEW_CONTROL_OF_STUB} \\
\verb|min::uns64 MUP::|
...
\LABEL{MUP::NEW_GC_CONTROL_OF_STUB} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_gc\_control\_stub}\ARGBREAK
      \verb| ( min::uns64 c, min::stub * s )|
\LABEL{MUP::RENEW_GC_CONTROL_STUB} \\
\verb|min::stub * MUP::|
    & \MUPKEY{stub\_of\_gc\_control}\verb| ( min::uns64 c )|
