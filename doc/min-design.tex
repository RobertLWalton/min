...
context, which contains a vector of \skey{variable}s that hold the attribute
...
may be when during garbage collection, or when the objects are
published (\pagref{PUBLISH}).
...
\verb|min::gen min::| & \MINKEY{new\_obj\_gen\REL}%
    ...
    \verb|   unsigned hash_size = 0,| \\
    \verb|   unsigned variables_size = 0 )| \\
    \end{tabular}
\LABEL{MIN::NEW_OBJ_GEN} \\
\end{tabular}\end{indpar}

The exact unused area and variables vector sizes are given.
...
A read-only \verb|min::vec_pointer| permits read-only access to the body
vector elements.
A read-write or updatable
{\tt min::\EOL updatable\_\EOL vec\_\EOL pointer}
permits read-write access to the body the body vector elements,
...
A vector level protected object pointer can be either a
`\mkey{master}{object pointer}' or a `\mkey{slave}{object pointer}'.
...
If an object has neither of these flags, a master pointer
to the object may be created, and this will set the \verb|min::OBJ_PRIVATE|
flag.  When the pointer is destructed, this flag will be cleared.
While this flag is set, no other master pointer to the object may be
...
(\pagref{PUBLISH}).
When this is set, any number of read-only and read-write (updatable) master
pointers to the object may be created, but no insertable master pointers
to the object may be created.  Creating pointers in this case does not
set object flags.  The \verb|min::OBJ_PUBLIC| flag may not be cleared by
...
A slave pointer $S$ of any other pointer $P$ may be created as long
as $P$ exists during the lifetime of $S$, subject to the following
restrictions.  If $P$ is read-only, $S$ must be read-only.
If $P$ is read-write, $S$ may not be insertable.  Thus $S$
cannot have greater access to the object than $P$ has.
Note that $P$ need not be a master pointer; it may be a slave of
yet another pointer.  $P$ is called the `\mkey{parent}{of pointer}'
of $S$ and $S$ is called the `\mkey{child}{of pointer}' of $P$.
...
A master read-only \minkey{vec\_\EOL pointer}
to the body vector may be created by the following functions:
...
A slave read-only \minkey{vec\_\EOL pointer}
can be created by the following functions:

\begin{indpar}[0.2in]\begin{tabular}{r@{}l}
\verb|(constructor) min::| & \MINKEY{vec\_pointer}%
       \verb| ( min::vec_pointer & pvp )|
\LABEL{MIN::VEC_POINTER_OF_VEC_POINTER} \\
\verb|(constructor) min::| & \MINKEY{vec\_pointer}%
       \verb| ( min::updatable_vec_pointer & pvp )|
\LABEL{MIN::VEC_POINTER_OF_UPDATABLE_VEC_POINTER} \\
\verb|(constructor) min::| & \MINKEY{vec\_pointer}%
       \verb| ( min::insertable_vec_pointer & pvp )|
\LABEL{MIN::VEC_POINTER_OF_INSERTABLE_VEC_POINTER} \\
\end{tabular}\end{indpar}
...
\verb|void min::|
    & \MINKEY{initialize}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::vec_pointer & cvp,| \\
      \verb|   min::vec_pointer & pvp )| \\
      \end{tabular}
\LABEL{MIN::INIT_OF_VEC_POINTER_OF_VEC_POINTER} \\
\verb|void min::|
    & \MINKEY{initialize}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::vec_pointer & cvp,| \\
      \verb|   min::updatable_vec_pointer & pvp )| \\
      \end{tabular}
\LABEL{MIN::INIT_OF_VEC_POINTER_OF_UPDATABLE_VEC_POINTER} \\
\verb|void min::|
    & \MINKEY{initialize}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::vec_pointer & cvp,| \\
      \verb|   min::insertable_vec_pointer & pvp )| \\
      \end{tabular}
\LABEL{MIN::INIT_OF_VEC_POINTER_OF_INSERTABLE_VEC_POINTER} \\
\end{tabular}\end{indpar}

Here \verb|pvp| is the parent pointer, and \verb|cvp| or
the created pointer is the child or slave pointer.
