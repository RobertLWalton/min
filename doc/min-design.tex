...
Packed structures and vectors and objects with particular
...
\LABEL{MIN::DEFINED_FORMAT_FUNCTION}\ARGBREAK
      \verb|( const min::printer printer,|\ARGBREAK
      \verb|  min::gen v,|\ARGBREAK
      \verb|  const min::gen_format * gen_format,|\ARGBREAK
...
\verb|     min::defined_format_function |
	& \TTMKEY{defined\_format\_function}{in {\tt min::defined\_format\_header}} \\
\verb|}                                   | \\
\end{tabular}\end{indpar}

There is a \key{packed defined format map}
...
about to print a packed structure or vector, it looks up the subtype in
the packed defined format map, and if the value found is a
...
Similarly there is a \key{typed defined format map} of \TT{.type}
...
When \TT{min::standard\_pgen} (\pagref{MIN::STANDARD_PGEN}) is
about to print an object with a \TT{.type} value, it looks up
the \TT{.type} value in
the typed defined format map, and if the value found is a
defined format and not \TT{min::\EOL NULL\_\EOL STUB}, the defined
format function is called to do the printing.  This call passes
...
When the defined format function is called it must act as a substitute
...
\TT{obj\_\EOL format->obj\_\EOL op\_\EOL flags}.
