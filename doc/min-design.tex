...
\ttmkey{rw}{abbreviation}	& read-write (\pagref{RW}) \\
...
This macro can be defined independently for each separately compiled
...
A \key{special general value} is a unique value that has some
special meaning.  For example, \verb|min::MISSING| is a special
general value that means that data is missing. 
...
a 12-bit+12-bit indirect indexed auxiliary pointer\COMPACT \\
a 20-bit+20-bit indirect indexed auxiliary pointer\LOOSE \\
a 24-bit index \\
a 24-bit control code \\
a 40-bit control code\LOOSE \\
a 24-bit special general value \\
\end{tabular}
\end{center}
...
less than $2^{32}-2^{29}$.  There are three
\skey{stub address packing scheme}s,\label{STUB-ADDRESS-PACKING} and the fastest
is selected for each type of general value.
...
using the stub address as the general value.  This scheme can be used if
the absolute address of the stub is not too large.  The
macro \TTKEY{MIN\_\EOL MAXIMUM\_\EOL ABSOLUTE\_\EOL STUB\_\EOL ADDRESS}%
\LABEL{MIN_MAXIMUM_ABSOLUTE_STUB_ADDRESS}
defines the maximum absolute address of any stub as an unsigned integer.
...
address is stored.  The stub base
address is defined as the value of the
\TTKEY{MIN\_STUB\_BASE}\LABEL{MIN_STUB_BASE}
macro, which can be a non-negative constant or the name of a global
variable holding an unsigned integer.  The relative stub address is
just the absolute stub address minus the stub base.  This scheme can be
used if the relative address of the stub is not too large.  The
macro \TTKEY{MIN\_\EOL MAXIMUM\_\EOL RELATIVE\_\EOL STUB\_\EOL ADDRESS}%
\LABEL{MIN_MAXIMUM_RELATIVE_STUB_ADDRESS}
defines the maximum relative address of any stub as an unsigned integer.

The slowest scheme is the \key{stub index} scheme, where the
relative stub address divided by the stub length is stored.
The stub length is choosen to be a power of 2 so that this
...
indirect indexed auxiliary pointer general values
the datum holds a 20-bit auxiliary pointer
and a 20-bit index.  For index and special general values the datum holds a
...
\begin{center}
\begin{tabular}{ll}
\TTKEY{MIN\_MAXIMUM\_ABSOLUTE\_STUB\_ADDRESS}
			& See \pagref{STUB-ADDRESS-PACKING} \\
\TTKEY{MIN\_STUB\_BASE} & See \pagref{STUB-ADDRESS-PACKING} \\
\TTKEY{MIN\_MAXIMUM\_RELATIVE\_STUB\_ADDRESS}
			& See \pagref{STUB-ADDRESS-PACKING} \\
\end{tabular}
\end{center}
...
\verb|unsigned min::|
    & \MINKEY{indirect\_index\_of}\verb| ( min::gen v )|
\LABEL{MIN::INDIRECT_INDEX_OF} \\
...
\verb|min::indirect_aux_of|), \verb|min::index_of|, and
{\tt min::\EOL special\_\EOL index\_\EOL of} functions
...
The \verb|min::indirect_aux_of| and \verb|min::indirect_index_of| functions
...
The \verb|min::special_index_of| function returns
a 24-bit unsigned integer for both 32-bit and 64-bit general values.
...
\verb|unsigned MUP::|
    & \MUPKEY{indirect\_index\_of}\verb| ( min::gen v )|
\LABEL{MUP::INDIRECT_INDEX_OF} \\
...
\verb|min::gen min::|
    & \MINKEY{new\_indirect\_indexed\_aux\_gen}%
...
fit into a direct string.
The {\tt unsigned} arguments used to make auxiliary pointers, indices,
and special general values
must fit within 24 bits, except for {\tt min::new\_indirect\_indexed\_aux\_gen}
...
auxiliary pointer, index, control code, and special general values
...
\verb|min::gen MUP::|
    & \MUPKEY{new\_indirect\_indexed\_aux\_gen}%
      \verb| ( unsigned p, unsigned i )|
...
is not a legal general value.

There are several general value functions that concern atoms.  An \key{atom}
...
The chain pointer is a packed stub address
(see \pagref{STUB-ADDRESS-PACKING})
...
take time proportional to the string length.
...
unlike the protected {\tt min::hash\_of} function that computes any uncomputed
hash value.
...
The {\tt min::str\_of} function returns this pointer, taking care to ensure
the vector of {\tt char}'s is NUL-terminated.  The {\tt min::relocate}
function resets the pointer after the relocated flag has been set,
in case this is necessary.
The value of {\tt min::str\_of} is not updated after the relocated flag has
been set \underline{until} the {\tt min::relocate} function is called on the
string pointer.
...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{new\_obj\_gen}%
    \begin{tabular}[t]{@{}l@{}}
...
\LABEL{MIN::ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|void min::|
	& \MINKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|void min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MIN::AUX_AREA_PUSH_OF_GEN} \\
\verb|void min::|
	& \MINKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MIN::AUX_AREA_PUSH_OF_GENS} \\
...
Both these functions check that the unused area has sufficient space for
the push, and signal a fatal program error if it does not.  The number
...
Two of the above functions push multiple values, as described by a vector
...
\verb|void MUP::|
	...
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::ATTRIBUTE_VECTOR_PUSH_OF_GEN} \\
\verb|void MUP::|
	& \MUPKEY{attribute\_vector\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::ATTRIBUTE_VECTOR_PUSH_OF_GENS} \\
\verb|void MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen value )|
\LABEL{MUP::AUX_AREA_PUSH_OF_GEN} \\
\verb|void MUP::|
	& \MUPKEY{aux\_area\_push} \\
	& \verb|    ( MUP::short_obj * so, min::gen * p, unsigned n )|
\LABEL{MUP::AUX_AREA_PUSH_OF_GENS} \\
\end{tabular}\end{center}
...
\begin{indpar}
{\bf No Superflous {\tt \bf min::LIST\_END}'s}.%
...
{\bf No Superflous List Auxiliary Pointers}.%
...
an implementation may use the free area or space in the object's header
...
one must use a {\tt MUP::rw\_\EOL list\_\EOL pointer}
({\tt rw} stands for `read-write', as opposed to `read-only'),%
\label{RW}
...
\begin{center}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{rw\_list\_pointer}\verb| ( min::stub * s )|
\LABEL{MUP::RW_LIST_POINTER_OF_STUB} \\
(constructor)~\verb|MUP::|
	& \MUPKEY{rw\_list\_pointer}\verb| ( min::gen obj )|
\LABEL{MUP::RW_LIST_POINTER_OF_GEN} \\
\verb|min::gen min::|
	& \MINKEY{start\_hash}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::rw_list_pointer & lp,| \\
	  \verb|   unsigned index )|
	  \end{tabular}
\LABEL{MIN::START_HASH_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{start\_vector}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::rw_list_pointer & lp,| \\
	  \verb|   unsigned index )|
	  \end{tabular}
\LABEL{MIN::START_VECTOR_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{start\_copy}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::rw_list_pointer & lp,| \\
	  \verb|   min::rw_list_pointer & lp2 )|
	  \end{tabular}
\LABEL{MIN::START_COPY_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{next}\verb| ( min::rw_list_pointer & lp )|
\LABEL{MIN::NEXT_OF_LIST_POINTER_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{current}\verb| ( min::rw_list_pointer & lp )|
\LABEL{MIN::CURRENT_OF_LIST_POINTER_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{start\_sublist}\verb| ( min::rw_list_pointer & lp )|
\LABEL{MIN::START_SUBLIST_OF_RW} \\
\end{tabular}\end{center}
...
second call to {\tt min::insert\_reserve\REL}.
...
\verb|min::gen min::|
	& \MINKEY{remove}%
	  \verb| ( min::rw_list_pointer & lp, unsigned n = 1 )|
...
\pagref{NO-SUPERFLUOUS-LIST}, {\tt MUP::rw\_list\_pointer}'s keep
...
by an auxilary value (or equivalent)
...
\begin{center}
\begin{tabular}{l}
\TTKEY{MIN\_USES\_OBJECT\_AUX\_STUBS} \\
...
Thus the control represents the auxiliary area element immediately
following the list element represented by the value.
...
Any stub pointer to an auxiliary stub of type \minkey{LIST\_AUX} is treated
as a list auxiliary pointer.  Such stub pointers may be stored in
auxiliary area \verb|min::gen| values or in auxiliary stub controls.

Any stub pointer to an auxiliary stub of type \minkey{SUBLIST\_AUX} is treated
as a sublist auxiliary pointer.  Such stub pointers may be stored in
...
If the control of an auxiliary stub holds a stub pointer, that pointer
must point at an auxiliary stub $S$ of type \verb|min::LIST_AUX| and the
...
An auxiliary stub \verb|min::gen| value that points at
...
an object auxiliary stub (in which case $S$ must be of type
...
\minkey{UNDEFINED} that indicates the node is not yet defined, and might
...
can be \skey{index general value}s that hold indeces of variables
...
given the initial value \minkey{UNDEFINED},
...
\subsection{Special General Values}
\label{SPECIAL-GENERAL-VALUES}
...
The special general values defined by MIN are:
...
Special general values each have a unique \ikey{index}{of special general value}
that identifies them relative to other special general values.  These
...
system.  Other special general values can be created by other systems,
and indices near 0 are reserved for non-system users.

For efficiency reasons it is desirable for special general values to be
...
general values.  This macro is

\begin{center}\begin{tabular}{r@{}l}
\verb|(macro) min::gen |
	& \TTKEY{MIN\_NEW\_SPECIAL\_GEN}\verb| ( i )|
\LABEL{MIN_NEW_SPECIAL_GEN} \\
\end{tabular}\end{center}

where \verb|i| is the index of the special general value to be
...
\LABEL{MIN::NEW_INDIRECT_PAIR_AUX_GEN} \\
\verb|min::gen min::|
    & \MINKEY{new\_indirect\_indexed\_aux\_gen}\ARGBREAK
      \verb| ( unsigned p, unsigned i )|
\LABEL{MIN::NEW_INDIRECT_INDEXED_AUX_GEN} \\
\verb|min::gen min::| & \MINKEY{new\_index\_gen}\verb| ( unsigned i )|
\LABEL{MIN::NEW_INDEX_GEN} \\
...
\LABEL{MUP::NEW_INDIRECT_PAIR_AUX_GEN} \\
\verb|min::gen MUP::|
    & \MUPKEY{new\_indirect\_indexed\_aux\_gen}\ARGBREAK
      \verb| ( unsigned p, unsigned i )|
\LABEL{MUP::NEW_INDIRECT_INDEXED_AUX_GEN} \\
\verb|min::gen MUP::| & \MUPKEY{new\_index\_gen}\verb| ( unsigned i )|
\LABEL{MUP::NEW_INDEX_GEN} \\
...
\verb|min::gen min::|
	& \MINKEY{start\_hash}\ARGBREAK
	  \verb| ( min::rw_list_pointer & lp,|\ARGBREAK
	  \verb|   unsigned index )|
\LABEL{MIN::START_HASH_OF_RW} \\
\verb|min::gen min::|
	& \MINKEY{start\_vector}\ARGBREAK
...
\INDEXHEADER{Special General Values}
