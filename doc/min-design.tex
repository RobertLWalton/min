...
\newcommand{\ttbkey}[1]{\TT{[#1]}\index{#1@{\tt [#1]}}}
...
\newcommand{\minmkey}[3]%
           {\TT{min::#1}\ttmindex{min::#2}{#3}\ttmindex{#2}{#3}}
...
`\ttmkey{MUP}{abbreviates \TT{min::unprotected}}\TT{::}'\label{MUP}
...
The {\tt min::}\MINKEY{gen}\LABEL{MIN::GEN}
type is defined as a C++ class that
consists of a single \TT{private} element of type
{\tt min::}\MINKEY{unsgen}, which is an unsigned integer.
...
and {\tt min::}\EOL \MINKEY{unsgen} is \TT{typedef}'ed to the appropriate one
of these two types.
...
if {\tt MIN\_IS\_COMPACT} = 1:	& $2^{28} - 2^{25}$ \\
...
\TTKEY{MIN\_STUB\_BASE}
	& The value of the `stub base' for relative addressing
	  ...
	  \TT{0} if {\tt MIN\_MAX\_NUMBER\_OF\_STUBS} is set by
...
\multicolumn{2}{l}{\TTKEY{MIN\_MAX\_RELATIVE\_STUB\_ADDRESS}} \\
	...
	& ~~{\tt 16*MIN\_MAX\_NUMBER\_OF\_STUBS - 1}.
...
\LABEL{MIN::REF_STUB} \\
\verb|const min::unsptr r| & \TTDMKEY{offset}{in {\tt min::ref<T>}}
...
`\TT{r->\ldots}', `\TT{operator~->}' of a \TT{min::\EOL ref<T>}
...
written using \TT{min::\EOL ref<T>} values.

The protected \TT{min::new\_ref} function may be used to construct
a \TT{min::\EOL ref<T>} value from a non-relocatable location,
...
\TT{min::\EOL ref<T>} value references \TT{min::\EOL ZERO\_\EOL STUB},
...
Another difference is the if a \TT{min::\EOL ref<T>} value \TT{r}
...
A \TT{min::\EOL ptr<T>} value can also be created by the
unprotected \TT{MUP::\EOL new\_\EOL ptr} function which
constructs a \TT{min::\EOL ptr<T>} value
...
A \TT{min::\EOL ptr<T>} value can be created by the
...
\begin{indpar}
\TTKEY{MIN\_STACKCOPY} \verb|( T, name, length, source )|
\LABEL{MIN_STACKCOPY}
\\[1ex]
expands to \\[1ex]
\verb|T name [length];| \\
...
In order to be locatable by the garbage collector, \TT{min::gen}
values in static or stack memory must be stored in locations of type
{\tt min::}\MINKEY{locatable\_\EOL gen}, or, if they a \underline{known}
with certainty to be numbers, in locations of type
{\tt min::}\MINKEY{locatable\_\EOL num\_\EOL gen}:
...
\verb|min::ref<min::gen> |
    & \TTOMKEY{min::ref<min::gen>}{min::ref<min::gen>}%
              {of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( const min::locatable_gen & var )|
...
In particular the \TT{min::\EOL packed\_\EOL \ldots ptr<\ldots>}
pointers
...
\verb|(constructor) min::|
    & \MINKEY{locatable\_var<T>}\verb| var ( void )|
\LABEL{MIN::LOCATABLE_VAR} \\
\verb|(constructor) min::|
    & \MINKEY{locatable\_var<T>} \verb| var ( T value )|
...
Here \TT{T} must be
`\TT{const min::stub~*}'
or one of the \TT{min::\EOL packed\_\ldots ptr<\ldots>} classes
whose only data member is of type \TT{const min::stub~*}.
In either case, a \TT{min::\EOL locatable\_\EOL var<T>}
...
\TT{min::\EOL locatable\_\EOL var<T>}, so expressions of
the forms \TT{\ldots=var},
\TT{var->\ldots}, \TT{var[\ldots]}, etc will work for a
\TT{min::\EOL locatable\_\EOL var<T>} value \TT{var} if they
...
to a \TT{min::\EOL locatable\_\EOL var<T>} constructor
...
\TT{min::\EOL locatable\_\EOL var<T>}in that it is not
...
To make the above easier to code, the \TT{MIN\_REF} macro
...
This methodology and the \TT{MIN\_\EOL REF} macro are frequently
...
encodes the NUL character as the `overlong' 2-byte string `\TT{0xC0,0x80}'
...
illegal encoding it returns \verb|min|::\MINKEY{UTF8\_\EOL ILLEGAL},%
\label{UTF8_ILLEGAL}
...
\ILLEGALCHAR.
In this case \TT{s} is updated to point just after the illegal encoding.
Applying the \TT{min::uft8\_to\_unicode} function repeatedly
until \TT{s >= ends} and will produce a sequence of UNICODE
characters from any byte string, even one that is not legal UTF-8,
though if the string is not legal some \TT{min::\EOL UTF8\_\EOL ILLEGAL}
...
bounded by \TT{s} and \TT{ends}.

The \TT{min::unicode\_to\_utf8} function will output
7-byte encodings (first byte \TT{0xFE}) for
UNICODE character values \TT{=> 0x80000000}, and will output
the overlong encoding \TT{0xC0},\TT{0x80}
...
\TT{s} must point to a byte string buffer with at least 7 bytes.
...
returns the UNICODE class of the UNICODE character \TT{c}.
...
\TT{"},
comma (\TT{,}), or
\TT{/} with a letter, as in
\`A(\TT{`A}),
\'A(\TT{'A}),
\^A(\TT{\^{}A}),
\~A(\TT{\textasciitilde A}),
\"A(\TT{"A}),
\c{C}(\TT{,C}), and
\O(\TT{/O}), with the remaining cases
\AA(\TT{AA}), and
\aa(\TT{aa}) using a compose sequence consisting of just the letter doubled.
...
characters (characters with codes \TT{< 256})
are the characters themselves, e.g., the UNICODE class of
`\TT{*}' is \TT{'*'}, \underline{except}:
\begin{center}
\begin{tabular}{l@{~}l}
\TT{'U'} & is the class of upper case letters; \\
           & \TT{A~\ldots~Z~\`A~\'A~\^A~\~A~\"A~\AA~\AE~\c{C}
	      \ldots~\DH~\ldots~\O~\ldots~\'Y}. \\
\TT{'L'} & is the class of lower case letters; \\
           & \TT{a~\dots~z \TH~\ss~\`a~\'a~\^a~\~a~\aa~\ae~\c{c}
	      \ldots~\dh~\~n \ldots~\o~\ldots~\th~\"y}. \\
\end{tabular}
\end{center}

The UNICODE class of a character with code \TT{>= 256} is determined
...
\end{tabular}
\end{center}
\caption{\TT{min::unicode\_class} values}
...
an immutable \TT{min::\EOL uns64} stub value that holds a NUL padded
...
Short string values are NUL (zero) padded 0 to 8 \TT{char}
...
which holds an arbitrary length NUL terminated \TT{char} string.
...
`\TT{S const *}' value with respect to the \TT{->} and
...
return `\TT{S~*}' and `\TT{S~\&}' values instead of
`\TT{S~const~*}' and `\TT{S~const~\&}' values.
...
`\TT{H const *}'.  This can be used to access the `\TT{length}'
...
\begin{center}
\TT{(\& pvp[0])[i]} is the same as \TT{pvp[i]}
\end{center}
...
The \TT{min::ptr<E const>} pointer will still be valid if the packed vector
body is relocated.
...
\TT{pvp->length}.
The \TT{min::ptr<E const>} values of these functions are implicitly
convertible to `\TT{E const *}' values, and so can be passed to
functions like \TT{memcpy}.
...
\verb|H * |
	& \TTOMKEY{->}{->}%
	  ...
	  \verb|( const min::packed_vec_updptr<E,H,L> & pvup )|
...
The \TT{->}, \TT{[]}, and \TT{+} operators and
\TT{min::begin\_ptr\_of()} and \TT{min::end\_ptr\_of()} functions are
redefined (i.e., \underline{not}
inherited) so that they
return `\TT{H~*}', `\TT{E~\&},
or `\TT{min::\EOL ptr<E>}' values
instead of `\TT{H~const~*}', `\TT{E~const~\&}', or
`\TT{min::\EOL ptr<E const>}' values.
The \TT{[]} operator\label{PACKED_VEC_[]_REF}
is further redefined if \TT{E} is a locatable type
so that a \TT{min::\EOL ref<E>} value is returned instead of
a `\TT{E~\&}' value.  This does for vector elements that are
not structures what
\TT{MIN\_REF} defined \TT{\ldots\_ref} functions do for structure
elements.
...
If the element type \TT{E} is locatable, \TT{min::\EOL push\RESIZE}
returns a \TT{min::\EOL ref<E>} reference.%
\label{PACKED_VEC_PUSH_REF}
Otherwise, if \TT{E} is not locatable, an
`\TT{E \&}' reference is returned.

Note that \TT{\& min::push(pvip)}
is an \underline{unprotected} body pointer if \TT{E} is not
locatable.
Very importantly, this means that \TT{v} \underline{must not}
be a relocating expression.  Since \TT{v} does not have locatable type,
this should not normally be an issue, but if \TT{v} is
...
It can either be a non-body pointer or a \TT{min:\EOL ptr<const E>} pointer.
In either case \TT{min::\EOL acc\_\EOL write\_\EOL update} 
...
modified by using the overlong 2-byte encoding of NUL, so that strings
may both contain the NUL character (in the overlong encoding)
and be NUL terminated.}
and NUL terminated.
A non-NUL-terminated, modified UTF-8
encoded, incomplete
...
NUL characters into files.  Line terminating
NUL characters should be loaded by the \TT{min::end\_line} function.
...
buffer using \TT{min::\EOL push} and \TT{min::\EOL end\_\EOL line}
...
line-terminating NUL character in the \TT{buffer}.  Or \TT{0} if there
are no line-terminating NUL characters.
This is \TT{0} when the buffer is empty.
Initialized to \TT{0} when the file is created and set appropriately by
\TT{min::\EOL init\_\EOL input\ldots} functions.

\item[\ttmkey{end\_\EOL count}{in {\tt min::file}}]
The number of line-terminating NUL characters that have ever been
...
Incremented by \TT{min::\EOL end\_\EOL line}.
...
character of line number \TT{n} (or of the line-terminating NUL
...
Truncated when a file is rewound.
...
\TT{print\_\EOL format.flags} (see \pagref{PRINT_FORMAT_FLAGS}) used to print
...
\begin{center}
\begin{tabular}{l@{~~~~}l}
\TT{min::GRAPHIC\_NSPACE\_FLAG} only
	& {\tiny $\stackrel{\textstyle C~}{~R}$} ~~~ 1 column \\
~~ but no \TT{min::ASCII\_FLAG}
\\[1ex]
\TT{min::GRAPHIC\_NSPACE\_FLAG} & \TT{<CR>} ~~~ 4 columns \\
~~ and \TT{min::ASCII\_FLAG}
\\[1ex]
\TT{min::ALLOW\_NSPACE\_FLAG}   & the carriage return \\
~~ but no \TT{min::GRAPHIC\_NSPACE\_FLAG} & character itself, \\
					   & 0 columns
\\[1ex]
no \TT{min::ALLOW\_NSPACE\_FLAG}   & omitted from output, \\
~~ and no \TT{min::GRAPHIC\_NSPACE\_FLAG}  & 0 columns
\end{tabular}
\end{center}

The \TT{min::print\_line} function and the other functions that use
it print file lines with the printer \TT{print\_\EOL format.flags}
set as in this \TT{print\_\EOL flags} file member, but with
\TT{min::\EOL HBREAK\_\EOL FLAG} and \TT{min::\EOL GBREAK\_\EOL FLAG}
off (see \TT{print\_\EOL format.flags} on \pagref{PRINT_FORMAT_FLAGS}).
...
Note that \TT{print\_\EOL flags} is \underline{not} used
...
a file member.
For example, \TT{min::\EOL init\_\EOL print\_\EOL flags} just
creates the file if necessary and sets \TT{print\_\EOL flags},
...
All these functions take arguments that initialize \TT{print\_\EOL flags}
...
\TT{buffer} and updates \TT{end\_\EOL offset} to equal the
new buffer length.
...
to mark the file as being complete.  After doing this, you should
...
the file \TT{buffer}, but instead empties the \TT{buffer}
and sets the file \TT{istream} member
which causes \TT{min::\EOL next\_\EOL line} to load lines into the
file from the \TT{istream} as new lines are required.  Similarly
the \TT{min::\EOL init\_\EOL input\_\EOL file} function sets
\TT{ifile} which causes \TT{min::\EOL next\_\EOL line} to load lines
into the file from the \TT{ifile} as new lines are required.
...
\TT{file\_\EOL line} is set to the number of lines in the file.
In both cases, \TT{file\_\EOL name} is set to \TT{min::\EOL MISSING()}.
See \TT{min::\EOL next\_\EOL line} below for more details.

Initializing a file rewinds it.  Then calls to
\TT{min::\EOL next\_\EOL line} sequence through the file
lines:
...
the end of \TT{buffer}, until a linefeed, NUL, or end of file
is read.  Both linefeeds and NUL's invoke \TT{min::\EOL end\_\EOL line}
...
interaction with the use of \TT{min::\EOL end\_\EOL line} to
...
and possibly use of the \TT{min::\EOL flush\_\EOL spool} described below.
...
For other values of \TT{spool\_\EOL lines},
...
\TT{line\_\EOL number} are deleted.  If the \TT{line\_\EOL number}
...
returned line.  \TT{min::rewind(n)} only succeeds if
if \TT{n == 0} (the default),
or if \TT{n == next\_\EOL line\_\EOL number}
(in this case the rewind function does nothing),
or if \TT{min::line(n)} would \underline{not} return
\TT{min::\EOL NO\_\EOL LINE}.
It is a \underline{programming} error
for \TT{min::\EOL rewind} to fail.  Normally rewind with \TT{n > 0} is only
used with files whose \TT{spool\_\EOL length == min::\EOL ALL\_\EOL LINES},
which eliminates the possibility of failure.

If the last line in \TT{buffer} is a partial line,
...
\TT{offset} point where the NUL would be written.
...
\TT{min::\EOL next\_\EOL line}).
Lastly, if the \TT{copy\_\EOL completion} argument is \TT{true} and
the file has an \TT{ofile}, the \TT{min::\EOL complete\_\EOL file}
...
or \TT{min::\EOL skip\_\EOL remaining}.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{GRAPHIC\_HSPACE\_FLAG}
\LABEL{MIN::GRAPHIC_HSPACE_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{GRAPHIC\_VSPACE\_FLAG}
\LABEL{MIN::GRAPHIC_VSPACE_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{GRAPHIC\_NSPACE\_FLAG}
\LABEL{MIN::GRAPHIC_NSPACE_FLAG} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{ALLOW\_HSPACE\_FLAG}
\LABEL{MIN::ALLOW_HSPACE_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{ALLOW\_VSPACE\_FLAG}
\LABEL{MIN::ALLOW_VSPACE_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{ALLOW\_NSPACE\_FLAG}
\LABEL{MIN::ALLOW_NSPACE_FLAG} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{ASCII\_FLAG}
\LABEL{MIN::ASCII_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{DISPLAY\_EOL\_FLAG}
\LABEL{MIN::DISPLAY_EOL_FLAG} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{HBREAK\_FLAG}
\LABEL{MIN::HBREAK_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{GBREAK\_FLAG}
\LABEL{MIN::GBREAK_FLAG} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{EOL\_FLUSH\_FLAG}
\LABEL{MIN::EOL_FLUSH_FLAG} \\
\verb|const min::uns32 min::| & \MINKEY{FLUSH\_ID\_MAP\_FLAG}
\LABEL{MIN::FLUSH_ID_MAP_FLAG} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{GRAPHIC\_FLAGS}
...
\item[\ttmkey{ASCII\_FLAG}{in printer {\tt print\_\EOL format.flags}}]~\\
...
\item[\ttmkey{ALLOW\_HSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]
\item[\ttmkey{ALLOW\_VSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
\item[\ttmkey{ALLOW\_NSPACE\_FLAG}%
               {in printer {\tt print\_\EOL format.flags}}]%
\vspace{-1ex}
~\\
...
When a \ttkey{NUL} is copied into a printer file buffer, which can only happen
...
This function updates `\TT{column}'
by applying the previous \TT{pwidth} function successively to the
UNICODE characters encoded by the \TT{n} byte string \TT{s}.
...
prints the {\tt min::gen} value \TT{v} to the
{\tt printer} using
\begin{center}
\begin{tabular}{ll}
...
\begin{tabular}{@{}p{6.2in}@{}}
\TTKEY{MIN\_STACKCOPY} \verb|( type, name, length, source )|
\LABEL{MIN_STACKCOPY}
\end{tabular}
