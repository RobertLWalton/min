...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::gen MUP::|
    & \MUPKEY{new\_gen\COMPACT}\verb| ( min::gen v, min::uns32 p )|
\LABEL{MUP::NEW_GEN_COMPACT} \\
\verb|min::gen MUP::|
    & \MUPKEY{new\_gen\LOOSE}\verb| ( min::gen v, min::uns64 p )|
\LABEL{MUP::NEW_GEN_LOOSE} \\
\end{tabular}\end{center}
...
\verb|int min::| & \MINKEY{type\_of}\verb| ( min::stub * s )| \\
\verb|bool MUP::|
    & \MUPKEY{test\_flags\_of}\verb| ( min::stub * s, unsigned flags )|
...
\verb|bool MUP::|
    & \MUPKEY{set\_flags\_of}\verb| ( min::stub * s, unsigned flags )|
\LABEL{MUP::SET_FLAGS_OF} \\
\verb|bool MUP::|
    & \MUPKEY{clear\_flags\_of}\verb| ( min::stub * s, unsigned flags )|
...
functions deal only with parts.  The \verb|MUP::test_flags_of| function
...
It is required that the type of a stub \underline{not} be changed from
a collectible to an uncollectible type or vice versa
by the \MUPkey{set\_type\_of} or \MUPkey{set\_control\_of} functions.

A stub control holds an 8 bit type code, $M$ bits of flags, and $N$ bits
of value or pointer, where $8\leq M\leq 24$ and $32\leq N\leq 48$.
$N$ is the number of bits required to store a pointer, and is
determined by the hardware being used.
The following functions can be used to compute or dissect 64 bit stub
control values:
...
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_control\_flags}\verb| ( min::uns64 c, unsigned flags )|
\LABEL{MUP::RENEW_CONTROL_FLAGS} \\
\verb|min::uns64 MUP::|
...
\verb|bool MUP::|
    & \MUPKEY{test\_control\_flags}\verb| ( min::uns64 c, unsigned flags )|
\LABEL{MUP::TEST_CONTROL_FLAGS} \\
...
the range from $-128$ through $127$, and the $M$-bit flags are represented
as an {\tt unsigned} integer in the range from $0$ through $2^{24}-1$.
The `{\tt new}' functions compute a control value that can be set
by {\tt MUP::set\_control\_of}, the `{\tt renew}' functions modify control
values read by {\tt MUP::control\_of}
by inserting a new type, new flags, or a new value
or pointer, the `{\tt test}' function returns true if and only if the AND
of the control value flags and the {\tt flags} argument is non-zero,
and the other functions return the parts of a control value
read by {\tt MUP::control\_of}.
...
\verb|min::gen MUP::|
    & \MUPKEY{new\_long\_control\_code\_gen\LOOSE}\verb| ( min::uns64 c )|
\LABEL{MUP::NEW_LONG_CONTROL_CODE_GEN} \\[1ex]
...
\LABEL{MIN::STR_OF_STR_POINTER} \\
\verb|void min::| & \MINKEY{relocate}\verb| ( MUP::str_pointer & sp )|
\LABEL{MIN::RELOCATE_OF_STR_POINTER} \\[1ex]
...
\begin{tabular}{@{}p{1.6in}@{}p{4.6in}@{}}
\verb|min::float64 MUP::| & \MUPKEY{float\_of}\ARGBREAK
		\verb| ( min::stub * s )|
\LABEL{MUP::FLOAT_OF} \\
\verb|void MUP::|
