...
The {\tt min::update\_\EOL refresh} function must be called if and element
of the object has been updated by applying the
{\tt min::\EOL update} or {\tt min::\EOL remove} functions
to \underline{another} pointer.
The {\tt min::\EOL insert\_\EOL refresh} function must be called if the object
may have been resized by the insertion of an element into some part of
the object by use of \underline{another} insertable list pointer, or
more specifically, if the {\tt min::\EOL insert\_\EOL reserve} function was
called for another pointer and that function call returned \verb|true|.
The {\tt min::\EOL insert\_\EOL refresh} function also adjusts the pointer
as the {\tt min::\EOL update\_\EOL refresh} function would, so there is
no need to call both functions.
Both functions return the current element.
In some situations in which another insertable list pointer has
been used to insert or remove an element,
calling a {\tt min::\EOL \ldots\_\EOL refresh} function
is not adequate and the list pointer must be
...
{\tt min::\EOL update\_\EOL refresh~(~lp2~)} or
{\tt min::\EOL insert\_\EOL refresh~(~lp2~)}
has been executed.
...
{\tt min::\EOL insert\_after} is in error.  The list pointer continues to point
