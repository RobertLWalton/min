...
a 26-bit auxiliary list pointer (32 bit general value only) \\
a 40-bit auxiliary list pointer (64 bit general value only) \\
a 26-bit auxiliary sublist pointer (32 bit general value only) \\
...
6-bit subtype code
and a low order 26-bit auxiliary pointer (see~\pagref{AUXILIARY-POINTER}
...
The actual direct value, stub pointer, auxiliary pointer, and control code
...
\secref{AUXILIARY-STUBS} for functions associated with auxiliary stubs.
...
\verb|min::float64 min::| & \MINKEY{num\_of}\verb| ( min::gen v )|
\LABEL{MIN::NUM_OF_GEN} \\[1ex]
...
section must be modified as explained in \secref{AUXILIARY-STUBS}.
...
\subsection{Auxiliary Stubs}
\label{AUXILIARY-STUBS}
