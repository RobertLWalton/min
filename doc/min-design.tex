...
The {\tt min::\EOL attr\_\EOL info} structs for the first {\tt n}
attribute names in the list of such are returned in the `{\tt out}'
vector, and the total number of attribute names in the list
is returned as the function call value (regardless of the value of {\tt n}).
If {\tt n==0} the function just returns
the number of attribute names in the list.
...
\begin{center}
\tt template
    \BRACKETED{class T, const min::raw\_vec\_type\_info \& I}
...
A character string containing for each part of an element
...
\ttmkey{g}{in raw vector {\tt type\_info} format}
	& Part is \verb|min::gen| value. \\
\ttmkey{s}{in raw vector {\tt type\_info} format}
	& Part is \verb|min::stub *| value. \\
...
element of the vector.  Note this is read-only, and
that $0\leq$\verb|i|$<$\verb|min::length_of(rvp)|,
...
The other functions above are the same for insertable pointers as they
are for updatable or read-only pointers.
...
with \verb|0| length and with a maximum length defined by the
\verb|type_info| structure \verb|I| described at the end of this
section.  The \verb|new_gen| function with a single \verb|max_length|
...
Here {\tt max\_\EOL increment} and {\tt increment\_\EOL ratio}
