...
The high order 8 bits of the stub control contains the stub type code.
If the high order bit of the type code (which is also the high order
bit of the stub control) is off, the stub is said to be
...
If an object has a body, its stub value is a pointer at that body.
Any pointer into an object body is called a `body pointer'.
...
Otherwise the stub is attached to an object and the type of the stub
...
values are general values.  See \secref{UNPROTECTED-STUB-FUNCTIONS}
for functions that can read and write auxiliary stubs.

\subsubsection{Body Pointers}
\label{BODY-POINTERS}
...
\subsubsubsection{Protected Body Pointers}
...
\subsubsubsection{Unprotected Body Pointers}
...
\subsubsection{Allocator/Collector/Compactor Interface}
\label{ACC-INTERFACE}
...
or write stubs.  These functions do not check type codes of stubs.

These functions are only useful for debugging or for constructing protected
functions.
...
\verb|int min::| & \MINKEY{type\_of}\verb| ( const min::stub * s )| \\
\verb|bool MUP::|
    & \MUPKEY{test\_flags\_of}\verb| ( const min::stub * s, min::uns64 flags )|
...
collector (ACC).  The ACC has to maintain a list of all allocated stubs,
which it does (normally) using the chain pointers in the stub.  For
the case of strings, the ACC maintains this list as a hash table, which
...
\INDEXHEADER{Unprotected Stub Functions}
...
\INDEXHEADER{Control Value Functions (Unprotected)}
