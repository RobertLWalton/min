...
Note also that deallocation of an object body is treated as
...
are C/C++ pointers that point directly into an object body.
...
min::stub * xstub = min::stub_of ( x );
// The following only works if the string is longer
// than 8 characters.
...
value, and long which stores more than 8 characters in the object body.  The
above code only works for long strings.
...
a caller who is using unprotected body pointers must check the relocated flag,
and if on, the caller must:

\begin{center}
\begin{tabular}{l}
optionally check that object bodies pointed at have not been deallocated
(see \secref{DEALLOCATION}) \\
...
A function that computes an unprotected pointer into an object body does not
...
access any part of a deallocated object body beyond the specified limit
