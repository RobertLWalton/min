...
about the character is available.  In
\TT{min::\EOL standard\_\EOL char\_\EOL flags} all characters defined
by the UNICODE standard (or at least the most recent version incorporated
into MIN) are supported.  But the support control part of a print format
...
(useful is the printer does not support them).
...
So the assignment of these next three flags is:
...
So the assignment of these next two flags is:
...
is typically used to reset the flags of a character not supported by
...
to mark graphic characters which when followed immediately by a line break
...
The next two flags are used by the
\TT{min::\EOL standard\_\EOL str\_\EOL classifier}
(\itemref{STRING-CLASSIFIERS}) to identify strings that
need to be quoted because they contain separator
...
A string classifier is used to identify character strings that
must be quoted, that are leading or trailing separators,
and that are marks used as types.
