...
machines), there is nothing to gain by either the relative stub address
...
\ttkey{include/min\_parameters.h}\ttindex{min\_parameters.h}.
...
\LABEL{MIN_MAX_NUMBER_OF_STUBS} \\[1ex]
...
\LABEL{MIN_MAX_RELATIVE_STUB_ADDRESS} \\[1ex]
...
\subsubsection{Protected Body Pointer Management}
\label{PROTECTED-BODY-POINTER-MANAGEMENT}

The protected way uses special pointer data that is adapted to
...
is re-read for every different iteration of the `\verb|for|'
...
\subsubsection{Unprotected Body Pointer Management}
\label{UNPROTECTED-BODY-POINTER-MANAGEMENT}

Functions that obtain body pointers from stubs are unprotected because
the body pointers they return are invalidated if the body pointed as
it relocated.  In order to track possible relocation,
names of functions that might relocate bodies are marked
with the superscript {\REL} in this document.

Relocation can only happen inside such relocating functions
(\pagref{RELOCATING-FUNCTIONS}).  When relocation happens,
...
If a stub is uncollectable, its stub control can be organized in different
ways according to the type code value.  The standard way of organizing
an uncollectable stub control is:
