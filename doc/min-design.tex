...
pointer points at a variable in a variable vector.  Such a variable
may point at an object, and attribute values of the object may be
indices that effectively point at other variables in the variable vector.
...
its index as a vector element.  The usual convention is for the variable with
index {\tt 0} of a vector to point at the root object of the
cluster of objects associated with the vector.
It is an undefined operation to access a variable pointer that
has never been pointed at a variable by a {\tt point} function call.
...
points at $X$ and the value of $X$ is
an indirect pointer to another variable $Y$
as its value, the {\tt get} proceeds as if the variable pointer pointed
at $Y$.

The {\tt set} function set the value of the variable pointed at,
after following all indirections.  That is, if the variable pointer
points at $X$ and the value of $X$ is an indirect pointer to
another variable $Y$ as its value,
the {\tt set} proceeds as if the variable pointer pointed
at $Y$.
