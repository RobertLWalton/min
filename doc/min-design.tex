...
\\[1ex]
\emkey{vector-list} ::=
    {\em attribute-descriptor} {\em attribute-name-descriptor-pair}$\,^\star$
\\[1ex]
...
a single {\em value}, a common case, it may be replaced by this single
{\em value}, in the interests of optimizing memory.

A {\em vector-list} is just like a {\em hash-list} but omits the first
element of the list, which by implication equals the vector index of the
...
attribute vector index, of it the {\em attribute-name}
begins with such an integer, then the attribute's
{\em attribute-name-descriptor-pair}
is put in the {\em vector-list} of the attribute vector element
indexed by the integer.
Otherwise the {\em attribute-name-descriptor-pair} is put
...
An {\em X-pair} whose second element is an empty {\em attribute-sublist}
or {\em value-sublist} can be added or removed without changing
the apparent contents
of the object at the attribute level.  There is one exception: if
a {\em vector-list} contains a {\em attribute-name-descriptor-pair}
the initial {\em attribute-descriptor} in the {\em vector-list} cannot
be omitted, even if it is an empty sublist.
...
such names, the functions replace them with their sole element, which is
an {\em atom}.
...
node.  The {\em node-descriptor} also contains a {\em child-sublist}
...
children of the object root node.  A {\em vector-attribute-entry}
...
{\em node-descriptor} is stored in the object's attribute vector.
Otherwise the nodes's {\em attribute-name-component} and
{\em node-descriptor} are placed in the {\em hash-table-entry}
...
A node whose {\em node-descriptor} is an empty list can be added or removed
without changing the apparent contents
of the object at the attribute level.
...
to a label whose sole component is the atom.
...
The \skey{attribute locator function}s\index{locator function!attribute} that
...
\verb|min::gen min::|
	& \MINKEY{locateg}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::attribute_pointer & ap, min::gen name )|
	  \end{tabular}
...
An attribute pointer can be thought of as storing an attribute name
...
reverse attribute name.  The reverse attribute name designates the
value set pointed at.  If it is {\tt min::NONE}, these are the values
not associated with any particular reverse attribute name.  If it is
{\tt min::ANY}, the value set is the set of \underline{all}
values associated with reverse attribute names.  Otherwise the value
set pointed at is that associated with the particular reverse attribute
name stored in the pointer.

The {\tt locateg} function sets the attribute name of the pointer.
If the `{\tt length}' argument is \underline{not} given, {\tt locateg}
saves the entire attribute name.  If the `{\tt length}' argument
is given, this function saves the longest initial segment of the
...
The {\tt reverse\_locateg} function sets the reverse attribute name of
a pointer.  This can take the special value {\tt min::NONE} or
{\tt min::ANY}, as noted above.  A call to {\tt locateg} sets the
reverse attribute name to {\tt min::NONE}.

If the object is reorganized, the attribute pointers of the object
will be obsoleted, and must be be reset by calling either the
{\tt locateg} and perhaps {\tt reverse\_locateg} functions, or by
calling the {\tt relocate} function.
...
The {\tt countg} function returns the number of values in the value
set pointed at.
The {\tt getg} function gets these values and stores them
in the {\tt out} vector.  The number of values
stored is the {\tt getg} return value.  The argument {\tt n} is the
...
\verb|min::uns32| vector required to store all the flags in the flag
set pointed at.  It returns {\tt 0} if no flags are set.
The {\tt get\_flagsg} function returns these flags in the \verb|min::uns32|
...
The above functions can read but not write attribute values and flags.
To write these you need to use a {\tt writable\_attribute\_pointer}
and the following locator and access functions:
...
\verb|min::gen min::|
	& \MINKEY{locateg}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::writable_attribute_pointer & ap,| \\
	  \verb|   min::gen name )|
	  \end{tabular}
...
The {\tt setg} function sets all the values in the value set the
...
reverse attribute names, the values set or deleted represent double arrows.
In this case, let the object being pointed at by the object pointer be $O_1$,
the pointer attribute name be $N$, and the reverse
attribute name associated with a double arrow value be $R$.
...
The {\tt set\_flagsg} function sets the flags of the flag set the attribute
pointer points at.
...
are added.  The reverse attribute name can be \verb|min::NONE|,
but can only be \verb|min::ANY| if {\tt v} is \verb|min::NONE|.
...
All the above functions in this section use complete attribute names
and work regardless of the setting of 
{\tt MIN\_ALLOW\_PARTIAL\_ATTRIBUTE\_LABELS}%
(\pagref{MIN_ALLOW_PARTIAL_ATTRIBUTE_LABELS}).  The following
variangs of those functions above that get but do not set information
use partial attribute names and are defined only if
{\tt MIN\_ALLOW\_PARTIAL\_ATTRIBUTE\_LABELS} is set to \verb|1|:

\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{countg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( unsigned & length, min::gen obj,| \\
    \verb|   min::gen name, min::gen reverse_name )|
    \end{tabular}
\LABEL{MIN::COUNTG_PARTIAL} \\
\verb|unsigned min::| & \MINKEY{getg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, unsigned n,| \\
    \verb|   unsigned & length, min::gen obj,| \\
    \verb|   min::gen name, min::gen reverse_name )|
    \end{tabular}
\LABEL{MIN::GETG_PARTIAL} \\
\verb|unsigned min::| & \MINKEY{count\_flagsg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb|   unsigned & length, min::gen obj,| \\
    \verb|   min::gen name )|
    \end{tabular}
\LABEL{MIN::COUNT_FLAGSG_PARTIAL} \\
\verb|unsigned min::| & \MINKEY{get\_flagsg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::uns32 * out, unsigned n,| \\
    \verb|   unsigned & length, min::gen obj,| \\
    \verb|   min::gen name )|
    \end{tabular}
\LABEL{MIN::GET_FLAGSG_PARTIAL} \\
\end{tabular}\end{center}

There functions use the longest initial segment of the attribute name
that locates an attribute which has either a non-empty set of values,
or at least one set flag, or at least one value associated with the
attribute name initial segment and any reverse attribute name.  The
length of this initial segment is returned in the `\verb|length|'
argument.  If \verb|0| is returned in `\verb|length|',
no initial segment meeting the requirements was found.
