...
An \key{argument pointer} is a pair of integers that is used by a general value
stored inside the body of an object called a \key{type object}
to point after the manner of an indirect pointer to an element of another
object called an \key{argument object}.
See~\pagref{ARGUMENT-POINTER}.
...
a 24-bit argument pointer (32 bit general value only) \\
a 40-bit argument pointer (64 bit general value only) \\
...
\verb|bool min::| & \MINKEY{is\_argument}\verb| ( min::gen v )|
\LABEL{MIN::IS_ARGUMENT} \\
...
\verb|unsigned min::| & \MINKEY{argument\_of}\verb| ( min::gen v )|
\LABEL{MIN::ARGUMENT_OF} \\
...
The \verb|argument_of| function only returns
...
\verb|unsigned MUP::| & \MUPKEY{argument\_of}\verb| ( min::gen v )|
\LABEL{MUP::ARGUMENT_OF} \\
...
\verb|min::gen min::| & \MINKEY{new\_argument\_gen}\verb| ( unsigned a )|
\LABEL{MIN::NEW_ARGUMENT_GEN} \\
...
The {\tt unsigned} argument used to make either argument pointers
...
\verb|min::gen MUP::| & \MUPKEY{new\_argument\_gen}\verb| ( unsigned a )|
\LABEL{MUP::NEW_ARGUMENT_GEN} \\
...
\verb|const unsigned min::| & \MINKEY{GEN\_ARGUMENT}
\LABEL{MIN::GEN_ARGUMENT} \\
...
an {\em argument} is a \verb|min::gen| argument value,
and a {\em typed-object} is a \verb|min::gen| value pointing at a
typed-object stub.
All these are list elements but not lists.

Note that {\em argument} and {\em typed-object}
values are not treated in any special way
at the object attribute level.  They can be used as values just
like {\em atoms} or {\em objects}.
...
Type and argument objects.\label{ARGUMENT-POINTER}
