...
\TT{min::find\_\EOL or\_\EOL add} on \TT{g} \underline{after}
performing \TT{min::map} on all the objects that are elements or
attribute values of \TT{g} that do not already have identifiers and are
also not arguments to the recursive calls to \TT{min::map}.
It also treats strings of length greater than \TT{strlen} as
objects and gives them identifiers.  The effect is to assign
identifiers so that if an object references another object or a string,
the referenced object or string has a smaller identifier
than the referencing object, unless there are
...
\ttmindex{display\_control}{in {\tt min::str\_format}}\ARGBREAK
    \verb|  min::uns32            id_map_if_longer;|%
...
equivalent of \TT{min::\EOL TYPED\_\EOL OBJECT}s
as pointers in place of the equivalent of \TT{min::\EOL gen}
...
method the context is given the \TT{min::\EOL OBJ\_\EOL TYPED} flag
and its first variable points at the type.  In the second method
the typed object is represented by a stub of \TT{min::\EOL TYPED\_\EOL OBJECT}
...
just a single index \TT{min::gen} value that holds the index of a
variable vector element.  This always refers to a variable in the context.
The second is an indirect \TT{min::gen} value that points at a pair
of values in an auxiliary area: the first being an index \TT{min::gen}
value and the second a pointer-to-object-stub \TT{min::gen} value.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{ENABLE\_MAPPED\_ID}
\LABEL{MIN::ENABLE_MAPPED_ID_FLAG} \\
...
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK
\verb|+ min::ENABLE_COMPACT|\ARGBREAK
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK
\verb|+ min::EMBEDDED_LINE|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK
\verb|+ min::ISOLATED_LINE|\ARGBREAK
\end{tabular}
...
the object already has an {\em object-id} in \TT{printer->\EOL id\_\EOL map}
and the \ttkey{ENABLE\_\EOL MAPPED\_\EOL ID} object format operation flag is on.
Objects may be given {\em object-ids} by {\TT min::map}: see
\pagref{MIN::MAP_OF_ID_MAP}.
...
Otherwise the object is printed as the ID `\TT{@}{\em <object-id>}' by
\TT{min::print\_id} (\pagref{MIN::PRINT_ID})
if the \ttkey{PRINT\_\EOL ID} object format operation flag is on.
The object is assigned an {\em object-id} in \TT{printer->\EOL id\_\EOL map}
if it does not already have one.

Otherwise the
object is printed in `\mkey{isolated line format}{for printing object}'
if the \ttkey{ISOLATED\_\EOL LINE} object format operation flag.
...
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK[0in]
\verb|+ min::ENABLE_COMPACT|\ARGBREAK[0in]
\end{TABULAR}
...
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK[0in]
\verb|+ min::EMBEDDED_LINE|\ARGBREAK[0in]
...
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|  min::ENABLE_MAPPED_ID                  // obj_op_flags|\ARGBREAK[0in]
\verb|+ min::ISOLATED_LINE|\ARGBREAK[0in]
