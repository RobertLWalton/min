...
An \key{object} is conceptually a hash table that maps attribute names to
attribute values.
...
arrow in the data base.  Flags, called \key{attribute flags},
can be attached to this arrow.  If the arrow is a double arrow,
a reverse direction label, called the \key{reverse attribute name}
is attached to the arrow, and flags for the reverse direction,
called \key{reverse attribute flags} may be attached.
...
labels are small unsigned integers.  The \key{auxilary area}
...
the hash table, attribute vector,
...
\verb|const min::gen min::| & \MINKEY{EMPTY\_LIST} \LABEL{MIN::EMPTY_LIST}
	& Denotes a list element that is itself an empty sublist.  This actually
	  equals a sublist pointer with zero index.
\end{tabular}\end{center}

Each hash table or attribute vector element is a \key{list head}.
Each list head is a \key{list continuation}.

If a list continuation is the value \verb|min::LIST_END|, the list has
no more elements, and may be empty if it has no previous elements.
If a list continuation is a continuation pointer,
that pointer points at another list continuation that continues the list.
Otherwise the list continuation is a \key{list element} which represents an
element of the list.
...
that is an auxilary sublist pointer or the value \verb|min::EMPTY_LIST|
...
it points at a continuation element of the sublist.  If it is the
value \verb|min::EMPTY_LIST|, it represents an empty sublist.
...
{\tt start} function, which takes as an argument the index of a list
continuation in the object body vector.  This index may point into the
hash table, attribute vector, or auxilary area.  The {\tt start} function
...
is that of a list continuation, but the index returned is that of
a list element or {\tt 0}.

The {\tt next} function moves the list pointer to the next list element of the
list the pointer points at, and returns the body vector index of that
list element.  It returns {\tt 0} if there is no list element because the end
...
it is the index of a list element and not of a list continuation that is
not a list element.  The {\tt next} function knows when the list element
pointed at by a list pointer is a list head by checking to see if the
index of the element lies within the hash table or attribute vector of
an object.

The {\tt current} function just returns the body vector index of the list
element the list pointer currently points at, or returns {\tt 0} if there is no
such element because the pointer is at the end of a list.
...
to set a list pointer to point at the first list element of the sublist.
The index given
the {\tt start\_sublist} function must index a body vector element
whose value is a sublist pointer or \verb|min::EMPTY_LIST|.
The {\tt start\_sublist} function returns the body vector index of the
first list element of the sublist, or returns {\tt 0} if the sublist is
empty.
