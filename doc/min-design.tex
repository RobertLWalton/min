...
(\pagref{PRINT_FORMAT_SUPPORT_CONTROL})
...
The \minkey{CONDITIONAL\_\EOL BREAK} flag is used in
...
\begin{center}
\begin{tabular}{l@{~~~~~}l}
\ttkey{min::CONDITIONAL\_BREAK}: & characters in categories \TT{Pc} and \TT{Pd}
...
the \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
string classifier (\itemref{STRING-CLASSIFIERS})
...
The \minkey{IS\_SEPARATOR} flag is used by the standard string classifier
to identify characters that cannot be combined with other characters
in unquoted strings; e.g. \TT{(}, \TT{)}, and \TT{|}.
The \minkey{IS\_REPEATER} flag is used by the standard string classifier
to identify characters that may be repeated when they appear in unquoted
separators; e.g. \TT{|}.
The \minkey{NEEDS\_QUOTES} flag is used by the standard string classifier
to identify graphic characters that require any string containing them to be
quoted; e.g., \TT{"}.
See the \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
string classifier (\itemref{STRING-CLASSIFIERS}) for details.
...
as the type of a parenthesized list: see the 
...
See \pagref{SUPPORT-CONTROL} for details.
...
or trailing, and if the string is a mark that can be used as a type.
...
no flags (0 returned)	& The string is used just for formatting and
			  ...
			  For the standard string classifier no flags are
			  returned if the string is empty or contains only
			  characters which have the \TT{IS\_\EOL VHSPACE}
			  flag.
...
\TT{min::NEEDS\_QUOTES}	& The string needs to be quoted.
			  ...
			  For the standard string classifier this is
			  returned if the string contains any character
			  that does not have the \TT{IS\_\EOL GRAPHIC}
			  or \TT{IS\_\EOL VHSPACE}
			  flags.  It is also returned if the string contains
			  any character with the \TT{NEEDS\_\EOL QUOTES}
			  flag, or if any of the following cases apply:
...
\TT{min::IS\_GRAPHIC}	& Unless the \TT{NEEDS\_QUOTES} flag is present,
			  ...
			  For the standard string classifier this is
			  returned for all strings that contain only
			  graphic characters unless the \TT{NEEDS\_\EOL QUOTES}
			  flag is being returned as per above.
...
\TT{min::IS\_LEADING}	& The string is a leading separator.
			  ...
			  For the standard string classifier this is
			  returned for strings whose first character has
			  the \TT{IS\_\EOL LEADING} flag and
			  \TT{NEEDS\_\EOL QUOTES} is not being returned
			  as per above.
...
\TT{min::IS\_TRAILING}	& The string is a trailing separator.
			  ...
			  For the standard string classifier this is
			  returned for strings whose last character has
			  the \TT{IS\_\EOL TRAILING} flag and
			  \TT{NEEDS\_\EOL QUOTES} is not being returned
			  as per above.
...
\TT{min::IS\_MARK}	& The string is a mark, and as such can be
			  ...
			  For the standard string classifier this is
			  returned for strings all of whose characters
			  have the \TT{IS\_MARK} flag, provided
			  \TT{NEEDS\_\EOL QUOTES} is not being returned
			  as per above.
\label{IS_MARK} \\
\end{tabular}\end{indpar}
...
\TT{min::\EOL NEEDS\_\EOL QUOTES} otherwise.
...
\multicolumn{2}{l}{\tt const min::display\_control
	...
	\verb|{ min::IS_VHSPACE + min::IS_GRAPHI, 0 };|
