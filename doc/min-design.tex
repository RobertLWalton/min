...
The C type of a packed structure
must be a C \verb|struct|
...
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG} \verb|pstype|\ARGBREAK
	  ...
	  \verb|   const min::uns32 * stub_ptr_disp = NULL,|\ARGBREAK
	  \verb|   const min::packed_struct_id &|\ARGBREAK
	  \verb|         base_class_id =|\ARGBREAK
	  \verb|         * (min::packed_struct_ID *) NULL )|
...
\verb|const min::packed_struct_id * const |
	& \verb|pstype.|\TTMKEY{id}
	                       {in {\tt min::packed\_struct}}
\LABEL{PACKED_STRUCT_ID} \\
\verb|min::packed_struct_id |
	& \MINKEY{packed\_struct\SARG}\verb|::id|
\LABEL{PACKED_STRUCT_::ID} \\
\verb|const min::packed_struct_id * |
	& \MINKEY{packed\_struct\SARG}\verb|::id.base|
\LABEL{PACKED_STRUCT_::ID.BASE} \\
\end{tabular}\end{indpar}
...
\item[\ttmkey{id}{in {\tt min::packed\_struct}}]
For each structure type \verb|S|,
{\tt min::\EOL packed\_\EOL struct\SARG::id} is a static member of
the class {\tt min::\EOL packed\_\EOL struct\SARG} whose \underline{address}
uniquely identifies the structure type \verb|S|.
The value of {\tt pstype.id} is this address,
and therefore equals `{\tt \&~min::\EOL packed\_\EOL struct\SARG::id}'.

This unique identifier of \verb|S| is used to verify that a conversion
of a `{\tt const min::\EOL stub~*}' value to a
{\tt min::\EOL packed\_\EOL struct\_\EOL ...ptr\SARG} value is legal.

If \verb|S| has a base class \verb|T|, then
{\tt min::\EOL packed\_\EOL struct\BRACKETED{T}::id} must be given
as the last argument to the constructor of \verb|pstype|.  That is,
if
\begin{indpar}\begin{verbatim}
struct S : public struct T { ... }
\end{verbatim}\end{indpar}
then
\begin{indpar}\begin{verbatim}
min::packed_struct<S> pstype
    ( ..., ..., ..., min::packed_struct<T>::id );
\end{verbatim}\end{indpar}

This must be done in order to make a conversion of a
{\tt min::\EOL packed\_\EOL struct\_\EOL ...ptr\SARG}
to a
{\tt min::\EOL packed\_\EOL struct\_\EOL ...ptr\BRACKETED{T}} value legal.

In this case {\tt min::\EOL packed\_\EOL struct\SARG::id.base} equals
`{\tt \&~min::\EOL packed\_\EOL struct\BRACKETED{T}::id}'.
...
has just the require header members: \verb|type|, \verb|length|, and
...
static min::packed_vec<pvh,pve> pvtype
    ( "pvtype", NULL, NULL,
