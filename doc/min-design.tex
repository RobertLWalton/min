...
\verb|min::ref<T> const & |
    & \TTOMKEY{=}{=}{of {\tt min::ref<T>}}\ARGBREAK
      \verb|( min::ref<T> const & r, V const & value )| \\
...
\TT{min::packed\_\ldots{}xxxptr<\ldots>} values and \TT{const min::stub~*}
values must be locatable.
...
\verb|(constructor) min::| & \MINKEY{locatable\_gen}
    \verb|var ( min::locatable_gen const & var )|
\LABEL{MIN::LOCATABLE_GEN_OF_VAR} \\
\verb|(constructor) min::| & \MINKEY{locatable\_gen}
    \verb|var ( V const & value )| \\
    & where \TT{V} values are convertable to \TT{min::gen} values
\LABEL{MIN::LOCATABLE_GEN_OF_VALUE} \\
\end{tabular}\end{indpar}
...
\verb|min::locatable_gen & |
    & \TTOMKEY{=}{=}{of {\tt min::locatable\_gen}}\ARGBREAK
      ...
      \verb|  min::locatable_gen const & var2 )|
...
\verb|min::gen |
    & \TTOMKEY{min::gen}{min::gen}%
      ...
      \verb|( min::locatable_gen const & var )|
\LABEL{MIN::LOCATABLE_GEN_TO_GEN} \\
\end{tabular}\end{indpar}

\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|min::ref<const min::gen> |
    & \TTOMKEY{min::ref<min::gen>}{min::ref<min::gen>}%
              {of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( min::locatable_gen const & var )|
\LABEL{MIN::CONST_LOCATABLE_GEN_TO_REF_GEN} \\
\verb|min::ref<min::gen> |
    & \TTOMKEY{min::ref<min::gen>}{min::ref<min::gen>}%
              {of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( min::locatable_gen & var )|
\LABEL{MIN::LOCATABLE_GEN_TO_REF_GEN} \\
\verb|min::ptr<const min::gen> |
    & \TTOMKEY{\&}{\&}%
              {of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( min::locatable_gen const & var )|
\LABEL{MIN::AMPERSAND_CONST_LOCATABLE_GEN} \\
...
Setting a \TT{min::\EOL locatable\_\EOL gen} location to a value \TT{v}
that is convertable to a \TT{min::\EOL gen} value writes the
converted value to the location.
A \TT{min::\EOL locatable\_\EOL gen} locatable variable
is implicitly convertible to a \TT{min::\EOL gen} type, which is
implemented by simply reading the value of the variable.
...
will be \TT{min::\EOL ref<const min::gen>}, which does not allow
...
will be \TT{min::\EOL ptr<const min::gen>}, which does not allow
the variable to be written.

Some \TT{min::locatable\_gen} examples are:
...
\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|(constructor) min::| & \MINKEY{locatable\_var<T>}
    \verb| var ( void )|
\LABEL{MIN::LOCATABLE_VAR} \\
\verb|(constructor) min::| & \MINKEY{locatable\_var<T>}
    \verb|var ( min::locatable_var<T> const & var )|
...
\verb|T |
    & \TTOMKEY{=}{=}{of {\tt min::locatable\_var}}\ARGBREAK
      \verb|( min::locatable_var<T> & location,|\ARGBREAK
      \verb|  min::locatable_var<T> const & p )|
\LABEL{MIN::=LOCATABLE_VAR_OF_LOCATABLE_VAR} \\
\end{TABULAR}
