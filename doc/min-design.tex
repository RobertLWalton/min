...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{ENABLE\_ID}
\LABEL{MIN::ENABLE_ID_FLAG} \\
...
and the \ttkey{ENABLE\_\EOL ID} object format operation flag is on.
Objects may be given {\em object-ids} by the \ttkey{PRINT\_\EOL ID}
object format operation flag or the {\TT min::map} function: see
...
Otherwise, if the \ttkey{PRINT\_\EOL ID} object format operation flag is on.
the object is printed by \TT{min::print\_id} (\pagref{MIN::PRINT_ID})
which maps the object using
the {\TT min::find\_\EOL or\_\EOL add} function
(see \pagref{MIN::FIND_OR_ADD_OF_ID_MAP})
and prints it as the ID `\TT{@}{\em <object-id>}'.
