...
and \verb|min::NONE| is a special value used only internally to
...
There are a few general value functions that concern atoms.  An \key{atom}
is an immutable value that has a hash value, so that an atom can be
used as a hash table index.

\begin{center}\begin{tabular}{r@{}l}
\verb|bool min::| & \MINKEY{is\_atom}\verb| ( min::gen v )|
\LABEL{MIN::IS_ATOM} \\
\verb|min::uns32 min::| & \MINKEY{hash}\verb| ( min::gen v )|
\LABEL{MIN::HASH} \\
\end{tabular}\end{center}

The \verb|min::is_atom| function returns true if and only if its argument is
an atom, which is defined as either
a number (\secref{NUMBERS}),
a string (\secref{STRINGS}),
or label (\secref{LABELS}).
The \verb|min::hash| function returns a non-zero hash value if its argument
is an atom, and returns zero otherwise.
...
is distinct from the element itself, even though both have the same
...
general value is an atom.  The hash of a label is computed from
the hash of each of its atoms using the following
...
atoms are numbers and strings of fewer than 10 characters,
the hash of a label is equivalent to the hash of the
string made by concatenating its elements, where each
...
They return the index of the last attribute vector element
remaining after the pop, or the first auxiliary element remaining
after the pop.  If there are no elements remaining after the pop,
the index returned
is the index of the first body element before the attribute vector
or the index of the virtual body element after the auxiliary area.
...
The index is usually the hash value of an atom (number, string, or label),
...
A {\tt MUP::list\_pointer} cannot be used to modify a list.  Instead,
one must use a {\tt MUP::writ\-able\_\EOL list\_\EOL pointer}
for which all the functions
defined for {\tt MUP::list\_\EOL pointer}'s are also defined:
...
\verb|min::gen min::|
	& \MINKEY{remove}%
	  \verb| ( min::writable_list_pointer & lp, unsigned n = 1 )|
...
pointer is pointed at the end of the list.
Element removal requires no reservation.

If there are two list pointers, $P_1$ and $P_2$, pointing at list
elements in the same object, and $P_1$
is used to insert or remove elements from the list is points at,
...
\pagref{NO-SUPERFLUOUS-LIST}, {\tt MUP::writable\_list\_pointer}'s keep
track of any auxiliary value
(or equivalent as per \secref{OBJECT-AUXILIARY-STUBS})
pointing a the current
element.  Insertions before or after the current element will change
...
The capability of omitting {\em map-sublists} at the beginning of a
{\em segment-sublist} is an optimization.  An omitted
{\em map-sublist} is equivalent
to an empty {\em map-sublist}.  If only one {\em map-sublist} is a
omitted, it must be the {\em reverse-attribute-map-sublist} that is
omitted, to avoid ambiguity.

An object is represented by a map that maps attribute names to flags and
multi-sets of values and maps attribute name/reverse name pairs to multi-sets
of values.
...
\verb|min::gen min::|
	& \MINKEY{remove}\ARGBREAK
	  \verb| ( min::list_pointer & lp, unsigned n = 1 )|
\LABEL{MIN::REMOVE_FROM_LIST} \\
\end{TABULAR}
