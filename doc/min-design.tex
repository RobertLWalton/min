...
\label{LINE-LENGTH}
Nominal maximum line length in columns.  Initialized to {\tt 0}.
...
because a {\tt min::\EOL push\_\EOL break} was executed,
...
Specifically, when
a break is automatically pushed into a printer file buffer, the
...
Used with {\tt min::push\_break} and {\tt min::pop\_break} to set
...
Used with {\tt min::push\_format} and {\tt min::pop\_format} to save and
...
The format parameters of a printer, namely its {\tt format\_\EOL flags}
and {\tt pgen\_\EOL format}, may be saved in the
...
The {\tt break\_stack}\label{BREAK-STACK}
allows the setting of multiple simultaneous break points,
modifies what happens when exceeding the {\tt line\_\EOL length}
triggers a line break, and
is specialized to printing nested lists.
It is assumed that a list header exists preceeded by a list header
break point and followed by a list indentation
point which is used as the indentation inside the list.
The first break needed inside the list uses the list header break
point, but thereafter this break point is never reused.  When and
if it is used, however, it resets {\tt indent} to the list indentation
point.  At the end of the list {\tt indent} is restored to its
pre-list value, and the list break point is forgotten.

This is implemented by a {\tt break\_stack} with
{\tt break\_\EOL entry} elements:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|struct min::| & \MINKEY{break\_entry}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  min::uns32 break_offset;|\ARGBREAK
    \verb|  min::uns32 break_column;|\ARGBREAK
    \verb|  min::uns32 break_indent;|\ARGBREAK
    \verb|  min::uns32 saved_indent;|\ARGBREAK
    \verb|  bool used;|\ARGBREAK
    \verb|};|
\ttmindex{break\_offset}{in {\tt min::break\_entry}}
\ttmindex{break\_column}{in {\tt min::break\_entry}}
\ttmindex{break\_indent}{in {\tt min::break\_entry}}
\ttmindex{saved\_indent}{in {\tt min::break\_entry}}
\ttmindex{used}{in {\tt min::break\_entry}}
\LABEL{MIN::BREAK_ENTRY_STRUCT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|typedef min::|
	& \verb|packed_vec_ptr<min::break_entry>|
	  \verb|    min::|\MINKEY{break\_stack}
\LABEL{MIN::BREAK_STACK} \\
\end{tabular}\end{indpar}

When the {\tt break\_stack} is empty it has no effect on
printing.

The {\tt min::push\_break} operation pushes an entry into
the break stack.  The current printer {\tt break\_\EOL offset},
{\tt break\_\EOL column}, {\tt column}, and {\tt indent}
are copied into the entry, with {\tt column} becoming the entry
{\tt break\_\EOL indent} and {\tt indent} becoming the entry
{\tt saved\_\EOL indent}.  The {\tt used} entry member is set to
{\tt false}.

The {\tt min::pop\_break} operation pops an entry from
the stack and sets the printer {\tt indent} to the value of
the popped entry's {\tt saved\_\EOL indent} member.
