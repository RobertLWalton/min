...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::uns32 min::|
    & \MINKEY{flush\_\EOL line\RESIZE}
	  \begin{argstack}
          \verb| ( min::file file,|\\
	  \verb|   min::uns32 offset )|
	  \end{argstack}
...
that operate on object vector pointers.  These are described in the section
just mentioned, and are merely listed here:
...
[Typed object are not yet implemented and are not discussed outside
this section.]
...
and if necessary {\tt min::\EOL reverse\_\_EOl locate} that positioned
{\tt ap2} to where it was before {\tt ap1} was used to modify the object.

Note that no special functions need to be called when an attribute
