...
A \key{packed structure} is a C `struct' stored in a body.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{init}\verb| ( min::file & file )|
\LABEL{MIN::INIT_FILE} \\
\verb|void min::|
    & \MINKEY{init}
          \verb| ( min::file & file,|\ARGBREAK
	  \verb|   min::uns32 printer_flags )|
\LABEL{MIN::INIT_FILE_WITH_PRINTER_FLAGS} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::|
    & \MINKEY{init\_output\_ostream}\ARGBREAK
          \verb| ( min::file & file,|\ARGBREAK
	  \verb|   std::ostream & ostream,|\ARGBREAK
	  \verb|   min::uns32 spool_lines = 0,|\ARGBREAK
	  \verb|   min::gen filename = min::MISSING )|
\LABEL{MIN::INIT_OUTPUT_OSTREAM_OF_FILE} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::|
    & \MINKEY{init\_input\_istream}\ARGBREAK
          \verb| ( min::file & file,|\ARGBREAK
	  \verb|   std::istream & istream,|\ARGBREAK
	  \verb|   min::uns32 spool_lines = min::ALL_LINES,|\ARGBREAK
	  \verb|   min::gen filename = min::MISSING )|
\LABEL{MIN::INIT_INPUT_ISTREAM_OF_FILE} \\
\end{tabular}\end{indpar}
...
\verb|min::uns32 file| & \TTARMKEY{line}{in {\tt min::file}}
\LABEL{MIN::FILE_LINE} \\
\verb|min::uns32 file| & \TTARMKEY{line\_\EOL offset}{in {\tt min::file}}
\LABEL{MIN::FILE_LINE_OFFSET} \\
\verb|min::uns32 file| & \TTARMKEY{output\_offset}{in {\tt min::file}}
\LABEL{MIN::FILE_OUTPUT_OFFSET} \\
...
line is UTF-8 encoded and NUL terminated.  A non-NUL-terminated partial
last line may appear at the end of the buffer.

Overlong encodings (i.e., encodings with more bytes than necessary)
are considered legal.  Thus {\tt NUL} may be encoded as {\tt 0xC0},{\tt 0x80}
(UTF-8 in which this is the \underline{only} permitted overlong encoding
is called `modified UTF-8').
Encodings of character codes up to $2^{32}-1$ are considered legal.
The 7-byte encoding (first byte {\tt 0xFE})
is allowed so that $2^{32}-1$ can be encoded.

Initialized to the empty vector.
...
\item[\ttmkey{line}{in {\tt min::file}}]
The number of the line currently being assembled at the end of the
{\tt buffer}, or of the next line if 
the buffer ends with a line-terminating {\tt NUL}.  The
first line number is {\tt 0}.  Initialized to {\tt 0}.

\item[\ttmkey{line\_\EOL offset}{in {\tt min::file}}]
If the buffer ends with a line-terminating {\tt NUL}, this
equals {\tt buffer->\EOL length}.  Otherwise it is the offset
in the buffer of the first character of the current line; that
is, this first line character is {\tt buffer[buffer->\EOL line\_\EOL offset]}.
Initialized to {\tt 0}.

\item[\ttmkey{output\_\EOL offset}{in {\tt min::file}}]
The number of buffer elements that have already been
output (e.g., sent the {\tt std::ostream}, read by
{\tt min::\EOL next\_\EOL line}, etc.).  Equivalently, the
offset in the buffer of the first {\tt char} element that has
not yet been output, or {\tt buffer->length} if there are no such characters.
Initialized to {\tt 0}.
...
\begin{center}
{\tt m = line\_index->length - ( line - n )}
\end{center}
If {\tt spool\_lines~==~min::ALL\_LINES}, this last reduces to {\tt m~=~n}.
Otherwise one must take into account that only the lines at the end of
the file are stored in {\tt buffer} and {\tt line\_\EOL index}.

Initialized to {\tt min::NULL\_STUB}.
Created by {\tt min::\EOL init\_\EOL line\_\EOL index}, and
also automatically created if {\tt spool\_\EOL lines} is \underline{neither}
{\tt 0} nor {\tt min::\EOL ALL\_\EOL LINES}.

\item[\ttmkey{spool\_\EOL lines}{in {\tt min::file}}]
The buffer is only required to retain the last {\tt spool\_\EOL lines}
of a file.  If there are more lines, then when new lines are output
from the buffer by {\tt min::\EOL next\_\EOL line} or {\tt min::\EOL flush}
older lines previously output may be removed from the beginning of the buffer.
When a line is deleted from the beginning of the buffer, any
corresponding element is deleted from {\tt line\_\EOL index}.

If {\tt line~<=~spool\_lines}, all file lines input so far are
retained in the buffer.  If {\tt spool\_EOL lines~==~min::\EOL ALL\_\EOL lines}, all lines in the file are retained.
...
\begin{center}
\begin{tabular}{l@{~~~~}l@{~~~~}l}
{\tt min::GRAPHIC\_FLAG} only
	& {\tiny $\stackrel{\textstyle C~}{~R}$} & 1 column \\[1ex]
{\tt min::GRAPHIC\_FLAG} & {\tt <CR>} & 4 columns \\
~~ and {\tt min::ASCII\_FLAG} \\[1ex]
no {\tt min::GRAPHIC\_FLAG}
	& omitted from output	     & 0 columns
\end{tabular}
\end{center}

File lines are to be printed with these
3 flags set as in {\tt print\_\EOL flags} and \underline{without}
any {\tt min::AUTOBREAK\_\EOL FLAG}.
...
Note that it is illegal for {\tt istream != NULL}
and {\tt ifile != NULL\_\EOL STUB}.

\item[\ttmkey{ostream}{in {\tt min::file}}]
If not {\tt NULL}, the {\tt min::\EOL flush} function
outputs complete lines to this {\tt std::\EOL ostream} when it finds
there are complete lines not yet output.

\item[\ttmkey{printer}{in {\tt min::file}}]
If not {\tt min::NULL\_STUB}, the {\tt min::\EOL flush} function
outputs complete lines to this {\tt min::\EOL printer} when it finds
there are complete lines not yet output.  The lines are output
with the printer {\tt min::\EOL ASCII\_\EOL FLAG} and
{\tt min::\EOL GRAPHICS\_\EOL FLAG} turned off, and the line-terminating
NUL is translated into sending {\tt min::eol} to the printer.

\item[\ttmkey{ofile}{in {\tt min::file}}]
If not {\tt min::NULL\_STUB}, the {\tt min::\EOL flush} function
outputs complete lines to this {\tt min::\EOL file} when it finds
there are complete lines not yet output.
The lines are simply appended to the end of the file, and
for each line-terminating NUL,
{\tt min::\EOL line\_\EOL end~(~ofile~)} is called.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::uns32 min::|
    & \MINKEY{flush}\verb| ( min::file & file )|
\LABEL{MIN::FLUSH_FILE} \\
\verb|min::uns32 min::|
    & \MINKEY{flush}\verb| ( min::file & file,|\\&{\tt ~~~~}
                    \verb|   min::uns32 offset )|
\LABEL{MIN::FLUSH_FILE_WITH_OFFSET} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|(constructor) min::| & \MINKEY{pline}\ARGBREAK
    \verb| ( min::file & file,|\ARGBREAK
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 prtr| & \TTARMKEY{length}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_LENGTH} \\
\verb|const min::uns32 prtr| & \TTARMKEY{line}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_LINE} \\
\verb|const min::uns32 prtr| & \TTARMKEY{column}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_COLUMN} \\
\verb|const min::uns32 prtr| & \TTARMKEY{line\_offset}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_LINE_OFFSET} \\
\verb|const min::uns32 prtr| & \TTARMKEY{break\_offset}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_BREAK_OFFSET} \\
\verb|const min::uns32 prtr| & \TTARMKEY{break\_column}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_BREAK_COLUMN} \\
\verb|const min::uns32 prtr|
    & \TTARMKEY{flush\_\EOL offset}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_FLUSH_OFFSET} \\
\end{tabular}\end{indpar}
...
\verb|min::printer_parameters prtr|
    & \TTARMKEY{saved\_\EOL parameters}{in {\tt min::printer}}
\LABEL{MIN::PRINTER_SAVED_PARAMETERS} \\
\verb|std::ostream * prtr| & \TTARMKEY{ostream}{in {\tt min::printer}}
...
The 2-argument form of {\tt min::\EOL init} sets the
{\tt ostream} printer member (see below).
...
\item[\ttmkey{length}{in {\tt min::printer}}]
The number of {\tt char} elements in the printer vector
(as per the printer being a packed vector: see \itemref{PACKED-VECTORS}).
Initialized to {\tt 0}.

\item[\ttmkey{line}{in {\tt min::printer}}]
The number of the line currently being assembled, or of the next line if 
the printer vector ends with a line-terminating {\tt NUL}.  The
first line number is {\tt 0}.  Initialized to {\tt 0}.
...
\item[\ttmkey{line\_\EOL offset}{in {\tt min::printer}}]
If the printer vector ends with a line-terminating {\tt NUL}, this
equals {\tt length}, the vector length.  Otherwise it is the offset
in the vector of the first character of the current line; that
is, for printer {\tt prtr} this first line character is
{\tt prtr[prtr->\EOL line\_\EOL offset]}.  Initialized to {\tt 0}.

\item[\ttmkey{break\_\EOL offset}{in {\tt min::printer}}]
If there is no break point in the current line, this equals
{\tt line\_\EOL offset}.  Otherwise it is the printer vector offset of the
...
\item[\ttmkey{flush\_\EOL offset}{in {\tt min::printer}}]
The number of printer vector elements that have already been
sent to an {\tt std::ostream} or to another printer.  Or the
offset in the vector of the first {\tt char} element that has
not yet been written, or {\tt length} if there are no such characters.
Initialized to {\tt 0}.
...
when a printer is created, and to the value of
{\tt printer\_\EOL saved\_\EOL parameters} when an existing
printer is initialized by {\tt min::\EOL init}.

\item[\ttmkey{saved\_\EOL parameters}{in {\tt min::printer}}]
A copy of the {\tt parameters}.  The end-of-message {\tt min::\EOL eom}
printer operation copies {\tt saved\_\EOL parameters} to {\tt parameters},
thereby restoring the parameters to their `normal' values.
This member may be written.
Initialized to the value of
{\tt min::\EOL default\_\EOL printer\_\EOL parameters} when a printer
is created; not reset by {\tt min::\EOL init}.

\item[\ttmkey{ostream}{in {\tt min::printer}}]
If not {\tt NULL}, designates a {\tt std::\EOL ostream}
to which printer lines are copied by the {\tt flush()} printer operation.
The 2-argument form of {\tt min::\EOL init} sets this printer member.
This member may be written.
Initialized to {\tt NULL} when a printer
is created; not reset by 1-argument {\tt min::\EOL init}.
...
\verb|const min::uns32 min::| & \MINNBKEY{GRAPHIC\_FLAG}
\LABEL{MIN::GRAPHIC_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{DISPLAY\_EOL\_FLAG}
\LABEL{MIN::DISPLAY_EOL_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{AUTOBREAK\_FLAG}
\LABEL{MIN::AUTOBREAK_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{EOL\_FLUSH\_FLAG}
\LABEL{MIN::EOL_FLUSH_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{EOM\_FLUSH\_FLAG}
\LABEL{MIN::EOM_FLUSH_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{KEEP\_FLAG}
\LABEL{MIN::KEEP_FLAG} \\
\end{tabular}\end{indpar}
...
\begin{center}
\verb|prtr->break_column > prtr->parameters.indent|
\end{center}
...
Breaks may be inserted automatically by using the
{\tt min::\EOL AUTOBREAK\_\EOL FLAG}: see below.
...
The following flag bits control printing:
...
\item[\ttnbmkey{GRAPHIC\_FLAG}{{\tt min::printer\_parameters} flag}]
...
\item[\ttnbmkey{AUTOBREAK\_FLAG}{{\tt min::printer\_parameters} flag}]
If this flag is on, a line break is automatically set
just after every single space
character, and both before and after every graphic mode character
representative.  Note that horizontal tabs are equivalent to from
1 to 8 single space characters for this and other purposes.

\item[\ttnbmkey{EOL\_FLUSH\_FLAG}{{\tt min::printer\_parameters} flag}]
If this flag is on, a {\tt min::flush()} printer operation is
executed at the very end of n {\tt min::\EOL eol} printer operation.

\item[\ttnbmkey{EOM\_FLUSH\_FLAG}{{\tt min::printer\_parameters} flag}]
If this flag is on, a {\tt min::flush()} printer operation is
executed at the very end of n {\tt min::\EOL eom} printer operation.
...
Encodings with fewer that the required
number of continuation bytes, and unexpected continuation bytes,
...
When a horizontal tab is copied to a printer vector in non-graphic mode,
it is converted to between 1 and 8 single space characters.

When a {\tt NUL} is copied into a printer vector, it is encoded as
...
{\tt <<} operator to a {\tt min::}\MINKEY{printer\_op}:
...
\verb|min::printer & |
    & \TTOMKEY{<<}{\LT\LT}%
              {of {\tt min::printer}}\ARGBREAK
      \verb| ( min::printer & prtr,|\ARGBREAK
      \verb|   const min::printer_op & op )|
\LABEL{OPERATOR<<_OF_PRINTER_OP} \\
\end{tabular}\end{indpar}

Printer operations behave for printers as {\tt iomanip}
C++ objects behave for {\tt iostreams}.  For example, the
printer analogs of {\tt std::\EOL endl} are:

\begin{indpar}[1em]\begin{tabular}{r@{}l@{~~~~~}l}
\verb|const min::printer_op min::| & \MINKEY{eol} & (end of line)
\LABEL{MIN::EOL} \\
\verb|const min::printer_op min::| & \MINKEY{eom} & (end of message)
\LABEL{MIN::EOM} \\
\end{tabular}\end{indpar}

and the {\tt min::setbreak} printer operation can be used to
explicitly set a break point:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::printer_op min::| & \MINKEY{setbreak}
\LABEL{MIN::SETBREAK} \\
\end{tabular}\end{indpar}

More specifically, {\tt prtr<<min::setbreak} sets a break point after
...
Both {\tt min::eol} and {\tt min::eom} end the current line.  The
{\tt min::\EOL eom} operation then restores the printer
{\tt parameters} from the printer {\tt saved\_\EOL parameters}.
The last thing these operations do is flush the printer using
the {\tt min::\EOL flush()} operation, provided the appropriate
printer {\tt parameters.flag} is on: i.e., the
{\tt min::EOL\_\EOL FLUSH\_\EOL FLAG} for {\tt min::\EOL eol}, and the
{\tt min::EOM\_\EOL FLUSH\_\EOL FLAG} for {\tt min::\EOL eom}.

The flush printer operations copy the printer vector contents
to an {\tt std::ostream}:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::printer_op min::|
...
The 0-argument {\tt min::flush} uses the printer {\tt ostream}
member.  The 1-argument {\tt min::\EOL flush} specifies the {\tt ostream}.
Line terminating {\tt NUL}s
in the printer vector are translated to line feeds, but if the
last line in the printer vector is incomplete, it is output without
any following line feed.  Characters previously flushed from the printer
vector are not included: only characters at or after the printer
\ttmkey{flush\_\EOL offset}{in {\tt min::printer}}
member are copied, and this member is then reset
to the length of the printer vector.
These functions both remove all terminated lines copied to the {\tt ostream}
from the printer vector, unless the {\tt min::\EOL KEEP\_\EOL FLAG}
printer {\tt parameters.flag} is on.
If the {\tt ostream} has
{\tt NULL} address, the {\tt min::flush} functions do not do anything.

The {\tt <<} operator can also be used to copy a printer vector
to either an {\tt ostream} or to another printer vector.
...
If {\tt out} is an {\tt ostream}, `{\tt out<<prtr}' does the
same thing as `{\tt prtr<<min::\EOL flush(out)}', except
that the latter does nothing if `{\tt out}' has {\tt NULL}
address.

If {\tt prtr1} and {\tt prtr2} are printers,
`{\tt prtr1<<prtr2}' copies the not-previously-flushed
characters of {\tt prtr2} to {\tt prtr1}.  Line terminations
are translated to `{\tt prtr1<<min::\EOL eol}'.  Other characters
are copied to {\tt prtr1} as if that printer had its
{\tt min::\EOL ASCII\_\EOL FLAG} and
{\tt min::\EOL GRAPHIC\_\EOL FLAG} both off.
Note that since horizontal tabs input to {\tt prtr2} have
been converted to 1 to 8 single space characters, it is these
single space characters that are copied to {\tt prtr1}, so the
effective tab stops are those of {\tt prtr2} and not {\tt prtr1}.
After copying characters, the
{\tt prtr2->}\ttmkey{\tt flush\_\EOL offset}{in {\tt min::printer}}
member is reset to the length of {\tt prtr2}.

Printer operations can also be used to print values with specified
formats.  The {\tt min::\EOL pgen} printer operation prints
{\tt min::gen} values using a {\tt min::\EOL printer\_\EOL format}:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::printer_op min::|
    & \MINKEY{pgen}\verb| ( min::gen v,|\\&{\tt ~~~~}
      \verb|   const min::printer_format *|\\&{\tt ~~~~}
      \verb|        format = NULL )|
\LABEL{MIN::PGEN} \\
\end{tabular}\end{indpar}

The expression `{\tt prtr<<min::pgen(v,f)}' prints the {\tt min::gen}
value \verb|v| to the printer {\tt prtr} using the
{\tt min::\EOL printer\_\EOL format} value pointed at by {\tt f}.
If {\tt f} is {\tt NULL}, {\tt prtr->\EOL parameters.for\-mat} is used
instead.  If that is also {\tt NULL},
{\tt \&min::default\_\EOL printer\_\EOL format} is used.

Note that `{\tt prtr<<v}' for a {\tt min::gen} value {\tt v} will
...
A single UNICODE character, or a string of UNICODE characters,
can be printed using the following printer operations:
...
printed by using the following printer operations:
...
For example, `{\tt prtr<<min::set\_printer\_flags(min::ASCII\_FLAG)}'
sets the {\tt ASCII\_FLAG} of {\tt prtr->\EOL parameters.flags}.

Particular {\tt parameters->flags} may be set and cleared by the
following

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::printer_op min::| & \MINKEY{ascii}
\LABEL{MIN::ASCII} \\
\verb|const min::printer_op min::| & \MINKEY{utf8}
\LABEL{MIN::UTF8} \\
\verb|const min::printer_op min::| & \MINKEY{graphic}
\LABEL{MIN::GRAPHIC} \\
\verb|const min::printer_op min::| & \MINKEY{nographic}
\LABEL{MIN::NOGRAPHIC} \\
\verb|const min::printer_op min::| & \MINKEY{display\_eol}
\LABEL{MIN::DISPLAY_EOL} \\
\verb|const min::printer_op min::| & \MINKEY{nodisplay\_eol}
\LABEL{MIN::NODISPLAY_EOL} \\
\verb|const min::printer_op min::| & \MINKEY{autobreak}
\LABEL{MIN::AUTOBREAK} \\
\verb|const min::printer_op min::| & \MINKEY{noautobreak}
\LABEL{MIN::NOAUTOBREAK} \\
\verb|const min::printer_op min::| & \MINKEY{eol\_flush}
\LABEL{MIN::EOL_FLUSH} \\
\verb|const min::printer_op min::| & \MINKEY{noeol\_flush}
\LABEL{MIN::NOEOL_FLUSH} \\
\verb|const min::printer_op min::| & \MINKEY{eom\_flush}
\LABEL{MIN::EOM_FLUSH} \\
\verb|const min::printer_op min::| & \MINKEY{noeom\_flush}
\LABEL{MIN::NOEOM_FLUSH} \\
\verb|const min::printer_op min::| & \MINKEY{keep}
\LABEL{MIN::KEEP} \\
\verb|const min::printer_op min::| & \MINKEY{nokeep}
\LABEL{MIN::NOKEEP} \\
\end{tabular}\end{indpar}

For example, `{\tt ptrt<<min::autobreak}' sets the
{\tt prtr->\EOL parameters.flags} {\tt AUTOBREAK\_FLAG}
and `{\tt prtr<<min::noautobreak}' clears this flag.

The {\tt parameters} of a printer may be saved in the
\ttmkey{saved\_\EOL parameters}{in {\tt min::printer}}
member of the printer, and restored from that member.
This may be done explicitly by the following printer
operations:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::printer_op min::|
...
The idea behind {\tt saved\_parameters} is that output consists
of a sequence of messages, such as error messages, and that
flags set producing these messages, such as the
{\tt min::\EOL GRAPHIC\_\EOL FLAG}, may need to be cleared at the
end of the message.  This can be done by setting the normal
printer parameters when the printer is initialized, saving them
with `{\tt prtr<<min::\EOL save\_\EOL printer\_\EOL parameters}',
and then ending messages with `{\tt prtr<<min::eom}', which restores
the parameters to their normal state by copying
{\tt prtr->\EOL saved\_\EOL parameters}
to {\tt prtr->\EOL parameters}.
...
The {\tt prv.charbuf\_\EOL pr\_\EOL stub} function does for
{\tt min::\EOL charbuf}'s what
the {\tt prv.ostream\_\EOL pr\_\EOL stub} function does for
{\tt std::\EOL ostreams}'s: see \pagref{MIN::OSTREAM_PR_STUB}.
...
\verb|min::gen | & \MINKEY{aux}%
    \verb| ( min::obj_vec_ptr & vp, min::unsptr aux_pointer )|
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{attr\_ptr}\verb| ap ( min::obj_vec_ptr & vp )|
\LABEL{MUP::ATTR_PTR_OF_OBJ_VEC_PTR} \\
\verb|void min::|
	& \MINKEY{locate}%
	  \begin{tabular}[t]{@{}l@{}}
...
\INDEXHEADER{ACC Stub Pointer Locator Vectors}

\begin{indpar}[0.2in]

\begin{TABULAR}{1.8in}
\verb|(constructor) min::| & \MINKEY{static\_gen}%
    \verb|<min::unsptr size>|\ARGBREAK
    \verb|    vec ( void )|
\LABEL{MIN::STATIC_GEN} \\
\verb|(constructor) min::| & \MINKEY{stack\_gen}%
    \verb|<min::unsptr size>|\ARGBREAK
