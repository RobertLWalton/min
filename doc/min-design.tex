...
Note: {\tt gen\_format} must \underline{not} be {\tt NULL}.
...
These members control the {\tt min::maskable\_space} operation, which
...
and which is also used to put the space between object attribute vector elements
if {\tt SUPPRESS\_\EOL OBJ\_\EOL SPACE\_\EOL FLAG} is on.
If the flags are not set as indicated,
{\tt min::\EOL maskable\_\EOL space} is not used for printing labels or
objects.

If either {\tt space\_\EOL prefix\_\EOL mask} or
{\tt space\_\EOL postfix\_\EOL mask} is {\tt NULL},
{\tt min::\EOL maskable\_\EOL space} outputs a single space character.
Otherwise it outputs a single space character if and only if
\begin{center}
\tt space\_prefix\_mask[A] \& space\_postfix\_mask[B] == 0
\end{center}
where
\begin{center}
\tt
...
a = {\rm the previous printed UNICODE character} \\
b = {\rm the following printed UNICODE character} \\
\end{tabular}
\end{center}
...
for the UNICODE classes
`{\tt )}', `{\tt ]}', `{\tt \KET}', `{\tt \guillemotright}',
`{\tt ,}', and `{\tt ;}'
