...
vector elements may be any C type whose data can be initialized by setting
their bytes to all zeros and can be copied by \verb|memcpy|.  Both
\verb|min::gen| and \verb|min::stub *| values are permitted, either
by themselves or as members of \verb|structs| or \verb|unions|.
C++ classes are not permitted, as the types must not have constructors
or destructors and must be copyable with
...
More explicitely, to create a new type of packed structure
...
declared.  This type must such that the body begins with
a \verb|min::uns32| value that holds a code effectively
pointing at \verb|pstype| plus some flags.
This beginning value is initialized by \verb|new_gen| and must not
...
be given (should be \verb|NULL|) if there are no \verb|min::gen|
