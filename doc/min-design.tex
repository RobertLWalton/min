...
set pointed at and stores them
in the `{\tt out}' vector.   The total number of values in the value
set is returned (regardless of the value of {\tt n}).
...
function returns the flags in the flag set pointed at and stores
these as control code
values in the \verb|min::gen| vector `{\tt out}'.
...
If there are $J$ flags in the flag set, and $N\geq J*\mbox{\tt VSIZE}$, then
flag $N$ is zero.
