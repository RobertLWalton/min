...
\footnote{There is currently a limit of $2^62$ bytes but currenly
the maximum virtual address space size is $2^48$ bytes.}.
...
growing downward into th unused area.  The variable vector and
hash table sizes, however, are fixed, though they can be changed
by the {\tt min::\EOL resize} function
(\pagref{MIN::RESIZE_UNUSED_AND_VAR_OF_VEC_INSPTR}).
...
\verb|bool min::| & \MINKEY{expand\REL}%
      ...
      \verb|   min::unsptr unused_size )| \\
      \end{tabular}
\LABEL{MIN::EXPAND_UNUSED_OF_VEC_INSPTR} \\
\end{tabular}\end{indpar}
...
the variable vector size, hash table size, or total size, but should support
