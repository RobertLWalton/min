...
\secref{AUXILIARY-STUBS}, though the only use of these functions on
...
and an {\em label} is a \verb|min::gen| value pointing at a label.
...
An object is represented by a map from attribute names to flags and from
attribute names and reverse attribute names to values.
...
a set of flags and a set of values.
...
\item If $E$ is a {\em value} and not a sublist,
...
Any sublists beginning $E$ are skipped over.  Any
...
number of bits in a control code integer.  $B$ is 26 if \verb|min::gen|
