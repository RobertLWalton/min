...
just a single index \TT{min::gen}\label{INDEX} value
(\itemref{GENERAL-VALUES})
in the graph type
that holds the index of a
variable vector element.  This always refers to a variable in the context.
The second is a \TT{min::gen} value pointing at a
stub of \minkey{VAR\_\EOL PTR}\label{MIN::VAR_PTR} type
...
of objects, and the non-root objects in the tree do \underline{not}
have the \minkey{OBJ\_\EOL IS\_\EOL GTYPE} flag, unless they are
roots of graph subtypes.  However the non-root objects must have
the \TT{OBJ\_\EOL PUBLIC} flag.
...
and sets this object's \TT{min::\EOL OBJ\_\EOL IS\_\EOL GTYPE} flag.
...
The argument \TT{obj} may by an object or may be a \TT{GTYPED\_\EOL OBJ}
...
the attribute pointer.  These can be accessed by the functions:
...
\verb|min::obj_vec_updptr & min::|
	& \MINMKEY{obj\_vec\_ptr\_of}{of {\tt attr\_updptr}}\ARGBREAK
	  \verb|( min::attr_updptr & ap )|
[See \pagref{MIN::OBJ_VEC_PTR_OF_ATTR_UPDPTR}] \\
\end{tabular}\end{indpar}

which point at the graph type (or at the object \TT{obj} if that is
a non-graph-typed object) and:
...
which point at the context of a graph typed object, or are set to
\TT{min::\EOL NULL\_\EOL STUB} if \TT{obj} is a non-graph-typed object.
