...
However, the length difference between a {\tt min::gen32} value
and a {\tt min::gen64} value (\secref{64-BIT-GENERAL-VALUES})
is used to distinguish between overloaded
functions that accept either kind of value.
...
in loose implementations (\pagref{LOOSE}).  64-bit general values
are used as function arguments and values in compact implementations
(\pagref{COMPACT}).
...
arguments cannot be used to identify overloaded functions.
However, the length difference between a {\tt min::gen32} value
(\secref{32-BIT-GENERAL-VALUES})
and a {\tt min::gen64} value is used to distinguish between overloaded
functions that accept either kind of value.

A 0 to 5 character string store in the \verb|min::gen64| value itself is
called a `\key{direct string}', as opposed to a short string stored in a stub
(\pagref{SHORT-STRING-STUB})
that is pointed at by the \verb|min:gen64| value, or a long string
stored in a body associated with a stub
(\pagref{LONG-STRING-STUB})
pointed at by the \verb|min:gen64|
value.
