...
\begin{indpar}
\begin{tabular}{@{}p{2.0in}p{3.5in}@{}}
\multicolumn{2}{l}{\TTNBKEY{MIN\_MAX\_RELATIVE\_STUB\_ADDRESS}} \\
...
\begin{indpar}
\begin{tabular}{@{}p{2.0in}p{3.5in}@{}}
\multicolumn{2}{l}{\TTNBKEY{MIN\_MAX\_ABSOLUTE\_STUB\_ADDRESS}} \\
...
\verb|bool |
    & \TTOMKEY{\LT}{\LT}{of {\tt min::ptr<T>}}\ARGBREAK
...
\verb|min::ptr<T> |
    & \TTOMKEY{\MINUS\MINUS}{\MINUS\MINUS}{of {\tt min::ptr<T>}}
    & [prefix \TT{-{}-}]\ARGBREAK
      \verb|( min::ptr<T> & r )|
...
instead of \TT{min::gen} members.

Members of the types `\TT{const min::stub *}', `\TT{min::stub const *}', and
`\TT{const min::\EOL stub \EOL const~*}' are treated like
`\TT{min::\EOL stub~*}' members for these purposes.

\TT{min::\EOL packed\_\EOL struct\_\EOL xxxptr<S>} ~and~
\TT{min::\EOL packed\_\EOL vec\_\EOL xxxptr<E,H,L>} members
are also treated as \TT{const min::\EOL stub~*} members.
...
\verb|typedef |
	& \verb|min::packed_struct_ptr|{\tt <S>}\ARGBREAK
	  \verb|min::|\MINKEY{packed\_struct<S>::ptr}
...
circumstances, such as inside a \TT{template} class,
component names like \TT{min::\EOL packed\_\EOL struct<S>::ptr} will not work
and the alternative \TT{min::\EOL packed\_\EOL struct\_\EOL ptr<S>}
must be used instead.  The later are usually preferred anyway because
it can be defined without defining \TT{S}, and therefore can be
...
\verb|typedef |
	& \verb|min::packed_struct_updptr|{\tt <S>}\ARGBREAK
	  \verb|min::|\MINKEY{packed\_struct<S>::updptr}
...
similar to C-language \TT{struct}, number, or pointer types in that:
\begin{enumerate}
...
header members described below, the \TT{control}, \TT{length},
and \TT{max\_\EOL length} members, are given values.
No constructor is called.
\item
Destruction of a datum of the type does nothing.
...
Numeric, \TT{min::gen}, and \TT{min::stub *} values are permitted as
...
\end{tabular}\end{indpar}
where \TT{H} is the type of the packed vector header struct,
\TT{E} is the type of the packed vector element, and
\TT{L} is the unsigned integer type of the \TT{length}
...
\verb|typedef |
	& \verb|min::packed_vec_ptr<E,H,L>|\ARGBREAK
	  \verb|min::|\MINKEY{packed\_vec<E,H,L>::ptr}
...
(e.g., \TT{pvp[i]} executes a \TT{MIN\_\EOL ASSERT}
...
\TT{pvp.begin\_ptr()} is the same as `\TT{pvp + 0}'
except the latter would fail if \TT{pvp->\EOL length == 0}.
\TT{pvp.end\_ptr()} is the same as `\TT{pvp + pvp->length}'
...
from the vector, use \TT{begin\_\EOL ptr()}:
\begin{indpar}\begin{verbatim}
memcpy ( ..., pvup.begin_ptr(), sizeof ( E ) * pvup->length );
\end{verbatim}\end{indpar}
Using \TT{\& pvup[0]} instead of \TT{pvup->begin\_ptr()}
will fail with a \TT{MIN\_\EOL ASSERT} fault if \TT{pvup->\EOL length == 0}.
...
\verb|typedef |
	& \verb|min::packed_vec_updptr<E,H,L>|\ARGBREAK
	  \verb|min::|\MINKEY{packed\_vec<E,H,L>::updptr}
...
\TT{begin\_ptr()} and \TT{end\_ptr()} members are
...
a `\TT{E~\&}' value.  This does for vector elements what
...
\verb|typedef |
	& \verb|min::packed_vec_insptr<E,H,L>|\ARGBREAK
	  \verb|min::|\MINKEY{packed\_vec<E,H,L>::insptr}
