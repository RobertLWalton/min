...
defines the labeled arrows sourced at the node, or the variable is set to
a number or string that is the value of the node.  When a variable $X$
...
all line long objects in this respect.
...
The data structure of an variable vector body can be viewed at any of
two levels: vector level and variable level.
...
is an unsigned integer part of which is the index of a \verb|min::gen|
value in the body vector.  This is distinct from the use of
auxiliary pointers in objects, as in that case the entire integer is
the index.  Details are below in \secref{VARIABLE-VECTOR-VARIABLE-LEVEL}.
...
\begin{indpar}
\key{indirect pair pointer} ~~~	This points at an \key{indirect pair},
...
\key{indirect indexed pointer} ~~~ The integer value of this auxiliary value is
