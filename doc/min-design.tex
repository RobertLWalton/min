...
\end{tabular}\end{indpar}
\begin{indpar}[5em]\begin{tabular}{l}
    \verb|  const char * const *     special_names;|\\
    \verb|  min::uns32               special_names_size;|\\
\ttmindex{special\_names}{in {\tt min::gen\_format}}
\ttmindex{special\_names\_size}{in {\tt min::gen\_format}}
\end{tabular}\end{indpar}
\begin{indpar}[5em]\begin{tabular}{l}
    \verb|  min::suppression_matrix  suppression_matrix;|\\
    \verb|};|
...
\verb|    //     [B][.] for B == '(', '[', '{', 0A8 (|\LAQUOTE\verb|)| \\
\verb|    //     [.][C] for C == ')', ']', '}', 0B8 (|\RAQUOTE\verb|), ',', ';'|
\end{tabular}\end{indpar}
...
\multicolumn{2}{l}{\tt const min::gen\_format
    ...
    \verb|  min::default_suppression_matrix|\ARGBREAK
    \verb|};|
...
\verb|const min::uns32 min::| & \MINNBKEY{SUBOBJ\_ID\_FLAG}
\LABEL{MIN::SUBOBJ_ID_FLAG} \\
...
and no attributes are treated specially.

\item[\ttnbmkey{OBJ\_ID\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set, an object is printed as an `\verb|@|\verb|<|{\em id}\verb|>|'
object identifier.  The identifier is retrived using the printer
{\tt obj\_\EOL id\_\EOL map}, and if the object is not alread in
in the map, it is first pushed into the map.

\item[\ttnbmkey{SUBOBJ\_ID\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set, subobjects that are parts of object elements or attribute
values are printed as if the {\tt OBJ\_\EOL ID\_\EOL FLAG} were set
