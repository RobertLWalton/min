...
a 26-bit auxilary list pointer (32 bit general value only) \\
a 26-bit auxilary sublist pointer (32 bit general value only) \\
a 26-bit control code (32 bit general value only) \\
a 40-bit auxilary list pointer (64 bit general value only) \\
a 40-bit auxilary sublist pointer (64 bit general value only) \\
...
6-bit subtype code
and a low order 26-bit auxilary pointer (see~\pagref{AUXILARY-POINTER}
for a definition of auxilary pointers).
32-bit general value control codes consist of a high order
6-bit subtype code and a low order 26-bit unsigned integer that
...
stub pointers, auxilary pointers, and control codes into IEEE Nan (Not-a-Number)
...
a 26-bit unsigned integer for both 32-bit and 64-bit general values.
For 64-bit general values it returns only the low order 26 bits of the
code.
...
fit within 26 bits if {\tt min::gen} values are 32-bits, and
within 40 bits if {\tt min::gen} values are 64-bits.
The {\tt unsigned} argument used to make control codes must
fit within 26 bits regardless of the size of
{\tt min::gen} values.
...
auxilary pointer, and control code
...
A \key{number stub} has \minkey{NUMBER}\LABEL{MIN::NUMBER} stub type code and
...
An uncollectible number stub with a stub control that is zero except
for its type code can be created by
...
Collectible number stubs only exist in compact implementations;
...
\minkey{SHORT\_STR}\LABEL{MIN::SHORT_STR} stub type code and
...
\minkey{LONG\_STR}\LABEL{MIN::LONG_STR} stub type code and
...
An uncollectible short or long string stub with a stub control that
is zero except for its type code can be created by

\begin{center}\begin{tabular}{r@{}l}
\verb|void min::stub * min::|
	& \MINKEY{new\_str\REL}\verb| ( const char * p )|
\LABEL{MIN::NEW_STR} \\
\end{tabular}\end{center}

If the input has 8 or fewer {\tt char}'s, a short stub is created; otherwise
...
[This section is obsolete and should be deleted]
...
A \key{label stub}\label{LABEL-STUB} has \minkey{LABEL}\LABEL{MIN::LABEL}
...
has \minkey{LABEL\_PAIR}\LABEL{MIN::LABEL_PAIR}
...
names begin with small positive integers.  The \key{auxilary area}
...
\minkey{SHORT\_OBJ}\LABEL{MIN::SHORT_OBJ} stub type code and
...
\minkey{LONG\_OBJ}\LABEL{MIN::LONG_OBJ} stub type code and
...
A list element may be a non-auxilary-pointer \verb|min::gen|
value, such as a direct number, direct string, control code, or
stub pointer.  None of these represent sublists.  A list element
that is an auxilary sublist pointer or the value \verb|min::EMPTY_SUBLIST|
...
there is only one way to reach any element of the auxilary area.
Or more specifically,
auxilary pointers must point at elements of the auxilary area,
two auxilary pointers are not permitted to point at the same auxilary
area element, and an auxilary area element that is preceded
in the auxilary area by a list element may not be pointed at by
an auxilary pointer.

Thus if the one way to reach an element of
the auxilary area is deleted, the element may be put on a list of
free elements for the auxilary area.  As an optimization,
an implementation may use the
free area to hold the head of a list of free elements of the auxilary
area.

As an optimization, \skey{auxilary stub}s can be used in place of object
auxilary area elements.  When this is done, the description of this
section must be modified as explained in \secref{AUXILARY-STUBS}.
...
into the auxilary area may change, invalidating positioning information
...
calls to {\tt next} will do nothing.
...
value is a sublist auxilary pointer, is the value {\tt min::EMPTY\_SUBLIST},
or is a pointer to a sublist auxilary stub as described in
\secref{AUXILARY-STUBS}.
...
will be repointed at X, and all other list pointers will not be affected.
...
\subsubsubsection{Auxilary Stubs}
\label{AUXILARY-STUBS}

As an optimization, \skey{auxilary stub}s can be used instead of object
auxilary area elements.  Auxilary stubs are a way of adding elements to
...
eliminate the use of auxilary stubs extending the object
by moving information from these into a new larger object auxilary area.

The value of an auxilary stub is treated as an auxilary area element value.
The chain pointer of an auxilary stub is treated as the next auxilary
...
auxilary stub is treated as a list pointer pointing at that stub.
A chain pointer can also hold an auxilary pointer value that is
list pointer pointing back into
the auxilary area of the object the auxilary stub extends.

An auxilary stub has one of the following two uncollectible stub type
codes:

\begin{indpar}
\begin{list}{}{}
\item[{\tt const int min::}\MINKEY{LIST\_AUXILARY}]~%
	\LABEL{MIN::LIST_AUXILARY}\\
A \verb|min::gen| value pointing at this stub behaves like a list pointer.
\item[{\tt const int min::}\MINKEY{SUBLIST\_AUXILARY}]~%
	\LABEL{MIN::SUBLIST_AUXILARY}\\
A \verb|min::gen| value pointing at this stub behaves like a sublist pointer.
\end{list}
\end{indpar}

Stubs with the first type code are called \skey{list auxilary stub}s,
and those with the second type code are called \skey{sublist auxilary stub}s.
...
A \verb|min::gen| value that points at list auxilary stub is
treated as a list pointer.
A \verb|min::gen| value that points at sublist auxilary stub is
treated as a sublist pointer.

Therefore a \verb|min::gen| value pointing at a list auxilary stub
cannot be a list element value, and
a \verb|min::gen| value pointing at a sublist auxilary stub
is a list element value that represents a sublist.

If an auxilary stub is pointed at by a chain pointer, the stub must
be a list auxilary stub.

An auxilary stub can be pointed at by only one pointer stored in an
object or in some auxilary stub extending an object.
Thus auxilary
stubs can be freed as soon as the pointer to them is deleted.  This
mechanism, and not garbage collection, frees auxilary stubs.
...
\\[1ex]
\emkey{map-list} ::= {\em map-pair}\ldots
\\[1ex]
\emkey{map-pair} ::= {\em name-component } {\em segment-entry}
\\[1ex]
\emkey{name-component} ::= {\em atom }
\\[1ex]
\emkey{segment-entry} ::= {\em value} $|$ {\em segment-list}
\\[1ex]
\emkey{segment-list} ::= {\em map-list-option} {\em attribute-list}
\\[1ex]
\emkey{attribute-list} ::= {\em flag-set}
    {\em value}\ldots {\em reverse-attribute-list}\ldots
\\[1ex]
\emkey{flag-set} ::= {\em control-code}\ldots
\\[1ex]
\emkey{value} ::= {\em atom} $|$ {\em object}
\\[1ex]
\emkey{reverse-attribute-list} ::= {\em reverse-attribute-name}
    {\em value}\ldots
\\[1ex]
\emkey{reverse-attribute-name} ::= {\em attribute-name}
\end{tabular}
\end{center}
...
an {\em object} is a \verb|min::gen| value pointing at an object,
and an {\em attribute-name} is a \verb|min::gen| value pointing at an
attribute name.  All these are list elements but not lists.
...
An object is represented by a map from attribute names to {\em attribute-lists}.
This map is based on the following search algorithm:

\begin{enumerate}

\item Let $C$ be the first component (atom) of the attribute name.
...
\item $M$ is a {\em map-list}.  Look through the {\em map-pairs} in this
list for one whose {\em component-name} equals $C$.  If none is found,
the search fails.
If one is found, let $E$ be the {\em segment-entry} part of the {\em map-pair}.

\item If $E$ is a value and not a list,
change $E$ to a list whose only element is $E$.
Note this will be interpreted below as
an {\em attribute-list} with one {\em value} and nothing else.

\item $E$ is a {\em segment-entry}.
If the attribute name has more components, let $C$ be the next
component.  If $E$ does not begin with a sublist, the search fails.
But if $E$ begins with a sublist, let this be $M$, and go back to step 4.

\item Otherwise, if there is no next component of the attribute name,
remove from $E$ any sublist, i.e., any {\em map-list}, that begins $E$.
The resulting $E$ is the {\em attribute-list} to which the attribute name maps.

\end{enumerate}

Once an attribute name $N$ has been mapped to an attribute list, the
attribute list is interpreted as follows:

\begin{enumerate}

\item The {\em flag-set} and the beginning of an {\em attribute-list}
specifies the attribute flags of the attribute name.\label{FLAG-SET}
See below \pagref{ATTRIBUTE-FLAGS} for details.

\item The {\em values} in the {\em attribute-list} before any
{\em reverse-attribute-list} are the values of the attribute name paired with
the empty reverse attribute name.

\item The {\em values} in any {\em reverse-attribute-list} are
the values of the attribute name paired with the {\em reverse-attribute-name}
of the {\em reverse-attribute-list}.

\end{enumerate}

\ikey{Attribute flags}{attribute flag!representation}\label{ATTRIBUTE-FLAGS}
are represented by a {\em flag-set} in an {\em attribute-list}
(\pagref{FLAG-SET}).
The {\em flag-set} is a sequence of \verb|min::gen|
control codes.

The flags are numberred 0, 1, 2, \ldots.  Flag $N$ corresponds to
the bit in the $I$'th control code
selected by the mask $2^K$ where $I=\mbox{floor}(N/C)+1$,
$K=N~\mbox{mod}~C$, and $C$ is the
number of bits in a control code integer.  $C$ is 26 if \verb|min::gen|
...
bit is present and set in the {\em flag-set} of the {\em attribute-list}
that the attribute name maps to.
...
reverse attribute name can be the special value {\tt min::MISSING}
the count is to be returned for the attribute name with no reverse attribute
name.

The {\tt getg} function gets the values which {\tt countg} counts.
The values are stored in the {\tt out} vector.  The number of values
stored is the function return value.  The argument {tt n} is the
...
The {\tt count\_flagsg} function returns the length of a \verb|min::uns32|
vector required to store all the flags for a given object and attribute name.
...
the number of elements of the vector written is the return value of the
function. The argument {\tt n} is the maximum number of elements that
may be written, the length of {\tt out}.  If there are more flags
than will fit in {\tt n} elements, only {\tt n} elements are written.
...
When an attribute value $O_2$ of an attribute name/reverse-name pair
of object $O_1$ is set, and the reverse-name is not missing,
the value $O_2$ is the other end of a double arrow.
It must be an object, and the attribute value $O_1$ is simultaneoulsy
set for the attribute of $O_2$ with the reverse-name/name pair:w

TBD
Double arrow deletion.

When an attribute value of an attribute name/reverse-name pair is deleted,
the value is the other end of a 
...
The following functions are analogous to the above functions, but
optimize for the case where the attribute name is a single atom that
is an integer in the range of a hash table index:

\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{geti}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, unsigned n,| \\
    \verb|   min::gen obj,| \\
    \verb|   int name, min::gen reverse_name)|
    \end{tabular}
\LABEL{MIN::GETI} \\
\verb|unsigned min::| & \MINKEY{counti}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen obj,| \\
    \verb|   int name, min::gen reverse_name)|
    \end{tabular}
\LABEL{MIN::COUNTI} \\
\verb|unsigned min::| & \MINKEY{get\_flagsi}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::uns32 * out, unsigned n,| \\
    \verb|   min::gen obj,| \\
    \verb|   int name)|
    \end{tabular}
\LABEL{MIN::GET_FLAGSI} \\
\verb|unsigned min::| & \MINKEY{count\_flagsi}%
    \verb| ( min::gen obj, int name)|
\LABEL{MIN::COUNT_FLAGSI} \\
\verb|void min::| & \MINKEY{seti\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( const min::gen * in, unsigned n,| \\
    \verb|   min::gen obj,| \\
    \verb|   int name, min::gen reverse_name)|
    \end{tabular}
\LABEL{MIN::SETI} \\
\verb|void min::| & \MINKEY{set\_flagsi\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( const min::uns32 * in, unsigned n,| \\
    \verb|   min::gen obj,| \\
    \verb|   int name)|
    \end{tabular}
\LABEL{MIN::SET_FLAGSI} \\
\end{tabular}\end{center}
...
the set of values for the attribute and reverse attribute names consists
of a single value $V$.  If the set is empty, the special value
\verb|min::MISSING| is returned, and if the set has more than one value,
the special value \verb|min::MULTI_VALUED| is returned.

This version of the {\tt setg} function deletes any previous values of
the attribute and reverse attribute name and makes the one value {\tt v}
the only value of the attribute and reverse attribute names.

The following functions deal with the case where the attribute
name/reverse-name pair has a set of values:
...
values of the attribute name/reverse-name pair if and only if
{\tt v} is not already in the set.  The {\tt add\_to\_multiset}
function adds {\tt v} even if the value is already in the set.
...
\item For every function of this section with a name ending in `{\tt g}'
and \verb|min::gen| {\tt name} argument, there is another function with
the name ending `{\tt g}' replaced by `{\tt i}' and the type of the
{\tt name} argument changed to {\tt int}.  These functions do that same
thing as would be done if the {\tt int} {\tt name} argument were converted
to a \verb|min::gen| value, but they do it more efficiently in the case
where the {\tt name} argument can be used as an index to the attribute vector
of the object.

\item Functions which have {\tt reverse\_name} as a last argument
have versions without this last argument, which act as
if the value \verb|min::MISSING| were given to this argument.

\item Functions with \verb|min::gen| {\tt obj} arguments also have
versions with `\verb|min::stub *|' {\tt obj} arguments.
...
\\
\verb|const int min::|\MINKEY{LABEL\_PAIR} \LABEL{MIN::LABEL_PAIR}
\\
\verb|const int min::|\MINKEY{SHORT\_OBJ} \LABEL{MIN::SHORT_OBJ}
\\
\verb|const int min::|\MINKEY{LONG\_OBJ} \LABEL{MIN::LONG_OBJ}
\\
\verb|const int min::|\MINKEY{LIST\_AUXILARY} \LABEL{MIN::LIST_AUXILARY}
\\
\verb|const int min::|\MINKEY{SUBLIST\_AUXILARY} \LABEL{MIN::SUBLIST_AUXILARY}
...
\begin{tabular}{@{}r@{}p{4.8in}}
\verb|unsigned min::| & \MINKEY{geti}\ARGBREAK
    \verb| ( min::gen * out, unsigned n,|\ARGBREAK
    \verb|   min::gen obj,|\ARGBREAK
    \verb|   int name, min::gen reverse_name)|
\LABEL{MIN::GETI} \\
\verb|unsigned min::| & \MINKEY{counti}\ARGBREAK
    \verb| ( min::gen obj,|\ARGBREAK
    \verb|   int name, min::gen reverse_name)|
\LABEL{MIN::COUNTI} \\
\verb|unsigned min::| & \MINKEY{get\_flagsi}\ARGBREAK
    \verb| ( min::uns32 * out, unsigned n,|\ARGBREAK
    \verb|   min::gen obj,|\ARGBREAK
    \verb|   int name)|
\LABEL{MIN::GET_FLAGSI} \\
\verb|unsigned min::| & \MINKEY{count\_flagsi}\ARGBREAK
    \verb| ( min::gen obj, int name)|
\LABEL{MIN::COUNT_FLAGSI} \\
\verb|void min::| & \MINKEY{seti\REL}\ARGBREAK
    \verb| ( const min::gen * in, unsigned n,|\ARGBREAK
    \verb|   min::gen obj,|\ARGBREAK
    \verb|   int name, min::gen reverse_name)|
\LABEL{MIN::SETI} \\
\verb|void min::| & \MINKEY{set\_flagsi\REL}\ARGBREAK
    \verb| ( const min::uns32 * in, unsigned n,|\ARGBREAK
    \verb|   min::gen obj,|\ARGBREAK
    \verb|   int name)|
\LABEL{MIN::SET_FLAGSI} \\
