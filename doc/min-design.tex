...
(\verb|min::PRIVATE|, \verb|min::PUBLIC|, and \verb|min::TYPED|)
...
object indirectly using a logrithmic code.  Therefore not every
...
The variables vector of the returned object is filled with
\verb|min::UNDEFINED| values.
...
permits read-write access to the body the body vector elemets,
...
An object has two flags that regulate the creation of such
pointers: \minnbkey{PRIVATE} and \minnbkey{PUBLIC}.

If an object has neither of these flags, a master pointer
to the object may be created, and this will set the \verb|min::PRIVATE|
...
If an object has neither flag, its \verb|min::PUBLIC| flag may be set.
...
set object flags.  The \verb|min::PUBLIC| flag may not be cleared by
protected functions.  The idea here is that setting the \verb|min::PUBLIC|
flag fixes the attribute label structure of an object, and the size of
...
Also object bodies may be reorganized if neither the
\verb|min::PRIVATE| or \verb|min::PUBLIC| flags are set, or
if the reorganization is needed by an insertable pointer.
When the \verb|min::PUBIC| is set, the object is reorganized to
compact it before setting the flag.
