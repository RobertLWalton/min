...
\subsection{Stubless Bodies}
\label{STUBLESS-BODIES}

Body pointers have types such as {\tt long\_str} and {\tt long\_obj}.
It is possible to have a body without a corresponding stub.  Such
bodies are called \ikey{stubless bodies}{stubless body}.  They
allocated in the C++ stack, C++ heap, C++ global data area,
or in some other way.  The rules of \secref{BODY-POINTERS} should
not be applied to these pointers.

For example, function frames are allocated in the C++ stack as stubless
{\tt long\_obj} bodies.
