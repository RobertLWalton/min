...
and their compose key sequences for Linux keyboard entry are:
\textexclamdown(!!),
\textcent(/c),
\textsterling(-l),
\textcurrency(xo),
...
$\times$(xx),
$\div$(-:).
At the moment (2012), other non-ASCII, non-letter
...
\"A({\tt "A}),
\c{C}({\tt ,C}), and
\O({\tt /O}), with the remaining cases
\AA({\tt AA}), and
\aa({\tt aa}) using a compose sequence consisting of just the letter doubled.
However the LATIN-1 letters \dh{} and \DH{} cannot currently be input, as
composing `- d' or `- D' give non-LATIN-1 UNICODE character codes.
LATIN-1 control codes cannot currently be input and have
...
The following functions can be used to convert \verb|min::gen|
string values to numbers:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::| & \MINKEY{strto} \verb|( min::int32 & value, min::gen g )|
\LABEL{MIN::STRTO_INT32} \\
\verb|bool min::| & \MINKEY{strto} \verb|( min::int64 & value, min::gen g )|
\LABEL{MIN::STRTO_INT64} \\
\verb|bool min::| & \MINKEY{strto} \verb|( min::uns32 & value, min::gen g )|
\LABEL{MIN::STRTO_UNS32} \\
\verb|bool min::| & \MINKEY{strto} \verb|( min::uns64 & value, min::gen g )|
...
If the \verb|min::gen| argument is a
string consisting of a space surrounded number legal to the UNIX
{\tt strtol}, {\tt strtoll},
{\tt strtoul}, or {\tt strtoull} functions with {\tt base} argument
{\tt 0}, or to the UNIX
{\tt strtof}, or {\tt strtoud} functions, respectively,
{\tt true} is returned and the number is returned in the {\tt value}
argument.  Otherwise {\tt false} is returned and the {\tt value}
argument is left untouched.  This includes the case where the
\verb|min::gen| argument is not a string.
...
{\tt NULL} state in which using it to access the string will almost
certainly cause a memory fault.  The constructor with no argument also
sets the string pointer to the {\tt NULL} state.  The operator to
convert a string pointer to a {\tt bool} returns {\tt true} if the
string pointer is \underline{not} in the {\tt NULL} state, and {\tt false}
if the string pointer \underline{is} in the {\tt NULL} state.

The {\tt =} operator can reset the string
pointer to point at a different \verb|min::gen| value, or at a stub,
or at the
value pointed at by another string pointer.  Like the constructor,
the string pointer is set to the {\tt NULL} state
if the \verb|min::gen| value or stub pointer is not a string,
or the string pointer being assigned from is itself in the {\tt NULL} state.
...
Numbers can be read using from a string pointed at by a string
...
If the string pointer is not in the {\tt NULL} state and
the part of the string beginning with {\tt sp[i]} is
space followed by a number legal to the UNIX
{\tt strtol}, {\tt strtoll},
{\tt strtoul}, or {\tt strtoull} functions with {\tt base} argument
{\tt 0}, or to the UNIX
{\tt strtof}, or {\tt strtoud} functions, respectively,
{\tt true} is returned, the number is returned in the
{\tt value} argument, and the {\tt i} argument is updated to point
just after the number.  Otherwise {\tt false} is returned and
{\tt value} and {\tt i} are left untouched.  This includes the
case where the string pointer is in the {\tt NULL} state.
...
to translate {\tt const min::\EOL stub~*} pointers to small unsigned
integer \smkey{identifier}s{in identifier map} that can be used
as surrogates for the pointers in external files.
A {\tt min::\EOL id\_\EOL map} is a packed vector with
{\tt const min::\EOL stub~*} elements such that index of an element
in the vector is the identifier of the element value.
This identifier may appear in files as \verb|@|{\em index}.
...
\verb|    min::uns32    | & \TTMKEY{flush\_\EOL gen\_\EOL flags}%
                                       {in {\tt min::print\_format}}
			          & \verb|// Default:| \\
...
\LABEL{MIN::FLUSH_GEN_FLAGS_IN_PRINT_FORMAT} \\
\verb|    min::uns32    | & \TTMKEY{flush\_\EOL attr\_\EOL gen\_\EOL flags}%
                                       {in {\tt min::print\_format}}
			          & \verb|// Default:| \\
			      &   & \verb|//    min::GRAPHIC_STR_FLAG| \\
			      &   & \verb|//  + min::OBJ_EXP_FLAG| \\
			      &   & \verb|//  + min::OBJ_ID_FLAG|
\LABEL{MIN::FLUSH_ATTR_GEN_FLAGS_IN_PRINT_FORMAT} \\
...
\item[\ttnbmkey{FLUSH\_ID\_MAP\_FLAG}%
    {in printer {\tt print\_\EOL format.flags}}]~\\
...
\item[\ttmkey{print\_format.flush\_gen\_flags}{in {\tt min::printer}}]%
\vspace{-2ex}
\item[\ttmkey{print\_format.flush\_attr\_gen\_flags}{in {\tt min::printer}}]%
\vspace{-2ex}
...
\multicolumn{2}{l}{\tt min::print\_format
    ...
    \verb|  min::OBJ_EXP_FLAG,                  // gen_flags|\ARGBREAK
...
to control the printing.  If the latter is {\tt NULL}, then
{\tt \&\,min::default\_\EOL gen\_\EOL format} is used in its place.

The expression `{\tt printer\,<{}<\,min::pgen(v,gen\_flags)}'
is similar except that it uses the {\tt gen\_\EOL flags} argument
in place of
{\tt printer->\EOL print\_\EOL format.flush\_\EOL gen\_\EOL flags}:
...
The expression `{\tt printer\,<{}<\,min::flush\_pgen(v)}'
is similar except that it uses
{\tt printer->\EOL print\_\EOL format.flush\_\EOL gen\_\EOL flags}
for values \verb|v| that are not objects, and for the top level of
\verb|v| if that is an object, but uses
{\tt printer->\EOL print\_\EOL format.flush\_\EOL attr\_\EOL gen\_\EOL flags}
instead for names and values of object attributes.

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::op min::|
    & \MINKEY{flush\_\EOL pgen}
	  \verb|( min::gen v )|
\LABEL{MIN::FLUSH_PGEN} \\
\end{tabular}\end{indpar}

This is
used by the {\tt min::\EOL flush\_\EOL id\_\EOL map} printer operation
(\pagref{MIN::FLUSH_ID_MAP}) to print objects \verb|v|
that are being flushed from the {\tt printer->\EOL id\_\EOL map},
when, to take an example, it is
permissible to print attribute values as identifiers of the
form `{\tt @}i', but not permissible to print \verb|v| as such.

The three argument form of {\tt min::pgen} may be used to give
{\tt gen\_\EOL flags} and {\tt attr\_\EOL gen\_\EOL flags} explicitly:
...
\verb|min::op min::|
    & \MINKEY{pgen}\ARGBREAK
	  ...
	  \verb|  min::uns32 attr_gen_flags )|
\LABEL{MIN::PGEN_WITH_ATTR_GEN_FLAGS} \\
\end{tabular}\end{indpar}

The one and two argument forms of {\tt min::pgen} in effect set
both {\tt attr\_\EOL gen\_\EOL flags} and {\tt gen\_\EOL flags}
equal.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|typedef const bool ( min::|
	& \MINKEY{suppress\_matrix}\verb| )[256][256]|
...
\end{tabular}\end{indpar}
\begin{indpar}[5em]\begin{tabular}{l}
    \verb|  min::suppress_matrix     suppress_matrix;|\\
...
\verb|min::printer min::| & \MINKEY{default\_pgen}\ARGBREAK
    ...
    \verb|  min::gen v,|\ARGBREAK
    \verb|  min::uns32 gen_flags,|\ARGBREAK
    \verb|  min::uns32 attr_gen_flags,|\ARGBREAK
    \verb|  const min::gen_format * g );|
...
\begin{indpar}[1em]\begin{tabular}{l}
\verb|min::suppress_matrix min::|\MINKEY{default\_\EOL suppress\_\EOL matrix}
\LABEL{MIN::DEFAULT_SUPPRESS_MATRIX} \\
\verb|    // All entries false except:|\\
\verb|    //     [B][.] for B == '(', '[', '{', '|\LACCENT
...
{\tt 0 <= i < special\_\EOL names->\EOL length}.
...
See {\tt surpress\_matrix} above.

\item[\ttnbmkey{OBJ\_EXP\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set and if all attributes of an object have names listed in
{\tt exp\_\EOL ok\_\EOL attrs} (see above), then the object is
...
\item[\ttnbmkey{OBJ\_ID\_FLAG}{{\tt min::gen\_format} flag}]~\\
...
\item[\ttnbmkey{OBJ\_INDENT\_FLAG}{{\tt min::gen\_format} flag}]~\\
...
\item[\ttnbmkey{STR\_ID\_FLAG}{{\tt min::gen\_format} flag}]~\\
...
These {\tt id\_map} flushing operations use {\tt min::\EOL flush\_\EOL pgen}
(\pagref{MIN::FLUSH_PGEN}) to print values in lines of the form:
\begin{center}
...
If the datum is an object not being printed as per
...
The printer flags in {\tt gen\_flags}, {\tt flush\_\EOL gen\_\EOL flags},
and {\tt flush\_\EOL attr\_\EOL gen\_\EOL flags} can be set or cleared
by the following printer operations.  Here
{\tt min::\EOL set\_\EOL xxx\_\EOL flags}, logically OR's
its argument into {\tt print\_\EOL format.xxx\_\EOL flags}, and
{\tt min::\EOL clear\_\EOL xxx\_\EOL flags} logically AND's
the bitwise complement of its argument into
{\tt print\_\EOL format.xxx\_\EOL flags}:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::op min::|
    & \MINKEY{set\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 gen_flags )|
\LABEL{MIN::SET_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 gen_flags )|
\LABEL{MIN::CLEAR_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{set\_\EOL flush\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 flush_gen_flags )|
\LABEL{MIN::SET_FLUSH_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_\EOL flush\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 flush_gen_flags )|
\LABEL{MIN::CLEAR_FLUSH_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{set\_\EOL flush\_\EOL attr\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 flush_attr_gen_flags )|
\LABEL{MIN::SET_FLUSH_ATTR_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_\EOL flush\_\EOL attr\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 flush_attr_gen_flags )|
\LABEL{MIN::CLEAR_FLUSH_ATTR_GEN_FLAGS} \\
\end{tabular}\end{indpar}

Particular {\tt print\_format.gen\_flags} may be set and cleared by the
following printer operations:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::op min::| & \MINKEY{graphic\_\EOL str}
\LABEL{MIN::GRAPHIC_STR} \\
\verb|const min::op min::| & \MINKEY{nographic\_\EOL str}
\LABEL{MIN::NOGRAPHIC_STR} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::op min::| & \MINKEY{bracket\_\EOL str}
\LABEL{MIN::BRACKET_STR} \\
\verb|const min::op min::| & \MINKEY{nobracket\_\EOL str}
\LABEL{MIN::NOBRACKET_STR} \\
\verb|const min::op min::| & \MINKEY{bracket\_\EOL lab}
\LABEL{MIN::BRACKET_LAB} \\
\verb|const min::op min::| & \MINKEY{nobracket\_\EOL lab}
\LABEL{MIN::NOBRACKET_LAB} \\
\verb|const min::op min::| & \MINKEY{bracket\_\EOL special}
\LABEL{MIN::BRACKET_SPECIAL} \\
\verb|const min::op min::| & \MINKEY{nobracket\_\EOL special}
\LABEL{MIN::NOBRACKET_SPECIAL} \\
\verb|const min::op min::| & \MINKEY{bracket\_\EOL implicit}
\LABEL{MIN::BRACKET_IMPLICIT} \\
\verb|const min::op min::| & \MINKEY{nobracket\_\EOL implicit}
\LABEL{MIN::NOBRACKET_IMPLICIT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::op min::| & \MINKEY{suppress\_\EOL lab\_\EOL space}
\LABEL{MIN::SUPPRESS_LAB_SPACE} \\
\verb|const min::op min::| & \MINKEY{nosuppress\_\EOL lab\_\EOL space}
\LABEL{MIN::NOSUPPRESS_LAB_SPACE} \\
\verb|const min::op min::| & \MINKEY{suppress\_\EOL obj\_\EOL space}
\LABEL{MIN::SUPPRESS_OBJ_SPACE} \\
\verb|const min::op min::| & \MINKEY{nosuppress\_\EOL obj\_\EOL space}
\LABEL{MIN::NOSUPPRESS_OBJ_SPACE} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::op min::| & \MINKEY{obj\_\EOL exp}
\LABEL{MIN::OBJ_EXP} \\
\verb|const min::op min::| & \MINKEY{noobj\_\EOL exp}
\LABEL{MIN::NOOBJ_EXP} \\
\verb|const min::op min::| & \MINKEY{obj\_\EOL id}
\LABEL{MIN::OBJ_ID} \\
\verb|const min::op min::| & \MINKEY{noobj\_\EOL id}
\LABEL{MIN::NOOBJ_ID} \\
\verb|const min::op min::| & \MINKEY{str\_\EOL id}
\LABEL{MIN::STR_ID} \\
\verb|const min::op min::| & \MINKEY{nostr\_\EOL id}
\LABEL{MIN::NOSTR_ID} \\
\end{tabular}\end{indpar}

The {\tt gen\_format} of a printer may be changed by the printer operation:
...
New formats should be made by copying an old format and then making
...
\verb|char * min::| & \MINKEY{strcpy}\ARGBREAK
      \verb|( char * p, const min::str_ptr & sp )|
\LABEL{MIN::STRCPY_OF_STR_PTR} \\
\verb|char * min::|
    & \MINKEY{strncpy}\ARGBREAK
      \verb|( char * p,|\ARGBREAK
...
\LABEL{MIN::PRINTER_ID_MAP} \\
\verb|min::uns32 printer->obj_id_map->|
    & \TTMKEY{next}{in {\tt min::\EOL printer::\EOL id\_\EOL map}}
\LABEL{MIN::PRINTER_ID_MAP_NEXT} \\
\end{TABULAR}
...
\verb|struct min::| & \MINKEY{gen\_format}\ARGBREAK
    ...
    \verb|  min::suppress_matrix   suppress_matrix;|\ARGBREAK
    \verb|  min::uns32             str_max_length;|\ARGBREAK
    \verb|  min::packed_vec_ptr<min::gen>  exp_ok_attrs;|\ARGBREAK
    \verb|};|
\LABEL{MIN::GEN_FORMAT} \\
\end{TABULAR}
...
\begin{TABULAR}{2.0in}
\multicolumn{2}{l}{\tt min::packed\_vec\_ptr<const char \&>}\ARGBREAK
    \verb|min::|\MINKEY{standard\_\EOL special\_\EOL names}
\LABEL{MIN::STANDARD_SPECIAL_NAMES} \\
\end{TABULAR}

\begin{TABULAR}{2.0in}
\multicolumn{2}{l}{\tt min::suppress\_matrix}\ARGBREAK
    \verb|min::|\MINKEY{default\_\EOL suppress\_\EOL matrix}
\LABEL{MIN::DEFAULT_SUPPRESS_MATRIX} \\
\end{TABULAR}

\begin{TABULAR}{2.0in}
\multicolumn{2}{l}{\tt min::packed\_vec\_ptr<min::gen>}\ARGBREAK
