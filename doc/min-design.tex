...
\key{continuation pointer}
			& The list is continued at the target of the list
			  pointer. \\
...
The list ends here.  This actually equals a continuation pointer
with zero index.
...
If the list continuation has a continuation pointer value,
...
(constructor)~\verb|MUP::|
	& \MUPKEY{list\_pointer}\verb| ( min::short_obj * so )|
\LABEL{MUP::LIST_POINTER_OF_SHORT_OBJ} \\
(constructor)~\verb|MUP::|
	& \MUPKEY{list\_pointer}\verb| ( min::long_obj * lo )|
\LABEL{MUP::LIST_POINTER_OF_LONG_OBJ} \\
(constructor)~\verb|MUP::|
...
\verb|min::gen |
	& \TTKEY{start\_sublist}\verb| ( min::list_pointer & lp, min::gen p )|
\LABEL{START_SUBLIST_OF_LIST_POINTER} \\
\end{tabular}\end{center}

A list pointer is created to move around in a particular object.
Once created it can be started on a new list by the {\tt start} function,
or on a new sublist by the {\tt start\_sublist} function.

A list pointer points into the relocatable body of an object.  When the
object is relocated, the list pointer must be reinitialized by recalling
the {\tt start} function with a list head index.  Relocation of an object
can also reorganize the object, in which case body vector indices that
are not of list heads may become invalid.

The {\tt start} function positions a list pointer at the beginning of a
list, and takes as an argument the index of a list head
in the object body vector.  This index may point into the
hash table or attribute vector.  The {\tt start} function
returns the value of the first element of the list, or returns
{\tt min::LIST\_END} if the list is empty.
...
The value of a list element can be an auxilary sublist pointer or
\verb|min::EMPTY_SUBLIST|.  Such a list element value represents a
sublist, and can be passed to the {\tt start\_sublist} function to point
a list pointer at the first element of the sublist.  The
{\tt start\_sublist} function returns the value of this first element,
or returns {\tt min::LIST\_END} if the sublist is empty.
...
object.  If the object is later relocated, it can be reorganized to
eliminate the use of auxilary stubs by moving information from these
into a new larger object auxilary area.

An auxilary stub holds a \verb|min::gen| value
that plays the role of an auxilary element value, and has an
\key{auxilary chain pointer} that can point at another auxilary stub,
and which plays the role of incrementing the index of an object
auxilary area element to get to the next auxilary area element.  Auxilary
stubs are uncollectible, and their chain pointer is stored in the stub
control.

There are two types of auxilary stubs:
