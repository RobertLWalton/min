...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{new\_obj}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( unsigned total_size,| \\
    \verb|   unsigned hash_table_size )| \\
    \end{tabular}
\LABEL{MIN::NEW_OBJ} \\
\end{tabular}\end{center}

The total and hash table sizes of the returned object are possible
total and hash table sizes at least as large as those given as
parameters.
The returned object is short if the total size argument is less than
$2^{16}$, and long otherwise.
It is a fatal programming error if no possible size
is at least as large as its corresponding argument, but this is
unlikely, given the large sizes allowed by a long object.

The hash table
of the returned object is zeroed, and therefore empty.  The attribute
vector and auxiliary areas of the returned object are zero length,
and all space not used by the header and hash table is allocated to
the unused area.  The attribute vector and auxiliary area can be
...
object vector.  The elements of the lists are
...
A list element that is not a sublist may be any \verb|min::gen|
value that is not a list or sublist auxililary pointer.
A list element
that is an sublist auxiliary pointer or the value \verb|min::EMPTY_SUBLIST|
...
The following function can be used to delete {\tt n} consecutive elements
...
\LABEL{MIN::DELETE_FROM_LIST} \\
\end{tabular}\end{center}

Element deletion requires no reservation.
...
~~~~~ 1 if code to use object auxilary stubs is to be compiled; \\
