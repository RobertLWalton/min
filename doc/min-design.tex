...
A \verb|min::gen| value can be used as a
\ikey{component of a name}{name component}\index{component!of name}
if and only if it is a
a number (\secref{NUMBERS}),
a string (\secref{STRINGS}),
or a label (\secref{LABELS}), i.e., it is an atom or a label.
Name components are all immutable values which have an associated
hash value.

The following functions concern name components:
...
a name component (number, string, or label).
...
greater than its \verb|v2| argument.  Both arguments must be name
components.  In this ordering numbers are
before strings and strings are before labels.  Numbers are ordered
...
The \verb|min::hash| and \verb|min::compare| functions produce
\verb|MIN_ASSERT| errors if passed non-name-component arguments.
...
The {\tt initialize} functions destruct the vector pointer and then
reconstruct it.
...
Exceptions are destructors and the {\tt min::\EOL ini\-tial\-ize}
functions, which
...
were not in the list of all attribute names.  However, note that if
the {\tt flag\_count} is non-zero, it is still
possible that all flags will be zero.
...
raw vector pointer (\pagref{MIN::RAW_VEC_POINTER})
...
Here \verb|v| must point at the stub of a raw vector,
and given a raw vector pointer \verb|rvp| pointing at
this raw vector, \verb|rvp[i]| is the \verb|i|+1's
