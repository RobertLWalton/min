...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{lab\_of}%
	  \begin{tabular}[t]{@{}l@{}}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{lablen}\verb| ( const min::stub * s )|
\LABEL{MIN::LABLEN} \\
\verb|min::unsptr min::| & \MINKEY{lablen}\verb| ( min::gen v )|
...
\newcommand{\TARG}{\BRACKETED{T,I}}

A \key{raw vector} is a variable sized vector whose elements
are C language characters, numbers, or structs.  Elements or struct members
may also be \verb|min::gen| values, \verb|min::stub *|
pointers, or values of any type that is
\verb|typedef|'ed from a C language character, number, or struct.
All elements must have the same size.

Elements may \underline{not} be C++ classes.  In particular,
they may \underline{not} have constructors or destructors, and must
be copyable with a simple `{\tt =}' operator or with the C language
\verb|memcpy| function.

There are three kinds of raw vector pointers that can be used to
access elements of raw vectors.
A read-only {\tt min::\EOL raw\_\EOL vec\_\EOL pointer\TARG}
permits read-only access to raw vector elements.
An updatable {\tt min::\EOL updatable\_\EOL raw\_\EOL vec\_\EOL pointer\TARG}
permits read-write access to raw vector elements.
An insertable {\tt min::\EOL insertable\_\EOL raw\_\EOL vec\_\EOL pointer\TARG}
permits creation of raw vectors,
read-write access to raw vector elements, and operations that push
and pop elements from the end of a a raw vector.

All three types of raw vector pointer are templates based on the
following parameterization:
\begin{center}
\tt template
    \BRACKETED{~typename T, const min::raw\_vec\_type\_info \& I~}
\LABEL{MIN::TEMPLATE_OF_RAW_VEC_POINTER} \\
\end{center}
...
\ttmkey{g}{in raw vector {\tt type\_info} format}
	& Part is a \verb|min::gen| value. \\
\ttmkey{s}{in raw vector {\tt type\_info} format}
	& Part is a \verb|min::stub *| value. \\
...
{\tt min::\EOL raw\_\EOL vec\_\EOL pointer\TARG} are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{raw\_vec\_pointer\TARG}%
	  \verb| rvp ( min::gen v )|
...
The form of constructor that has no argument creates a raw pointer
...
{\tt min::\EOL updatable\_\EOL raw\_\EOL vec\_\EOL pointer\TARG} are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{updateable\_raw\_vec\_pointer\TARG}%
	  \verb| rvp ( min::gen v )|
\LABEL{MIN::UPDATABLE_RAW_VEC_POINTER} \\
(constructor)~\verb|min::|
	& \MINKEY{updatable\_raw\_vec\_pointer\TARG}%
	  \verb| rvp ( min::stub * s )|
...
The functions for using a
{\tt min::\EOL insertable\_\EOL raw\_\EOL vec\_\EOL pointer\TARG}
to access an existing raw vector are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{insertable\_raw\_vec\_pointer\TARG}%
	  \verb| rvp ( min::gen v )|
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min|
	& {\tt ::insertable\_raw\_vec\_pointer\TARG} \\
...
\begin{TABULAR}{1.8in}
\verb|min::unsptr min::| & \MINKEY{lab\_of}\ARGBREAK
    \verb| ( min::gen * p, min::unsptr n,|\ARGBREAK
    \verb|   const min::stub * s )|
\LABEL{MIN::LAB_OF} \\
\verb|min::unsptr min::| & \MINKEY{lab\_of}\ARGBREAK
    \verb| ( min::gen * p, min::unsptr n,|\ARGBREAK\verb|   min::gen v )|
\LABEL{MIN::LAB_OF_GEN} \\
\end{TABULAR}
