...
\verb|const min::uns32 min::| & \MINNBKEY{NOBREAKS\_FLAG}
\LABEL{MIN::NOBREAKS_FLAG} \\
...
break point exists after the {\tt indent} (see below), a break,
consisting of a line end followed by indentation spaces,
is pushed into the printer vector before the character representative.

The break points in the current line inside the printer vector
are just before a sequence of consecutive single space
characters, or just before the representative of a character sent to the
printer in graphics mode (see below).  Horizontal tabs cause {\tt 1} to
{\tt 8} single space characters to be inserted in the current line,
and these count in computing break points.  When the break point just
before a sequence of single spaces is used, the single spaces are
deleted, and the character representative that cause the break begins
in the {\tt indent} column.
...
Other values presented to {\tt <<} are converted to {\tt char} strings
as follows:
...
Both these printer operations end the current line.  The
...
\verb|const min::printer_op min::| & \MINKEY{nobreaks}
\LABEL{MIN::NOBREAKS} \\
\verb|const min::printer_op min::| & \MINKEY{breaks}
\LABEL{MIN::BREAKS} \\
...
For example, `{\tt ptrt<<min::nobreaks}' sets the
{\tt prtr->\EOL parameters.flags} {\tt NOBREAKS\_FLAG}
and `{\tt prtr<<min::breaks}' clears this flag.
