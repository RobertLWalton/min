...
If there are several list pointers pointing into a list,
and one is used to insert an element after a element of the list,
or to insert an element before the end of the list,
...
into sublists of the list or superlists of the list are not affected.

TBD

If there are several list pointers pointing into a list,
and one is used to insert an element X before a element Y of the list
that is not the end of the list, any other list pointers pointing
into the list or at the end of an empty sublist of the list may be
affected in an undefined way.
...
\begin{center}
\begin{tabular}{l}
\emkey{hash-table-entry} ::= {\em map-list}\ldots
\\[1ex]
\emkey{attribute-vector-entry} ::= {\em segment-entry}\ldots
\\[1ex]
...
\\[1ex]
\emkey{attribute-list} ::= {\em flag-option}
    {\em value}\ldots {\em reverse-component-list}\ldots
\\[1ex]
\emkey{flag} ::= {\em control-code}
\\[1ex]
\emkey{value} ::= {\em atom} $|$ {\em object}
\\[1ex]
\emkey{reverse-component-list} ::= {\em reverse-attribute-name}
    {\em value}\ldots
\\[1ex]
...
Here
an {\tt atom} is a \verb|min::gen| value representing a number or string,
...
What the object does is map attribute names to {\em attribute-lists}.
It does this by mapping using the following search algorithm
to map an attribute name to an {\em attribute-list}.
...
The initial segment of an attribute name consisting of just the first
component of the name, i.e., the first atom, is mapped as follows.
If the first component is an integer from 1 through the size of the attribute
vector, inclusive, the attribute vector entry whose index is this component
is the {\em segment-entry} to which the initial segment is mapped.  Otherwise


If a {\em segment-list} begins with a
{\em map-list}, 

If 

Hash table entries are {\em hash-table-entries}, which are {\em map-pairs}
mapping attribute name components onto {\em component-entries}.



If the name is an unsigned integer small enough to index an
