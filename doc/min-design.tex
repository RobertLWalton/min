...
\ikey{Locator functions}{locator function!of attribute pointer}
...
\ikey{Accessor functions}{accessor function!of attribute pointer}
...
\ikey{Information functions}{information function!of attribute pointer}
...
returns the \TT{reverse\_\EOL name} argument of the last call to a
\TT{min::\EOL reverse\_\EOL locate}
function, or returns \TT{min::\EOL NONE()} if no
\TT{min::\EOL reverse\_\EOL locate} calls have been made
...
\ttmkey{name}{member of {\tt min::attr\_info}}
    & Name of the current attribute, i.e., the
...
\ttmkey{reverse\_attr\_count}{member of {\tt min::attr\_info}}
    & Number of reverse attribute labels associated with the current
      attribute which have one or more values.
      Zero if no current attribute.
\end{tabular}
\end{indpar}
...
have a non-zero value for one of their three counts.  The number $N$ of such
attributes is returned.  Information about the first $\min(N,$\TT{n}$)$
of these attributes is returned, in arbitrary order.
...
Attributes accessed through the attribute vector are not returned
...
of these reverse attributes is returned, in arbitrary order.
