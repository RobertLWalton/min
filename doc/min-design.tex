...
The \verb|min::subsequence| function
returns the index of the first occurence
of its first argument \verb|v1| as a subsequence of its second
argument \verb|v2|, or returns \verb|-1| if there is no such
occurence.  For this function,
a non-label must be an atom, and is equivalent to a length {\tt 1} label
...
\verb|   min::uns32   | & \TTMKEY{value\_\EOL gen\_\EOL flags}%
...
\LABEL{MIN::VALUE_GEN_FLAGS_IN_PRINT_FORMAT} \\
\verb|   min::uns32   | & \TTMKEY{name\_\EOL gen\_\EOL flags}%
                                       {in {\tt min::print\_format}}
			& \verb|// Default:| \\
			&   & \verb|//    min::GRAPHIC_STR_FLAG| \\
...
\LABEL{MIN::NAME_GEN_FLAGS_IN_PRINT_FORMAT} \\
...
\begin{itemlist}[0.6in]
\item[\ttmkey{print\_format.value\_gen\_flags}{in {\tt min::printer}}]
\item[\ttmkey{print\_format.name\_gen\_flags}{in {\tt min::printer}}]%
\vspace{-2ex}~
...
{\tt printer} using
{\tt printer->\EOL print\_\EOL format.value\_\EOL gen\_\EOL flags}
and
...
\verb|min::op min::|
    & \MINKEY{pgen}\ARGBREAK
	  \verb|( min::gen v,|\ARGBREAK
	  \verb|  min::uns32 value_gen_flags,|\ARGBREAK
	  \verb|  min::uns32 name_gen_flags )|
...
\begin{indpar}[5em]\begin{tabular}{l}
    ...
    \verb|       ( min::printer printer,|\\
    \verb|         min::uns32 context,|\\
    \verb|         min::gen v,|\\
    \verb|         min::uns32 value_gen_flags,|\\
    \verb|         min::uns32 name_gen_flags,|\\
...
\verb|min::printer min::| & \MINKEY{default\_pgen}\ARGBREAK
    \verb|( min::printer printer,|\ARGBREAK
    \verb|  min::uns32 context,|\ARGBREAK
    \verb|  min::gen v,|\ARGBREAK
    \verb|  min::uns32 value_gen_flags,|\ARGBREAK
    \verb|  min::uns32 name_gen_flags,|\ARGBREAK
    \verb|  const min::gen_format * g );|
...
\LABEL{MIN::PGEN_VALUE} \\
\verb|const min::uns32 min::| & \MINNBKEY{PGEN\_FLUSH}
\LABEL{MIN::PGEN_FLUSH} \\
\verb|const min::uns32 min::| & \MINNBKEY{PGEN\_NAME}
\LABEL{MIN::PGEN_NAME} \\
\verb|const min::uns32 min::| & \MINNBKEY{PGEN\_ELEMENT}
\LABEL{MIN::PGEN_ELEMENT} \\
\end{tabular}\end{indpar}
...
The printer flags in {\tt value\_\EOL gen\_\EOL flags},
and {\tt name\_\EOL gen\_\EOL flags} can be set or cleared
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::op min::|
    & \MINKEY{set\_\EOL value\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 value_gen_flags )|
\LABEL{MIN::SET_VALUE_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_\EOL value\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 value_gen_flags )|
\LABEL{MIN::CLEAR_VALUE_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{set\_\EOL name\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 name_gen_flags )|
\LABEL{MIN::SET_NAME_GEN_FLAGS} \\
\verb|min::op min::|
    & \MINKEY{clear\_\EOL name\_\EOL gen\_\EOL flags}\ARGBREAK
      \verb|( min::uns32 name_gen_flags )|
\LABEL{MIN::CLEAR_NAME_GEN_FLAGS} \\
\end{tabular}\end{indpar}

Particular {\tt print\_format.value\_gen\_flags} may be set and cleared by the
following printer operations:
