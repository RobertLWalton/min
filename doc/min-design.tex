...
even if {\tt min::gen} values can accept 40-bit auxilary pointers.}
...
attribute values are variable.  The variable attribute values are
called `\skey{variable}s' and are elements of the variable vector.
The attribute values actually stored in the associated objects are index
general values that point at variables in the variable vector.
In effect these indeces are indirect pointers at the variables.

A variable $X$ can hold either a general value,
or an indirect pointer to another variable $Y$.
An indirect point directs all references to $X$ to the
other variable $Y$.  $Y$ can be in the same variable vector as $X$,
in which case the indirect pointer is just the index of $Y$ as an element
of this variable vector, or $Y$ can be in a different variable vector from $X$,
in which case the indirect pointer is a pair consisting
of a pointer to the stub of the variable vector containing $Y$
and an index value specifying $Y$ as an element of this vector.

The objects associated with a variable vector are required to satisfy
some restrictions.  First, values with reverse attribute names cannot
be indices.  Second, if an attribute with no reverse attribute name
has more than one value, its values cannot be indices.

\subsubsubsection{Variables}
\label{Variables}

A \MUPkey{variable} can be used to move around in a object at
the variable level, and get or set the values of variables.
The functions for using a variable are:

\begin{center}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{variable}\verb| ( )|
\LABEL{MUP::VARIABLE} \\
\verb|min::gen min::| & \MINKEY{point}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::variable & var,| \\
    \verb|   min::gen vector, unsigned index_name)|
    \end{tabular}
\LABEL{POINT_VARIABLE} \\
\verb|min::gen min:: |
	& \TTKEY{get}\verb| ( min::variable & var )|
\LABEL{GET_VARIABLE} \\
\verb|min::gen min:: |
	& \TTKEY{set}\verb| ( min::variable & var, min::gen value )|
\LABEL{SET_VARIABLE} \\
\verb|bool min::|
	& \TTKEY{is\_variable}\verb| ( min::variable & var )|
\LABEL{IS_VARIABLE} \\
\verb|min::gen min::| & \MINKEY{getg}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::variable & var,| \\
    \verb|   min::gen name, min::gen reverse_name)|
    \end{tabular}
\LABEL{MIN::GETG_OF_VARIABLE} \\
\verb|min::gen min::| & \MINKEY{geti}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::variable & var,| \\
    \verb|   int name, min::gen reverse_name)|
    \end{tabular}
\LABEL{MIN::GETI_OF_VARIABLE} \\
\end{tabular}\end{center}
