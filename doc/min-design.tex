...
\TT{min::standard\_char\_flags} is computed using data in
\ldots\TT{/min/\EOL unicode/\EOL unicode\_data.h}%
...
\verb|const min::support_control min::|%
    \MINKEY{inclusive\_support\_control} \verb|=| \\
\verb|            { 0xFFFF, min::IS_UNSUPPORTED };|
\LABEL{MIN::INCLUSIVE_SUPPORT_CONTROL} \\
\end{tabular}\end{indpar}
...
\verb|const min::display_control min::|%
    \MINKEY{verbatim\_display\_control} \verb|=| \\
\verb|            { 0xFFFF, 0 };|
...
\LABEL{MIN::GRAPHIC_AND_CONTROL_DISPLAY_CONTROL} \\
\end{tabular}\end{indpar}
...
\LABEL{MIN::NO_AUTO_BREAK_BREAK_CONTROL} \\
\verb|const min::break_control min::|%
    \MINKEY{break\_after\_hyphenators\_break\_control} \verb|=| \\
\verb|            { min::IS_HSPACE, 0, min::BREAK_CONTROL, 4 };|
\LABEL{MIN::BREAK_AFTER_HYPHENATORS_BREAK_CONTROL} \\
\end{tabular}\end{indpar}
...
if ( cflags & dc.display_char )
    ...
    // represented by a sequence of ' ' spaces if EXPAND_HT
    // op_flag is on.
    ...
    // the char_name_format prefix and postfix, and X is
    // min::unicode::name[cindex] if that is not NULL, or is the
    // character code in hexa-decimal with a leading digit
    // otherwise.
\end{verbatim}\end{indpar}

\item Compute automatic break point settings for \TT{c}:
\begin{indpar}\begin{verbatim}
