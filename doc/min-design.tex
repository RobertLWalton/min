...
These have names beginning with \ttnbkey{MIN\_}.
...
Code in \verb|include/min.h| that is not meant to be accessed
by users is in the \ttkey{min::internal} namespace, which in
\verb|src/min.cc| is abbreviated by 
`\verb|#define |\ttnbmkey{MINT}{abbreviates {\tt min::internal}}'%
\label{MINT}\verb| min::internal|'.
This code and its interfaces are \underline{not} described
in this document.
...
interface names begin with `\minkey{unprotected}{\tt ::}'.  User code
that accesses the unprotected interface typically abbreviates this long
prefix to
...
{\tt MIN\_ASSERT} macro.  The
`\verb|-DMIN_PROTECT=0|' compiler option (see \pagref{MIN_PROTECT})
disables \verb|MIN_ASSERT| and removes much of the
protection provided by the protected interface.
...
unprotected MIN functions are in the \ttnbmkey{MUP}{namespace} namespace
and have names beginning with `\ttnbmkey{MUP::}{in function name}'.

The {\tt min} and {\tt min::unprotected} namespaces hold \underline{all}
the stable interfaces of the MIN implementation.  Other namespaces
({\tt min::os} or {\tt MOS}, {\tt min::acc} or {\tt MACC},
{\tt min::internal} or {\tt MINT})
hold code whose interfaces are subject to change without notice.
...
TBD: above is used only as size of deallocated block.
...
declared in \verb|include/min_os.h| and defined in \verb|src/min_os.cc|.
...
Details of the \verb|min::os| interface are in \verb|include/min_os.h|.  The
...
\label{ALLOCATOR/COLLECTOR/COMPACTOR}
...
provided at compile time in \verb|include/min_acc_parameters.h|.
Documention for these parameters is in this last file.
...
Details of the ACC code are in \verb|include/min_acc.h|.  The
following is an overview.
