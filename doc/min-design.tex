...
to appear all on one line.  Then if the text extends beyond the
limits defined by the printer {\tt line\_\EOL length}, a line end
...
equal {\tt indent} if {\tt indent} spaces were inserted after the line end.
Thus {\tt min::\EOL eol} and {\tt min::\EOL indent} both end by executing
an implied {\tt min::\EOL setbreak}, and an implied {\tt min::\EOL setbreak}
is also executed just after inserting a line end and following spaces
at a break point.
...
Unless you do, an adjustment
that extends beyond the printer {\tt line\_\EOL length} may format badly.

The {\tt min::indent} operation forces a line break if the current
printer column is greater than {\tt indent}, and otherwise inserts
single spaces until the current printer column is equal to
{\tt indent}.  In either case the operation
...
just as would happen if a break stack entry were not used.  Note also
that if no break stack entry is enabled, break operation proceeds
...
\end{center}
with the {\tt HBREAK\_\EOL FLAG} flag on and with
{\tt min::save\_break} called after printing
`{\tt \BRA~}', `{\tt [~}', and `{\tt (~}'
and {\tt min::restore\_break} called after printing% {
`{\tt ~\KET}', `{\tt ~]}', and `{\tt ~)}'.
...
\begin{center}
\tt \BRA aaa, bbb, [ ccc, ddd, eee, ( fff, ggg )
\end{center}
a break is triggered just before printing the `{\tt ,}' following
`{\tt ggg)}'.  The first element in the break stack was pushed
...
is not executed.  When the second element is popped, it is
