...
multi-sets of values.  The part of this map for each given attribute
name is independent of the other parts of the map for other attribute
names.  These parts are represented by final segments of lists.

TBD

An attribute name is a sequence of atoms.  Each atom of the

\begin{enumerate}

\item If the first component of an attribute name is an integer I in the
range from 1 through the object attribute vector size S, then
the component list begins with the I-1'st element of the attribute vector.

\item Otherwise the component is located in the pair list that begins
with the H'th element of the hash table, where H = hash of component
modulo the size of the hash table.

\item A pair list is an alternating sequence of atoms and component sublists.
Each atom denotes an attribute name component, and is followed in the pair
list by the component sublist of that component.

\item If a component sublist begins with a sublist with at least 2 elements,
that sublist is a pair list for the next component of an attribute name.
The rest of the component sublist is the attribute name list for the
attribute name ending in the given component.
