...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::ref<T> | & \verb|p|\TTBMKEY{i}{of {\tt min::ptr}}
\LABEL{MIN::PTR_[]} \\
\verb|min::ptr<T> |
...
\verb|T * |
	& \TTOMKEY{!}{!}{of {\tt min::ptr<T>}}
	  \verb|( const min::ptr<T> & p )| & [unprotected]
\LABEL{MIN::!_OF_PTR} \\
\verb|min::ptr<T> |
	& \MINKEY{ptr<T>}
...
This is because
the \TT{->} operator has been defined for \TT{min::\EOL ptr<T>}
types to return \TT{\&v}, but for \TT{min::\EOL ref<T>} types
it returns \TT{v} (see \pagref{REF->}).
...
from a pointer \TT{s} to a stub and either a pointer to
within the body associated with that stub, or an offset
relative to the beginning of that body.
However, unlike \TT{MUP::\EOL new\_\EOL ref},
the location pointed at within a body may not be \TT{const} unless \TT{T} is
\TT{const}.  Also unlike \TT{MUP::\EOL new\_\EOL ref}, an offset
can be given instead of a pointer to within the body, but
then the type \TT{T} must be included in the function name,
which is \TT{MUP::\EOL new\_\EOL ptr<T>}
and not just \TT{MUP::\EOL new\_\EOL ptr}, as \TT{T} cannot
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|char |
    & \verb|sp|\TTBMKEY{i}{of {\tt min::str\_ptr}}
...
\verb|min::gen |
    & \verb|labp|\TTBMKEY{i}{of {\tt min::lab\_ptr}}
...
\verb|min::gen |
    & \verb|labp|\TTBMKEY{i}{of {\tt MUP::lab\_ptr}}
...
The paritial line (\pagref{PARTIAL-LINE}) that may be in the file when
...
If this condition is met, all lines before the one with number
\begin{center}
...
is \TT{0} and \TT{line\_\EOL index} does not exist.
...
\TT{min::\EOL flush\_\EOL line} except that, instead of copying
