...
\begin{tabular}{l}
a pointer to a stub \\
a 64-bit IEEE floating point number atom (64 bit general value only) \\
...
For 32-bit general values, a direct integer value consists of a high order 4-bit
...
\LABEL{MUP::STUB_OF} \\
\verb|unsigned MUP::| & \MUPKEY{aux\_of}\verb| ( min::gen v )|
\LABEL{MUP::AUX_OF} \\
\end{tabular}\end{center}
...
\verb|min::gen min::| & \MINKEY{new\_aux\_gen}\verb| ( unsigned p )|
\LABEL{MIN::NEW_AUX_GEN} \\
\end{tabular}\end{center}

These protected functions check for argument range errors.  The function
with an {\tt int} argument will convert that to an IEEE 64-bit floating
point number if the \verb|min::gen| type is 64-bits.  The function
with an {\tt min::float64} argument will always have an argument range
...
\verb|min::gen MUP::| & \MUPKEY{new\_aux\_gen}\verb| ( unsigned p )|
\LABEL{MUP::NEW_AUX_GEN} \\
\end{tabular}\end{center}
...
\begin{center}\begin{tabular}{r@{}l@{\hspace*{0.3in}}l}
\verb|const unsigned min::| & \MINKEY{GEN32\_DIRECT\_INT} & 4 bits
\LABEL{MIN::GEN32_DIRECT_STR} \\
\verb|const unsigned min::| & \MINKEY{GEN32\_DIRECT\_STR} & 8 bits
\LABEL{MIN::GEN32_DIRECT_STR} \\
\verb|const unsigned min::| & \MINKEY{GEN32\_AUX} & 4 bits
\LABEL{MIN::GEN32_AUX} \\
\verb|const unsigned min::| & \MINKEY{GEN64\_DIRECT\_STR}	& 20 bits
\LABEL{MIN::GEN64_DIRECT_STR} \\
\verb|const unsigned min::| & \MINKEY{GEN64\_STUB}	& 20 bits
\LABEL{MIN::GEN64_STUB} \\
\verb|const unsigned min::| & \MINKEY{GEN64\_AUX}	& 20 bits
\LABEL{MIN::GEN64_AUX} \\
\end{tabular}\end{center}
