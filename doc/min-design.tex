...
output in the non-graphic state.
\item \TT{min::leading} outputs a single space unless that space
would be preceeded or followed by a non-graphic character or line
end, or unless the printer
is in the leading state, the \TT{min::\EOL FORCE\_\EOL SPACE}
...
is in the trailing state, the \TT{min::\EOL FORCE\_\EOL SPACE}
printer operation flag is off, and the next thing output is one of:
\begin{enumerate}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|(const min::ustring *) "\x01\x01" " "    // obj_sep|\ARGBREAK
\end{tabular}
...
\item The object does not have any attributes other than \TT{.position}
that have multiple values, flags, or reverse attributes (double arrows).
\item If the object has a \TT{.type} attribute, the value of that
attribute is a name (as per \TT{min::\EOL is\_\EOL name}:
\pagref{MIN::IS_NAME}).
\item The value of any attribute besides \TT{.position} and \TT{.type}
is a string.
...
In compact format with an \TT{.initiator} attribute, the only one of the
...
\begin{center}
\TT{[} {\em L} {\em <element>}
    \OPEN{} {\em T2} \TT{,} \TT{\textvisiblespace} {\em <element>} \CLOSE\STAR{}
    {\em T} \TT{]}
\end{center}
where
\begin{center}
\begin{tabular}{lcl}
{\em L}			& is	& \TT{min::leading} \\
{\em T2}		& is	& \TT{min::trailing2} \\
...
`{\em T2} \TT{,} \TT{\textvisiblespace}' is replaced by what
...
In compact format without any \TT{.initiator} or \TT{.type} the format
is as before with the type omitted.  If there is a \TT{.type} and
the \TT{TRAILING\_TYPE} object format operation flag is on, then
`\TT{|\}}' is replaced by `\TT{|~T~\}}'.
...
\end{tabular}
\end{center}
In normal format \TT{obj\_\EOL bra} is followed by the
object's \TT{.type} and attributes followed by
\TT{obj\_\EOL braend}, and all this serves as the opening bracket for
the object's list of elements.
The closing bracket for the object's list of elements consists of
\TT{obj\_\EOL ketbegin}, optionally followed by the 
the object \TT{.type} if the \TT{TRAILING\_\EOL TYPE} object format
operation flag is on, followed by \TT{obj\_\EOL ket}.
If the type is missing, it is omitted, unless there are other attributes,
in which case the type is printed as the empty string \TT{""} right
after \TT{obj\_EOL bra}.
