...
are stack variables that may only hold numeric \verb|min::gen|
values.  These variables are called `\skey{acc locatable
variable}s'.  Static variable may also be global; i.e., they
...
Unprotected functions may be used to access the internals of short
and long strings.
...
numerically, strings are ordered lexigraphically as per the
C language \verb|strcmp| function, and labels are ordered lexigraphically
...
Strings are printed as they are immediately preceeded by
{\tt prv.str\_\EOL prefix} and followed by
{\tt prv.str\_\EOL postfix}.  The default is to enclose strings in
\verb|`'| quotes.

Labels are printed as comma separated lists of their elements preceeded by
...
preceeded by {\tt prv.special\_\EOL prefix} and followed by
...
but the control code value is printed in hexadecimal (preceeded by
...
range of the object's attribute vector indices, then the child nodes's
...
\LABEL{MIN::ATTR_INFO} \\
\verb|typedef min::| & \MINKEY{attr\_info\_pointer}
...
\verb|typedef min::| & \MINKEY{reverse\_attr\_info\_pointer}
...
The implementation dependent order of names may change if attribute or
reverse attribute values are
added to or removed from the object or attribute flags are set or cleared,
or if the object is reorganized.
...
A read-only {\tt min::\EOL packed\_\EOL struct\SARG::\EOL pointer}
permits read-only access to packed structure members.
An updatable {\tt min::\EOL packed\_\EOL struct\SARG::\EOL
updatable\_\EOL pointer}
...
the insertable {\tt min::\EOL packed\_\EOL struct\HEARG::\EOL
insertable\_\EOL pointer},
...
terminated by the value \verb|0|.  This displacements vector should not
...
\verb|min::gen| value pointing at it.
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::pointer} \verb|psp| \ARGBREAK
	  \verb| ( min::gen v )|
\LABEL{MIN::PACKED_STRUCT_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::pointer} \verb|psp| \ARGBREAK
	  \verb| ( min::stub * s )|
\LABEL{MIN::PACKED_STRUCT_POINTER_OF_STUB} \\[1ex]
\verb|const S * | & \verb|psp| \\[1ex]
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::updatable\_pointer} \verb|psup|\ARGBREAK
	  \verb| ( min::gen v )|
\LABEL{MIN::PACKED_STRUCT_UPDATABLE_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::updatable\_pointer} \verb|psup|\ARGBREAK
	  \verb| ( min::stub * s )|
\LABEL{MIN::PACKED_STRUCT_UPDATABLE_POINTER_OF_STUB} \\[1ex]
\verb|S * | & \verb|psup| \\
\end{tabular}\end{indpar}

In other words, \verb|psp| is implicitly convertable to a `\verb|const S *|'
pointer and
\verb|psup| is implicitly convertable to a `\verb|S *|' pointer.
Recall, however, that these `\verb|S *|' pointers, since they point at a body,
can change whenever a function is called, but the {\tt min::\EOL
packed\_\EOL struct\SARG::\EOL \ldots} pointers do not change.
...
\verb|min::gen |
	& \verb|pstype.|\TTKEY{new\_gen} \verb|( void )|
\LABEL{PACKED_VEC_NEW_GEN} \\
\verb|min::gen |
	& \verb|pstype.|\TTKEY{new\_gen} \ARGBREAK
	  \verb| ( min::uns32 length, const E * vp = NULL )|
\LABEL{INITED_PACKED_VEC_NEW_GEN} \\
\verb|min::gen |
	& \verb|pstype.|\TTKEY{new\_gen} \ARGBREAK
	  \verb| ( min::uns32 max_length,| \ARGBREAK
	  \verb|   min::uns32 length, const E * vp = NULL )|
\LABEL{MAX_INITED_PACKED_VEC_NEW_GEN} \\
\verb|const char * const |
	& \verb|pstype.|\TTMKEY{name}{in {\tt min::packed\_struct}}
\LABEL{PACKED_VEC_NAME} \\
\verb|const min::uns32 * const |
	& \verb|pstype.|\TTMKEY{header\_gen\_disp}
	                       {in {\tt min::packed\_struct}}
\LABEL{PACKED_VEC_HEADER_GEN_DISPLACEMENTS} \\
\verb|const min::uns32 * const |
	& \verb|pstype.|\TTMKEY{header\_stub\_ptr\_disp}
	                       {in {\tt min::packed\_struct}}
\LABEL{PACKED_VEC_HEADER_STUB_PTR_DISPLACEMENTS} \\
\verb|const min::uns32 * const |
	& \verb|pstype.|\TTMKEY{element\_gen\_disp}
	                       {in {\tt min::packed\_struct}}
\LABEL{PACKED_VEC_ELEMENT_GEN_DISPLACEMENTS} \\
\verb|const min::uns32 * const |
	& \verb|pstype.|\TTMKEY{element\_stub\_ptr\_disp}
	                       {in {\tt min::packed\_struct}}
...
In addition the following parameters may be set by the user:

\begin{indpar}[2.0in]
\begin{itemize}

\item[{\tt pvtype.}\ttmkey{initial\_max\_length}{in {\tt min::packed\_vec}}]
...
\item[{\tt pvtype.}\ttmkey{increment\_ratio}{in {\tt min::packed\_vec}}]
The \verb|min::reserve| function multiplies the old maximum length by
this ratio to get the maximim length increment: see formula on
\pagref{PACKED-VEC-EXPANSION-FORMULA}.  Defaults to \verb|0.5|.

\item[{\tt pvtype.}\ttmkey{max\_increment}{in {\tt min::packed\_vec}}]
...
to the initial value of the packed vector.  Or the maximum length
may not be given and will default to the length.  No parameters may
be given, in which case the length will default to \verb|0| and
the maximum length to {\tt pvtype.initial\_\EOL max\_\EOL length}.
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::pointer} \verb|pvp| \ARGBREAK
	  \verb| ( min::gen v )|
\LABEL{MIN::PACKED_VEC_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::pointer} \verb|pvp| \ARGBREAK
	  \verb| ( min::stub * s )|
\LABEL{MIN::PACKED_VEC_POINTER_OF_STUB} \\[1ex]
\verb|const H * | & \verb|pvp|
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::updatable\_pointer} \verb|pvup|\ARGBREAK
	  \verb| ( min::gen v )|
\LABEL{MIN::PACKED_VEC_UPDATABLE_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::updatable\_pointer} \verb|pvup|\ARGBREAK
	  \verb| ( min::stub * s )|
\LABEL{MIN::PACKED_VEC_UPDATABLE_POINTER_OF_STUB} \\[1ex]
\verb|H * | & \verb|pvup|
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::insertable\_pointer} \verb|pvip|\ARGBREAK
	  \verb| ( min::gen v )|
\LABEL{MIN::PACKED_VEC_INSERTABLE_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{packed\_vec\HEARG::insertable\_pointer} \verb|pvip|\ARGBREAK
	  \verb| ( min::stub * s )|
\LABEL{MIN::PACKED_VEC_INSERTABLE_POINTER_OF_STUB} \\[1ex]
\verb|H * | & \verb|pvip|
\LABEL{MIN::PACKED_VEC_INSERTABLE_POINTER_CONVERSION} \\
\verb|E | & \verb|pvip[i]|
\LABEL{MIN::PACKED_VEC_INSERTABLE_POINTER_ELEMENT} \\[1ex]
\verb|void min::|
	& \MINKEY{push} \ARGBREAK
	  \verb| ( packed\_vec<H,E>::insertable\_pointer pvip,| \ARGBREAK
	  \verb|   const E & v )|
...
\verb|E min::|
	& \MINKEY{pop} \ARGBREAK
	  \verb| ( packed\_vec<H,E>::insertable\_pointer pvip )|
...
As for packed structures, packed vector pointers are implicitly convertable to
ordinary C pointers to the headers.  In addition subscripting packed vector
...
of the vector to ensure the access is legal.  The vector elements in a
packed vector are organized as a C language vector so that
\begin{center}
\verb|(& pvp[0])[i]| is the same as \verb|pvp[i]|
\end{center}
but the address of the packed vector can change if an out-of-line
function is called, so using C pointers into the packed vector is
generally a bad idea.  However \verb|memcpy| can be used as follows to copy
...
length is \verb|pvp->max_length| (see description of \verb|H| above).
...
exceed the maximum length, performs a \verb|min::reserve| function
(see below) to increase the maximum length.  A single element \verb|v|
...
if \verb|vp| is \verb|NULL|, \verb|n| zeros are pushed.
...
the current length is reset to the new maximim length.
...
\begin{indpar}\begin{verbatim}
new_maximum_length = old_maximum_length
                   + max ( reserve_length,
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|struct min::| & \MINKEY{packed\_struct\SARG} \verb|t| \ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  const char *    name;|\ARGBREAK
    \verb|  const char *    format;|\ARGBREAK
    \verb|  min::unsptr     element_size;|\ARGBREAK
    \verb|  min::unsptr     initial_max_length;|\ARGBREAK
    \verb|  min::float64    increment_ratio;|\ARGBREAK
    \verb|  min::unsptr     max_increment;|\ARGBREAK
    \verb|};|
\LABEL{MIN::RAW_VEC_TYPE_INFO} \\
\end{tabular}\end{indpar}



Here {\tt T} is the C language type of the vector element, and
{\tt I} is a {\tt min::\EOL raw\_\EOL vec\_\EOL type\_\EOL info}
structure defined as follows:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|struct min::| & \MINKEY{raw\_vec\_type\_info}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  const char *    name;|\ARGBREAK
    \verb|  const char *    format;|\ARGBREAK
    \verb|  min::unsptr     element_size;|\ARGBREAK
...
\item[\ttmkey{name}{in {\tt type\_info}}]
A name unique to the element type \verb|T|, typically the
fully qualified name of this type.  This character string may
be output to identify the element type if the raw vector is
output.

\item[\ttmkey{format}{in {\tt type\_info}}]
A character string containing for each consecutive part of an element
a character specifying the format of the part from the point
of view of the garbage collector.  The possible characters
are:
\begin{indpar}
\begin{tabular}{lp{3.0in}}
\ttmkey{g}{in raw vector {\tt type\_info} format}
	& Part is a \verb|min::gen| value, which must
	  \underline{not} point at an \underline{uncollectible}
	  stub. \\
\ttmkey{s}{in raw vector {\tt type\_info} format}
	& Part is a \verb|min::stub *| value pointing at
	  a \underline{collectible} stub. \\
\ttmkey{p}{in raw vector {\tt type\_info} format}
	& Part is a number the size of a pointer. \\ 
\ttmkey{.}{in raw vector {\tt type\_info} format}
	& Part is 1-byte piece of a number. \\
\end{tabular}
\end{indpar}

An $N$-byte number is represented by $N$ consecutive `{\tt .}'s.
For example, an \verb|uns32| is represented by `{\tt ....}'.

`{\tt .}'s at the end of a format may be omitted.

\item[\ttmkey{element\_size}{in {\tt type\_info}}]
Must equal `{\tt sizeof ( T )}'.

\item[\ttmkey{initial\_max\_length}{in {\tt type\_info}}]%
\label{INITIAL_MAX_LENGTH}
Value of the raw vector maximum length parameter
when the raw vector is created by a \verb|new_gen|
function with no arguments: see \pagref{MIN::RAW_VEC_NEW_GEN}.

\newsavebox{\RAWVECINCREMENT}
\begin{lrbox}{\RAWVECINCREMENT}
\begin{tabular}[b]{r}
\ttmkey{increment\_ratio}{in {\tt type\_info}} \\
\ttmkey{max\_increment}{in {\tt type\_info}}
\end{tabular}
\end{lrbox}
\item[\usebox{\RAWVECINCREMENT}]
Parameters used in the formula for automatically
expanding raw vectors: see \pagref{RAW-VEC-EXPANSION-FORMULA}.

\end{itemize}
\end{indpar}

In order to be used as a template type parameter, a
{\tt min::\EOL raw\_\EOL vec\_\EOL type\_\EOL info} structure
must be external.  The following is an example usage:
\begin{indpar}\begin{verbatim}
struct rvs {
    min::gen g;
    min::uns32 u;
};
// Note: If min::gen values are 8 bytes, sizeof ( rvs ) may
//       actually be 16, and there may be 4 bytes of hidden
...
typedef min::raw_vec_pointer<rvs,rvs_type_info>
    rvs_pointer;
typedef min::updatable_raw_vec_pointer<rvs,rvs_type_info>
    updatable_rvs_pointer;
typedef min::insertable_raw_vec_pointer<rvs,rvs_type_info>
    insertable_rvs_pointer;
\end{verbatim}\end{indpar}\label{RAW-VEC-TYPE-INFO-EXAMPLE}


The functions for using a read-only
{\tt min::\EOL raw\_\EOL vec\_\EOL pointer\SARG} are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::gen v )|
\LABEL{MIN::RAW_VEC_POINTER_OF_GEN} \\
(constructor)~\verb|min::|
	& \MINKEY{raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::stub * s )|
\LABEL{MIN::RAW_VEC_POINTER_OF_STUB} \\
(constructor)~\verb|min::|
	& \MINKEY{raw\_vec\_pointer\SARG}%
	  \verb| rvp ( void )|
\LABEL{MIN::RAW_VEC_POINTER_OF_VOID} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::gen v )|
\LABEL{MIN::INITIALIZE_RAW_VEC_POINTER_OF_GEN} \\
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::stub * s )|
\LABEL{MIN::INITIALIZE_RAW_VEC_POINTER_OF_STUB} \\
\verb|void min::|
	& \MINKEY{deinitialize}\ARGBREAK
	  \verb| ( min::raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::DEINITIALIZE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|const T & min::|
	& \ttkey{operator [ ]}\ARGBREAK
	  \verb| ( min::raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::unsptr i )|
\LABEL{MIN::RAW_VEC_POINTER[]} \\
\verb|min::unsptr min::|
	& \MINKEY{length\_of}\ARGBREAK
	  \verb| ( min::raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::LENGTH_OF_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}

Here \verb|v| or \verb|s| must point at the stub of a raw vector,
and given a raw vector pointer \verb|rvp| pointing at
a raw vector, \verb|rvp[i]| is the \verb|i|+1's
element of the vector, for
$0\leq$\verb|i|$<$\verb|min::length_of(rvp)|,
where the {\tt min::\EOL length\_\EOL of} function
returns the number of elements in the raw vector.
Note that \verb|rvp[i]| is read-only, but one can take
its address, as in:
\begin{indpar}\begin{verbatim}
memcpy ( ..., & rvp[0], sizeof ( T ) * min::length_of ( rvp ) );
\end{verbatim}\end{indpar}

The form of constructor that has no argument creates a raw vector pointer
that is not pointing at any raw vector.
In this last case the raw vector pointer must
be initialized by the {\tt min::\EOL initialize} function before the pointer
is used; use before initialization will give undefined results, though
attempts to access
raw vector elements using the pointer usually result in memory faults.

The {\tt min::initialize} functions destruct the raw vector pointer and then
reconstruct it.  The {\tt min::\EOL deinitialize} function destructs
the raw vector pointer and leaves it not pointing at any object stub.
Use of such a destructed pointer is undefined, but attempts to access
raw vector elements using it usually result in memory faults.

The functions for using a read-write
{\tt min::\EOL updatable\_\EOL raw\_\EOL vec\_\EOL pointer\SARG} are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{updateable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::gen v )|
\LABEL{MIN::UPDATABLE_RAW_VEC_POINTER} \\
(constructor)~\verb|min::|
	& \MINKEY{updatable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::stub * s )|
\LABEL{MIN::UPDATABLE_RAW_VEC_POINTER_OF_STUB} \\
(constructor)~\verb|min::|
	& \MINKEY{updatable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( void )|
\LABEL{MIN::UPDATABLE_RAW_VEC_POINTER_OF_VOID} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::updatable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::gen v )|
\LABEL{MIN::INITIALIZE_UPDATABLE_RAW_VEC_POINTER_OF_GEN} \\
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::updatable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::stub * s )|
\LABEL{MIN::INITIALIZE_UPDATABLE_RAW_VEC_POINTER_OF_STUB} \\
\verb|void min::|
	& \MINKEY{deinitialize}\ARGBREAK
	  \verb| ( min::updatable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::DEINITIALIZE_UPDATABLE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|T & min::|
	& \ttkey{operator [ ]}\ARGBREAK
...
\verb|min::unsptr min::|
	& \MINKEY{length\_of}\ARGBREAK
	  \verb| ( min::updatable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::LENGTH_OF_UPDATABLE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}

These functions are the same as for a read-only raw vector pointer
except that \verb|rvp[i]| can now be both read and written.

The functions for using a
{\tt min::\EOL insertable\_\EOL raw\_\EOL vec\_\EOL pointer\SARG}
to access an existing raw vector are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{insertable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::gen v )|
\LABEL{MIN::INSERTABLE_RAW_VEC_POINTER} \\
(constructor)~\verb|min::|
	& \MINKEY{insertable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( min::stub * s )|
\LABEL{MIN::INSERTABLE_RAW_VEC_POINTER_OF_STUB} \\
(constructor)~\verb|min::|
	& \MINKEY{insertable\_raw\_vec\_pointer\SARG}%
	  \verb| rvp ( void )|
\LABEL{MIN::INSERTABLE_RAW_VEC_POINTER_OF_VOID} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::gen v )|
\LABEL{MIN::INITIALIZE_INSERTABLE_RAW_VEC_POINTER_OF_GEN} \\
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::stub * s )|
\LABEL{MIN::INITIALIZE_INSERTABLE_RAW_VEC_POINTER_OF_STUB} \\
\verb|void min::|
	& \MINKEY{deinitialize}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::DEINITIALIZE_INSERTABLE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|T & min::|
	& \ttkey{operator [ ]}\ARGBREAK
...
\verb|min::unsptr min::|
	& \MINKEY{length\_of}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::LENGTH_OF_INSERTABLE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{push}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|\ARGBREAK
	  \verb|   const & T v )|
\LABEL{MIN::PUSH_OF_INSERTABLE_RAW_VEC_POINTER} \\
\verb|void min::|
	& \MINKEY{push}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|\ARGBREAK
	  \verb|   const T * p, min::unsptr n )|
\LABEL{MIN::PUSH_N_OF_INSERTABLE_RAW_VEC_POINTER} \\
\verb|T min::|
	& \MINKEY{pop}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::POP_OF_INSERTABLE_RAW_VEC_POINTER} \\
\verb|void min::|
	& \MINKEY{pop}\ARGBREAK
	  \verb| ( T * p, min::unsptr n,|\ARGBREAK
	  \verb|   min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::POP_N_OF_INSERTABLE_RAW_VEC_POINTER} \\
\end{tabular}\end{indpar}

The non-push/pop functions above are the same for insertable pointers as they
are for updatable pointers.

The {\tt min::\EOL push} functions push one new element or \verb|n| new
elements to the end of the vector, and the {\tt min::\EOL pop} functions
pop one element or \verb|n| elements from the end of the vector.
The push functions increment the vector length by \verb|1| or \verb|n|,
and the pop functions decrement the length by \verb|1| or \verb|n|.

Raw vectors have a \mkey{maximum length}{of raw vector}
in addition to having a \mkey{length}{of raw vector}.  The length
tells the number of elements in the vector.  The maximum length is
the largest value of the length allowed before the vector must be
resized, which reallocates the raw vector in memory.  The push functions 
automatically resize the raw vector if they would make the length
greater than the maximum length.  The difference between the maximum length
and the length is the number of \smkey{unused element}s{of raw vector}
in the raw vector memory area, and is called the
\mkey{unused length}{of raw vector} of the raw vector.

The following functions may be used to create new raw vectors and maintain
existing raw vectors:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min|
	& {\tt ::insertable\_raw\_vec\_pointer\SARG} \\
	& {\tt ::}\ttkey{new\_gen}\verb| ( void )|
\LABEL{MIN::RAW_VEC_NEW_GEN} \\
\verb|min::gen min|
	& {\tt ::insertable\_raw\_vec\_pointer\SARG} \\
	& {\tt ::}\ttkey{new\_gen}\verb| ( min::unsptr max_length )|
\LABEL{MIN::RAW_VEC_NEW_GEN_OF_1} \\
\verb|min::gen min|
	& {\tt ::insertable\_raw\_vec\_pointer\SARG} \\
	& {\tt ::}\ttkey{new\_gen}\verb| ( const T * p, min::unsptr n )|
\LABEL{MIN::RAW_VEC_NEW_GEN_OF_N} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::|
	& \MINKEY{max\_length\_of}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::MAX_LENGTH_OF_INSERTABLE_RAW_VEC_POINTER} \\
\verb|min::unsptr min::|
	& \MINKEY{unused\_length\_of}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp )|
\LABEL{MIN::UNUSED_OF_INSERTABLE_RAW_VEC_POINTER} \\
\verb|void min::|
	& \MINKEY{resize}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::unsptr new_max_length )|
\LABEL{MIN::RESIZE_INSERTABLE_RAW_VEC_POINTER} \\
\verb|void min::|
	& \MINKEY{reserve}\ARGBREAK
	  \verb| ( min::insertable_raw_vec_pointer<T,I> & rvp,|\ARGBREAK
	  \verb|   min::unsptr required_unused )|
\end{tabular}\end{indpar}

The {\tt min::\EOL insertable\_\EOL raw\_\EOL vec\_\EOL pointer<T,I>::\EOL
new\_\EOL gen} functions create a new raw vector and return a
\verb|min::gen| value pointing at its stub.  These functions are static
members of the
{\tt min::\EOL insertable\_\EOL raw\_\EOL vec\_\EOL pointer<T,I>} type.
The \verb|new_gen| function with zero arguments creates a new raw vector
with \verb|0| length and with a maximum length equal to
{\tt I.initial\_\EOL max\_\EOL length} (\pagref{INITIAL_MAX_LENGTH}).
The \verb|new_gen| function with a single \verb|max_length|
argument creates a new raw vector with \verb|0| length and the maximum
length given by the argument.
The \verb|new_gen| function with a vector of \verb|n| \verb|min::gen|
values specified by arguments creates a new raw vector whose length
and maximum length both equal \verb|n| and whose elements are given by
the argument specified vector.  Here memory order is preserved: if
\verb|rvp| is a raw vector pointer at the new raw vector,
\verb|rvp[0]| takes the value \verb|p[0]| and
\verb|rvp[n-1]| takes the value \verb|p[n-1]|.

The {\tt min::\EOL max\_\EOL length\_\EOL of} function returns the
maximum length of the raw vector pointed at, and
the {\tt min::\EOL unused\_\EOL length\_\EOL of} function returns the
difference of the maximum length and length of this raw vector.
The {\tt min::\EOL resize} function resets the maximum length of the
vector, and if the new maximum length is less than that old length,
also resets the length to the new maximum length.
Resizing usually causes the raw vector to be relocated in memory.
The {\tt min::\EOL reserve} function checks if the unused length
is at least as large as the {\tt required\_\EOL unused} argument,
and if not, resizes the raw vector so the unused length is at least
as large as the argument.  In the latter case the formula used to
choose the new maximum length is:
\begin{indpar}\begin{verbatim}
new_maximum_length = old_maximum_length
                   + max ( required_unused,
                           min ( I.max_increment,
                                   I.increment_ratio
                                 * old_maximum_length ) )
\end{verbatim}\end{indpar}\label{RAW-VEC-EXPANSION-FORMULA}

Here {\tt I.max\_\EOL increment} and {\tt I.increment\_\EOL ratio}
are given by the {\tt min::\EOL raw\_\EOL vec\_\EOL type\_\EOL info}
structure \verb|I| (\pagref{MIN::RAW_VEC_TYPE_INFO}).

Raw vector pointers do no caching and there is \underline{no need} for
