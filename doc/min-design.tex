...
in a {\em double-arrow-sublist} is not meaninful.
...
multiset pointed at is the set of values
...
multiset of values, or at least one set flag, or at least one value
...
multiset when the reverse attribute name is \verb|min::ANY| unless documented
otherwise.
...
as if they has empty value multisets and flag control code lists.
...
multiset pointed at and stores them
in the `{\tt out}' vector.
...
If there are $J$ flag control codes
...
pointer points at.
The flag set control codes are given in the `{\tt in}' vector which
