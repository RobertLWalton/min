...
% < and > do NOT print write in {\tt \bf <>}, i.e.,
...
For the printer {\tt prtr},
{\tt prtr}\ttarmkey{line}{{\tt min::printer} member}
is the number
of the line currently being assembled, or of the next line if 
the printer vector ends with a line-ending {\tt NUL}.  The
first line number is {\tt 0}.
{\tt prtr}\ttarmkey{column}{{\tt min::printer} member}
is the
number of columns currently in the line being assembled,
or the column number of the next character to be input to the
printer, with the first column number being {\tt 0}.

{\tt prtr}\ttarmkey{parameters}{{\tt min::printer} member}
contains parameters that control
the printing process.
{\tt prtr}\ttarmkey{saved\_\EOL parameters}{{\tt min::printer} member}
contains a copy of
{\tt prtr->\EOL parameters}.  The end-of-message {\tt min::\EOL eom}
printer operation restores the printer parameters by copying
{\tt prtr->\EOL saved\_\EOL parameters} to
{\tt prtr->\EOL parameters}.

{\tt prtr}\ttarmkey{ostream}{{\tt min::printer} member},
...
The parameters in {\tt prtr}\ttarmkey{parameters}{{\tt min::printer} member}
that control
the printing process are:

\begin{itemlist}

\item[\ttmkey{line\_length}{{\tt min::printer\_parameters} member}]
...
of columns in the line to exceed the {\tt line\_length}, a break,
...
\item[\ttmkey{ indent}{{\tt min::printer\_parameters} member}]
...
\item[\ttmkey{format}{{\tt min::printer\_parameters} member}]
The {\tt format}
controls the printing of
{\tt min::gen} values by the {\tt min::pgen} function described
below.

\item[\ttmkey{flags}{{\tt min::printer\_parameters} member}]
...
\ttnbmkey{<<}{of {\tt min::printer}} operator:
...
When a horizontal tab is copied to a printer vector, it is converted
to between 1 and 8 single space characters.
...
printer analogs of {\tt std::\EOL endl} are {\tt min::\EOL eol}
(end of line) and {\tt min::\EOL eom} (end of message):

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::printer_op min::| & \MINKEY{eol}
\LABEL{MIN::EOL} \\
\verb|const min::printer_op min::| & \MINKEY{eom}
\LABEL{MIN::EOM} \\
\end{tabular}\end{indpar}
...
any ending line feed.  Characters previously flushed from the printer
vector are not included: only characters at or after the printer
{\tt flush\_\EOL offset} member are copied, and this member is then reset
to the length of the printer vector.
These functions both remove all lines copied to the {\tt ostream}
...
After copying characters, the {\tt prtr2} {\tt flush\_\EOL offset}
...
and {\tt lab\_\EOL separator} member is printed between elements.
...
\ttmkey{saved\_\EOL parameters}{of {\tt min::printer}}
...
the parameters to their normal state.
...
\item[\ttmkey{stub\_disp}{in {\tt min::packed\_struct}}]
...
\item[\ttmkey{header\_stub\_disp}{in {\tt min::packed\_vec}}]
Ditto but for \verb|min::stub *| members of \verb|H|.
See \pagref{STUB_DISP} for more details.

\item[\ttmkey{element\_stub\_disp}{in {\tt min::packed\_vec}}]
