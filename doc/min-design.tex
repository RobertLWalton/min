...
a 12-bit index.  For index general values the datum is a 24-bit index.
...
and a 20-bit index.  For index general values the datum holds a
...
The {\tt unsigned} argument used to make auxiliary pointers and indices
...
The {\tt unsigned} argument used with {\tt min::new\_special\_gen}
must fit within 24 bits.

The subtype codes used for 64-bit {\tt min::gen} direct string, stub pointer,
auxiliary pointer, index, control code, and special
values -- that is, all the non-numeric \verb|min::gen| values --
...
for optimization.  Some implementations may use a different header
format not containing the above sizes directly.  For example, an
implementation may permit only particular hash table sizes that are
listed in a table, and use 8 bits to encode a table reference, freeing
up at least 8 bits in the header for use as flags.

An implementation is not required to support all possible sizes of
hash table and other areas,
but should support sizes that permit objects that are
read-only to be compacted.  In particular, zero sized unused areas
should be supported, along with arbitrary sized attribute vector
and auxiliary area.

All sizes are \underline{in units of {\tt min::gen} values}.
...
a specific use.  They are commonly used as return or argument
...
and indices beginning with 0 are reserved for non-system users.
...
\begin{center}\begin{tabular}{r@{}l}
\verb|(macro) min::gen |
	& \TTKEY{MIN\_NEW\_SPECIAL}\verb| ( i )|
\LABEL{MIN_NEW_SPECIAL} \\
\end{tabular}\end{center}
...
\begin{indpar}\begin{verbatim}
const min::gen MISSING = MIN_NEW_SPECIAL ( 0xFFFFFF );
\end{verbatim}\end{indpar}
...
A MIN control value holds a pointer, or a pointer sized unsigned integer,
...
Thus control values contain a pointer/value low order field, and optional
type code high order field, and flag bits.  The size of the pointer/value field
varies with the hardware: it may be anything from 32 bits to 48 bits.
The type code field, if present, is always 8 bits.  The number of flag bits
varies from 8 to 32.

The control values used as stub controls do have a type code field which
can be read by \verb|min::type_of| (which is unprotected) and
written by \verb|MUP::set_type_of|.
...
\begin{indpar}
\begin{list}{}{}
\item[{\tt const min::uns64 MUP::}\MUPKEY{STUB\_POINTER}]~%
	\LABEL{MIN::STUB_POINTER}\\
Indicates that the pointer/value field of the control holds a
stub pointer (which is never in VSN form).
\end{list}
\end{indpar}
...
\\
\multicolumn{2}{l}{$M$ is the size of the pointer/value field} \\
\\
...
in a control value that are not inside the pointer/value field,
...
next executes.  Flag bits must not be inside the pointer/value field
or inside the type code field if that is used.
...
Control values can be directly manipulated by the following functions:
...
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_control} \\
    & \verb|    ( int type_code, void * p, min::uns64 flags = 0 )|
...
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_control\_pointer}\verb| ( min::uns64 c, void * p )|
\LABEL{MUP::RENEW_CONTROL_POINTER} \\
...
\verb|void * MUP::| & \MUPKEY{pointer\_of\_control}\verb| ( min::uns64 c )|
...
by inserting a new type, value, or pointer, and
the other functions return the parts of a control value.
The \verb|MUP::stub_of_control| function is the same as
\verb|MUP::pointer_of_control| with the addition of casting
its return value to `\verb|min::stub *|' for convenience.
...
\verb|min::uns64 MUP::|
    & \MUPKEY{new\_control} \\
    & \verb|    ( int type_code, void * p,|\ARGBREAK
      \verb|      min::uns64 flags = 0 )|
\LABEL{MUP::NEW_CONTROL_OF_STUB} \\
\verb|min::uns64 MUP::|
...
\LABEL{MUP::RENEW_CONTROL_VALUE} \\
\verb|min::uns64 MUP::|
    & \MUPKEY{renew\_control\_pointer}\ARGBREAK
      \verb| ( min::uns64 c, void * p )|
\LABEL{MUP::RENEW_CONTROL_POINTER} \\
\verb|int MUP::| & \MUPKEY{type\_of\_control}\verb| ( min::uns64 c )|
\LABEL{MUP::TYPE_OF_CONTROL} \\
\verb|min::uns64 MUP::| & \MUPKEY{value\_of\_control}\verb| ( min::uns64 c )|
...
\begin{TABULAR}{2.0in}
\verb|const min::uns64 MUP::| & \MUPKEY{STUB\_POINTER}
\LABEL{MIN::STUB_POINTER}\\
\end{TABULAR}
