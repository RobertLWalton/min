...
The value set pointed at by a pointer is that of the attribute pointed
...
\verb|min::NONE| or \verb|min::ANY|.  It is an error to access a value
set when the reverse attribute name is \verb|min::ANY| unless documented
otherwise.
...
as if they has empty value and flag sets.  Similarly reverse attributes
whose value sets have never been set are treated as having empty value
sets.
...
the number of values in the value set.

The 1-argument {\tt min::\EOL get} function assumes that there is exactly
one value in the value set and returns that value, or returns the special value
\verb|min::NONE| (\pagref{MIN::NONE}) if the value set is empty,
or returns the special value
\verb|min::MULTI_VALUED| (\pagref{MIN::MULTI_VALUED}) if the value set
...
the number of values in the attribute value set in the {\tt value\_count}
member, the number of control codes in the attribute flag set in
the {\tt flag\_count} member, and the number of associated
reverse attribute names which have non-empty value sets in the
...
of values in the value set of the reverse attribute pointed at.
A reverse attribute name with an empty value set is treated as if it
were not in the list of reverse attribute names.

The implementation dependent order of names may change if attribute or
reverse atttribute values are
