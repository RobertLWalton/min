...
\verb|// [0 .. 7] == |\ARGBREAK
\verb|//        (min::ustring) "\x07\x07" "MISSING",|\ARGBREAK
\verb|//        (min::ustring) "\x04\x04" "NONE",|\ARGBREAK
\verb|//        (min::ustring) "\x03\x03" "ANY",|\ARGBREAK
\verb|//        (min::ustring) "\x0C\x0C" "MULTI_VALUED",|\ARGBREAK
\verb|//        (min::ustring) "\x09\x09" "UNDEFINED",|\ARGBREAK
\verb|//        (min::ustring) "\x07\x07" "SUCCESS",|\ARGBREAK
\verb|//        (min::ustring) "\x07\x07" "FAILURE",|\ARGBREAK
\verb|//        (min::ustring) "\x05\x05" "ERROR"|
\end{tabular}\end{indpar}
...
\verb|min::trailing_vbar_pstring               // obj_braend|\ARGBREAK
\verb|min::vbar_leading_pstring                // obj_ketbegin|\ARGBREAK
...
If a line is printed immediately after a separated line,
a single space is printed before the line; otherwise the
indent is saved with \TT{min::\EOL save\_\EOL indent} and
then incremented by 4.  If a line is not a separated line,
then \TT{min::\EOL restore\_\EOL indent} and then \TT{min::\EOL bol}
are executed after the line.
...
line compact format that has at least one line element preceeding
the paragraph.  First \TT{obj\_\EOL paragraph\_\EOL begin} is
printed (e.g. `\TT{:}'), followed by the object elements which are
printed using \TT{top\_\EOL element\_\EOL format}.

Define a \key{line group} as a sequence of separated lines followed
by a non-separated line, where a non-line is treated as if it were
a 1-element line (and therefore cannot contain a paragraph).
...
correctly set.  A paragrah is itself a sequence of line groups,
and each of these is preceded by executing \TT{min::indent} to
indent the line group.

TBD

Any \TT{.type} present in bracketed compact format with an \TT{.initiator}
...
defined by the \TT{mark\_\EOL classifier}\label{MARK_CLASSIFIER}
...
class defined by the \TT{mark\_\EOL classifier} member of the object
format, then the format is
\begin{center}
\TT{\{}\TT{T} {\em <element>}
    \OPEN{} {\em ta} \TT{,} \TT{\textvisiblespace} {\em <element>} \CLOSE\STAR{}
...
E.g., if \TT{"+"} is a \TT{.type} value in the \TT{mark\_\EOL classifier} class,
...
the \TT{mark\_\EOL classifier} class,
the format is `\TT{\{<}\ldots\TT{>\}}'.
...
\verb|min::bracket_lab_format                 // lab_format|\ARGBREAK[0in]
\verb|min::bracket_special_format             // special_format|\ARGBREAK[0in]
\verb|min::isolated_line_obj_format           // obj_format|\ARGBREAK[0in]
\verb|NULL                                    // id_map_format|
\end{TABULAR}
...
\verb|min::bracket_lab_format                 // lab_format|\ARGBREAK[0in]
\verb|min::bracket_special_format             // special_format|\ARGBREAK[0in]
\verb|min::id_obj_format                      // obj_format|\ARGBREAK[0in]
\verb|NULL                                    // id_map_format|
\end{TABULAR}
