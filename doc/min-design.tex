...
stub represents an object whose contents are yet unspecified.
The preallocated stub can be
referenced as is, and then later \TT{min::\EOL stub\_\EOL swap}
can be used to fill in the contents of the object represented
by the stub.
...
This should be done only if the count of the preallocated
stub is \TT{1}, indicating that the identifier has been input
from its file only once, at the same time as the value to be
assigned to the identifier was input.  Otherwise references to the
stub that pre-exist the assignment will be dangling.

If the object represented by the preallocated stub never
has its contents filled in, references to it become
dangling.  Then the ID recorded in the stub can be used
in debugging to find the source of the dangling references.
...
Later the contents of the stub can be set by using
\TT{MUP::stub\_swap} (\itemref{SWAPPING-STUBS}) if the value
is an object, even if other data already points at the
preallocated stub.
