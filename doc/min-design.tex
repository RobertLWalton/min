...
happens, the MIN system function sets the relocation flag.

Upon return from any function call, a function that uses pointers
into bodies stored in local variables must check the relocation flag,
...
remember it has cleared the relocation flag
\end{tabular}
\end{center}

When the function returns to its caller, it must set the relocation flag
if that flag was ever turned off by the function.
...
considered to be a fatal programming error, so typical code does not
...
code when reloading a possibly relocated body pointer.
...
write these values.  Unprotected functions are provided to obtain
