...
contain stub pointers.  The first subsection of this
section specifies how this is done, and includes
data types that must be used to hold \verb|min::gen|
values is static or stack memory.
...
interface described in the following subsections.
...
that might not be, so in loose implementations less overhead need
be encountered in tracking the former.

When \verb|min::gen| values are stored in static memory, they
should be stored in \verb|min::static_gen| vectors declared
as follows:
