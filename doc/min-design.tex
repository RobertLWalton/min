...
while \TT{"100" << min::trail\-ing << "\#"} does output a space.
...
\TT{min::\EOL leading} and \TT{min::\EOL trailing} would have all
produced spaces.
...
deals with `\ikey{items}{printer item}\index{item!printer}'.  A
character string is an item, but so is a number, and so are
\TT{min::gen} values that are not labels or objects.  Labels and
objects are `\key{composite}' and output multiple items.
The default rule is that \TT{min::gen} string values are classified by

\begin{center}
\TT{printer->print\_format.gen\_format->str\_format.str\_classifer}
\end{center}

if it exists, whereas in all other cases items are classified by
...
and the whole quoted string is treated as a single item with just the
\TT{IS\_ GRAPHIC} flag.

In general items may be separated by
consecutive sequences of one \underline{or more} of the operations
