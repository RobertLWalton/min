...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::|
	& \MINKEY{list\_legal} \verb|( min::gen v )|
\LABEL{MIN::LIST_LEGAL} \\
\end{tabular}\end{indpar}
...
\verb|min::obj_vec_ptr & min::|
	& \MINMKEY{obj\_vec\_ptr\_of}{of {\tt list\_ptr}}
...
\TT{min::\EOL start\_\EOL sublist} with \TT{lp2}$\equiv$\TT{lp}).
...
in a list element (\pagref{MIN::LIST_LEGAL}).
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::|
	& \MINKEY{attr\_legal} \verb|( min::gen v )|
\LABEL{MIN::ATTR_LEGAL} \\
\end{tabular}\end{indpar}
...
(\TT{min::gen} labels as per \itemref{LABELS}).  If
partial labels are allowed, a label that is an attribute
name may only have string or number components.  Reverse
attribute names, and attribute names when partial labels are
not allowed, may be labels with components that are also labels.
...
(see \pagref{MIN::ATTR_LEGAL}) or if the pointer's reverse attribute
