...
no more elements, and is empty if it has no previous elements.
...
{\tt min::insert\_after} is an error.  The list pointer continues to point
...
be no call to {\tt min::interrupt} (\pagref{MIN::INTERRUPT}) and no
second call to {\tt min::insert\_reserve}.
...
and one is used to insert elements into a list X, or delete elements from
X, other list pointers pointing into the list X
may be affected in unspecified ways.  Other list pointers not pointing
into X will not be affected, even if they point into other lists of
the same object.
...
points at a vector of elements, one of which is E, and the last of which is
a list pointer back to the element originally after E in the list.
As a special case, insertion before the end of a list that consists of
a single element headed in the hash table or attribute vector is transformed
into insertion after the single element.  It is this case that makes
it impossible to guarentee that list pointers not pointing at E will
be unaffected by an insertion even if they point into the list that
contains E.
\end{quote}
