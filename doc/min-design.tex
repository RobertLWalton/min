...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::erase_all_space_colon_pstring       // obj_paragraph_begin|\ARGBREAK
\end{tabular}
...
attribute, it has \underline{both}, or the object has just a
\TT{.terminator} and also has a \TT{.type} equal to \TT{line\_\EOL type}.
...
the object has a \TT{.type} equal to the object format
\TT{line\_\EOL type} and the object has no \TT{.separator}.
...
object with 2 or more elements and no \TT{.terminator}
printed with line compact format.
...
\TT{.initiator} or \TT{.separator} attributes,
...
If the object has a \TT{.terminator},
it is printed after the last element and the line is called a
\key{terminated line}.

\item[\ttmkey{paragraph\_type}{in {\tt min::obj\_format}}]
If an object being printed in compact format has \TT{.type} attribute
equal to \TT{paragraph\_\EOL type}, does not have
\TT{.initiator} or \TT{.separator} attributes,
and is the last element of a non-terminated line with 2 or more
elements,
the object is printed in paragraph compact format.
See \pagref{PARAGRAPH-COMPACT-FORMAT} for details.
...
are no attributes other than \TT{.type}, \TT{.terminator},
and attributes with a \TT{hide\_\EOL flag},
...
If there is a \TT{.terminator}, it is printed just after the line.
...
In \mkey{paragraph compact format}{of printed object},%
\label{PARAGRAPH-COMPACT-FORMAT}
where there are
are no attributes other than \TT{.type}
and attributes with a \TT{hide\_\EOL flag},
the \TT{.type} is equal to \TT{paragraph\_\EOL type},
and the object is the last element of a non-terminated line printed with
line compact format and
at least one line element preceeding
the paragraph.  First \TT{obj\_\EOL paragraph\_\EOL begin}
(e.g. \TT{min::\EOL erase\_\EOL all\_\EOL space} followed by `\TT{:}') is
printed, followed by \TT{min::eol}, and then the
...
This function takes the character string input from its \TT{text\_\EOL buffer}
...
of this vector.  The \TT{flag\_\EOL numbers} vector \underline{must have}
a length
...
\end{center}
which is made by from the input \TT{text\_\EOL buffer} by replacing
...
The \TT{min::standard\_attr\_flag\_parser} flag parser parses character strings
that contain flag names taken from \TT{min::\EOL standard\_\EOL
attr\_\EOL flag\_\EOL names}.  Thus if this parser is given
the \TT{text\_\EOL buffer} input `\TT{*a(b)\_c}' the
flag numbers output would be 0, 12, 13, 14 and the
\TT{text\_\EOL buffer} output would be `\TT{(,)\_}'.

Flag parsers in general accept sequences of digits as flag numbers,
...
A new flag parser can be constructed using:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|typedef min::uns32 ( * min::| & \MINKEY{flag\_parser\_function}\ARGBREAK
    \verb|( min::uns32 * flag_numbers,|\ARGBREAK
    \verb|  char * text_buffer,|\ARGBREAK
    \verb|  const min::flag_parser * flag_parser )|
\LABEL{MIN::FLAG_PARSER_FUNCTION} \\
\verb|struct min::| & \MINKEY{flag\_parser}\ARGBREAK
    \verb|  { min::flag_parser_function flag_parser_function }|
\LABEL{MIN::FLAG_PARSER} \\
\end{tabular}\end{indpar}

First write a function whose address conforms to the
\TT{min::\EOL flag\_\EOL parser\_\EOL function} type, then
make a structure whose first element is the address of this
function, and lastly recast a pointer to this structure as having the type
`\TT{const min::\EOL flag\_\EOL parser *}'.  A call to
\TT{min::\EOL parse\_\EOL flags} is simply remapped to a call to
the function whose address is the first element of the
\TT{min::\EOL flag\_\EOL parser}.
...
\verb|min::erase_all_space_colon_pstring       // obj_paragraph_begin|
\end{TABULAR}
