...
When the body of a stub is deallocated, the stub is given
the \minkey{DEALLOCATED}\LABEL{DEALLOCATED-TYPE} stub type code.
...
\begin{tabular}{l}
a pointer to a stub \\
a 64-bit IEEE floating point number (64 bit general value only) \\
a 28-bit integer atom (32 bit general value only) \\
a 0-5 character direct string (64 bit general value only) \\
a 0-3 character direct string (32 bit general value only) \\
an 28-bit auxilary pointer (32 bit general value only)
\end{tabular}
\end{center}

Numbers and character strings stored inside \verb|min::gen| value are
...
always stored in only one way be an implementation: if a number will fit
...
pointing at them point at the same place, and simularly
...
For 32-bit general values, an integer value consists of a high order 4-bit
...
Similarly auxilary pointers consist of a high order 4-bit subtype code
and a low order 28-bit auxilary pointer (see~\secref{AUXILARY-POINTERS}
for a definition of auxilary pointers).  A direct string consists of
...
code.  The values of this subtype code chosen to indicate direct strings,
pointers, and auxilary pointers make these values IEEE Nan (Not-a-Number)
values that would not be generated by hardware instructions.
...
whether the implementation uses addresses or VSN's in general value
that point at stubs must be made when code is compiled.  A C++ code that
is executed must be compiled to refect these decisions.

The following macros are defined to indicate these decisions:

\begin{center}
\begin{tabular}{ll}
\ttkey{MIN\_IS\_COMPACT}	& 1 if compact implemention; 0 if loose \\
\ttkey{MIN\_IS\_LOOSE}		& 1 if loose implemention; 0 if compact \\
\ttkey{MIN\_USES\_ADDRESSES}	& 1 if implemention uses addresses;
				  0 if uses VSN's \\
\ttkey{MIN\_USES\_VSNS}		& 1 if implemention uses VSN's;
				  0 if uses addresses \\
\end{tabular}
\end{center}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|int min::| & \MINKEY{is\_direct\_num}\verb| ( min::gen v )|
\LABEL{MIN::IS_DIRECT_NUM} \\
\verb|int min::| & \MINKEY{is\_direct\_str}\verb| ( min::gen v )|
\LABEL{MIN::IS_DIRECT_STR} \\
\verb|int min::| & \MINKEY{is\_aux}\verb| ( min::gen v )|
\LABEL{MIN::IS_AUX} \\
\verb|int min::| & \MINKEY{is\_stub}\verb| ( min::gen v )|
\LABEL{MIN::IS_STUB} \\
\end{tabular}\end{center}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|int float::| & \MINKEY{direct\_num\_of}\verb| ( min::gen v )|
\LABEL{MIN::DIRECT_NUM_OF} \\
...
\LABEL{MIN::STUB_OF} \\
\verb|unsigned min::| & \MINKEY{aux\_of}\verb| ( min::gen v )|
\LABEL{MIN::AUXILARY_OF} \\
\end{tabular}\end{center}
...
These protected functions check for argument range errors.
...
\verb|int min::| & \MINKEY{is\_direct\_str}\verb| ( min::gen64 v )|
\LABEL{MIN::IS_DIRECT_STR_OF_GEN64} \\
...
\verb|min::uns64 min::| & \MINKEY{direct\_str\_of}\verb| ( min::gen64 v )|
\LABEL{MIN::DIRECT_STR_OF_GEN64} \\
...
Here the {\tt min::direct\_str\_of} function merely copies the
...
\verb|min::uns64 MUP::| & \MUPKEY{direct\_str\_of}\verb| ( min::gen64 v )|
\LABEL{MUP::DIRECT_STR_OF_GEN64} \\
...
\verb|min::gen64 min::| &
	\MINKEY{new\_direct\_str\_gen64}\verb| ( const char * p )|
\LABEL{MIN::NEW_DIRECT_STR_GEN64} \\
...
types.
The reason the direct string function has the name it does is that
{\tt new\_gen64} names a function that will create non-direct strings if
the string is too long to store in a direct string.
...
\verb|min::gen64 MUP::| &
	\MUPKEY{new\_direct\_str\_gen64}\verb| ( const char * p )|
\LABEL{MUP::NEW_DIRECT_STR_GEN64} \\
...
\begin{center}\begin{tabular}{r@{}l}
\verb|const unsigned min::| & \MINKEY{GEN64\_DIRECT\_STR}
\LABEL{MIN::GEN64_DIRECT_STR} \\
...
\verb|min::float64 min::| & \MINKEY{number\_of}\verb| ( min::gen32 v )|
\LABEL{MIN::NUMBER_OF_GEN32} \\
\verb|min::float64 min::| & \MINKEY{number\_of}\verb| ( min::gen64 v )|
\LABEL{MIN::NUMBER_OF_GEN64} \\
\end{tabular}\end{center}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{strlen}\verb| ( min::gen32 v )|
\LABEL{MIN::STRLEN_OF_GEN32} \\
\verb|unsigned min::| & \MINKEY{strlen}\verb| ( min::gen64 v )|
\LABEL{MIN::STRLEN_OF_GEN64} \\
\verb|unsigned min::| & \MINKEY{strhash}\verb| ( min::gen32 s )|
\LABEL{MIN::STRHASH_OF_GEN32} \\
\verb|unsigned min::| & \MINKEY{strhash}\verb| ( min::gen64 s )|
\LABEL{MIN::STRHASH_OF_GEN64} \\
\verb|min::uns64 min::| & \MINKEY{strsignature}\verb| ( min::gen32 s )|
\LABEL{MIN::STRSIGNATURE_OF_GEN32} \\
\verb|min::uns64 min::| & \MINKEY{strsignature}\verb| ( min::gen64 s )|
\LABEL{MIN::STRSIGNATURE_OF_GEN64} \\
\verb|char * min::| & \MINKEY{strcpy}\verb| ( char * p, min::gen32 s )|
\LABEL{MIN::STRCPY_OF_GEN32} \\
\verb|char * min::| & \MINKEY{strcpy}\verb| ( char * p, min::gen64 s )|
\LABEL{MIN::STRCPY_OF_GEN64} \\
\verb|char * min::| &
	\MINKEY{strncpy}\verb| ( char * p, min::gen32 s, unsigned n )|
\LABEL{MIN::STRNCPY_OF_GEN32} \\
\verb|char * min::| & \MINKEY{strncpy}\verb| ( char * p, min::gen64 s, unsigned n )|
...
argument, these functions take a {\tt min::gen32} or {\tt min::gen64}
source argument.
The {\tt min::gen64} source argument value can be a direct string, or
a stub pointer to either a short or long string stub.
The {\tt min::gen32} source argument value can be
a stub pointer to either a short or long string stub.
...
\begin{center}\begin{tabular}{r@{}l}
\verb|int min::| & \MINKEY{is\_str}\verb| ( min::gen32 v )|
\LABEL{MIN::IS_STR_OF_GEN32} \\
\verb|int min::| & \MINKEY{is\_str}\verb| ( min::gen64 v )|
\\
\verb|min::gen32 min::| &
    \MINKEY{new\_str\_gen32\REL} \verb| ( const char * p )|
\LABEL{MIN::NEW_STR_GEN32} \\
\verb|min::gen64 min::| &
    \MINKEY{new\_str\_gen64\REL} \verb| ( const char * p )|
\\
\end{tabular}\end{center}
...
attribute values.  Double arrows are handled specially: see
\secref{DOUBLE-ARROWS}.  The part of the hash table that maps small
integer attribute names is actually a vector.
...
no difference between a short and a long object.
...
\verb|char * min::| & \MINKEY{strncpy}\ARGBREAK
                      \verb| ( char * p,|\ARGBREAK
		      \verb|   min::gen32 s, unsigned n )|
\LABEL{MIN::STRNCPY_OF_GEN32} \\
\verb|char * min::| & \MINKEY{strncpy}\ARGBREAK
		      \verb| ( char * p,|\ARGBREAK
		      \verb|   min::gen64 s, unsigned n )|
\LABEL{MIN::STRNCPY_OF_GEN64} \\[1ex]
