...
return values (and \underline{cannot} be used as object attribute values) are:
...
Auxilarly values that are used to build lists cannot be stored as
...
Only auxilarly general values (with subtypes \TT{GEN\_...\_AUX})
...
new value of the element may be \TT{min::\EOL LIST\_\EOL END()},
...
\TT{min::\EOL EMPTY\_\EOL SUBLIST()}.  Thus \TT{min::update} can be used
...
value, or if the value argument is \TT{min::NONE()},
