...
sublists stored inside objects (\pagref{LIST-LEVEL}), but labels tend to
be reused by many object, and storing them inside each object that uses
...
This reads an intial segment of the label vector into the location
...
A hash value for a label may be computed by the following functions:

\begin{center}\begin{tabular}{r@{}l}
\verb|unsigned | & \TTKEY{labhash} \verb| ( min::stub * s )|
\LABEL{LABHASH_OF_STUB} \\
\verb|unsigned min::| & \MINKEY{labhash} \verb| ( min::gen v )|
\LABEL{MIN::LABHASH} \\
\verb|unsigned | & \TTKEY{labhash} \verb| ( min::gen * const p, unsigned n )|
\LABEL{LABHASH_OF_GEN_VECTOR} \\
\end{tabular}\end{center}
...
\begin{center}\begin{tabular}{r@{}l}
\verb|min::stub * | & \TTKEY{new\_label}%
    \verb| ( min::gen * const p, unsigned n )|
\LABEL{NEW_LABEL} \\
\verb|min::gen | & \TTKEY{new\_gen}%
    \verb| ( min::gen * const p, unsigned n )|
\LABEL{NEW_GEN_OF_LAB} \\
\end{tabular}\end{center}

and tested by the following function:

\begin{center}\begin{tabular}{r@{}l}
\verb|bool | & \MINKEY{is\_label} \verb| ( min::gen v )|
\LABEL{IS_LABEL_OF_GEN} \\
\end{tabular}\end{center}
...
value of the attribute, some object Y, the label of the attribute, and
also the label of the attribute in Y that is the other end of the double
arrow.  Double arrow labels are represented by \key{label pair} stubs.%
\footnote{Ditto for label pairs.}
