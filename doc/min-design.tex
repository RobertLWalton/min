...
value is any auxiliarly pointer (i.e., list, sublist, etc.).
...
of any \verb|min::gen| value containing an auxiliarly pointer (list,
sublist, or indirect).
...
\begin{indpar}\begin{tabular}{@{}r@{}l@{}}
\verb|(constructor) MUP::| & \MUPKEY{resize\_body} \\
    & \verb|    ( min::stub * s,| \\
...
the value set of each attribute, but permits the existing values of an
...
Auxiliary area indeces are defined in this manner
...
(constructor)~\verb|min::|
	& \MINKEY{list\_pointer}\verb| ( min::vec_pointer & vp )|
...
The {\tt min::\EOL next} function moves forward in the current list.
The list is always pointing at a current element, whose value is
returned by the {\tt min::\EOL current} function.  If the current element
is written using another updatable or insertable list pointer,
the {\tt min::\EOL refresh} function must be used to reestablish the
current element's value, which is cached in the list pointer.
This {\tt min::\EOL refresh} function must also be used if list elements
are inserted into or removed from the object by using another
insertable list pointer, but in some such situations using
{\tt min::\EOL refresh} is not adequate and the list pointer must be
restarted (see \pagref{RESTARTING_LIST_POINTER}).
...
The {\tt min::refresh} function must be called if the object
may have been resized by the addition of an element to some part of
the object, as by use of an insertable list pointer, or if
the element the list pointer is pointing at is written, as by use
of an updatable list pointer.  It returns the current element.
In some situations in which another insertable list pointer has
been used to insert or remove an element, calling {\tt min::\EOL refresh}
is not adequate and the list pointer must be
restarted by a {\tt min::\EOL start\_\ldots} function:
see \pagref{RESTARTING_LIST_POINTER}.
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{updatable\_list\_pointer} \\
	& \verb|    ( min::updatable_vec_pointer & vp )|
...
\verb|min::gen min::|
	& \MINKEY{refresh}%
	  ...
	  \verb| ( min::updatable_list_pointer & lp )| \\
	  \end{tabular}
\LABEL{MIN::REFRESH_OF_UPDATABLE_LIST_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::|
...
The {\tt min::set} function can be used to replace the
...
{\tt min::set} using a different updatable list pointer {\tt lp1},
then {\tt lp2} will not have valid current value until
{\tt min::refresh~(~lp2~)} has been executed.
...
\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{insertable\_list\_pointer} \\
	& \verb|    ( min::insertable_vec_pointer & vp )|
...
\verb|min::gen min::|
	& \MINKEY{refresh}%
	  ...
	  \verb| ( min::insertable_list_pointer & lp )| \\
	  \end{tabular}
\LABEL{MIN::REFRESH_OF_INSERTABLE_LIST_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::|
...
\verb|void min::|
	& \MINKEY{set}%
	  ...
	  \verb|   min::gen value )| \\
	  \end{tabular}
\LABEL{MIN::SET_OF_INSERTABLE_LIST_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::|
...
Also, there is one function, {\tt min::set}, that can do more for
...
the current value replaced by {\tt min::set}
may be a sublist, and/or the replacing value may be
{\tt min::\EOL EMPTY\_\EOL SUBLIST}.  Thus {\tt min::set} can be used
...
{\tt min::insert\_reserve} for any list pointer
...
the object pointed at, and if a resizing occurs,
all list pointers become invalid and must be refreshed by
a call to {\tt min::refresh}.
...
Also, as noted above, you can use {\tt min::\EOL set} with an
...
create and position list pointers, and to read, set, and remove
list elements.

The {\tt min::remove} function can be used to remove
...
If compliled, the code can be enabled or disabled by setting the
following variable:
...
\emkey{value} ::= {\em atom} $|$ {\em label} $|$ {\em object}
                             $|$ {\em indirect-pointer}
\\[1ex]
\emkey{indirect-pointer} ::= {\em index} $|$ {\em label}
    $|$ {\em indirect-auxiliary}
\end{indpar}
...
When partial attribute names are supported, an attribure name is viewed
...
An \key{attribute pointer} can be used to access attribute flags and values
in an object.
...
A read-only {\tt min::\EOL attribute\_\EOL pointer}
permits read-only access to read attribute values and flags.
An updatable {\tt min::\EOL updatable\_\EOL attribute\_\EOL pointer}
...
An insertable {\tt min::\EOL insertable\_\EOL attribute\_\EOL pointer}
permits read-write access to attribute values and flags
and also permits values to be added or removed from attributes.

The functions for using a read-only {\tt min::\EOL attribute\_\EOL pointer}
are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{attribute\_pointer}\verb| ( min::vec_pointer & vp )|
\LABEL{MUP::ATTRIBUTE_POINTER_OF_VEC_POINTER} \\
\verb|min::gen min::|
	& \MINKEY{locate}%
	  \begin{tabular}[t]{@{}l@{}}
...
\verb|min::gen min::|
	& \MINKEY{locate}%
	  ...
	  \end{tabular}
\LABEL{MIN::LOCATE_PARTIAL_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locate\_reverse}%
	  ...
	  \end{tabular}
\LABEL{MIN::LOCATE_REVERSE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{relocate}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::attribute_pointer & ap )| \\
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{count}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::attribute_pointer ap )| \\
    \end{tabular}
\LABEL{MIN::COUNT_OF_ATTRIBUTE} \\
\verb|min::unsptr min::| & \MINKEY{get}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, min::unsptr n,| \\
    \verb|   min::attribute_pointer ap )| \\
    \end{tabular}
\LABEL{MIN::GET_OF_ATTRIBUTE} \\
\verb|min::gen min::| & \MINKEY{get}%
    \verb| ( min::attribute_pointer ap )|
\LABEL{MIN::GET1_OF_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
\verb|unsigned min::| & \MINKEY{count\_flags}%
    ...
    \verb| ( min::attribute_pointer ap )| \\
    \end{tabular}
\LABEL{MIN::COUNT_FLAGS_OF_ATTRIBUTE} \\
\verb|unsigned min::| & \MINKEY{get\_flags}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, unsigned n,| \\
    \verb|   min::attribute_pointer ap )| \\
    \end{tabular}
\LABEL{MIN::GET_FLAGS_OF_ATTRIBUTE} \\
\verb|bool min::| & \MINKEY{test\_flag}\ARGBREAK
    \verb| ( min::attribute_pointer wap,|\ARGBREAK
    \verb|   unsigned n )|
\LABEL{MIN::TEST_FLAG_OF_ATTRIBUTE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{get\_attributes}\ARGBREAK
...
\verb|min::unsptr min::| & \MINKEY{get\_reverse\_attributes}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK
    \verb|   min::attribute_pointer ap )|
...
{\tt min::\EOL locate} and if necessary {\tt min::\EOL locate\_reverse} functions, or by
calling the {\tt min::\EOL relocate} function.  This last function uses the
...
The {\tt min::\EOL count} function returns the number of values in the value
set pointed at.
The 3-argument {\tt min::\EOL get} function gets these values and stores them
in the `{\tt out}' vector, and returns the number of values stored.
The argument {\tt n} is the
...
one value and returns that value, or returns the special value
\verb|min::NONE| (\pagref{MIN::NONE}) if there are no values,
or returns the special value
\verb|min::MULTI_VALUED| (\pagref{MIN::MULTI_VALUED}) if there is
more than one value.

The {\tt min::\EOL count\_\EOL flags}
function returns the length of the smallest
...
function returns these flags in the \verb|min::gen|
vector `{\tt out}'.  The argument {\tt n} is the maximum number of
vector elements that may be written, the length of `{\tt out}'.
The return value is the same as that of {\tt count\_\EOL flags} if this is
...
dependent fashion.  The current attribute name of an attribute
pointer is a pointer into this list.
The {\tt min::\EOL get\_\EOL attributes} function gets up to {\tt n}
of these attribute names, starting at the current attribute name.
The names are returned as {\em atoms} or {\em labels} in the `{\tt out}'
vector, and the number of names returned is returned from the function.
...
After the last name is returned the pointer is put in a special state
where it has no attribute name and calling
{\tt min::\EOL get\_\EOL attributes} again will return nothing but {\tt 0}.
The {\tt min::\EOL rewind\_\EOL attributes} function resets the pointer
to point at the first name.  Setting the attribute pointer to point
at an attribute name is the equivalent of calling {\tt min::\EOL locate}
to set the current attribute name of the pointer.

The {\tt min::\EOL get\_\EOL reverse\_\EOL attributes}
...
pointer.  However in this case there is no rewind function, and
instead setting the reverse attribute name to \verb|min::NONE|
using {\tt min::\EOL locate\_\EOL reverse} has the effect of setting
the pointer so the next call to
{\tt min::\EOL get\_\EOL reverse\_\EOL attributes} will get the first
reverse name in the list.

An attribute name or reverse attribute name is returned
only if its value set is non-empty or it has a set flag.

The implementation dependent order of names may change if values are
added to or removed from the object or attribute flags are set or cleared,
so after any of these actions the 
{\tt min::\EOL rewind\_\EOL attributes} function should be called before
any names are gotten.
...
{\tt min::\EOL updatable\_\EOL attribute\_\EOL pointer}
are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{updatable\_attribute\_pointer}\ARGBREAK
	  \verb| ( min::updatable_vec_pointer & vp )|
\LABEL{MUP::UPDATABLE_ATTRIBUTE_OF_UPDATABLE_VEC_POINTER} \\
\verb|min::gen min::|
	& \MINKEY{locate}\ARGBREAK
	  \verb| ( min::updatable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::gen name )|
\LABEL{MIN::LOCATE_ATTRIBUTE_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locatei}\ARGBREAK
	  \verb| ( min::updatable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   int name )|
\LABEL{MIN::LOCATEI_ATTRIBUTE_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locate}\ARGBREAK
	  \verb| ( min::updatable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::unsptr & length, min::gen name )|
\LABEL{MIN::LOCATE_PARTIAL_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locate\_reverse}\ARGBREAK
	  \verb| ( min::updatable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::gen reverse_name )|
\LABEL{MIN::LOCATE_REVERSE_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{relocate}\ARGBREAK
	  \verb| ( min::updatable_attribute_pointer & ap )|
\LABEL{MIN::RELOCATE_ATTRIBUTE_OF_UPDATABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{count}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer ap )|
\LABEL{MIN::COUNT_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::unsptr min::| & \MINKEY{get}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK 
    \verb|   min::updatable_attribute_pointer ap )|
\LABEL{MIN::GET_OF_UPDATABLE_ATTRIBUTE} \\
\verb|min::gen min::| & \MINKEY{get}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer ap )|
\LABEL{MIN::GET1_OF_UPDATABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{count\_flags}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer ap )|
\LABEL{MIN::COUNT_FLAGS_OF_UPDATABLE_ATTRIBUTE} \\
\verb|unsigned min::| & \MINKEY{get\_flags}\ARGBREAK
    \verb| ( min::gen * out, unsigned n,|\ARGBREAK
    \verb|   min::updatable_attribute_pointer ap )|
\LABEL{MIN::GET_FLAGS_OF_UPDATABLE_ATTRIBUTE} \\
\verb|bool min::| & \MINKEY{test\_flag}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer wap,|\ARGBREAK
    \verb|   unsigned n )|
\LABEL{MIN::TEST_FLAG_OF_UPDATABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
\verb|min::unsptr min::| & \MINKEY{get\_attributes}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK
    \verb|   min::updatable_attribute_pointer ap )|
\LABEL{MIN::GET_ATTRIBUTES_OF_UPDATABLE} \\
\verb|void min::| & \MINKEY{rewind\_attributes}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer ap )|
\LABEL{MIN::REWIND_ATTRIBUTES_OF_UPDATABLE} \\
\verb|min::unsptr min::| & \MINKEY{get\_reverse\_attributes}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK
    \verb|   min::updatable_attribute_pointer ap )|
\LABEL{MIN::GET_REVERSE_ATTRIBUTES_OF_UPDATABLE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{update}\ARGBREAK
    \verb| ( min::updatable_attribute_pointer wap,|\ARGBREAK
    \verb|   const min::gen v )|
\LABEL{MIN::UPDATE_OF_UPDATABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
value set have a single value, which this function replaces.
It is an error if the value set is empty or has more than one
value.
...
The functions for using a
{\tt min::\EOL insertable\_\EOL attribute\_\EOL pointer}
are:

\begin{indpar}\begin{tabular}{r@{}l}
(constructor)~\verb|MUP::|
	& \MUPKEY{insertable\_attribute\_pointer}\ARGBREAK
	  \verb| ( min::insertable_vec_pointer & vp )|
\LABEL{MUP::INSERTABLE_ATTRIBUTE_OF_INSERTABLE_VEC_POINTER} \\
\verb|min::gen min::|
	& \MINKEY{locate}\ARGBREAK
	  \verb| ( min::insertable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::gen name )|
\LABEL{MIN::LOCATE_ATTRIBUTE_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locatei}\ARGBREAK
	  \verb| ( min::insertable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   int name )|
\LABEL{MIN::LOCATEI_ATTRIBUTE_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locate}\ARGBREAK
	  \verb| ( min::insertable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::unsptr & length, min::gen name )|
\LABEL{MIN::LOCATE_PARTIAL_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{locate\_reverse}\ARGBREAK
	  \verb| ( min::insertable_attribute_pointer & ap,|\ARGBREAK
	  \verb|   min::gen reverse_name )|
\LABEL{MIN::LOCATE_REVERSE_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::gen min::|
	& \MINKEY{relocate}\ARGBREAK
	  \verb| ( min::insertable_attribute_pointer & ap )|
\LABEL{MIN::RELOCATE_ATTRIBUTE_OF_INSERTABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{count}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer ap )|
\LABEL{MIN::COUNT_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::unsptr min::| & \MINKEY{get}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK 
    \verb|   min::insertable_attribute_pointer ap )|
\LABEL{MIN::GET_OF_INSERTABLE_ATTRIBUTE} \\
\verb|min::gen min::| & \MINKEY{get}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer ap )|
\LABEL{MIN::GET1_OF_INSERTABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}

\begin{indpar}\begin{tabular}{r@{}l}
\verb|unsigned min::| & \MINKEY{count\_flags}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer ap )|
\LABEL{MIN::COUNT_FLAGS_OF_INSERTABLE_ATTRIBUTE} \\
\verb|unsigned min::| & \MINKEY{get\_flags}\ARGBREAK
    \verb| ( min::gen * out, unsigned n,|\ARGBREAK
    \verb|   min::insertable_attribute_pointer ap )|
\LABEL{MIN::GET_FLAGS_OF_INSERTABLE_ATTRIBUTE} \\
\verb|bool min::| & \MINKEY{test\_flag}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer wap,|\ARGBREAK
    \verb|   unsigned n )|
\LABEL{MIN::TEST_FLAG_OF_INSERTABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
\verb|min::unsptr min::| & \MINKEY{get\_attributes}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK
    \verb|   min::insertable_attribute_pointer ap )|
\LABEL{MIN::GET_ATTRIBUTES_OF_INSERTABLE} \\
\verb|void min::| & \MINKEY{rewind\_attributes}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer ap )|
\LABEL{MIN::REWIND_ATTRIBUTES_OF_INSERTABLE} \\
\verb|min::unsptr min::| & \MINKEY{get\_reverse\_attributes}\ARGBREAK
    \verb| ( min::gen * out, min::unsptr n,|\ARGBREAK
    \verb|   min::insertable_attribute_pointer ap )|
\LABEL{MIN::GET_REVERSE_ATTRIBUTES_OF_INSERTABLE} \\
\end{tabular}\end{indpar}
...
\verb|void min::| & \MINKEY{update}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer wap,|\ARGBREAK
    \verb|   const min::gen v )|
\LABEL{MIN::UPDATE_OF_INSERTABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
\LABEL{MIN::SET_OF_INSERTABLE_ATTRIBUTE} \\
\verb|void min::| & \MINKEY{set\REL}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer wap,|\ARGBREAK
    \verb|   const min::gen v )|
\LABEL{MIN::SET1_OF_INSERTABLE_ATTRIBUTE} \\
\end{tabular}\end{indpar}
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{remove\_one}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer wap,|\ARGBREAK
    \verb|   const min::gen * in, min::unsptr n )|
...
\verb|void min::| & \MINKEY{remove\_all}\ARGBREAK
    \verb| ( min::insertable_attribute_pointer wap,|\ARGBREAK
    \verb|   const min::gen * in, min::unsptr n )|
...
The {\tt min::\EOL set}
function sets all the values in the value set the
...
in which case there are no new value and all previous values are simply
deleted.
...
adds values to the multiset set of value,
but adds each value if and only if the value is not already in the multiset set,
using {\tt ==} to compare values for equality.
The {\tt min::\EOL add\_\EOL to\_\EOL multiset} function
adds values even if they are already in the multiset set.
...
from the multiset of values (it is less efficient if
...
The {\tt min::\EOL set\_\EOL flags} function sets the flags of the flag set the attribute
pointer points at.
The flags are given in the `{\tt in}' vector which
has length {\tt n}: see the discussion of
{\tt min::\EOL get\_\EOL flags} on \pagref{MIN::GET_FLAGS_OF_ATTRIBUTE}
for the format of this vector.
All flags are zeroed before the new set of flags
...
\begin{TABULAR}{1.8in}
\verb|(constructor) MUP::| & \MUPKEY{resize\_body}\ARGBREAK
...
\LABEL{MIN::CURRENT_OF_LIST_POINTER} \\
\verb|min::gen min::|
	& \MINKEY{refresh}\verb| ( min::list_pointer & lp )|
\LABEL{MIN::REFRESH_OF_LIST_POINTER} \\
\end{TABULAR}
...
\begin{TABULAR}{1.6in}
(constructor)~\verb|min::|
	& \MINKEY{updatable\_list\_pointer}\ARGBREAK
	  \verb| ( min::updatable_vec_pointer & vp )|
\LABEL{MIN::UPDATABLE_LIST_POINTER_OF_UPDATABLE_VEC_POINTER} \\
\multicolumn{2}{@{}l}{\tt min::updatable\_vec\_pointer \&} \\
\verb|min::|
...
\begin{TABULAR}{1.4in}
(constructor)~\verb|min::|
	& \MINKEY{insertable\_list\_pointer}\ARGBREAK
	  \verb| ( min::insertable_vec_pointer & vp )|
\LABEL{MIN::INSERTABLE_LIST_POINTER_OF_INSERTABLE_VEC_POINTER} \\
\multicolumn{2}{@{}l}{\tt min::insertable\_vec\_pointer \&} \\
\verb|min::|
