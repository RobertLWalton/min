...
and pops.  All auxiliary pointers in the object will
be adjusted by adding to each the new total size minus the old total size.
Otherwise no change is made to object contents, outside the unused
...
is not too large, that the auxiliary pointer \verb|p| is within the
auxiliary area,
that the unused area
is non-empty for pushes, and that the attribute vector or auxiliary area
is non-empty for pops,
and for set and push operation
...
An implementation is not required to support all possible value of
...
\begin{TABULAR}{1.2in}
\verb|void min::| & \MINKEY{resize\REL}\ARGBREAK
      \verb| ( min::insertable_vec_pointer & vp,|\ARGBREAK
      \verb|   min::unsptr unused_size )|
\LABEL{MIN::RESIZE_UNUSED_OF_INSERTABLE_VEC_POINTER} \\
\verb|void min::| & \MINKEY{resize\REL}\ARGBREAK
