...
\multicolumn{2}{l}{\tt struct
    ...
    \verb|{|\ARGBREAK
    \verb|  min::uns32    unquote_of_first;|%
\ttmindex{unquote\_if\_first}{in {\tt min::quote\_control}}\ARGBREAK
    \verb|  min::uns32    unquote_of_none_of;|%
...
string is quoted.  Otherwise if the first character of the string
has a flag that is in \TT{qc.unquote\_\EOL if\_\EOL first}, the string
is not quoted, and if not, the string is quoted.%
\label{UNQUOTE_IF_FIRST}  See the \TT{min::\EOL QUOTE\_\EOL SUPPRESS}
flag on \pagref{QUOTE-SUPPRESS}.
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
    \verb|  const min::ustring *                 obj_propinit;|%
\ttmindex{obj\_propinit}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  const min::ustring *                 obj_propeol;|%
\ttmindex{obj\_propeol}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  const min::ustring *                 obj_propsep;|%
\ttmindex{obj\_propsep}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  const min::ustring *                 obj_propneg;|%
\ttmindex{obj\_propneg}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  const min::ustring *                 obj_propeq;|%
\ttmindex{obj\_propeq}{in {\tt min::obj\_format}}\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|(const min::ustring *) "\x02\x82" ": "   // obj_propinit|\ARGBREAK
\verb|(const min::ustring *) "\x01\x81" ":"    // obj_propeol|\ARGBREAK
\verb|(const min::ustring *) "\x02\x82" ", "   // obj_propsep|\ARGBREAK
\verb|(const min::ustring *) "\x03\x03" "no "  // obj_propneg|\ARGBREAK
\verb|(const min::ustring *) "\x03\x03" " = "  // obj_propeq|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|NULL                                     // obj_propinit|\ARGBREAK
\verb|(const min::ustring *) "\x01\x81" ":"    // obj_propeol|\ARGBREAK
\verb|NULL                                     // obj_propsep|\ARGBREAK
\verb|(const min::ustring *) "\x03\x03" "no "  // obj_propneg|\ARGBREAK
\verb|(const min::ustring *) "\x03\x03" " = "  // obj_propeq|\ARGBREAK
\end{tabular}
...
object elements and properties in the absence of object
\TT{.initiator} and \TT{.terminator} attributes.
In this case \TT{obj\_\EOL bra} followed optionally by the
object's \TT{.type} and properties followed by
...
followed optionally by the object \TT{.type} and properties
followed by \TT{obj\_\EOL ket} serves as the closing bracket
for the object.  See text below for more details.

\item[\ttmkey{obj\_propinit}{in {\tt min::obj\_format}}]~\\
\item[\ttmkey{obj\_propeol}{in {\tt min::obj\_format}}]%
\vspace{-5ex}~\\
\item[\ttmkey{obj\_propsep}{in {\tt min::obj\_format}}]%
\vspace{-5ex}~\\
\item[\ttmkey{obj\_propneg}{in {\tt min::obj\_format}}]%
\vspace{-5ex}~\\
\item[\ttmkey{obj\_propeq}{in {\tt min::obj\_format}}]%
\vspace{-5ex}~\\
These are the \TT{min::ustring}'s printed to introduce
an object property list, separate properties, indicate
negation of properties whose value is \TT{FALSE}, and
separator a property label from its value.
\TT{Obj\_\EOL propbegin} is used to begin a property
list that is not an indented paragraph, whereas
\TT{obj\_\EOL propeol} is used just before an end-of-line
to begin a property list that is an indented
paragraph.  \TT{Obj\_\EOL propsep} is used to separate
properties when these are \underline{not} in an indented
paragraph; \TT{obj\_\EOL propeq} separates a property
label from the property's value; and \TT{obj\_\EOL propneg}
is the property label prefix used to indicate a property
