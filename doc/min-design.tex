...
\subsubsection{Classifying Characters and Strings}
\label{CLASSIFYING-CHARACTERS-AND-STRINGS}

The inline function:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::uns32 min::| & \MINKEY{char\_flags}\ARGBREAK
      \verb|( const min::char_flags * char_flags,|\ARGBREAK
      \verb|  min::support_control sc,|\ARGBREAK
      \verb|  min::Uchar c )|
\LABEL{MIN::CHAR_FLAGS} \\
\end{tabular}\end{indpar}

returns the character flags \TT{cflags} of the UNICODE character
...
the returned number of columns; but if it is
zero, the string may contain other characters, such as single spaces,
...
a string is in the specified class if:
\begin{indpar}
(1) \label{STR_CLASSIFIER} \underline{Every} character in the string has some
...
true unless \TT{n == 0}.
\end{indpar}
Note that the empty string is never in the specified class.
Also see the \TT{min::\EOL QUOTE\_\EOL SUPPRESS}
...
class (\pagref{STR_CLASSIFIER})
defined by the \TT{marking\_\EOL type} member of the object
