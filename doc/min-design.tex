...
\verb|T & p| & \TTBMKEY{i}{of {\tt min::ptr}}
    \hspace*{1.0in} [if \verb|T| \underline{not} locatable]
\LABEL{MIN::PTR_[]} \\
\verb|min::ref<T> p| & \TTBMKEY{i}{of {\tt min::ptr}}
    \hspace*{1.0in} [if \verb|T| \underline{is} locatable]
...
overloading it with \verb|char[8]|, as in the code:
...
characters, the encoding of the second may be truncated after the 8'th
byte.
...
Here \verb|line| is the line number of the line containing the position
(the first line is line number \verb|0|), \verb|offset| is the
number of bytes before the position in the line within the
file, and \verb|column| is the number of print columns
before the position in the printed version of the line if
printing is done as per {\tt min::\EOL print\_\EOL line}.
Note that \verb|column| is partly determined by the
{\tt print\_\EOL flags} of the file passed to {\tt min::\EOL print\_\EOL line},
so if positions are computed assuming one value of
{\tt print\_\EOL flags}, the \verb|column| values in these positions will
not be valid for a different value of {\tt print\_\EOL flags}.

If the positions of the beginning and ending of an erroneous text in
a file are known, then in place of the above \verb|for| loop with its calls
to {\tt min::\EOL print\_\EOL line}, one can use the function:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::uns32 min::|
    & \MINKEY{print\_\EOL item\_\EOL lines\REL}\ARGBREAK
      \verb| ( min::printer printer,|\ARGBREAK
      \verb|   min::file file,|\ARGBREAK
      \verb|   const min::position & begin,|\ARGBREAK
      \verb|   const min::position & end,|\ARGBREAK
      ...
      \verb|   const char * unavailable_line =|\ARGBREAK
      \verb|       "<UNAVAILABLE-LINE>" )|
\LABEL{MIN::PRINT_ITEM_LINES} \\
\end{tabular}\end{indpar}

Here the `item' is the erroneous text,
the \verb|begin| argument is the position of the first
character of the item, the \verb|end| argument is the position
just \underline{after} the last character of the item, the
lines containing the item are printed, and under each item
...
\begin{indpar}\begin{tabular}{r@{}l@{\hspace*{1.4in}}l}
\verb|min::ref<min::gen> |
	& \TTOMKEY{[ ]}{[ ]}{of {\tt min::vec\_updptr}}
	& [same as {\tt min::attr}]\\&
	  \multicolumn{2}{l}{~(~const min::vec\_updptr \& vp,}\\&
	  \multicolumn{2}{l}{~~~min::unsptr index )}
\LABEL{MIN::[]_OF_VEC_UPDPTR} \\
\end{tabular}\end{indpar}
...
\begin{TABULAR}{1.6in}
\verb|min::gen |
	& \TTOMKEY{[ ]}{[ ]}{of {\tt min::vec\_ptr}}\ARGBREAK
	  \verb| ( const min::vec_ptr & vp,|\ARGBREAK
	  \verb|   min::unsptr index )|
\LABEL{MIN::[]_OF_VEC_PTR} \\
\verb|min::unsptr min::| & \MINKEY{size\_of}
          \verb|( min::obj_vec_ptr & vp )|
...
\begin{TABULAR}{1.7in}
\verb|min::ref<min::gen> |
	& \TTOMKEY{[ ]}{[ ]}{of {\tt min::vec\_updptr}}\ARGBREAK
	  \verb| ( const min::vec_updptr & vp,|\ARGBREAK
	  \verb|   min::unsptr index )|
\LABEL{MIN::[]_OF_VEC_UPDPTR} \\
\end{TABULAR}
