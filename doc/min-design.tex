...
\underline{not} allocated by these functions, so the {\tt int}
...
\LABEL{MIN::GEN_STUB} \\
\verb|const unsigned min::| & \MINKEY{GEN\_AUX}
\LABEL{MIN::GEN_AUX} \\
...
20 bits of the value, and then zeros the retrieved value if it
indicates a direct IEEE 64-bit floating point number.
For 32-bit \verb|min::gen| values, this retrieves
the high order 8 bits, and then zeros the low order 4 bits of that if
the 8 bit value is less than \verb|min::GEN_DIRECT_STR|.
...
The \verb|min::new_gen| function for 64-bit \verb|min::gen| values
is just another name for {\tt min::new\_\EOL direct\_\EOL gen}, which
simply changes the type of its argument.
For 32-bit \verb|min:gen| values the function creates a direct integer
...
The {\tt min::gen} source argument value can be a direct string, or
...
\begin{center}\begin{tabular}{r@{}l}
\verb|void | &
    \TTKEY{putg}\verb| ( min::stub * s, min::gen name, min::gen value )|
\LABEL{PUTG_TO_STUB} \\
\verb|void min::| &
    \MINKEY{putg}\verb| ( min::gen obj, min::gen name, min::gen value )|
\LABEL{MIN::PUTG} \\
\verb|void | &
    \TTKEY{puti}\verb| ( min::stub * s, int index, min::gen value )|
\LABEL{PUTI_TO_STUB} \\
\verb|void min::| &
    \MINKEY{puti}\verb| ( min::gen obj, int index, min::gen value )|
\LABEL{MIN::PUTI} \\
\end{tabular}\end{center}
...
object it the attribute previously had a value.  It is never an error
