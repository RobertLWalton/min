...
\verb|min::ref<T> const & |
    & \TTOMKEY{=}{=}{of {\tt min::ref<T>}}\ARGBREAK
      \verb|( const min::ref<T> & r, V value )| \\
    & where \TT(V) values are convertable to \TT(T) values
\LABEL{MIN::=REF_OF_T} \\
\verb|min::ref<T> const & |
    & \TTOMKEY{=}{=}{of {\tt min::ref<T>}}\ARGBREAK
...
This is because
the \TT{->} operator has been defined for \TT{min::\EOL ptr<T>}
types to return \TT{\&loc}, but for \TT{min::\EOL ref<T>} types
...
The following permit a \TT{min::\EOL ptr<T>} equivalent of the
\TT{NULL} value:

\begin{indpar}\begin{tabular}{r@{}l@{~~~~}l}
\verb|bool |
    & \TTOMKEY{==}{==}{of {\tt min::ptr<T>}}\ARGBREAK
      \verb|( const min::ptr<T> & p1, const min::ptr<T> & p2 )|
\LABEL{MIN::==_OF_PTR_OF_T} \\
\verb|bool |
    & \TTOMKEY{!=}{!=}{of {\tt min::ptr<T>}}\ARGBREAK
      \verb|( const min::ptr<T> & p1, const min::ptr<T> & p2 )|
