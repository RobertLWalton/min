...
which can be tested by AND'ing the unsigned flags value with
the \TT{min::USTRING\_LEADING/TRAILING} or
\TT{min::USTRING\_LEADING/TRAILING\_\EOL ALWAYS} flags.
The flags can be set into a \TT{ustring} by adding them to the first
or second byte, as in \TT{"\textbackslash x81\textbackslash x41"~","}
which prints as
\begin{center}
...
\verb|(const min::ustring *) "\x01\x01" " "    // lab_separator|\ARGBREAK
