...
\verb|min::gen min::| & \MINKEY{line\_feed}
		      & \verb|= min::new_str_gen ( "\n" )|
\LABEL{MIN::LINE_FEED} \\
\verb|min::gen min::| & \MINKEY{stx}
		      & \verb|= min::new_str_gen ( "\02" )|
\LABEL{MIN::STX} \\
\verb|min::gen min::| & \MINKEY{etx}
		      & \verb|= min::new_str_gen ( "\03" )|
\LABEL{MIN::ETX} \\
...
converts to \TT{true} if it does not equal \TT{min::\EOL MISSING\_\EOL POSITION}
and to \TT{false} otherwise.
...
\ttmindex{quote\_type}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  min::gen                             line_initiator;|%
\ttmindex{line\_initiator}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  min::gen                             paragraph_terminator;|%
\ttmindex{paragraph\_terminator}{in {\tt min::obj\_format}}
\end{tabular}
...
\verb|min::NONE()                              // line_initiator|\ARGBREAK
\verb|min::NONE()                              // paragraph_terminator|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::stx                                 // line_initiator|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::etx                                 // paragraph_terminator|\ARGBREAK
\end{tabular}
...
\begin{enumerate}
\item The `\key{line compact format}' is used if
the object has an \TT{.initiator} equal to the object format
\TT{line\_\EOL initiator} and the object has no \TT{.separator}.

\item Otherwise the `\key{paragraph compact format}' is used if
the object has a \TT{.terminator} equal to the object format
\TT{paragraph\_\EOL terminator}, the object has no \TT{.separator},
and the object is being printed as the last element of a containing
object with 2 or more elements and \TT{.terminator} equal to
\TT{"<LF>"} printed with line compact format.
...
\item[\ttmkey{line\_initiator}{in {\tt min::obj\_format}}]
If an object being printed in compact format has \TT{.initiator} attribute
equal to \TT{line\_\EOL initiator} and does not have a
...
See \pagref{LINE-COMPACT-FORMAT} for details.

If the object has a \TT{.terminator} not equal to \TT{"<LF>"},
the terminator is printed after the last element and the line is called a
\key{terminated line}.

\item[\ttmkey{paragraph\_terminator}{in {\tt min::obj\_format}}]
If an object being printed in compact format has \TT{.terminator} attribute
equal to \TT{paragraph\_\EOL terminator}, does not have a
\TT{.separator} attribute,
and is the last element of a non-terminated line with 2 or more
elements being printed in line compact format,
...
In \mkey{line compact format}{of printed object},%
\label{LINE-COMPACT-FORMAT}
where there are
are no attributes other than \TT{.initiator}, \TT{.terminator},
and attributes with a \TT{hide\_\EOL flag},
with \TT{.initiator} being equal to \TT{line\_\EOL initiator},
the object elements are just printed using
...
are no attributes other than \TT{.initiator}, \TT{.terminator},
and attributes with a \TT{hide\_\EOL flag},
the \TT{.terminator} is equal to \TT{paragraph\_\EOL terminator},
and the object is the last element of a non-terminated line printed with
line compact format which has
...
\LABEL{MIN::OBJ_FORMAT}\ARGBREAK[0in]
    ...
    \verb|  min::gen                             line_initiator;|\ARGBREAK[0in]
    \verb|  min::gen                             paragraph_terminator;|\ARGBREAK[0in]
...
\verb|min::NONE()                              // line_initiator|\ARGBREAK[0in]
\verb|min::NONE()                              // paragraph_terminator|\ARGBREAK[0in]
\end{TABULAR}
...
\verb|min::stx                                 // line_initiator|\ARGBREAK[0in]
\end{TABULAR}
...
\verb|min::null_str_classifier                 // mark_classifier|\ARGBREAK[0in]
\verb|min::etx                                 // paragraph_terminator|\ARGBREAK[0in]
\end{TABULAR}
