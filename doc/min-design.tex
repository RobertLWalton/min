...
values must be stored in locations of type
...
\verb|min::gen |
    & \TTOMKEY{min::gen}{min::gen}%
      ...
      \verb|( const min::locatable_gen & location )|
\LABEL{MIN::LOCATABLE_GEN_TO_GEN_REFERENCE} \\
\end{tabular}\end{indpar}

\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|(constructor) min::| & \MINKEY{locatable\_num\_gen} \verb| ( void )|
\LABEL{MIN::LOCATABLE_NUM_GEN} \\
\verb|(constructor) min::| & \MINKEY{locatable\_num\_gen}%
    \verb|( min::gen value )|
\LABEL{MIN::LOCATABLE_NUM_GEN_OF_VALUE} \\
\verb|min::gen |
    & \TTOMKEY{=}{=}{of {\tt min::locatable\_num\_gen}}\ARGBREAK
      \verb|( min::locatable_num_gen & location,|\ARGBREAK
      \verb|  min::gen value )|
\LABEL{MIN::=LOCATABLE_NUM_GEN_OF_GEN} \\
\verb|min::gen |
    & \TTOMKEY{min::gen}{min::gen}%
              {of {\tt min::locatable\_num\_gen}}\ARGBREAK
      \verb|( const min::locatable_num_gen & location )|
\LABEL{MIN::LOCATABLE_NUM_GEN_TO_GEN_REFERENCE} \\
\end{tabular}\end{indpar}
...
The only operations that can be performed on a
{\tt min::locatable\_\EOL gen} location are to copy a {\tt min::gen}
value into it or out of it.
...
\verb|(constructor) min::|
    & \MINKEY{locatable\_ptr}\BRACKETED{T}\verb| ( void )|
\LABEL{MIN::LOCATABLE_PTR} \\
\verb|(constructor) min::|
    & \MINKEY{locatable\_ptr}\BRACKETED{T}\verb|( T value )|
\LABEL{MIN::LOCATABLE_PTR_OF_VALUE} \\
\verb|T |
    & \TTOMKEY{=}{=}{of {\tt min::locatable\_gen}}\ARGBREAK
      \verb|( min::locatable_gen<T> & location,|\ARGBREAK
      \verb|  T value )|
\LABEL{MIN::=LOCATABLE_PTR} \\
\end{tabular}\end{indpar}

Here {\tt T} must be a class whose only data member is of type
{\tt const min::stub~*}.  The {\tt min::\EOL packed\_\ldots ptr}
classes that contain pointers to packed structures and vectors satisfy
this constraint.  The class {\tt T} is a public base class of
of {\tt min::\EOL locatable\_\EOL ptr<T>}, permitting functions with
arguments of type {\tt T} to be applied to values of type
{\tt min::\EOL locatable\_\EOL ptr<T>}.

An example usage is:

\begin{indpar}\begin{verbatim}
static min::locatable_gen p;
. . . . . . . . . .
... some_function ( ... )
{
    min::locatable_gen q = p;
    . . . use p, q . . .
}

min::packed_vec_insptr<char> bufvec;
min::locatable_ptr<bufvec> x;
... some_function ( ... )
{
    min::locatable_ptr<bufvec> y = x;
    . . . use x, y . . .
}
\end{verbatim}\end{indpar}
...
there are elements not yet output.
Non-NUL elements in a complete
line are output with the printer {\tt min::\EOL ASCII\_\EOL FLAG},
{\tt min::\EOL GRAPHICS\_\EOL FLAG},
and {\tt min::\EOL AUTOBREAK\_\EOL FLAG}
turned off, and the line-terminating
NUL elements are translated into sending {\tt min::eol} to the printer.
Non-NUL elements in an incomplete
line at the end of the file {\tt buffer}
are output with just the printer {\tt min::\EOL ASCII\_\EOL FLAG},
and {\tt min::\EOL GRAPHICS\_\EOL FLAG}
turned off, and with the
{\tt min::\EOL AUTOBREAK\_\EOL FLAG} left untouched.
...
given the code:
\begin{indpar}\begin{verbatim}
min::stack_stub<2> stubs;
min::file & file1 = * (min::file *) & stubs[0];
min::file & file2 = * (min::file *) & stubs[1];
init_input ( file1 );
init_input_... ( file2, ... );
...
min::file & file = * (min::file *) & stubs[0];
init_input ( file );
...
...
\multicolumn{2}{l}{\tt const min::printer\_parameters
    ...
    \verb|  min::AUTOBREAK_FLAG              // flags|\ARGBREAK
...
appropriate point by {\tt printer << "~"}.
...
\verb|min::gen |
    & \TTOMKEY{min::gen}{min::gen}%
              {of {\tt min::locatable\_num\_gen}}\ARGBREAK
      \verb|( min::locatable_num_gen & location )|
\LABEL{MIN::LOCATABLE_NUM_GEN_TO_GEN_REFERENCE} \\
\end{TABULAR}
