...
The \TT{id} is used only for debugging; when a preallocated
stub is created from an identifier input from a file, the
ID of the identifier is recorded in the preallocated stub
in case the identifier is later defined as a non-object
value, in which case the preallocate stub will become
a dangling reference, and knowing the ID that created it
can help find the bad identifer in the file.
...
is the vector index of the vector element containing the
...
It need not be done for non-object values.
...
When an object is to be output, \TT{min::\EOL find\_\EOL and\_\EOL add}
will assign it an identifier so it can be
output as `\TT{@}{\em identifier}', even if the object contents have not
been output.  Later the object contents should be output,
usually in a line of the
form
\begin{center}
\TT{@}{\em identifier} \TT{=} {\em object}
\end{center}

In this context objects are usually output by such lines in order of
...
when an object is outputted.  Thus identifiers from
\TT{map->next} through \TT{map->length-1} are for \TT{min::gen}
values that have been assigned an identifier index but
whose contents have not yet been output.
...
If there is no value associated with \TT{id}, a stub \TT{min::gen}
...
It merely returns the identifier associated with a value \TT{g} if one exists,
or returns \TT{0} if no such identifier exists.
\TT{min::\EOL find} also returns
\TT{0} if \TT{s} is \TT{min::NONE()}.
