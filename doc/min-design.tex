...
\item
Execute \TT{MUP::\EOL acc\_\EOL write\_\EOL update} for any pointer
stored in the new contents.  The garbage collector
may interrupt this step.
...
preallocated stub.  In this case modify Step
\ref{PREALLOCATED-CHECK-TYPE} to check the types of both
stubs, modify Step \ref{PREALLOCATED-SET-FILLING} to set the
types of both stubs to \TT{min::\EOL FILLING}, and modify Step
\ref{PREALLOCATED-SET-TYPE} to set the type of the second stub
to \TT{min::\EOL DEALLOCATED}.

Note that if a stub is temporarily given type \TT{min::\EOL FILLING} 
the normal operations on the stub may not work, because they check
for a different type.  Also note that preallocated stubs may be
garbage collected as soon as their type is changed from \TT{min::\EOL FILLING}
in Step \ref{PREALLOCATED-SET-TYPE},
as preallocated stubs are usually visible to the garbage collector.
In normal circumstances preallocated stubs are protected from
garbage collection before the above filling algorithm is executed.
