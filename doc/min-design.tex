...
Initialized to {\tt min::\EOL NO\_\EOL LINE} if {\tt buffer} is initialized
to hold what is possibly just part of the file.
In this case {\tt file\_\EOL lines} is set to the number of complete
lines in the file when {\tt min::\EOL next\_\EOL line} reads the
end of file.

Otherwise initialized to the number of complete lines in the file if
{\tt buffer} is initialized to the complete contents of the file.
...
\verb|min::uns32 min::|
    & \MINKEY{flush\_\EOL line}\verb| ( min::file & file,|\\&{\tt ~~~~}
                    \verb|   min::uns32 offset )|
...
If the {\tt ASCII\_FLAG} is on
and {\tt GRAPHIC\_FLAG} is \underline{off},
the single space character prints as itself, the horizontal
tab character prints as 1 to 8 single spaces, other
control characters
print as if they were letters that have zero width (consume
no columns), and the
ill-formed UTF-8 character prints as `{\tt <ILL>}'.
...
are printed as character 2368 (hexadecimal), the `\key{smirk}', a.k.a
...
If the {\tt ASCII\_FLAG} and
{\tt GRAPHIC\_FLAG} are both off,
single space prints as itself, horizontal
tab prints as 1 to 8 single spaces, other
control characters
print as if they were letters that have zero width (consume
no columns), and the
ill-formed UTF-8 character prints as the `smirk' (see above).
...
if {\tt min::\EOL EOM\_\EOL FLUSH\_\EOL FLAG} is set,
