...
{\tt column}.
~{\tt min::restore\_\EOL indent} first
executes {\tt min::\EOL restore\_\EOL line\_\EOL break} and
then executes
{\tt min::\EOL indent} if the line break stack element
it popped was disabled and was not the first element of the stack.
It executes {\tt min::\EOL indent}
because it assumes a disabled element is a used element so
the printed text bracketed by the {\tt min::\EOL save\_\EOL line\_\EOL break}
that pushed the element and the {\tt min::\EOL restore\_\EOL line\_\EOL break}
that is popping the element was preceded by an actual line break and should be
ended by a {\tt min::indent}.  However, if the popped element was the
first in the stack, it is assumed that instead an explicit {\tt min::\EOL eol}
...
reflects the point just after `{\tt (~}'.  When the third element
of the stack is popped, it is enabled, and {\tt min::\EOL indent}
is not executed.
...
When the second element is popped, it is
disabled, so {\tt min::\EOL indent} is executed.
When the first element is popped,
{\tt min::\EOL indent} is not executed, because the element was the first
element of the stack, and it is expected that
{\tt min::\EOL eol} will be explicitly executed after
{\tt min::\EOL restore\_\EOL indent}.
...
\verb|struct min::| & \MINKEY{gen\_format}\ARGBREAK
    ...
    \verb|  const char *           implicit_initiator;|\ARGBREAK
    \verb|  const char *           implicit_terminator;|\ARGBREAK
...
\ttmindex{implicit\_initiator}{in {\tt min::gen\_format}}
\ttmindex{implicit\_terminator}{in {\tt min::gen\_format}}
...
\multicolumn{2}{l}{\tt const min::gen\_format
    ...
    \verb|  "","",               // implicit_initiator/terminator|\ARGBREAK
...
\verb|const min::uns32 min::| & \MINNBKEY{NAME\_SPECIAL\_FLAG}
\LABEL{MIN::NAME_SPECIAL_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{BRACKET\_IMPLICIT\_FLAG}
\LABEL{MIN::BRACKET_IMPLICIT_FLAG} \\
\verb|const min::uns32 min::| & \MINNBKEY{DELETE\_SPACES\_FLAG}
\LABEL{MIN::DELETE_SPACES_FLAG} \\
...
\item[\ttmkey{implicit\_initiator}{in {\tt min::gen\_format}}]
\item[\ttmkey{implicit\_terminator}{in {\tt min::gen\_format}}]\vspace{-2ex}
If not {\tt NULL},
the {\tt implicit\_\EOL \BRA initiator,terminator\KET} members
are treated as
the {\tt .initiator} and {\tt .terminator} of an object that is being
printed with both {\tt EXPRESSION\_\EOL FLAG} and {\tt IMPLICIT\_\EOL
PAREN\-THESIS\_\EOL FLAG} on, provided that
the object has \underline{neither}
an {\tt .initia\-tor} or a {\tt .terminator} attribute of its own.
...
vector elements is suppressed if
\begin{center}
...
\item[\ttnbmkey{PREFIX\_SEPARATOR\_FLAG}{{\tt min::gen\_format} flag}]
...
\item[\ttnbmkey{POSTFIX\_SEPARATOR\_FLAG}{{\tt min::gen\_format} flag}]
...
\begin{indpar}[0.1in]\begin{itemlist}[1.4in]
...
\item[\ttnbkey{IMPLICIT\_PARENTHESES\_FLAG}]\vspace{-1ex}
...
\item[\ttnbmkey{IMPLICIT\_PARENTHESES\_FLAG}{{\tt min::gen\_format} flag}]~\\
If set, and if in addition {\tt implicit\_\EOL initiator} and
{\tt implicit\_\EOL terminator} are both non-{\tt NULL},
and if the {\tt EXPRESSION\_\EOL FLAG} is set,
then subobjects
that have no {\tt .initiator} or {\tt .terminator} attributes
are printed surrounded by {\tt implicit\_\EOL initiator}
and {\tt implicit\_\EOL terminator}.
