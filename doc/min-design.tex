...
(see Section~\secref{PACKED-STRUCTURES-AND-VECTORS} must be
locatable.
...
and a packed structure (\secref{PACKED-STRUCTURES-AND-VECTORS} is:
...
These \verb|min::acc_write_update| functions need not and indeed
\underline{must not} be called if \verb|s1| is of
{\tt min::ACC\_\EOL FREE} type, which is the case immediately after
{\tt min::\EOL new\_\EOL acc\_\EOL stub} has been called.
However \verb|min::gen| and
other pointers to stubs can be stored in such a stub's data.
After initializing the stub's data its type must be reset to
something other than {\tt min::ACC\_\EOL FREE} and then it must
be stored in an acc locatable place before any relocating
function is called.
