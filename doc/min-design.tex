...
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::pointer} \verb|psp|
	  \verb| ( min::gen v )|
...
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::pointer} \verb|psp|
	  \verb| ( void )|
\LABEL{MIN::PACKED_STRUCT_POINTER_OF_VOID} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
...
\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::updatable\_pointer} \verb|psup|
	  \verb| ( min::gen v )|
...
(constructor)~\verb|min::|
	& \MINKEY{packed\_struct\SARG::updatable\_pointer} \verb|psup|
	               \verb|( void )|
\LABEL{MIN::PACKED_STRUCT_UPDATABLE_POINTER_OF_VOID} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::|
	& \MINKEY{initialize}\ARGBREAK
...
but should only be used temporarily inside a single statement that
does not call a relocating function.

For the constructors and {\tt min::initialize} functions \verb|v| or
\verb|s| must point at packed structure.
...
pointing at anything.  The constructor with no arguments creates
a pointer that does not point at anything.  A pointer not pointing at
anything produces undefined results if used, though most often
the result will be a memory fault.  Such a pointer
may be initialized by {\tt min::initialize} and then used.
