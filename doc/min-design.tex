...
in which case the count is returned is for the attribute name
with no reverse attribute name.
...
\verb|min::MISSING|, the values deleted or set represent double arrows.
In this case let the object be $O_1$, the name be $N$, and the reverse
name be $R$.  If a value $O_2$ is set (or deleted), the value $O_1$ will also
be set (or deleted) from the attribute named $R$ with reverse name $N$ of the
object $O_2$.  That is, the other end of the double arrow will also be
set (or deleted).
...
the set of values for object, attribute name, and reverse attribute namer
...
attribute name, and reverse attribute name.
...
implementations \verb|int| may be implicitly convertable to \verb|min::gen|
...
making objects during inititalization, and making the special
