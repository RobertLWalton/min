...
are no attributes other than \TT{.type}, \TT{.terminator}, and \TT{.posi\-tion},
...
separated by a single space but with an exception described below for
the space before the last element.
A break is set by
\TT{min::set\_\EOL break} just before each element is printed.

If the \TT{.type} equals \TT{line\_type} then either there is
a \TT{.terminator} that is printed just after the line, or
if not, and if there is more than one element,
the last element separating space is replaced by execution
of \TT{min::\EOL trailing\_\EOL always} and the last element of the
line is printed in paragraph compact format if it has the type
\TT{top\_\EOL element\_\EOL format->\EOL paragraph\_\EOL type}.

If the \TT{.type} equals \TT{line\_\EOL sep\_\EOL type},
then the line is called a \key{separated line} and
there can no \TT{.terminator}.  Then
...
printed (e.g. `\TT{:}'), followed by \TT{min::eol}, and then the
...
If instead \TT{T} is a string
whose \TT{mark\_\EOL classifier}\label{MARK_CLASSIFIER_USAGE} defined
...
\verb|min::trailing_always_semicolon_pstring   // obj_line_sep|
\end{TABULAR}
