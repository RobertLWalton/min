...
An empty {\em value-set} is equivalent to a missing
{\em double-arrow-name-descriptor-pair}.
{\em Double-arrow-name-descriptor-pairs} are not
removed from the object until the object is completely reorganized,
in order to avoid invalidating other attribute pointers referencing the object.
...
optional {\em double-arrow-sublist}, and flags, which are associated
...
value set have a single value and that it be the value set
...
\verb|bool min::| & \MINKEY{remove\_one}\ARGBREAK
    \verb| ( min::insertable_attr_pointer wap,|\ARGBREAK
    \verb|   min::gen v )|
...
\verb|bool min::| & \MINKEY{remove\_all}\ARGBREAK
    \verb| ( min::insertable_attr_pointer wap,|\ARGBREAK
    \verb|   min::gen v )|
...
is a no-operation.  The 2-argument versions return {\tt true} if they
remove at least one value from the value set and {\tt false} if they
did not find any value to remove.  The 3-argument versions return a count
of the number of values removed from the value set.
For these functions the pointer's reverse attribute name
can be \verb|min::NONE|
but cannot be \verb|min::ANY|.
