...
The \TT{disable\_mapping} argument is normally \TT{false}, but
is set \TT{true} when defined format functions decline
to specially format a \TT{min::gen} value: see
\itemref{PRINTING-USING-DEFINED-FORMATS}.
...
\begin{indpar}\begin{verbatim}
    ( * gen_format->pgen ) ( printer, v, gen_format, true )
\end{verbatim}\end{indpar}

with the \TT{disable\_\EOL mapping} argument set to \TT{true} to
suppress defined format mapping.
...
\TT{min::\EOL new\_\EOL gtype} by giving it a second argument:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{new\_gtype}\ARGBREAK
        \verb|( min::gen gtype,|\ARGBREAK
	\verb|  min::packed_vec_insptr<min::gen> vartab )|
...
\TT{.type} attribute equal to the symbol \ttkey{*VARNAME*},
no other non-hidden attributes (i.e., no other attributes that do
\underline{not} have an \TT{H} flag), and a single element that
is a symbol or label.  This symbol is looked up in \TT{vartab}
...
equal to \TT{min::\EOL MISSING()}.  Other elements may or may not
be preset to \TT{min::\EOL MISSING()} or to a symbol or label
in order to prevent the use of an index or pre-assign an index
to a name.
