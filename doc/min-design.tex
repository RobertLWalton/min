...
stub attached to \verb|s1|.  This is done by the follwing functions:
...
returns the size of the stub body, that is, the same size
...
returns a pointer to a pointer to the stub body.
The first pointer points to a location P with a fixed address
(P is the stub value); P holds
a pointer to the stub body, which may be relocated by a call to
...
In order to change the size of a stub body, the following
can be used:
...
\verb|void MUP::| & \MUPKEY{void\_resize\_body}%
	     \verb| ( MUP::resize_body & rb )| 
\LABEL{MUP::VOID_RESIZE_BODY} \\
\end{tabular}\end{indpar}

When constructed the \verb|MUP::resize_body| datum allocates
a new body for a stub \verb|s|, and when desconstructed the datum
installs the new body in the stub \verb|s| while deallocating the
old body of \verb|s|.  Stub \verb|s|
is not altered until the {\tt resize\_\EOL body}
datum is deconstructed, and the {\tt void\_\EOL resize\_\EOL body}
function can be used to prevent stub s from ever being altered.
The sizes of the old and new body of \verb|s| must be passed to
the {\tt resize\_\EOL body} constructor.

Given a {\tt resize\_\EOL body} datum \verb|rb|,
\begin{center}
...
as the {\tt resize\_\EOL body} datum \verb|rb| exists.   P holds
...
the old body to the new body before the {\tt resize\_\EOL body}
datum is deconstructed.  The new body will not be touched by
the garbage collector while the {\tt resize\_\EOL body} datum
exists, but it may be relocated.  The existing stub \verb|s|
\underline{must} be protected by the from garbage collection and
its body protected from reorganization by the user while the
{\tt resize\_\EOL body} datum exists, but that body may
also be relocated.  \verb|s| must NOT be deallocated
while the {\tt resize\_\EOL body} datum exists, unless
{\tt void\_\EOL resize\_\EOL body} has been called.
...
arrow has an arrow with directions and direction names reversed.
...
and contains attribute labels, and one called the typed object or
...
that give the index of the element relative to the beginning of the
object body.
Auxiliary pointers with zero index do not address a body vector
element, since they point into the header, and these are given
special meanings (e.g., {\tt  min::\EOL LIST\_\EOL END}).
...
and possible converted from short to long or vice versa.  Frequently an
...
Objects are compacted by the `{\tt publish}' function that sets the
...
An object header encodes the hash table size and total size of the
object indirectly using a logrithmic or pseudo-floating-point
code.  Therefore not every
hash table size or total size is possible.
In order to create an object it is necessary to determine possible
hash table and total sizes.
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::uns32 min::|
    & \MINKEY{short\_obj\_hash\_size}\verb| ( min::uns32 u )|
\LABEL{MIN::SHORT_OBJ_HASH_TABLE_SIZE} \\
\verb|min::uns32 min::|
    & \MINKEY{short\_obj\_total\_size}\verb| ( min::uns32 u )|
\LABEL{MIN::SHORT_OBJ_TOTAL_SIZE} \\
\verb|min::uns32 min::|
    & \MINKEY{long\_obj\_hash\_size}\verb| ( min::uns32 u )|
\LABEL{MIN::LONG_OBJ_HASH_TABLE_SIZE} \\
\verb|min::uns32 min::|
    & \MINKEY{long\_obj\_total\_size}\verb| ( min::uns32 u )|
...
\verb|min::gen min::| & \MINKEY{new\_obj\_gen\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::uns32 unused_size,| \\
    \verb|   min::uns32 hash_size = 0,| \\
    \verb|   min::uns32 variable_size = 0 )| \\
    \end{tabular}
\LABEL{MIN::NEW_OBJ_GEN} \\
\end{tabular}\end{indpar}

The exact unused area and variable vector sizes are given.
...
the unused area and variables vector sizes given, and the header size.
...
A read-only \verb|min::vec_pointer| permits read-only access to body
vector elements.
...
object's attibute vector or auxiliary area.
...
by calling the `\verb|publish|' function
(\pagref{PUBLISH}).
...
flag fixes the attribute label structure of the object, and the size of
...
Object bodies may be reorganization when insertions are made
using an insertable pointer, or when the \verb|publish| function
is called to set the \verb|min::OBJ_PUBLIC| flag.
...
\verb|min::uns32 min::| & \MINKEY{var\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MIN::VAR_SIZE_OF_VEC_POINTER} \\
\verb|min::uns32 min::| & \MINKEY{hash\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MIN::HASH_SIZE_OF_VEC_POINTER} \\
\verb|min::uns32 min::| & \MINKEY{attr\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MIN::ATTR_SIZE_OF_VEC_POINTER} \\
\verb|min::uns32 min::| & \MINKEY{unused\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MIN::UNUSED_SIZE_OF_VEC_POINTER} \\
\verb|min::uns32 min::| & \MINKEY{aux\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MIN::AUX_SIZE_OF_VEC_POINTER} \\
\verb|min::uns32 min::| & \MINKEY{total\_size\_of}%
    \verb| ( min::vec_pointer & vp )|
...
\verb|min::gen | & \MINKEY{var}%
    \verb| ( min::vec_pointer & vp, min::uns32 index )|
...
\verb|min::gen | & \MINKEY{aux}%
    \verb| ( min::vec_pointer & vp, min::uns32 aux_pointer )|
...
\begin{center}
$\mbox{\tt MUP::aux\_offset\_of(vp)}\leq p<\mbox{\tt min::total\_size\_of(vp)}$
\end{center}
(See \pagref{MUP::AUX_OFFSET_OF_VEC_POINTER} for 
{\tt MUP::aux\_offset\_of(vp)}).
...
An exception is the {\tt min::\EOL initialize} functions, which
will suffer a {\tt MIN\_\EOL ASSERT} error if applied to a
downcast object vector pointer.

The {\tt min::set\_}\,\ldots{} functions above invoke the
...
These push and pop functions preserve the order of the values in memory,
...
\verb|min::uns32 MUP::| & \MUPKEY{var\_offset\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MUP::VAR_OFFSET_OF_VEC_POINTER} \\
\verb|min::uns32 MUP::| & \MUPKEY{attr\_offset\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MUP::ATTR_OFFSET_OF_VEC_POINTER} \\
\verb|min::uns32 MUP::| & \MUPKEY{unused\_offset\_of}%
    \verb| ( min::vec_pointer & vp )|
\LABEL{MUP::UNUSED_OFFSET_OF_VEC_POINTER} \\
\verb|min::uns32 MUP::| & \MUPKEY{aux\_offset\_of}%
    \verb| ( min::vec_pointer & vp )|
...
auxiliary area.  The following are equivalences except for
the omission of {\tt MIN\_\EOL ASSERT} checks that the unused area
is non-empty for pushes or that the attribute vector or auxiliary area
is non-empty for pops,
and for push operation calls to {\tt MUP::\EOL acc\_\EOL write\_\EOL update}
...
sizes in \verb|min::gen| units; e.g., a short object header size
...
for optimization.  Most implementations do not all the required
sizes directly.  For example, an implementation usually permits only
particular hash table sizes, and uses 5 bits for short objects
or 11 bits for long object to encode a reference to a table
of hash table sizes, freeing up at 3 bits for short objects and
5 for long objects to be used
as flags.  The {\tt min::\EOL OBJ\_\EOL PRIVATE},
{\tt min::\EOL OBJ\_\EOL PUBLIC},
and {\tt min::\EOL OBJ\_\EOL TYPED} flags are stored in this way.
Additional flags may be used to optimize garbage collection.
...
{\tt start\_hash}, {\tt start\_attr}, or {\tt start\_copy}
(note {\tt start\_sublist} is \underline{not} a start list function).
The `{\tt next}' function moves forward in the current list, and
the {\tt start\_sublist} function starts down a sublist.
The list is always pointing at a current element, whose value is
returned by the `{\tt current}' function.  If the current element
is written using another updatable or insertable list pointer,
the `{\tt refresh}' function must be used to reestablish the
current element's value, which is cached in the list pointer.
This `{\tt refresh}' function must also be used if list elements
are inserted into or removed from the object by using another
insertable list pointer, but in some such situations using
`{\tt refresh}' is not adequate and the list pointer must be
restarted (see \pagref{RESTARTING_LIST_POINTER}).

The {\tt min::\EOL start\_hash} function positions
...
The {\tt min::\EOL start\_attr} function is analogous except it is given
...
The {\tt min::\EOL start\_copy} function positions a list pointer ({\tt lp})
to the same place as another list pointer ({\tt lp2}).
The two list pointers must have been constructed from the same
vector pointer, but {\tt lp2} may be a read-only, updatable, or
insertable list pointer.
...
{\tt min::\EOL start\_\EOL sublist} function can be called to position the
...
It is an error to call {\tt min::start\_sublist}
if the list pointer points at an
...
An {\tt min::updatable\_list\_pointer} allows read-write access to
...
\verb|min::|
	& \MINKEY{updatable\_vec\_pointer\_of} \\
	& \verb|    ( min::updatable_list_pointer & lp )|
\LABEL{MIN::UPDATABLE_VEC_POINTER_OF_UPDATABLE_LIST_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::|
	& \MINKEY{start\_hash}%
...
and neither may be the head of a sublist (even an empty sublist).
...
\verb|min::|
	& \MINKEY{insertable\_vec\_pointer\_of} \\
	& \verb|    ( min::insertable_list_pointer & lp )|
\LABEL{MIN::INSERTABLE_VEC_POINTER_OF_INSERTABLE_LIST_POINTER} \\
\end{tabular}\end{indpar}
\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::|
	& \MINKEY{start\_hash}%
...
\verb|void min::|
	& \MINKEY{insert\_reserve\REL}%
	  \begin{tabular}[t]{@{}l@{}}
	  \verb| ( min::insertable_list_pointer & lp,| \\
	  \verb|   min::uns32 insertions,| \\
	  \verb|   min::uns32 elements = 0,| \\
...
may be a sublist, and the replacing value may be
{\tt min::\EOL EMPTY\_\EOL SUBLIST}.  Thus {\tt min::set} can be used
to remove a sublist or to create and empty sublist.
...
list pointers will be detected if unsufficient memory is reserved,
but not otherwise.
...
\item the first element of a sublist that is adjacent to
      an element that is inserted or removed
\item the end of an empty sublist that is adjacent to
      an element that is inserted or removed
\end{enumerate}
...
A typed object is a pair of objects: the \mkey{type}{of typed object}
which provides attribute labels, and the \mkey{context}{of typed object}
which provides variables that give attribute values.  They type
...
just a single index \verb|min::gen| value that hold the index of a
...
TBD: The `publish' function.\label{PUBLISH}
...
\verb|min::unsptr min::| & \MINKEY{get\_attributes}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, min::unsptr n,| \\
    \verb|   min::attribute_pointer ap )| \\
    \end{tabular}
...
\verb|min::unsptr min::| & \MINKEY{get\_reverse\_attributes}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen * out, min::unsptr n,| \\
    \verb|   min::attribute_pointer ap )| \\
    \end{tabular}
...
\verb|void * & MUP::| & \MUPKEY{new\_body\_pointer\_ref}\ARGBREAK
	     \verb|    ( MUP::resize_body & rb )| 
\LABEL{MUP::NEW_BODY_POINTER_REF} \\
\verb|void MUP::| & \MUPKEY{void\_resize\_body}\ARGBREAK
	     \verb|    ( MUP::resize_body & rb )| 
\LABEL{MUP::VOID_RESIZE_BODY} \\
\end{TABULAR}
...
\verb|const char * MUP::| & \MUPKEY{str\_of}\verb| ( MUP::long_str * str )|
\LABEL{MUP::STR_OF_LONG_STR} \\
\verb|min::uns32 MUP::| & \MUPKEY{length\_of}\verb| ( MUP::long_str * str )|
\LABEL{MUP::LENGTH_OF_LONG_STR} \\
\verb|min::uns32 MUP::| & \MUPKEY{hash\_of}\verb| ( MUP::long_str * str )|
\LABEL{MUP::HASH_OF_LONG_STR} \\
\end{TABULAR}
...
\verb|min::gen min::| & \MINKEY{new\_obj\_gen\REL}%
    ...
    \verb| ( min::uns32 unused_size,| \\
    \verb|   min::uns32 hash_size = 0,| \\
    \verb|   min::uns32 variables_size = 0 )| \\
    \end{tabular}
\LABEL{MIN::NEW_OBJ_GEN} \\
\end{TABULAR}
...
\multicolumn{2}{@{}l}{\tt min::updatable\_vec\_pointer \&} \\
\verb|min::|
	& \MINKEY{updatable\_vec\_pointer\_of}\ARGBREAK
	  \verb| ( min::updatable_list_pointer & lp )|
\LABEL{MIN::UPDATABLE_VEC_POINTER_OF_UPDATABLE_LIST_POINTER} \\
\end{TABULAR}
...
\multicolumn{2}{@{}l}{\tt min::insertable\_vec\_pointer \&} \\
\verb|min::|
	& \MINKEY{insertable\_vec\_pointer\_of}\ARGBREAK
	  \verb| ( min::insertable_list_pointer & lp )|
\LABEL{MIN::INSERTABLE_VEC_POINTER_OF_INSERTABLE_LIST_POINTER} \\
\end{TABULAR}
...
\verb|void min::|
	& \MINKEY{insert\_reserve\REL}\ARGBREAK
	  \verb| ( min::insertable_list_pointer & lp,|\ARGBREAK
	  \verb|   min::uns32 insertions,|\ARGBREAK
	  \verb|   min::uns32 elements = 0,|\ARGBREAK
