...
{\tt buffer} holds the complete contents of the file is
{\tt buffer->\EOL length - end\_\EOL offset}, and if this
is zero, then there is no partial line.
The offset of a partial line is {\tt end\_\EOL offset}.  The
following functions get the partial line length and offset in
a more mnemonic fashion:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|bool min::|
    & \MINKEY{file\_\EOL is\_\EOL complete}\verb| ( min::file file )|
\LABEL{MIN::FILE_COMPLETE} \\
\verb|min::uns32 min::|
    & \MINKEY{partial\_\EOL length}\verb| ( min::file file )|
\LABEL{MIN::PARTIAL_LENGTH} \\
\verb|min::uns32 min::|
    & \MINKEY{partial\_\EOL line}\verb| ( min::file file )|
\LABEL{MIN::PARTIAL_LINE} \\
\end{tabular}\end{indpar}

The first function returns {\tt file\_\EOL lines != min::\EOL NO\_\EOL LINE},
and the other functions must not be used unless the first function returns
{\tt true}.  This will always be the case after
...
({\tt file\_\EOL lines} still equals {\tt min::\EOL NO\_\EOL LINE})
