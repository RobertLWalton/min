...
{\tt vp[i]} differs from {\tt min::\EOL attr(vp,i)} in that
the former can be cast to an address within the object body vector,
whereas the latter produces a {\tt min::gen}
temporary value whose address is useless.  Note, however,
that {\tt \&vp[i]} is a relocatable address, and so it should only
...
or {\tt min::\EOL ptr<min::gen>} while read-only
element access functions return
{\tt min::gen}, `{\tt const min::\EOL gen~\&}', or
{\tt min::\EOL ptr<const min::gen>}.
