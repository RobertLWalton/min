...
\TTKEY{MIN\_ASSERT}\TT{(}$e$\TT{,}$m$\TT{,...)}
	& By default, defined to evaluate $e$, and if the value is
	  ...
	  \TT{printf(}$m$\TT{,...)} and then calls \TT{abort()}.
\LABEL{MIN_ASSERT} \\
\end{tabular}\end{indpar}

The definition of \TT{MIN\_ASSERT} can be changed by defining
exactly one of the following (which by default are undefined):
\begin{indpar}\begin{tabular}{lp{4.0in}}
\TTKEY{MIN\_ASSERT\_CALL\_ALWAYS}
    & If defined, then \TT{MIN\_A\EOL SSERT} is defined to always call the
      \TT{min::\EOL assert\_\EOL hook} function, passing it the
      value of $e$.  By default, this function will return immediately
      if $e$ is true, and will print an error message and call
      \TT{abort()} if $e$ is false.  However global variables may
      be set to modify this behavior.
\LABEL{MIN_ASSERT_CALL_ALWAYS} \\
\TTKEY{MIN\_ASSERT\_CALL\_NEVER}
    & If defined, then \TT{MIN\_\EOL ASSERT} is defined to do nothing at all,
      not even evaluate its argument.
\LABEL{MIN_ASSERT_CALL_NEVER} \\
\end{tabular}\end{indpar}

Two alternatives to \TT{MIN\_ASSERT} are:

\begin{indpar}\begin{tabular}{lp{4.0in}}
\TTKEY{MIN\_REQUIRE}\TT{(}$e$\TT{,}$m$\TT{,...)}
	& Does the same thing as \TT{MIN\_ASSERT} unless the
	  \TT{MIN\_\EOL ASSERT\_\EOL CALL\_\EOL NEVER} macro
	  is defined, in which case it does what \TT{MIN\_\EOL ASSERT}
	  would do if \TT{MIN\_\EOL ASSERT\_\EOL CALL\_\EOL NEVER} were
	  not defined (i.e., evaluate $e$ and if false call
	  \TT{min::\EOL assert\_\EOL hook}).
\LABEL{MIN_REQUIRE} \\
\TTKEY{MIN\_ABORT}\TT{(}$m$\TT{,...)}
	& Always calls
	  \TT{min::\EOL assert\_\EOL hook} with a false $e$ value,
	  regardless of whether other macros are defined.
\LABEL{MIN_ABORT} \\
\end{tabular}\end{indpar}
...
values of the $m$ argument to \TT{MIN\_\EOL ASSERT}, and the arguments
following that.
The \TT{function\_\EOL name} argument to \TT{min::\EOL assert\_\EOL hook}
may be \TT{NULL} if the function name is not known.
...
The next four flags are used in conjunction with printer format
...
\ttkey{min::QUOTE\_SUPPRESS}: & categories \TT{L} and \TT{M}
				(letters and combining marks) \\
\ttkey{min::QUOTE\_SKIP}: & \TT{.} (period) \\
...
true (and the code, being optimized, does not have to read the entire
string).
\end{itemize}
\underline{and}
\begin{itemize}
\item[(2)] The string is of length \TT{n} and begins with \TT{0} to \TT{n-1}
...
\multicolumn{2}{l}{\tt struct
                       min::\MINKEY{str\_classifier}}
\LABEL{MIN::STR_CLASSIFIER}\ARGBREAK
    \verb|{|\ARGBREAK
