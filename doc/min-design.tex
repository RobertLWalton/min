...
\ttkey{min::IS\_VHSPACE}: & \TT{FF} \TT{VT} \TT{LF} \TT{CR},
                            plus characters with the {\tt IS\_HSPACE} flag
\end{tabular}
\end{indpar}
...
\ttkey{min::IS\_NON\_SPACING}: & characters with the \TT{IS\_CONTROL} flag,\\
			& but without the \TT{IS\_HSPACE} flag, \\
			       & plus characters in subcategory \TT{Mn} \\
\end{tabular}
\end{indpar}
...
\TT{NULL} \TT{min::str\_\EOL format} and returns 0 if all characters
...
of print columns these take; the maximum is 15.
...
\verb|// Same as min::element_gen_format except for:|\ARGBREAK
\verb|min::paragraph_element_obj_format       // obj_format|
\end{tabular}\end{indpar}
...
\verb|// Same as min::element_gen_format except for:|\ARGBREAK
\verb|min::line_element_obj_format            // obj_format|
\end{tabular}\end{indpar}
...
\ttmindex{flag\_format}{in {\tt min::obj\_format}}
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::line_element_gen_format             // top_element_format|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::trailing_always_semicolon_space_pstring  // obj_line_sep|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::paragraph_element_gen_format        // top_element_format|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::left_curly_vbar_vbar_right_curly_pstring // obj_empty|\ARGBREAK
\end{tabular}
...
\verb|// All other members are NULL or NONE().|
\end{tabular}
...
\begin{enumerate}
\item The `\key{bracketed compact format}'
...
\item Otherwise the `\key{line compact format}' is used if
the object has a \TT{.type} equal to either the object format
\TT{line\_\EOL type} or \TT{line\_\EOL sep\_\EOL type}
and the object has no \TT{.separator}.

\item Otherwise the `\key{paragraph compact format}' is used if
the object has a \TT{.type} equal to the object format
\TT{paragraph\_\EOL type}, the object has no \TT{.separator},
and the object is being printed as the last element of a containing
object printed with line compact format.
...
any other kinds of objects or inside lines not representing objects.
...
being printed using the \TT{quote\_\EOL element\EOL format}.
...
as a list of its elements with each element
being printed using the \TT{top\_EOL element \_\EOL format}
and the elements being separated by \TT{obj\_\EOL sep}.

If the \TT{.type} is equal to \TT{line\_\EOL sep\_\EOL type},
\TT{obj\_\EOL line\_\EOL sep} is printed after the last element.
...
other than \TT{.position} and \TT{.separator}.
...
This is the \TT{min::pstring} printed in line formats at the end of a line
...
WARNING: The \TT{flag\_format->flag\_names} is
\underline{not} locatable by the ACC, and its value must therefore be
stored elsewhere in a locatable variable.  It is expected that values of
...
printing the first elements, and a break is set by
...
are no elements and no attributes other than
\TT{.position} and \TT{.separator},
...
are no attributes other than \TT{.type} and \TT{.position},
...
In \mkey{line compact format}{of printed object}, where there are
are no attributes other than \TT{.type} and \TT{.posi\-tion},
with \TT{.type} being equal to \TT{line\_\EOL type}
or \TT{line\_\EOL sep\_\EOL type},
the object elements are just printed using
\TT{top\_\EOL element\_\EOL format} with elements being
separated by a single space.  A break is set by
\TT{min::set\_\EOL break} just before each element is printed.
If the \TT{.type} equals \TT{line\_\EOL sep\_\EOL type},
then the line is called a \key{separated line} and
\TT{obj\_\EOL line\_\EOL sep} is printed (e.g. `\TT{;}') just after the line.
...
In \mkey{paragraph compact format}{of printed object}, there are
are no attributes other than \TT{.type} and \TT{.posi\-tion},
the \TT{.type} is equal to \TT{paragraph\_\EOL type},
and the object is the last element of a non-separated line printed with
line compact format with at least one line element preceeding
the paragraph.  First \TT{obj\_\EOL paragraph\_\EOL begin} is
printed (e.g. `\TT{:}'), followed \TT{min::eol}, and then the
...
line compact format, and elements which are not thus are printed as if
they were one element non-separated lines.
Define a \key{line group} as a maximal sequence of separated lines followed
by a non-separated line, or a maximal sequence of separated lines at the end
of a paragraph.
Because the paragraph is the last thing in a line group, the indent
...
and in paragraph compact format each of these is preceded by
executing \TT{min::\EOL indent} to
indent the line group and then \TT{min::\EOL place\_\EOL indent(4)}
to indent continuations of the line group beyond the beginning
...
In \mkey{default compact format}{of printed object}, there are
are no attributes other than \TT{.type}, \TT{.position}, and
\TT{.separator}, and none of the above compact formats apply.
Then if
...
If in this format the \TT{NO\_\EOL TRAILING\_\EOL TYPE}
object format operation flag on, then \TT{T} is omitted from `\TT{|T\}}' which
becomes `\TT{|\}}'.

If instead \TT{T} is a string whose \TT{mark\_\EOL classifier} defined
...
\TT{"T2"}'s string class has the \TT{min::\EOL IS\_\EOL TRAING} flag,
then the format is
\begin{center}
\TT{\{}\TT{T1} {\em <element>}
    \OPEN{} {\em ta} \TT{,} \TT{\textvisiblespace} {\em <element>} \CLOSE\STAR{}
...
\TT{flag\_format->flag\_prefix}	& is	& \TT{"]"} \\
\TT{flag\_format->flag\_prefix}	& is
...
\LABEL{MIN::OBJ_FORMAT}\ARGBREAK[0in]
    ...
    \verb|  min::gen                             line_type;|\ARGBREAK[0in]
    \verb|  min::gen                             line_sep_type;|\ARGBREAK[0in]
    \verb|  min::gen                             paragraph_type;|\ARGBREAK[0in]
    ...
    \verb|  min::pstring                         obj_line_sep;|\ARGBREAK[0in]
    \verb|  min::pstring                         obj_paragraph_begin;|
\end{TABULAR}
...
\verb|min::NONE()                              // line_type|\ARGBREAK[0in]
\verb|min::NONE()                              // line_sep_type|\ARGBREAK[0in]
\verb|min::NONE()                              // paragraph_type|\ARGBREAK[0in]
\end{TABULAR}
...
\begin{TABULAR}{0.3in} & \hspace*{0in}
\verb|NULL                                     // obj_line_sep|\ARGBREAK[0in]
\verb|NULL                                     // obj_paragraph_begin|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINLKEY{paragraph\_element}{\_obj\_format}:}%
\LABEL{MIN::PARAGRAPH_ELEMENT_OBJ_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|min::line_element_gen_format             // top_element_format|\ARGBREAK[0in]
\verb|min::null_str_classifier                 // mark_classifier|\ARGBREAK[0in]
\verb|min::line_feed                           // line_type|\ARGBREAK[0in]
\verb|min::semicolon                           // line_sep_type|\ARGBREAK[0in]
\verb|min::trailing_always_semicolon_space_pstring  // obj_line_sep|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINKEY{line\_element\_obj\_format}:}%
\LABEL{MIN::LINE_ELEMENT_OBJ_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|min::paragraph_element_gen_format        // top_element_format|\ARGBREAK[0in]
\verb|min::null_str_classifier                 // mark_classifier|\ARGBREAK[0in]
\verb|min::colon                               // paragraph_type|\ARGBREAK[0in]
\verb|min::trailing_always_colon_pstring       // obj_paragraph_begin|
\end{TABULAR}
...
\verb|min::null_str_classifier                 // mark_classifier|\ARGBREAK[0in]
\verb|min::left_curly_vbar_vbar_right_curly_pstring // obj_empty|\ARGBREAK[0in]
