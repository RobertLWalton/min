...
\verb|T & |
	& \TTOMKEY{[ ]}{[ ]}%
	          {of {\tt min::ptr\TARG}}
	  \verb| ( const min::ptr<T> & p, I index )|
\LABEL{MIN::[]_OF_PTR} \\
\verb|min::ptr<T> |
	& \TTOMKEY{+}{+}%
	          {of {\tt min::ptr\TARG}}
...
{\tt p[i]} is a `{\tt T \&}' reference value for the {\tt i+1}'st element of
the vector with elements of type {\tt T} pointed at by {\tt p}, and
{\tt p+i} is a {\tt min::\EOL ptr<T>} pointer to that element.

Note that {\tt p[i]} is only a `{\tt T \&}' reference, and
\underline{not} a {\tt min:\EOL ref<T>} reference, and as such is
in effect an unprotected body pointer that will become undefined whenever
a relocatable function is called.  This has been done because a
...
common when {\tt T} is a structure type.
...
inherited) so that they
return `{\tt H~*}', `{\tt E~*}' `{\tt E~\&}, or `{\tt min::\EOL ptr<E>}' values
instead of `{\tt H~const~*}', `{\tt E~const~*}', `{\tt E~const~\&}', or
...
It can either be non-body pointer or a {\tt min::ptr<E>} pointer.
...
\verb|min::printer |
    & \TTOMKEY{<<}{\LT\LT\REL}%
              {of {\tt min::printer}}\ARGBREAK
      \verb| ( min::printer printer, min::ptr<char> s )|
