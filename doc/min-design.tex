...
a \TT{min::\EOL VAR\_\EOL PTR} type \TT{min::\EOL gen} value.  Instead,
the single argument \TT{min::\EOL get} function stores information
in the attribute pointer that can be used by the following functions
(provided no other \TT{min::\EOL get} function execution on the
attribute pointer intervenes):

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::|
	& \MINMKEY{var\_ptr\_to}{of {\tt attr\_updptr}}\ARGBREAK
	  \verb|( min::attr_updptr & ap )|
\LABEL{MIN::VAR_PTR_TO_ATTR_UPDPTR} \\
\verb|bool min::|
	& \MINMKEY{set\_var}{of {\tt attr\_updptr}}\ARGBREAK
	  \verb|( min::attr_updptr & ap, min::gen v )|
\LABEL{MIN::SET_VAR_OF_ATTR_UPDPTR} \\
\end{tabular}\end{indpar}

The \TT{min::\EOL var\_\EOL \_ptr\_\EOL to} function returns
a \TT{min::\EOL VAR\_\EOL PTR} pointing to the variable whose
value the \TT{min::\EOL get} returned, or returns
\TT{min::\EOL NULL\_\EOL STUB} if the \TT{min::\EOL get}
returned a value from a location that was \underline{not}
an object variable.  The \TT{min::\EOL set\_\EOL var}
function stores its \TT{v} argument in the variable whose
value the \TT{min::\EOL get} returned and returns \TT{true},
or does nothing but return \TT{false} if the \TT{min::\EOL get}
returned a value from a location that was \underline{not}
an object variable.
