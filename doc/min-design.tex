...
Here any {\em attribute-label} is printed with
\TT{label\_\EOL format} and any {\em attribute-flags}
is printed with \TT{flags\_\EOL format}
...
and the {\em object-id} is printed with \TT{min::\EOL print\_\EOL id}.
What would be `{\em attribute-label} \TT{= TRUE}' is replaced by
just `{\em attribute-label}\,', and what
would be `{\em attribute-label} \TT{= FALSE}' is replaced by
just `\TT{no }{\em attribute-label}\,'.
