...
\LABEL{MIN::PACKED_VEC_PTR_[]} \\
\verb|const E * pvp| & \TTDMKEY{begin\_ptr()}{of {\tt min::packed\_vec\_ptr}}
...
name, represents any ill-formed UTF-8 encoding
(see {\tt UTF8\_\EOL ILLEGAL}, \pagref{UTF8_ILLEGAL}, for a definition
of an illegal UTF-8 encoding).
...
no columns), and any
ill-formed UTF-8 encoding prints as `{\tt <ILL>}'.
...
range 2400-243F (hexadecimal), while ill-formed UTF-8 encodings
...
print as if they were letters that have zero width (consume
no columns), and any
ill-formed UTF-8 encoding prints as the `smirk' (see above).
