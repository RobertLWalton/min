...
The types of both the header and the vector elements must be such that:
\begin{enumerate}
...
\verb|struct min::| & \MINKEY{gen\_format}\ARGBREAK
    ...
    \verb|  const char *           implied_initiator;|\ARGBREAK
    \verb|  const char *           implied_terminator;|\ARGBREAK
...
\ttmindex{implied\_initiator}{in {\tt min::gen\_format}}
\ttmindex{implied\_terminator}{in {\tt min::gen\_format}}
...
\multicolumn{2}{l}{\tt const min::gen\_format
    ...
    \verb|  "","",       // implied_initiator/terminator|\ARGBREAK
...
printing of {\tt str\_\EOL pre/\EOL post\-fix/\EOL quote} (see below).

\item[\ttnbmkey{EXPRESSION\_FLAG}{{\tt min::gen\_format} flag}]
\item[\ttnbmkey{IMPLICIT\_PARENTHESES\_FLAG}%
    {{\tt min::gen\_format} flag}]\vspace{-1ex}
\item[\ttnbmkey{DATA\_FLAG}{{\tt min::gen\_format} flag}]\vspace{-1ex}
\item[\ttnbmkey{RAW\_FLAG}{{\tt min::gen\_format} flag}]\vspace{-1ex}
...
Object IDs are choosen using the printer {\tt obj\_\EOL id\_\EOL map}:
...
\item[\ttmkey{implied\_initiator}{in {\tt min::gen\_format}}]
\item[\ttmkey{implied\_terminator}{in {\tt min::gen\_format}}]\vspace{-2ex}
If not {\tt NULL},
the {\tt implied\_\EOL \BRA initiator,terminator\KET} members
are treated as
