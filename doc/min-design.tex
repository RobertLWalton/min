...
\begin{tabular}{ll}
\TT{L}		& letters \\
~~~\TT{Ll}	& lower case letters \\
~~~\TT{Lu}	& upper case letters \\
~~~\TT{Lo}	& other letter, includes
...
~~~\TT{Sm}	& math symbol, includes \TT{+} \TT{<} \TT{=} \TT{>} \TT{|}
		  ...
		  \TT{\textdiv}
		  \\
~~~\TT{Sc}	& currency symbol, includes
		  \TT{\textcent}
		  \TT{\textsterling}
		  \TT{\textcurrency}
		  \TT{\textyen}
...
\TT{C}		& control characters, includes \TT{SHY} and characters
                  with codes \\
		& in ranges [\TT{00}\ldots\TT{1F}]
		  and [\TT{7F}\ldots\TT{9F}] \\
\end{tabular}
\end{center}

Given this the assignment of flags in
\TT{min::standard\_char\_flags} is as follows:
...
The next flag, the 
\minkey{IS\_\EOL MARK} flag, is given to all punctuation and symbols
...
The specific assignment of these last four flags encoded in
...
\multicolumn{2}{l}{\tt const min::support\_control
	...
	\verb|{ 0xFFFFFFFF, min::IS_UNSUPPORTED };|
