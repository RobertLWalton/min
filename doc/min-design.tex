...
is objects, which can have stub type {\tt min::\EOL TINY\_\EOL OBJ},
{\tt min::\EOL SHORT\_\EOL OBJ},
{\tt min::\EOL LONG\_\EOL OBJ},
or {\tt min::\EOL HUGE\_\EOL OBJ},
and may be reorganized during relocation to
...
reverse attribut name and whose destination is the unreversed arrow's
source.
...
Similarly the auxilarly area is zero length, but can be expanded by
growing downward into the unused area.

Various sizes can be changed, however, by {\tt min::\EOL resize}
functions.  The variable vector, attribute vector, and unused area
sizes can be changed by applying {\tt min::\EOL resize} to
a {\tt min::\EOL obj\_\EOL vec\_\EOL insptr} object vector pointer
(\pagref{MIN::RESIZE_OF_OBJ_VEC_INSPTR}).
The these sizes can be changed and the
auxilarly area can be compacted by applying 
{\tt min::\EOL resize} to
a {\tt min::\EOL list\_\EOL insptr} list pointer
(\pagref{MIN::RESIZE_OF_LIST_INSPTR}).
All the sizes can be changed, including the hash table size, and the
auxilarly area can be compacted by applying 
{\tt min::\EOL resize} to
a {\tt min::\EOL attr\_\EOL insptr} attribute pointer
(\pagref{MIN::RESIZE_OF_ATTR_INSPTR}).
...
On the flip side, the compactor may reduce the size of the unused
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{obj\_resize\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    ...
    \verb|   bool expand = true )| \\
    \end{tabular}
\LABEL{MIN::OBJ_RESIZE} \\
\verb|void min::| & \MINKEY{obj\_resize\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr unused_size )| \\
    \end{tabular}
\LABEL{MIN::OBJ_RESIZE_UNUSED} \\
\verb|void min::| & \MINKEY{obj\_expand\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr unused_size )| \\
    \end{tabular}
\LABEL{MIN::OBJ_EXPAND} \\
\end{tabular}\end{indpar}

The 4-argument {\tt min::obj\_resize} function can change
...
The 2-argument {\tt min::obj\_resize} function and the {\tt min::\EOL expand}
function are both equivalent to {\tt min::\EOL resize} with no change
in variables vector size.  The 2-argument {\tt min::obj\_\EOL resize}
...
in its lists, and therefore no unnecessary invisible jumps (auxilary
...
reorganization just involves changing auxiliary pointers in the object and
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{obj\_reorganize\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
...
\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{obj\_compact\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object,| \\
    \verb|   min::unsptr variable_size,| \\
    \verb|   min::unsptr unused_size,| \\
    \verb|   bool expand = true )| \\
    \end{tabular}
\LABEL{MIN::OBJ_COMPACT_AND_RESIZE} \\
\verb|void min::| & \MINKEY{obj\_compact\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
\LABEL{MIN::OBJ_COMPACT} \\
\verb|void min::| & \MINKEY{obj\_publish\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
\LABEL{MIN::OBJ_PUBLISH} \\
\end{tabular}\end{indpar}

These functions are like the {\tt min::reoganize} function
...
is left unchanged if $2N/3\leq S\leq 3N/2$.  Otherwise the
...
the object {\tt OBJ\_\EOL PUBLISH} flag which prevents further changes to the
...
{\tt min::reorganize} and the following:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{hash\_count\_of\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
\LABEL{MIN::HASH_COUNT_OF} \\
\verb|void min::| & \MINKEY{set\_publish\_flag\_of\REL}%
    \begin{tabular}[t]{@{}l@{}}
    \verb| ( min::gen object )| \\
    \end{tabular}
\LABEL{MIN::SET_PUBLISH_FLAG} \\
\end{tabular}\end{indpar}
...
the object's {\tt min::\EOL OBJ\_\EOL PUBLISH} flag.

All of the above maintenance functions work by creating
...
{\tt min::\EOL OBJ\_\EOL PUBLISH} flag as already been set.  The
The {\tt min::hash\_count\_of} function will work, and just uses a
...
can also be used as a form of null pointer.  By indexing auxiliarly
...
{\tt const min::\EOL stub~*} value, provided the object is public.
...
are pushed into the attribute vector or auxiliarly area only changes
the unused size.
...
the attribure vector has zero length.
...
\verb|void min::| & \MINKEY{resize\REL}%
      ...
      \end{tabular}
\LABEL{MIN::RESIZE_OF_OBJ_VEC_INSPTR} \\
\verb|bool min::| & \MINKEY{resize\REL}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::obj_vec_insptr & vp,| \\
      \verb|   min::unsptr unused_size )| \\
      \end{tabular}
\LABEL{MIN::RESIZE_UNUSED_OF_OBJ_VEC_INSPTR} \\
\verb|bool min::| & \MINKEY{expand\REL}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::obj_vec_insptr & vp,| \\
      \verb|   min::unsptr unused_size )| \\
      \end{tabular}
\LABEL{MIN::EXPAND_UNUSED_OF_OBJ_VEC_INSPTR} \\
\end{tabular}\end{indpar}
...
The 4-argument {\tt min::resize\REL} function changes the size of the unused
area and variable vector.  The sizes can be either increased or
decreased.  Elements added to the variable vector are added to
its end and initialized with the value {\tt min::\EOL UNDEFINED}.
The unused area is zeroed.
The object body may be relocated by this function in order to
resize it, and its header size may be changed.
The sizes of the attribute vector and
auxiliary area are not changed, but can be changed by later pushes
and pops.
No change is made to retained object elements outside the unused area.

The `{\tt expand}' argument, like that `{\tt expand}'
of the {\tt min::\EOL new\_\EOL obj\_\EOL gen} function
(\pagref{MIN::NEW_OBJ_GEN}), controls whether the allocator rounds up
the unused area size of the new object up so that the new object body fills the
free block to which it is allocated.

The {\tt min::resize\REL} function returns \verb|true| if it
relocated the object's body, and \verb|false| if it did not.

The 2-argument {\tt min::resize} function is equivalent to the
4-argument function with no change in variable vector size and \verb|false|
{\tt expand} argument.

The 2-argument {\tt min::expand} function is equivalent to the
4-argument function with no change in variable vector size and \verb|true|
{\tt expand} argument.
...
for a push, {\tt MIN\_\EOL ASSERT} checks that element to be popped
actually exist,
...
\pagref{MIN::RESIZE_UNUSED_OF_OBJ_VEC_INSPTR}).
...
An object can be resized and reorganized so as to
compact the auxiliary area, move information out
of any auxiliary stubs attached to the object and into the
auxiliary area, and if desired change the size of the
hash table.  This can be done by the functions:
...
\verb|void min::| & \MINKEY{resize\REL}%
      ...
      \verb|   min::unsptr var_size )| \\
      \end{tabular}
\LABEL{MIN::RESIZE_OF_ATTR_INSPTR} \\
\verb|bool min::| & \MINKEY{resize\REL}%
      \begin{tabular}[t]{@{}l@{}}
      \verb| ( min::attr_insptr & ap,| \\
      \verb|   min::unsptr unused_size,| \\
      \verb|   min::unsptr hash_size )| \\
      \end{tabular}
\LABEL{MIN::RESIZE_HASH_OF_ATTR_INSPTR} \\
\end{tabular}\end{indpar}
