...
\subsubsection{Protected Body References and Pointer}
...
Internally it is a pointer to a location of type {\tt T}, and in this case the
pointer consists of a {\tt const min::\EOL stub~*} pointer to a stub
and an {\tt min::\EOL unsptr} byte offset of a location in the body
...
type {\tt r} equal to another reference type {\tt r2} does
...
differ according to whether or not the type {\tt T} is `locatable'.
TBD
...
When you write general values or pointers to stubs
into stubs or bodies, you need to call an acc function
to update garbage collection flags associated with stubs.
The first subsection of this section specifies how these things are
normally done, and includes
...
be called when a value that might point
...
\subsubsection{Storing General Values and Stub Pointers}
\label{STORING-GENERAL-VALUES-AND-STUB-POINTERS}
...
\footnote{More specifically, we need to disable the copy constructor
...
to another location would be a disaster.  Having done this by making
the copy constructor {\tt private}, we find that
the construction `{\tt T~x~=~y}' in our C++ compiler (GNU) works by
by compiling `{\tt T~x~(T(y))}' if {\tt y} is not of type {\tt T},
that is, the compiler uses the conversion constructor to convert {\tt y}
...
Here {\tt T} must be a \underline{class} whose only data member is of type
...
The {\tt min::locatable\_gen} type is in fact a specialization
...
\verb|T |
    & \TTOMKEY{=}{=}{of {\tt min::ref\TARG}}\ARGBREAK
      \verb|( min::ref<T> & p,|\ARGBREAK
      \verb|  min::locatable_var<T> var )|
\LABEL{MIN::=REF_OF_LOCATABLE_VAR} \\
\verb|min::ref<T> |
	& \TTOMKEY{ref\TARG}{ref\TARG}%
...
\INDEXHEADER{General Value Constants}
