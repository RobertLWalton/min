...
If the lexeme sequence begins and ends with matching brackets, these
are removed from the sequence, and become the {\tt .initiator} and
{\tt .terminator} attributes of the object being computed.

\item The lexeme sequence is then scanned from right to left to produce
the list elements of the object being computed.  At each stage of the
...
object whose only attributes are a {\tt 1} attribute that is the quoted string
...
Recall the in object representations there is no difference between
a quoted string representation of a mark and the mark, but the quoted
string representation of marks that can be operators is used, to prevent
them from being misread as operators.  Thus \verb|"+"| is used in the
