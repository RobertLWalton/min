...
\verb|min::line_format * file| & \TTARMKEY{line\_format}{in {\tt min::file}}
...
\verb|void min::|
    & \MINKEY{init\_line\_format\REL}\ARGBREAK
	  ...
	  \verb|  min::line_format * line_format )|
...
\verb|void min::| & \MINKEY{init\_\EOL input\RESIZE}\ARGBREAK
          \verb|( min::ref<min::file> file,|\ARGBREAK
	  \verb|  min::line_format * line_format = NULL,|\ARGBREAK
...
\TT{file->\EOL line\_\EOL format\EOL .op\_\EOL flags}
...
If \TT{file->\EOL line\_\EOL format} is \TT{NULL},
\TT{printer->\EOL print\_\EOL format.line\_\EOL format} is used in its
place.
...
\begin{indpar}\begin{verbatim}
    min::print_line_column
        ( file, position, file->line_format,
...
\verb|min::uns32 min::|
    & \MINKEY{print\_\EOL line\_\EOL column\RESIZE}\ARGBREAK
      \verb|( min::file file,|\ARGBREAK
      \verb|  min::phrase_position const & position,|\ARGBREAK
      \verb|  min::line_format * line_format,|\ARGBREAK
      \verb|  const min::print_format & print_format )|
\LABEL{MIN::PRINT_LINE_COLUMN} \\
\end{tabular}\end{indpar}

that, given a file, a position within that file, a line format,
...
with \TT{line\_\EOL format\EOL .op\_\EOL flags}.
If the \TT{line\_format} argument is \TT{NULL},
the \TT{print\_format} argument's \TT{line\_format} member (which must
\underline{not} be \TT{NULL}) is used
in its place.


The above functions use \TT{file->line\_format}
...
is on in \TT{file->line\_format}, \TT{<NL>} 
...
an extra \TT{line\_\EOL format} argument and use that in place of
\TT{file->\EOL line\_\EOL format}:
...
\LABEL{MIN::PRINT_LINE_WITH_LINE_FORMAT} \\
\end{tabular}\end{indpar}
...
\verb|min::uns32 min::|
    & \MINKEY{print\_\EOL phrase\_\EOL lines\RESIZE}\ARGBREAK
      ...
      \verb|  min::phrase_position const & position,|\ARGBREAK
      \verb|  char mark = '^' )|\ARGBREAK
\LABEL{MIN::PRINT_PHRASE_LINES_WITH_LINE_FORMAT} \\
\end{tabular}\end{indpar}
