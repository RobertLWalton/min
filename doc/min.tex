...
The variable:
\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{max\_id\_strlen} ~~~~[default \TT{63}]
...
to denote the \TT{min::gen} value associated with the given
vector {\em index}.
...
\subsubsection{Output Using Identifier Maps}
\label{OUTPUT-USING-IDENTIFIER-MAPS}
...
\subsubsection{Input Using Identifier Maps}
\label{INPUT-USING-IDENTIFIER-MAPS}

When an identifier map is used for input, map elements are
read and written by the functions:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINMKEY{get}{of id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id )|
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{set\REL}{of id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
...
When a `\TT{@}{\em index}' token is encountered in the input
and {\em index} is not in the map, normal procedure is
to set the \TT{map[{\em index}]} element using \TT{min::set}
to a new preallocated stub (\itemref{PREALLOCATED-STUBS}).
Then when a definition for
`\TT{@}{\em index}' is encountered later in the input, the preallocated
stub is filled in.
...
So a `\TT{@}{\em index}' token that is to be defines as
...
of the token.  The preallocated stub contains the {\em index} number
of the token that created it, so an error message
can be output identifying precisely the input tokens that are
left dangling.
...
\verb|min::ustring map->| & \TTMKEY{id\_char}{of {\tt min::id\_map}}
			  ~~~~ (defaults to \TT{"\textbackslash x01%
			                         \textbackslash x01" "@"})
\LABEL{MIN::ID_MAP_ID_CHAR} \\
\verb|min::ustring map->| & \TTMKEY{id\_assign}{of {\tt min::id\_map}}
			  ~~~~ (defaults to \TT{"\textbackslash x03%
			                         \textbackslash x03" "::="})
\LABEL{MIN::ID_MAP_ID_ASSIGN} \\
\verb|min::ustring map->| & \TTMKEY{id\_paragraph}{of {\tt min::id\_map}}
			  ~~~~ (defaults to \TT{"\textbackslash x02%
			                         \textbackslash x02" "::"})
\LABEL{MIN::ID_MAP_ID_PARAGRAPH} \\
\end{tabular}\end{indpar}
...
to print the string has a \TT{id\_\EOL strlen} member less than the
string length and the string length is also greater than
\TT{min::\EOL max\_id\_strlen}.
...
\begin{TABULAR}{1.8in}
\verb|min::unsptr min::| & \MINKEY{max\_id\_strlen}
\LABEL{MIN::MAX_ID_STRLEN} \\
\end{TABULAR}
