...
\begin{indpar}\begin{verbatim}
    ...
    evaluate ( EXPRESSION, CONTEXT ):
        if EXPRESSION is a single number atom:
            CONTEXT.CURRENT-OBJECT = EXPRESSION
        else if EXPRESSION is `...' bracketed list:
            quoted ( EXPRESSION, CONTEXT, 1 )
        else if EXPRESSION is {...} bracketed list:
    ...
    quoted ( EXPRESSION, CONTEXT, DEPTH ):
        ...
        foreach ITEM in EXPRESSION:
            ...
            if ITEM is [...] bracketed:
                if DEPTH > 1:
                    quote ( ITEM, C, DEPTH - 1 )
                    append C.CURRENT-OBJECT to RESULT
                else:
                    evaluate ( ITEM, C )
                    concatenate C.CURRENT-OBJECT to end of RESULT
    ...
    code ( EXPRESSION, CONTEXT ):
        ...
        foreach STATEMENT in EXPRESSION:
            ...
            while S.REPEAT-FLAG:
                C = push CONTEXT
                C.CURRENT-STATEMENT = STATEMENT
                       C.CURRENT-ITEM = first item of STATEMENT
                C.STATE = S
                evaluate ( STATEMENT, C )
                S = C.STATE
                if S.RETURN-FLAG:
                    CONTEXT.CURRENT-OBJECT = C.CURRENT-OBJECT
