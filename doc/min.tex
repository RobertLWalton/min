...
\verb|const min::str_classifier min::|
    & \MINLKEY{quote\_non\_word}{\_str\_classifier}
\LABEL{MIN::QUOTE_NON_WORD_STR_CLASSIFIER} \\
...
The \minlkey{quote\_non\_word}{\_str\_classifier} is just like the
\TT{min::\EOL standard\_\EOL str\_\EOL class\-ifier} except that
it adds the \TT{min::\EOL NEEDS\_\EOL QUOTES} flag if the
result does \underline{not} have the \TT{min::\EOL IS\_\EOL LETTER} flag.
...
In this format `\TT{@}', `\TT{::=}', and the \TT{min::gen\_format}
...
There is more information on Printing Using an Idenfitier Map
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINMKEY{get}{of numeric id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id )|
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{set\REL}{of numeric id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{clear}{of numeric id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id )|
...
The \TT{min::get} function returns \TT{min::NONE()} if the \TT{id}
is not mapped.  The \TT{min::set} function adds elements
...
equal \TT{min::NONE()}.  The \TT{min::clear} function removes
...
to set the \TT{map[}{\em id}\TT{]} element using \TT{min::set}
...
but the following identifier map parameter can be used by such
a scanner
to determine whether a token has the form `\TT{@}{\em id}\,'
where `\TT{@}' is the value of the following parameter and {\em id}
is a string of digits.

\begin{indpar}[1em]\begin{tabular}{r@{}l@{~~~~~~~~}l}
			  & \TT{map} Member	& \verb|//| Default
\\[1ex]
\verb|min::Uchar map->| & \TTMKEY{id\_Uchar}{of {\tt min::id\_map}}
			  & \verb|//| \TT{U'@'}
\LABEL{MIN::ID_MAP_ID_UCHAR} \\
\end{tabular}\end{indpar}
...
`\TT{@}{\em symbol}\,\TT{@}' tokens are not output by \TT{min} functions.
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINMKEY{get}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{set\REL}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{clear}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
\LABEL{MIN::CLEAR_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}

It is an error to use these functions if
the identifier map has an output hash table.
...
\TT{min::gen} MIN string identifer.  The string is the {\em symbol},
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
\verb|min::quote_non_word_str_format          // str_format|
\end{tabular}\end{indpar}
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
\verb|min::quote_non_word_id_str_format       // str_format|\ARGBREAK
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
\verb|min::quote_non_word_str_format          // str_format|\ARGBREAK
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\multicolumn{2}{l}{const min::str\_format *
                   min::\MINLKEY{quote\_non\_word}{\_str\_format}:}
\LABEL{MIN::QUOTE_NON_WORD_STR_FORMAT}\ARGBREAK
\verb|// Same as min::standard_str_format except for:|\ARGBREAK
\verb|min::quote_non_word_str_classifier // str_classifier|\ARGBREAK
\end{tabular}\end{indpar}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::erase_all_space_double_colon_pstring  // obj_attreol|\ARGBREAK
\end{tabular}
...
\multicolumn{2}{l}{\tt const min::str\_classifier} \\
\verb|    min::|
    & \MINKEY{quote\_non\_word\_str\_classifier}
\LABEL{MIN::QUOTE_NON_WORD_STR_CLASSIFIER} \\
\multicolumn{2}{l}{\tt const min::str\_classifier} \\
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_non_word_id_str_format       // str_format|\ARGBREAK[0in]
\verb|min::compact_id_obj_format              // obj_format|
\end{TABULAR}
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_non_word_str_format          // str_format|\ARGBREAK[0in]
\verb|min::isolated_line_id_obj_format        // obj_format|
\end{TABULAR}
...
\verb|min::erase_all_space_double_colon_pstring // obj_attrsep|\ARGBREAK[0in]
\verb|0                                        // hide_flags|
\end{TABULAR}
