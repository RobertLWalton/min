...
Cooked data representations differ from raw data representations
in that cooked data representations contain {\em expressions}.
An {\em expression} is a list of atoms, bracketted subexpressions,
and operators.

\secref{OPERATOR-EXPRESSIONS} describes how to preprocess
expressions containing operators to produce expressions not containing
operators.  Expressions bracketted with curly brackets \verb|{ }| are
pre-processed as explained in \secref{CODE-EXPRESSIONS} to remove
\verb|:| lexemes and insert \verb|{|, \verb|}|, and \verb|;| lexemes.
In the rest of this section we will assume all preprocessing has been
done, and {\em expressions} are just lists of {\em atoms} and
{\em bracketted-subexpressions}.

After being read, expressions are expanded by calling macros.  A macro
is a piece of code that computes the true value of an expression from
the expression that is read.

For example, the expression

\subsection{Operator Expressions}
\label{OPERATOR-EXPRESSIONS}
