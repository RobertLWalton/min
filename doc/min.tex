...
It also includes the cases where the converted number is outside
the range of the \TT{value} argument, including cases like
\TT{1e+500} and \TT{1e-500} which produce numbers too large or
too small to be represented in a {\tt min::float64} value.
However, if the \TT{min::gen} argument is a string such as
\TT{Inf} or \TT{NaN} representing a floating point infinity
or not-a-number, conversion to floating point will succeed.
...
and the case where the number that might be returned would be
outside the range of
numbers storable in the \TT{value} argument (e.g., \TT{1e+500}
and \TT{1e-500}).  However, strings such as \TT{Inf} and \TT{Nan}
representing floating point infinity of not-a-number will
be converted.
...
\\[1ex]
\verb|min::ustring map->| & \TTMKEY{id\_char}{of {\tt min::id\_map}}
			  & \verb|//|
			    \TT{"\textbackslash x01\textbackslash x01" "@"}
\LABEL{MIN::ID_MAP_ID_CHAR} \\
\verb|min::ustring map->| & \TTMKEY{id\_assign}{of {\tt min::id\_map}}
			  & \verb|//|
			    \TT{"\textbackslash x03\textbackslash x03" "::="}
...
and the \ttikey{PREFERRED\_\EOL ID}{PREFERRED\_ID}
