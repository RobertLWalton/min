...
Expressions are evaluated in an \key{evaluation context} that
...
the beginning of the remaining argument list.
\end{indpar}
...
expression.  The initial evaluator is used to set the current evaluator
when evaluation of new expression begins.  Thus when a subexpression
of the current expression is to be evaluated, a new evaluation context
is created for evaluating the subexpression, and the initial evaluator
becomes both the initial and current evaluator in the new context.
\end{indpar}
...
An object or frame has an `ttdkey{ancestors}' attribute that is a
list of all the ancestors of the object or frame.  The last element
of this list is called the `\key{lexical parent}' of the object or
frame.  Ancestors of objects must be other objects, while ancestors
...
\ttakey{frame}{of evaluation context} & MIN frame
    & Top frame on the frame stack whenever the evaluation
      context is the top evaluation context on the evaluation stack. \\[0.5ex]

\ttakey{caller}{of evaluation context} & evaluation context
    & Evaluation context of the expression which called the function
      execution of which the current evaluation context is a part. \\[0.5ex]

\ttakey{expression}{of evaluation context} & MIN expression
...
\ttakey{initial value}{of evaluation context} & MIN value
    & Initial value for evaluating the expression being evaluated. \\[0.5ex]

\ttakey{current value}{of evaluation context} & MIN value
    & Current value for evaluating the expression being evaluated.
...
\ttakey{arguments}{of evaluation context} & MIN expression
    & An expression containing the remaining arguments.  This often
      equals the expression being evaluated. \\[0.5ex]

\ttakey{next}{of evaluation context} & non-negative integer
    & The remaining arguments are all the elements of the `{\tt arguments}'
      expression list from the element whose index is `{\tt next}'
...
\ttakey{input values}{of evaluation context} & MIN list or MISSING
    & The possibly empty list of input values, or the special MISSING
      value.  The MISSING value need not be a MIN value, and
      may be the C++ NULL value.  \\[0.5ex]

\ttakey{output values}{of evaluation context} & MIN list or MISSING
...
\ttakey{condition code}{of evaluation context} & 32 bits
...
The MIN system \skey{condition code flag}s are:

\begin{indpar}
\begin{list}{}{}

\item[\ttakey{repeat}{condition code flag}]~\\
    If on at the end of evaluating a statement in a block,
    this flag is turned off and the statement is repeated.

\item[\ttakey{skip else}{condition code flag}]~\\
...
\item[\ttakey{skip catch}{condition code flag}]~\\
    Similar to the {\tt skip else} flag, but skips {\tt catch}
    statements instead of {\tt else} statements.  Set while
    executing {\tt try} statements.  Cleared if a {\tt throw}
    to the TBD

\item[\ttakey{return}{condition code flag}]~\\
    If on at the end of evaluating a statement in a block,
