...
until later in the file, by using a preallocated stub
(\pagref{MIN::PREALLOCATED})
as a place to put the object contents when
...
\verb|min::gen min::| & \MINKEY{get}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id )|
\LABEL{MIN::GET_OF_ID_MAP} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{put\REL}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id,|\ARGBREAK
    \verb|  min::gen g )|
\LABEL{MIN::PUT_OF_ID_MAP} \\
\end{tabular}\end{indpar}
...
If identifiers are being input, \TT{min::get(map,id)} can be used
to return the \TT{min::gen} value associated with \TT{id}.  If there
is no such value, \TT{min::\EOL get} will return \TT{min::\EOL NONE()},
and the \TT{min::\EOL put} function can be used to associate a
\TT{min::gen} value with {\tt id}.  The \TT{min::put} function
...
If there is no value associated with \TT{id}, the
