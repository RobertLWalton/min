...
other hand, behave like unchangeable constants and not variables.
...
For output a \TT{min::\EOL id\_\EOL map} has
an optional associated \mkey{numeric hash table}{of identifier map}
that maps \TT{min::gen} values to numeric identifier indices.
When the hash table is used, the map between numeric identifier indices
and \TT{min::gen} values can be made 1-to-1.  This should be done when
outputting object values,
so numeric identifiers act as pointers in the external file.

For input a \TT{min::\EOL id\_\EOL map} has
an optional associated \mkey{symbolic hash table}{of identifier map}
that maps symbols to \TT{min::gen} values.  The symbols are
represented as MIN strings.  Unlike the other identifier map
tables, the \TT{min:gen} value a symbol is mapped to can be changed.
