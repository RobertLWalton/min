...
A CURRENT-OBJECT. \\
A CURRENT-STATEMENT. \\
A CURRENT-ITEM in the current statement. \\
An EXECUTE-INDICATOR: NEXT-ITEM, NEXT-STATEMENT, REPEAT-STATEMENT. \\
A FAILURE-FLAG. \\
\end{indpar}
...
\begin{indpar}\begin{verbatim}
    ...
    evaluate ( EXPRESSION, CONTEXT ):
        if EXPRESSION is a single number atom:
            return EXPRESSION
        if EXPRESSION is `...' bracketed list:
            return quoted ( EXPRESSION, CONTEXT )
        if EXPRESSION is {...} bracketed list:
            return code ( EXPRESSION, CONTEXT )
        return search ( EXPRESSION, CONTEXT )

    quoted ( EXPRESSION, CONTEXT ):
        if EXPRESSION contains no [...] bracketted sublists:
	    return EXPRESSION
	// TBD: Handle depth, e.g., `foo `fum [[x]] [y]''

    code ( EXPRESSION, CONTEXT ):
        ` EXPRESSION is {...} bracketed'
        push CONTEXT
	     new CONTEXT has same SEARCH-OBJECT as old CONTEXT
	     and begins at first item of first statement in
	     expression, with FAILURE-FLAG off, and CURRENT-OBJECT
	     equal SEARCH-OBJECT.
	     ???
