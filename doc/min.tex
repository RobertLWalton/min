...
\begin{indpar}[0.5em]
Same as (\ref{LOCAL-WRITE}) but with the the ancestor in
place of the current object.
\end{indpar}
...
this string is the longest such string in case there are
several that name attributes of the current object,
and there are not two ancestors that have an attribute
with this name.

\begin{indpar}[0.5em]
Same as (\ref{LOCAL-READ}) but with the ancestor in
place of the current object.
\end{indpar}
...
\subsection{Builtin Operations}
\label{BUILTIN-OPERATIONS}

The builtin operations are the call operation and the error operation.
