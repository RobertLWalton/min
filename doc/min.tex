...
\item[\ttmkey{OUTPUT\_HTML}%
               {in {\tt print\_\EOL format.op\_\EOL flags}}]~\\
If this flag is on, HTML reserved characters that are output to
\TT{printer->\EOL file->\EOL ostream}
are replaced as per Figure~\ref{HTML-RESERVED-CHARACTERS}.
It is a \TT{MIN\_\EOL ASSERT} error if this flag is on and
...
between any operations that put characters into the buffer
and any use of the \TT{min::tag} \TT{ostream}.

Some example uses are:

\begin{indpar}\begin{verbatim}
min::tag(printer) << "<pre>\n";
min::tag(printer) << "<p class='" << p_class << "'>\n";
\end{verbatim}\end{indpar}
...
For example, printing an end of line by sending \TT{'\textbackslash n'}
to \TT{min::tag} does not set \TT{printer->\EOL column} to \TT{0}.
