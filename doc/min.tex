...
This document describes MIN, the Minimal Runtime System.
...
\TT{expression} with \TT{value} if \TT{expression} is not \TT{NULL},
and \TT{message\_\EOL format} and subsequent arguments
as per \TT{printf} if \TT{message\_\EOL format} is not \TT{NULL}.
In addition if \TT{value}, \TT{min::\EOL assert\_\EOL throw},
and \TT{min::\EOL assert\_\EOL print} are \underline{all} \TT{false},
...
there was an actual interrupt that might have relocated bodies.
If it returns \TT{false}, no bodies were relocated.

The inline function
\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::| & \MINKEY{pending} \verb|( void )|
\LABEL{MIN::PENDING} \\
\end{tabular}\end{indpar}%
\index{interrupt!pending}
checks an interrupt
flag, returns \TT{true} if that is set, and returns \TT{false}
otherwise.  It \underline{never} calls an out-of-line function
and \underline{never} relocates bodies.  If it returns \TT{true},
code should prepare for bodies to be relocated and then call
\TT{min::interrupt()}.
...
\item[\TT{const min::gen min::}\MINKEY{TRUE()}]~%
	\LABEL{MIN::TRUE}\\
Used to represent the `true' truth value.
\item[\TT{const min::gen min::}\MINKEY{FALSE()}]~%
	\LABEL{MIN::FALSE}\\
Used to represent the `true' truth value.
\end{list}
\end{indpar}
...
\subsubsection{Preallocated Stubs}
\label{PREALLOCATED-STUBS}

A stub can be \key{preallocated}, referenced, and even garbage
collected, before its contents are known.  Such stubs have the
\TT{PREALLOCATED} type.
...
\verb|min::uns32 min::| & \MINKEY{id\_of\_preallocated}
    \verb|( min::gen g )|
\LABEL{MIN::ID_OF_PREALLOCATED} \\
\verb|min::uns32 min::| & \MINKEY{count\_of\_preallocated}
    \verb|( min::gen g )|
\LABEL{MIN::COUNT_OF_PREALLOCATED} \\
\verb|void::| & \MINKEY{increment\_preallocated}
    \verb|( min::gen g )|
\LABEL{MIN::INCREMENT_PREALLOCATED} \\
\end{tabular}\end{indpar}
...
The last three of the above functions use all the
values \TT{p[0]}, \ldots, \TT{p[n-1]}
as a second argument in a call to
the corresponding function chosen from the first three
of the above functions.

The \TT{min::acc\_write\_update} function on
\pagref{MUP::ACC_WRITE_UPDATE_OF_OBJ_VEC_PTR} is similar to these
last functions, but takes all the pointers stored in an object
as second arguments.
...
The \TT{MUP::new\_\EOL body} and \TT{MUP::resize\_\EOL body} functions
(\itemref{UNPROTECTED-BODY-FUNCTIONS}) \underline{may} be called
to fill a \TT{min::ACC\_\EOL FREE} stub, and these may invoke
the garbage collector.
...
\begin{indpar}
\begin{tabular}{l@{~~~~~}l}
\ttkey{min::IS\_HSPACE}:\label{IS-HSPACE} & \TT{HT}, plus
...
\label{STANDARD_CHAR_FLAGS_FLAGS}
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{IS\_DIGIT}
\LABEL{MIN::IS_DIGIT} \\
\verb|const min::uns32 min::| & \MINKEY{IS\_NATURAL}
\LABEL{MIN::IS_NATURAL} \\
...
The following flags serve to identify various types of lexemes.
A lexeme that has a character with the \minkey{IS\_\EOL DIGIT} flag before
any character with the \minkey{IS\_\EOL LETTER} flag is a `\key{number}.
A number consisting of just characters with the \minkey{IS\_\EOL NATURAL}
flag is a `\key{natural number}', unless it begins with `\TT{0}' and
has more than one digit.
A lexeme that has a character with the \minkey{IS\_\EOL LETTER} flag before
any character with the \minkey{IS\_\EOL DIGIT} flag is a `\key{word}.
A lexeme all of whose characters have the \minkey{IS\_\EOL MARK} flag is
a `\key{mark}'.
See \TT{min::\EOL standard\_\EOL str\_\EOL classifier}
(\itemref{STRING-CLASSIFIERS})
and the 
\TT{mark\_\EOL classifier} of the object format,
\pagref{MARK_CLASSIFIER}.

The assignment of these flags in \TT{min::standard\_char\_flags}
is as follows:

\begin{center}
\begin{tabular}{l@{~~~~~}l}
\ttkey{min::IS\_DIGIT}: & category \TT{Nd} (digits) \\
\ttkey{min::IS\_NATURAL}: & the characters
                            \TT{0}, \TT{1}, \TT{2}, \TT{3}, \TT{4},
                            \TT{5}, \TT{6}, \TT{7}, \TT{8}, and \TT{9} \\
\ttkey{min::IS\_LETTER}: & category \TT{L} (letters) \\
...
The \minlkey{quote\_value}{\_str\_classifier} is just like the
\TT{min::\EOL standard\_\EOL str\_\EOL class\-ifier} except that
it adds the \TT{min::\EOL NEEDS\_\EOL QUOTES} flag if the
result does \underline{not} have either the \TT{min::\EOL IS\_\EOL LETTER} flag
or the \TT{min::\EOL IS\_\EOL DIGIT} flag.
...
\TT{min::str\_\EOL format} that is \TT{NULL} and returns 0
if \underline{all} characters
in the string have the \TT{IS\_\EOL VHSPACE} flag, and just
\TT{min::\EOL IS\_\EOL GRAPHIC} otherwise.

The following function is used by the \TT{min::standard\_str\_classifier}:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|bool min::|
    & \MINKEY{is\_number} \verb|( min::unsptr n, min::ptr<const min::Uchar> p )|
\LABEL{MIN::IS_NUMBER} \\
\end{tabular}
\begin{indpar}
Returns true iff the UNICODE {\tt n} character
string pointed at by {\tt p} is acceptable to {\tt strtod}.
This means that it has one of the following syntaxes:
\begin{center}
\begin{tabular}{l}
\{{\tt +}|{\tt -}\}\QMARK{}
$digit$\PLUS{} \{{\tt .}$digit$\STAR{}\}\QMARK{}
...
and \TT{min::\EOL unicode::\EOL extra\_\EOL names}
of a character is added to the table if the character's flags,
...
In addition, a string of up to 3 bytes can be stored within a 32-bit
\TT{min::gen} value, and a string of up to 5 bytes can be stored
...
A short string holds up to 8 bytes inside the string stub
(there must be more bytes than a direct string will hold).
A long string has an string body that holds the string bytes
(there must be more than 8)
along with the string length and hash value.

Sufficiently short strings are \skey{identifier string}s.
The variable:
\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{max\_id\_strlen} ~~~~[default \TT{32}]
\LABEL{MIN::MAX_ID_STRLEN} \\
\end{tabular}\end{indpar}
...
For floating point \TT{value}'s, the string representing the number
may be \TT{"-Inf"}, \TT{"NaN"}, or similar, with letter
case ignored.

The following function makes a copy of a \underline{non-identifier}
string pointed at
by a string pointer in a preallocated stub:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINKEY{copy}\ARGBREAK
	\verb|( min::gen preallocated,|\ARGBREAK
	\verb|  min::str_ptr & const sp )|
\LABEL{MIN::COPY_STR_TO_PREALLOCATED} \\
\end{tabular}\end{indpar}

Only non-identifier strings may be so copied, because they
are the only strings that
are not in the string hash table.
...
bytes than can be stored in a direct string
...
are often \skey{atom}s, i.e., single numbers or identifier strings.
...
Labels are sometimes used to list names (e.g., strings).  The following
returns the index (\TT{0}, \TT{1}, \TT{2}, etc.)
of the first occurence of the \TT{key} as an element of
the \TT{label}:

\begin{indpar}\begin{tabular}{r@{}l}
\verb|int min::| & \MINKEY{labfind} \verb|( min::gen key, min::gen label )|
\LABEL{MIN::LABFIND} \\
\end{tabular}\end{indpar}

If \TT{key} is not in \TT{label}, \TT{-1} is returned. 
...
the \TT{const min::gen} members of \TT{S},
...
be given (its address should be \TT{NULL}) if there are no \TT{const min::gen}
values in \TT{S}.  But if there are \TT{const min::gen} members, this
...
Ditto but for \TT{const min::stub * const} members of \TT{S}
instead of \TT{const min::\EOL gen} members.  The following member types are
treated as \TT{const min::\EOL stub * const} members for these purposes:
\begin{center}
\tt
\begin{tabular}{l}
const min::packed\_struct\_xxxptr<T> \\
const min::packed\_vec\_xxxptr<E,H,L> \\
\end{tabular}
\end{center}
...
the types mentioned above: \TT{const min::gen}, \TT{const min::stub~* const},
\TT{const min::\EOL packed\_\EOL \ldots{}ptr<\ldots>}, etc.

However a member does not have to be \TT{const}, as non-\TT{const}
members will be converted to \TT{const} members implicitely.
Thus members of type \TT{min::gen} and \TT{const min::stub~*} will
work, but \underline{not} one of type \TT{min::stub~*}, as that
is a different type from \TT{const min::stub~*}.

If you have a structure type \TT{T} containing a member \TT{m} of one of these
types, and at most one such member, you can define the following
that allows a structure \TT{S} to have members of type \TT{const T}:

\begin{indpar}\begin{verbatim}
template<typename S>
min::uns32 DISP ( const T S::* d )
{
    return min::OFFSETOF ( d ) + min::DISP ( & T::m );
}
\end{verbatim}\end{indpar}

The function call \TT{min::OFFSETOF\footnotemark ( \& S::m )}
returns the offset
of member \TT{m} in structs of type \TT{S}, but this may not
be what you want if \TT{m} contains a \TT{const min::gen} or a
\TT{const min::stub~* const} at offsets other than zero.  Hence the
second term \TT{min::\EOL DISP ( \& T::m )} is needed.
\footnotetext{The C/C++ macro \TT{offsetof} does not seem to
be reliable, so \TT{min::\EOL OFFSETOF} has been defined to
be used instead.}
...
\item[\ttmkey{subtype}{in {\tt min::packed\_vec}}]\label{PACKED_SUBTYPE}
...
A \TT{line\_format} also contains the following parameters:
\begin{center}
\begin{tabular}{rl@{~~~~~}l}
\verb|const char *| & \TT{blank\_line} & \pagref{LINE_FORMAT_BLANK_LINE} \\
\verb|const char *| & \TT{end\_of\_file} & \pagref{LINE_FORMAT_END_OF_FILE} \\
\verb|const char *| & \TT{unavailalable\_line}
                    & \pagref{LINE_FORMAT_UNAVAILABLE_LINE} \\
\verb|const char *| & \TT{line\_table\_class}
                    & \pagref{LINE_FORMAT_LINE_TABLE_CLASS} \\
\verb|const char *| & \TT{line\_number\_class}
                    & \pagref{LINE_FORMAT_LINE_NUMBER_CLASS} \\
\verb|char| & \TT{mark} & \pagref{LINE_FORMAT_MARK} \\
\verb|const char *| & \TT{line\_mark\_class}
                    & \pagref{LINE_FORMAT_LINE_MARK_CLASS} \\
\end{tabular}
\end{center}
...
For example,
\begin{indpar}\begin{verbatim}
min::locatable_var<min::file> file1, file2;
init_input ( file1 );
init_input_... ( file2, ... );
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|const min::uns32 min::| & \MINKEY{NO\_LINE} \verb|= 0xFFFFFFFF|
\LABEL{MIN::NO_LINE} \\
\verb|min::uns32 min::|
...
The length of the identifier map vector is \TT{map->length}.
The value of the vector element with index \TT{id} is
\TT{map[id]}.  \TT{map[0] == NONE()} is always true, but
if \TT{map[id] == NONE()} for \TT{0 < id < map->length} or if
\TT{id >= map->length} then \TT{id} has \underline{not} been given
a value by the map.

The \TT{min::init} function may be used to create or reset a \TT{map}
so that \TT{map->length == 1} and \TT{map[0] == NONE()} and
neither the output or input hash tables exist.

\subsubsection{Output Using an Identifier Map}
\label{OUTPUT-USING-AN-IDENTIFIER-MAP}

When an identifier map is used for output, map elements are
...
These functions maintain the identifier map
\mkey{output hash table}{of identifier map}
that maps \TT{min::gen} values back to vector indices:
\begin{center}
\TT{map[id]==g} if and only if \TT{find(map,g)==id} \\
provided \TT{g != NONE()}
\end{center}

Thus there is a 1-to-1 correspondance between identifier map
vector indices and \TT{min::gen} values stored in the identifier
map vector (except for the value \TT{min::NONE())}.

\TT{min::find(map,g)} returns \TT{0} if \TT{g} is not in the
identifier map vector or if \TT{g == min::NONE()}.
...
MIN does not provide input scanners to convert text to tokens,
but the \TT{ID\_character} identifier map parameter
(\pagref{MIN::ID_MAP_ID_CHARACTER})
can be used by such
a scanner
to determine whether a token has the form `\TT{@}{\em id}\,'
where `\TT{@}' is the value of the \TT{ID\_character} parameter and {\em id}
is a string of digits.

\subsubsection{Input Using a Symbolic Identifier Map}
\label{INPUT-USING-A-SYMBOLIC-IDENTIFIER-MAP}

Optionally, on input `\TT{@}{\em symbol}\,\TT{@}' tokens may be
used like variables with \TT{min:gen} values.  These
\smkey{symbolic identifier}s{in identifier map}\index{identifier!symbolic}
are useful when input is generated by people, and not
automatically.  The `\TT{@}{\em id}\,' tokens, on the
other hand, behave like unchangeable constants and not like variables.

`\TT{@}{\em symbol}\,\TT{@}' tokens are not input or
output by \TT{min} functions.

An identifier map used for input may have an associated
\mkey{input hash table}{of identifier map}
that maps symbols to \TT{min::gen} values.  Elements in this table
are gotten and set by:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::gen min::| & \MINMKEY{map\_get}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
\LABEL{MIN::MAP_GET_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{map\_set\REL}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol,|\ARGBREAK
    \verb|  min::gen g )|
\LABEL{MIN::MAP_SET_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINMKEY{map\_clear}{of symbolic id map}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
\LABEL{MIN::MAP_CLEAR_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}
...
\verb|     const char * |
	& \TTMKEY{line\_table\_class}{in {\tt min::line\_format}} \\
\verb|     const char * |
...
\verb|"MIN-LINE-TABLE",          // line_table_class|\ARGBREAK
...
\item[\ttmkey{OUTPUT\_HTML}%
               {in {\tt print\_\EOL format.op\_\EOL flags}}]~\\
If this flag is on, when HTML reserved characters are copied from the
printer buffer to \TT{printer->\EOL file->\EOL ostream}, they
are replaced as per Figure~\ref{HTML-RESERVED-CHARACTERS}.

It is a \TT{MIN\_\EOL ASSERT} error to set this flag when
\TT{printer->\EOL file->\EOL ostream} is \TT{NULL}.
Setting or clearing this flag flushes the printer buffer
before the flag change is made.

Also see \TT{min::tag} on \pagref{MIN::TAG}.
...
\item[\ttmkey{FORCE\_SPACE}%
    {in {\tt print\_\EOL format.op\_\EOL flags}}]~\\
This flag forces the \TT{min::leading}, \TT{min::trailing},
and \TT{min::space\_\EOL if\_\EOL needed}
...
\TT{min::\EOL leading/\EOL trailing/space\_\EOL if\_\EOL needed}.
...
The \TT{min::\EOL set\_\EOL break} operation may be delayed until
after a preceeding
\TT{min::\EOL leading/\EOL trailing/\EOL space\_\EOL if\_\EOL needed} operation
has executed.  See
\pagref{DELAYED-SET-BREAK} for details.
...
The \TT{min::\EOL set\_\EOL indent} operation may be delayed until
after a preceeding
\TT{min::\EOL leading/\EOL trailing/\EOL space\_\EOL if\_\EOL needed} operation
has executed.  See
\pagref{DELAYED-SET-BREAK} for details.
...
\TT{min::bom} executes a \TT{min::bol} and then both
...
After \TT{min::bom} the indent can be set to any point in the message
...
Here the contents of the error message will be indented by
...
one of the following print operations
...
\TT{min::}\MINKEY{space\_if\_needed}\label{MIN::SPACE_IF_NEEDED} \\
\TT{min::}\MINKEY{space\_if\_needed\_always}%
    \label{MIN::SPACE_IF_NEEDED_ALWAYS} \\
\end{tabular}
\end{center}
followed by either or both of the operations
\begin{center}
\TTALL
\begin{tabular}{l}
min::save\_indent \\
...
\item Otherwise if the sequence begins with
\begin{center}
\TTALL
\begin{tabular}{l}
min::leading \\
min::trailing \\
min::space\_if\_needed \\
\end{tabular}
\end{center}
and the \TT{FORCE\_\EOL SPACE} printer\label{FORCE_SPACE}
...
min::leading\_always \\
min::space\_if\_needed \\
min::space\_if\_needed\_always \\
\end{tabular}
\end{center}
and the preceding print item has the \TT{IS\_\EOL LEADING}
...
\end{tabular}
\end{center}
and the following print item has the \TT{IS\_\EOL TRAILING}
flag, no space is output.
\item Otherwise a space is output.
\end{enumerate}

For an operation sequence as described above%
\label{DELAYED-SAVE-INDENT}%
\label{DELAYED-SET-BREAK}
the \TT{min::\EOL save\_\EOL indent} and/or \TT{min::\EOL set\_\EOL break}
operations
are delayed until the space is output or a decision not to output
...
\verb|min::printer min::| & \MINKEY{print\_space\_if\_needed}\ARGBREAK
	\verb|( min::printer printer )|
\LABEL{MIN::PRINT_SPACE_IF_NEEDED} \\
...
\verb|min::printer min::| & \MINLKEY{print\_space\_}{if\_needed\_always}%
							\ARGBREAK
	\verb|( min::printer printer )|
\LABEL{MIN::PRINT_SPACE_IF_NEEDED_ALWAYS} \\
\end{tabular}\end{indpar}
...
The \TT{min::print\_leading/trailing/space\_if\_needed\ldots}
functions do the same thing as the corresponding
\TT{min::\EOL leading/\EOL trailing/\EOL space\_\EOL if\_\EOL needed\ldots}
printer operations.
...
\subsubsection{File Positions and Phrases}
\label{FILE-POSITIONS-AND-PHRASES}

The lines of an input file are typically saved and can be accessed
by the \TT{min::line} function (\pagref{MIN::LINE_OF_FILE}).
A position in such a file is defined as the point
\underline{between} to UTF-8 characters in the file, or the point
at the beginning of the file, or the point at the end of the file.
A position is encode in a \TT{min::position} structure:
...
(the line is encoded as a byte string of UTF-8 characters
ending with a NUL character instead of a line-feed).
...
\end{tabular}
\\[1ex]
\verb|const min::phrase_position| \MINKEY{MISSING\_PHRASE\_POSITION} =\\
\verb|    { min::MISSING_POSITION, min::MISSING_POSITION }|%
\LABEL{MIN::MISSING_PHRASE_POSITION}
\\[1ex]
\begin{tabular}{r@{}l}
\verb|bool |
    & \TTOMKEY{(bool)}{(bool)}%
              {of {\tt min::phrase\_position}}\ARGBREAK
      \verb|( const min::phrase_position & pp )|
\LABEL{OPERATOR(bool)_OF_PHRASE_POSITION} \\
\end{tabular}
\end{indpar}

Here `\TT{begin}' encodes the position before the first character
of the phrase in the file, and `\TT{end}' encodes the position
after the last character of the phrase in the file.
...
A \TT{phrase\_\EOL position} converts to \TT{false} if
it equals \TT{MISSING\_\EOL PHRASE\_\EOL POSITION}, and converts
to \TT{true} in other typical cases.  More specifically, a
\TT{phrase\_\EOL position} \TT{pp} converts to:
\begin{center}
\tt pp.begin.line != 0xFFFFFFFF
\end{center}

Positions are recorded when an input file is scanned for lexemes
and subexpressions.  Such scanning is not a part of the MIN system.
However, the output of the scanning may be stored in a
type of packed vector, the `\key{phrase position vector}\,',
...
(see \itemref{OBJECTS}), in which case it will be associated with
the attribute vector of the object (\pagref{ATTRIBUTE-VECTOR}).
...
If a MIN object has a \TT{.position} attribute equal to a
phrase position vector, its phrase position
...
\TT{min::line ( file, line\_number )} (\pagref{MIN::LINE_OF_FILE})
to print a file line:
...
\TT{min::\EOL DISPLAY\_\EOL EOL} flag, is returned.

If \TT{line\_\EOL format} is \TT{NULL},
\TT{file->line\_\EOL format} is used in its place.  If that is also
\TT{NULL},
\TT{printer->\EOL print\_\EOL format.line\_\EOL format} is used (the
end result of this must \underline{not} be \TT{NULL}).
...
then \label{LINE_FORMAT_UNAVAILABLE_LINE}
\TT{line\_\EOL format->\EOL unavail\-able\_\EOL line} is
printed on a line by itself instead, without any visible line feed,
provided that
\TT{line\_\EOL format->\EOL unavailable\_\EOL line} is not \TT{NULL}.
But if \TT{line\_\EOL format->\EOL unavail\-able\_\EOL line} is \TT{NULL},
nothing is done except to return \TT{min::\EOL NO\_\EOL LINE}.

If a line would print as blank\label{LINE_FORMAT_BLANK_LINE}, and if
\TT{line\_\EOL format->\EOL blank\_\EOL line} is not \TT{NULL}, then
\TT{line\_\EOL format->\EOL blank\_\EOL line}
is printed at the end of the line after any
whitespace characters.
More specifically, a line is deemed to print as blank if
\TT{min::DISPLAY\_\EOL EOL} is not in effect and either it contains
no characters, or 
it contains only
...
then any partial line at the end of the file buffer is printed if
there is a partial line, and then \label{LINE_FORMAT_END_OF_FILE}
\TT{line\_\EOL format->\EOL end\_\EOL of\_\EOL file} is printed,
all on one line, without an visible line feed, provided that
\TT{line\_\EOL format->\EOL end\_\EOL of\_\EOL file} is not \TT{NULL}.
However, if \TT{line\_\EOL format->\EOL end\_\EOL of\_\EOL file}
is \TT{NULL}, then
...
Note that \TT{line\_\EOL format->\EOL op\_\EOL flags} is used to print
...
\underline{graphic} characters.
Also, these strings may not be unprotected body pointers,
...
If
\begin{center}
\TT{min::OUTPUT\_HTML} is set in \TT{printer->print.format.op\_flags}
\\
and
\\
\TT{line\_format->line\_table\_class} is \underline{not} \TT{NULL}%
\label{LINE_FORMAT_LINE_TABLE_CLASS}
\end{center}
then what is printed to \TT{printer->file->ostream} is
\begin{center}
\TT{<tr><td>}{\em filtered-printed-line}\TT{</td></tr>}
\end{center}

Here {\em filtered-printed-line} is the line as it is printed
but with HTML reserved characters replaced by their corresponding HTML entities
(e.g., \TT{<} is replaced by \TT{\&lt;}), and it is required that
\TT{line\_\EOL format->\EOL line\_\EOL number\_\EOL class} be \TT{NULL}.

If instead \TT{line\_\EOL format->\EOL line\_\EOL number\_\EOL class} is
\underline{not} \TT{NULL},\label{LINE_FORMAT_LINE_NUMBER_CLASS}
the above is replaced by
\begin{center}
\TT{<tr><td class='}{\em line-number-class}\TT{'>}%
{\em N}\TT{:}\TT{</td><td>}{\em filtered-printed-line}\TT{</td></tr>}
\end{center}

where {\em N} = \TT{line\_number} + 1 and {\em line-number-class}
is the character string value of \TT{line\_\EOL format->\EOL
line\_\EOL number\_\EOL class}.

If lines are wrapped in HTML \TT{<tr>} and \TT{<td>} tags, the
code that calls \TT{min::\EOL print\_\EOL line} must wrap the
lines with a \TT{<table>} tag.  The \TT{min::\EOL print\_\EOL
phrase\_\EOL lines} function (\pagref{MIN::PRINT_PHRASE_LINES})
does this.

Error messages typically consist of a description of the error
accompanied by the file lines involved printed with the above function.
Typically the error is associated with a phrase in the file
(see \itemref{FILE-POSITIONS-AND-PHRASES}).  If the phrase is
specified by the \TT{position} argument to \TT{min::\EOL print\_\EOL line},
the phrase may be marked.

If a line is \underline{not} being wrapped in HTML tags,
a line containing marks is printed after the line,
so that under each line character in the phrase a mark is printed.
An empty phrase is marked as if it were 1-column wide, that column
being the character after the empty phrase position.

The mark is the character
\TT{line\_\EOL format->mark}\label{LINE_FORMAT_MARK}.
If it is \TT{0}, marking lines will be omitted, and the
phrase will not be marked.

If instead the line \underline{is} being wrapped in HTML tags,
the phrase characters are wrapped in
\begin{center}
\TT{<span class='}{\em line-mark-class}\TT{'>}\ldots\ldots\ldots\ldots%
\TT{</span>}
\end{center}
where {\em line-mark-class} is the value of \TT{line\_\EOL format->\EOL
line\_\EOL mark\_\EOL class}.\label{LINE_FORMAT_LINE_MARK_CLASS}
If this value is \TT{NULL},
the phrase characters are not wrapped and the phrase is not marked.

If the \TT{position} argument to \TT{min::\EOL print\_\EOL line} has
the value \TT{MIN::\EOL MISSING\_\EOL PHRASE\_\EOL POSITION}
(\pagref{MIN::MISSING_PHRASE_POSITION}), no marking is done.

To print all the lines of a phrase one can use the function:
...
The lines containing the phrase are printed and marked by
passing the \TT{position} and \TT{line\_\EOL format} parameters
to the \TT{min::\EOL print\_\EOL line} function.
If the \TT{min::\EOL OUTPUT\_\EOL HTML} flag is on in
\TT{printer->\EOL print\_\EOL format\EOL .op\_\EOL flags} and
\TT{line\_\EOL format->\EOL line\_\EOL table\_\EOL class}
is not \TT{NULL}, then the printed lines are wrapped in
\begin{center}
\TT{<table class='}{\em line-table-class}\TT{'>}\ldots\ldots\ldots\ldots%
\TT{</table>}
\end{center}
where {\em line-table-class} is the value of
\TT{line\_\EOL format->\EOL line\_\EOL table\_\EOL class}.
However \underline{no} line-feed (no \TT{std::endl}) will be
output after \TT{</table>}, as in certain contexts, e.g. a
\TT{<table>} inside a \TT{<pre>}, a line-feed would cause
a blank line after the \TT{<table>}.

When printing an error message associated with lines printed
by \TT{min::\EOL print\_\EOL phrase\_\EOL lines}, line numbers
need to be printed unless they are being output using HTML tags
and \TT{line\_\EOL format->\EOL line\_\EOL number\_\EOL class}.
...
which outputs:
\begin{center}
{\em file-name}\TT{: lines }{\em M}\TT{-}{\em N}
\end{center}
where {\em file-name} is the value of \TT{file->\EOL file\_\EOL name},
{\em M} = \TT{position.begin.line} + 1, and
{\em N} = \TT{position.end.line} + 1, with the following modifications:
\begin{itemize}
\item
If \TT{position.end.offset == 0} and
\TT{position.end.line > position.begin.line},
then \TT{position.end.line} is used in place of
\TT{position.end.line} + 1.
\item
If {\em M} = {\em N}, \TT{lines }{\em M}\TT{-}{\em N} is
replaced by \TT{line }{\em M}.
\item
If \TT{file->\EOL file\_\EOL name} is \TT{min::\EOL MISSING()},
`{\em file-name}\TT{: }' is omited.
\end{itemize}

An example using the above to print an error message is:
...
Internally the above \TT{min::\EOL pline\_\EOL numbers}
...
Internally, when it uses \TT{line\_\EOL format->\EOL mark},
the \TT{min::\EOL print\_\EOL phrase\_\EOL lines} function
calls the function:
...
which, given a file, a position within that file, print format,
...
The above functions that take a \TT{line\_format} argument
replace this with a non-\TT{NULL} value if it is \TT{NULL}.
First \TT{file->\EOL line\_\EOL format} is tried, but if that is
also \TT{NULL}, then \TT{printer->\EOL print\_\EOL format\EOL
.line\_\EOL format} is used.  The end result may \underline{not}
be \TT{NULL}.

The above functions that take a \TT{line\_format} argument use it
...
non-graphic, non-horizontal-space characters usually print as pictures
...
\subsubsection{Printing HTML Tags}
\label{PRINTING-HTML-TAGS}

When the \TT{min::OUTPUT\_HTML} flag is set in
\TT{printer->\EOL print\_\EOL format\EOL .op\_\EOL flags},
HTML reserved characters (e.g., \TT{<}, \TT{>}, \TT{\&})
are replaced as per Figure~\ref{HTML-RESERVED-CHARACTERS}
(e.g., by \TT{\&lt;}, \TT{\&gt;}, \TT{\&amp;}) when
the printed text is output to \TT{printer->\EOL file->\EOL ostream}.
However it is necessary to avoid replacing characters when
printing HTML tags.  This can be done with the function:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|std::ostream & min::| & \MINKEY{tag}
    \verb|( min::printer printer )|
\LABEL{MIN::TAG} \\
\end{tabular}\end{indpar}

which first flushes the printer buffer by calling
\TT{min::\EOL flush\_\EOL file ( printer->\EOL file )},
and then returns
\TT{* printer->\EOL file->\EOL ostream}.  Elements printed
directly to this \TT{ostream} do not have any characters
replaced.  But it is important to flush the printer buffer
before any operation that puts characters into the \TT{ostream}
returned by \TT{min::tag}.
...
Note that printing tags with \TT{min::tag} does not
update \TT{printer->column} and other printer parameters
used to insert spaces between print items.
Thus tags are invisible to normal printing.
For example, printing an end of line by using \TT{std::endl}
with \TT{min::tag} does not set \TT{printer->\EOL column} to \TT{0}.
...
\verb|min::printer min::| & \MINKEY{print\_gen}\ARGBREAK
    \verb|( min::printer printer,|\ARGBREAK
    \verb|  min::gen v,|\ARGBREAK
    \verb|  const min::gen_format * f,|\ARGBREAK
    \verb|  bool disable_mapping = false )|
\LABEL{MIN::PRINT_GEN_WITH_FORMAT} \\
\end{tabular}\end{indpar}

The fourth \TT{disable\_mapping} argument disables use of
...
the printing of components of \TT{v}, and is normally only used
by defined format printing functions.
...
\TT{printer->\EOL print\_\EOL format.gen\_\EOL format} \\
\end{center}
is \ttkey{min::compact\_gen\_format} (\pagref{MIN::COMPACT_GEN_FORMAT}).

\TT{min::\EOL pgen\_\EOL name(v)},
\TT{min::\EOL pgen\_\EOL quote(v)},
\TT{min::\EOL pgen\_\EOL bracket\_\EOL quote(v)}, and
...
min::pgen ( v, min::quote\_gen\_format ) \\
min::pgen ( v, min::bracket\_quote\_gen\_format ) \\
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINLKEY{text}{\_gen\_format}:}
\LABEL{MIN::TEXT_GEN_FORMAT}\ARGBREAK
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
\verb|min::standard_str_format                // str_format|\ARGBREAK
\verb|min::text_obj_format                    // obj_format|
\end{tabular}\end{indpar}
...
\begin{indpar}[1em]\begin{tabular}{r@{}l}
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{bracket\_quote\_gen\_format}:}
\LABEL{MIN::BRACKET_QUOTE_GEN_FORMAT}\ARGBREAK
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
\verb|min::quote_all_str_format               // str_format|\ARGBREAK
\verb|min::bracket_special_format             // special_format|
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{quote\_gen\_format}:}
\LABEL{MIN::QUOTE_GEN_FORMAT}\ARGBREAK
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK
...
\minkey{NEEDS\_QUOTES} flag or does \underline{not} have the
...
\minkey{IS\_BREAKABLE} flag, the string is printed by
...
otherwise, and this class is used to determine whether or not to
insert a space before the string:
see \itemref{LEADING-AND-TRAILING-SEPARATORS}.
...
\ttmindex{element\_format}{in {\tt min::lab\_format}}\ARGBREAK
...
\verb|NULL                                     // element_format|\ARGBREAK
\verb|NULL                                     // lab_prefix|\ARGBREAK
\verb|min::space_if_needed_pstring             // lab_separator|\ARGBREAK
\verb|     // Equivalent to printer << min::space_if_needed|\ARGBREAK
\verb|NULL                                     // lab_postfix|\ARGBREAK
\end{tabular}\end{indpar}
...
\verb|min::compact_value_gen_format            // element_format|\ARGBREAK
...
Given a label format \TT{lf = f->lab\_format},
a \TT{min::gen} label value is printed as:
\begin{enumerate}
\item
\TT{lf->\EOL lab\_\EOL prefix},
\item
followed by the label elements separated by
\TT{lf->\EOL lab\_\EOL separator}'s,
\item
followed by \TT{lf->\EOL lab\_\EOL postfix}. 
\end{enumerate}
Label elements are printed by \TT{lf->element\_format} if that is not
\TT{NULL}, and \TT{f} otherwise.
\TT{NULL} values for
\TT{lf->\EOL lab\_\EOL prefix/separator/postfix} act like empty strings
that print nothing.
A \TT{NULL} value for \TT{lf} acts like
...
\verb|min::leading_asterisk_pstring            // special_prefix   "*"|\ARGBREAK
\verb|min::trailing_asterisk_pstring           // special_postfix  "*"|\ARGBREAK
...
\verb|//     LOGICAL_LINE INDENTED_PARAGRAPH TRUE FALSE|
\end{tabular}\end{indpar}
...
\begin{indpar}[0.1in]\begin{tabular}{rl}
\multicolumn{2}{l}{{\tt struct min::}\MINKEY{defined\_format\_header}}
\LABEL{MIN::DEFINED_FORMAT_HEADER} \\
\verb|{                                       | \\
\verb|  const min::uns32 |
	& \TTMKEY{control}{in {\tt min::defined\_format\_header}} \\
\verb| const min::uns32 |
	& \TTMKEY{length}{in {\tt min::defined\_format\_header}} \\
\verb| const min::uns32 |
	& \TTMKEY{max\_length}{in {\tt min::defined\_format\_header}} \\
\verb|  const min::defined_format_function |
	& \TTMKEY{defined\_format\_function}{in {\tt min::defined\_format\_header}} \\
\verb|}                                       | \\
\end{tabular}\end{indpar}
...
\begin{indpar}\begin{tabular}{r@{}l}
\multicolumn{2}{l}{{\tt min::defined\_format\_insptr min::}
	\MINKEY{new\_defined\_format\REL}}\ARGBREAK[0.5in]
    \verb|( min::defined_format_function f,|\ARGBREAK[0.5in]
    \verb|  min::uns32_number of_arguments )|
\LABEL{MIN::NEW_DEFINED_FORMAT} \\
\end{tabular}\end{indpar}

There is a \key{defined format packed map}
of packed structure/vector subtypes (\pagref{PACKED_SUBTYPE})
to defined formats that can be edited by the function:
...
with the \TT{disable\_\EOL mapping} argument set to \TT{true} to
suppress defined format mapping for \TT{v} (but not for the
components of \TT{v}).
...
The \mkey{attribute vector}{of object}\label{ATTRIBUTE-VECTOR}
...
and the \TT{min::\EOL set\_\EOL public\_\EOL flag} function just sets
...
The new object copy may also be used to fill a preallocated stub
(\itemref{PREALLOCATED-STUBS}) 
...
\verb|min::gen min::| & \MINKEY{copy\REL}\ARGBREAK
    ...
    \verb|  min::gen object,|\ARGBREAK
    \verb|  min::unsptr unused_size )|
\LABEL{MIN::PREALLOCATED_COPY_UNUSED} \\
\end{tabular}\end{indpar}
where the \TT{preallocated} argument designates the preallocated stub.

The new object is \underline{not} public even if the original
object is.  The forms of \TT{min::\EOL copy} with no \TT{expand}
argument
always expand the unused area to fit the space actually allocated
to the new object, as if they had a \TT{true} \TT{expand} argument.
...
\verb|min::gen min::| & \MINKEY{copy\REL}\ARGBREAK
    ...
    \verb|  min::obj_vec_ptr & vp,|\ARGBREAK
    \verb|  min::unsptr unused_size )|
\LABEL{MIN::PREALLOCATED_COPY_UNUSED_VEC_PTR} \\
\end{tabular}\end{indpar}
...
a single \TT{min::gen} value (push),%
\label{OBJ_VEC_INSPTR_PUSH_REF}
of a \TT{min:gen} value (pop),
...
It the hash table does not exist (its size is 0)
\TT{min::\EOL start\_\EOL hash} sets the list pointer
to point to \TT{min::LIST\_END()}.
...
It the index is not less than the attribute vector size,
\TT{min::\EOL start\_\EOL attr} sets the list pointer
to point to \TT{min::LIST\_END()}.
...
\emkey{value} ::= {\em atom} $|$ {\em label} $|$ {\em object}
                             $|$ {\em indirect-pointer}
\\[1ex]
\emkey{atom} ::= {\em identifier string} $|$ {\em number}
\\[1ex]
...
Read-only attribute pointers\index{read-only!attribute pointer}%
\index{attribute pointer!read-only}
...
Attribute locator functions\index{locator function!of attribute pointer}
...
Attribute accessor functions\index{accessor function!of attribute pointer}
...
Attribute information functions\index{information function!of attribute pointer}
...
The 3-argument \TT{min::attr\_info\_of} function returns \TT{true} if
any of the 3 counts is non-zero, and \TT{false} otherwise.
Note that if \TT{include\_\EOL reverse\_\EOL attr} is \TT{false},
the \TT{reverse\_\EOL attr\_\EOL count} is always zero.

The 4-argument \TT{min::attr\_info\_of} function returns in the
\TT{info} vector the same information
for all attributes of the object pointed at by the attribute pointer that
have a non-zero value for at least one of their three counts.
The number $N$ of such
attributes is returned.  Information about the first $\min(N,$\TT{n}$)$
of these attributes is returned in the \TT{info} vector, in arbitrary order.
The \TT{min::\EOL sort\_\EOL attr\_\EOL info} function can be used
to sort the vector by \TT{name}.
The \TT{min::\EOL compare} function, \pagref{MIN::COMPARE},
is used to determine the ordering of names.
Note that if \TT{include\_\EOL reverse\_\EOL attr} is \TT{false},
only information about attributes which have values or flags is returned.

Attributes accessed through the object's attribute vector are not returned
by the 4-argument \TT{min::\EOL attr\_\EOL info\_\EOL of} function unless
...
The 3-argument \TT{min::attr\_info\_of} function can also take a read-only
\TT{min::\EOL obj\_\EOL vec\_\EOL ptr} pointing at the object instead
of a \TT{min::\EOL attr\_\EOL ptr}.

The 2-argument \TT{min::reverse\_attr\_info\_of} function returns information
about the current reverse attribute of an attribute pointer (i.e., the
attribute whose name was given by the last \TT{min::\EOL locate\ldots}~call
for the attribute pointer and whose reverse name was given by the last
subsequent \TT{min::\EOL locate\_\EOL reverse} call).  Values stored in the
\TT{min::\EOL reverse\_\EOL attr\_\EOL info}\ttindex{min::reverse\_attr\_info}
structure members are as follows:
...
Updatable attribute pointers\index{updatable!attribute pointer}%
\index{attribute pointer!updatable}
...
Insertable attribute pointers\index{insertable!attribute pointer}%
\index{attribute pointer!insertable}
...
taken, to any depth.  The first indirection must be an index general
value, but the remaining indirections can be either kind of indirect pointer.
...
\ttmindex{top\_element\_format}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  const min::gen_format *              text_element_format;|%
\ttmindex{text\_element\_format}{in {\tt min::obj\_format}}\ARGBREAK
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
    \verb|  const min::stub *                    text_type_table;|%
\ttmindex{text\_type\_table}{in {\tt min::obj\_format}}
\end{tabular}
...
\ttmindex{mark\_classifier}{in {\tt min::obj\_format}}
\end{tabular}
...
\ttmindex{obj\_sep}{in {\tt min::obj\_format}}\ARGBREAK
    \verb|  min::pstring                         obj_textsep;|
\ttmindex{obj\_textsep}{in {\tt min::obj\_format}}
\end{tabular}
...
\verb|const min::uns32 min::| & \MINKEY{ENABLE\_TEXT}
\LABEL{MIN::ENABLE_TEXT} \\
\end{tabular}\end{indpar}
...
\verb|NULL                                     // text_element_format|\ARGBREAK
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::NULL_STUB                           // text_type_table|\ARGBREAK
\end{tabular}
...
\verb|NULL                                     // obj_textsep|\ARGBREAK
\end{tabular}
...
\begin{tabular}{r@{}l}\hspace*{0.1in} \\[-3ex]
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINLKEY{text}{\_obj\_format}:}%
\LABEL{MIN::TEXT_OBJ_FORMAT}\ARGBREAK
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK
\end{tabular}
...
\verb|+ min::ENABLE_TEXT|\ARGBREAK
\end{tabular}

\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::text_gen_format                     // top_element_format|\ARGBREAK
\verb|min::text_gen_format                     // text_element_format|\ARGBREAK
\verb|min::space_if_needed_pstring             // obj_textsep|
\end{tabular}
...
\TT{printer->\EOL id\_\EOL map} exists
and the \ttikey{FORCE\_\EOL ID}{FORCE\_ID}
...
attributes with a \TT{hide\_\EOL flag}, and attributes
with no values, flags, or reverse attribute (double arrow)
values.

\item No attribute other than those with a \TT{hide\_\EOL flag}
has multiple values, flags, or reverse attribute (double arrow) values.

\item A value of any attribute
...
\item If the object has \TT{min::INDENTED\_PARAGRAPH()}
as its \TT{.terminator}, the object has no \TT{.separator}
and is an element of a logical line that is being printed in compact format.

\end{enumerate}

Otherwise, if the \ttikey{DEFERRED\_\EOL ID}{DEFERRED\_ID}
object format operation flag is on
and \TT{printer->\EOL id\_\EOL map} exists
...
\begin{enumerate}
\item If the object has a \TT{.type} attribute and no other
non-hidden attributes, and if any
\TT{disable\_\EOL mapping} argument to the function being used to
print the object (e.g., \TT{min::\EOL standard\_\EOL pgen} or
\TT{min::\EOL print\_\EOL obj}) is \TT{false}, and if the \TT{.type}
attribute value has a non-\TT{NULL\_\EOL STUB}
entry \TT{df} in the defined format type map
(\pagref{DEFINED-FORMAT-TYPE-MAP}), then the defined format function
is called.  E.g.,
\begin{center}
\TT{min::standard\_pgen ( printer, v, gen\_format, false )}
\end{center}
or
\begin{center}
\TT{min::print\_obj
    ( printer, v, gen\_format, obj\_format, obj\_op\_flags, false )}
\end{center}
is translated to
\label{DEFINED-FORMAT-TYPE-TRANSLATION}
\begin{center}
\TT{(* df->defined\_format\_function) ( printer, v, gen\_format, df )}
\end{center}
...
object format operation
flag is on,
and the object is being printed as an element of a containing
object that is being printed in logical line compact format.
...
In this case the object has a \TT{.type},
no \TT{.initiator}, 
no \TT{.terminator}, but may have a \TT{.separator}.

\end{enumerate}

Most of the compact format variants may or may not be in
`\key{text mode}'\label{TEXT-MODE}:
\begin{enumerate}
\item
A logical line compact format or indended paragraph compact format
is in text mode if and only if the \TT{ENABLE\_\EOL TEXT}
object format operation flag is on.
\item
A bracketed compact format
is in text mode if and only if the \TT{ENABLE\_\EOL TEXT}
object format operation flag is on and the object's \TT{.initiator}
has an entry in the object text type table
(see \itemref{OBJECT-TEXT-TYPE-TABLE}).
\item
A default compact format
is in text mode if and only if the \TT{ENABLE\_\EOL TEXT}
object format operation flag is on and the object has a \TT{.type} that
has an entry in the object text type table
(see \itemref{OBJECT-TEXT-TYPE-TABLE}).
\end{enumerate}

Text mode causes \TT{obj\_textsep} object format element to be
used in place of the \TT{obj\_\EOL sep} object format element (see below),
and causes the
\TT{text\_\EOL element\_\EOL format} object format element to be
used in place of either the \TT{element\_\EOL format}
object format element or \TT{top\_\EOL format} object format element.
If the printer's print format \TT{OUTPUT\_\EOL HTML} operation flag is on,
text mode also uses any object text type table entry to output
HTML tags before and after the object compact format printout:
see \itemref{OBJECT-TEXT-TYPE-TABLE}.
...
\item[\ttmkey{ENABLE\_TEXT}%
    {in {\tt obj\_\EOL format.obj\_\EOL op\_\EOL flags}}]\vspace{-1ex}~\\
If an object is being printed in compact format, this flag allows the
object to be printed in text mode\pagnote{TEXT-MODE}.
...
\item[\ttmkey{text\_element\_format}{in {\tt min::obj\_format}}]\vspace{-2ex}~\\
These are the \TT{min::gen\_format}'s used to print the
object vector elements.  If the object is being printed in
text mode compact format, \TT{text\_element\_format} is used.
Otherwise if the object is being printed in
...
format, and the \TT{.terminator} and \TT{.separator}
in logical line compact format.
The \TT{.initiator} in indented paragraph compact format is
printed using \TT{terminator\_format}, as it is in a terminator
position.

\item[\ttmkey{text\_type\_table}{in {\tt min::obj\_format}}]~\\
\label{TEXT_TYPE_TABLE}
Used to determine whether a compact format object is in
text mode\pagnote{TEXT-MODE} and to determine HTML tags for
text mode objects (see \itemref{OBJECT-TEXT-TYPE-TABLE}).
...
\item[\ttmkey{obj\_textsep}{in {\tt min::obj\_format}}]\vspace{-2ex}~\\
\TT{Obj\_sep} serves as the object element separator in compact
or normal format when the object does
not have a \TT{.separator} attribute.  E.g., a single space.
\TT{Obj\_\EOL textsep} replaces \TT{obj\_\EOL sep}
in compact format text mode\pagnote{TEXT-MODE}.
...
negation of attributes whose value is \TT{min::FALSE()}.
Attributes with value \TT{min::TRUE()} are printed as just
the attribute label with no following \TT{obj\_\EOL attreq} or value.
Attributes with value \TT{min::FALSE()} are printed as just
...
\begin{center}
\begin{tabular}{rcl}
\TT{[}			& is	& the \TT{.initiator} printed with
                                  \TT{initiator\_format} \\
$\ell$			& is	& \TT{min::leading} \\
{\em ta}		& is	& \TT{min::trailing\_always} \\
\TT{,}			& is	& the \TT{.separator} printed with
                                  \TT{separator\_format} \\
\TT{\textvisiblespace}	& is	& a single space character \\
{\em t}			& is	& \TT{min::trailing} \\
\TT{]}			& is	& the \TT{.terminator} printed with
                                  \TT{terminator\_format}
\\[1ex]
{\em <element>}		& is	& printed by \TT{min::print\_gen} \\
			&	& using the \TT{element\_format} \\
\end{tabular}
\end{center}
...
is printed just after the last line element.  Specifically, it is preceeded
by printing \TT{min::trailing} and then printed using the
\TT{terminator\_format}.
...
and the object is being printed as an element of a containing object
that is being printed in logical line compact format.
First single spaces are erased from the end of the
current line by \TT{min::\EOL print\_\EOL erase\_\EOL space},
then \TT{.initiator} is printed using
\TT{terminator\_format} (as it is in a terminating position),
then \TT{min::eol} is printed, and lastly the
object elements are printed using \TT{top\_\EOL element\_\EOL format}.

The paragraph indent is set to the indent of its containing logical line
plus 4 if the paragraph is the last and only element of its containing
logical line that is printed in indented paragraph compact format.
Otherwise it is set to the indent of its containing logical line plus 8,
while the indent of non-paragraph element sequences of the containing logical
line which separate paragraphs is set to the indent of the containing
logical line plus 4.

If a paragraph element is preceded by a paragraph element that is a
terminated line (i.e., has a non-line-feed \TT{.ter\-min\-ator})
and that is printed in the logical line
compact format, the paragraph element is separated from this preceding
element by a single space.  Otherwise the element is preceded by
executing \TT{min::\EOL indent}, \TT{min::\EOL save\_\EOL indent},
and \TT{min::\EOL place\_\EOL indent(4)} in order and followed by
executing \TT{min::\EOL restore\_\EOL indent}.
Lastly \TT{min::\EOL bol} is executed at the end of
the paragraph.
...
by `\TT{|T\}}' where `\TT{|}' is what \TT{obj\_\EOL ketbegin} prints and
...
\TT{obj\_valbegin}	& is	& \TT{"\{* "} \\
\TT{obj\_valsep}	& is	& \TT{", "} \\
\TT{obj\_valend}	& is	& \TT{" *\}"} \\
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{rcl}
{\em <attribute>} & ::= & {\em attribute-label} {\em attribute-flags}\QMARK{}
                        \{ \TT{\textvisiblespace=\textvisiblespace}
			{\em attribute-values} \}\QMARK{} \\
		& $|$ & {\em attribute-label} {\em attribute-flags}\QMARK{}
			\begin{tabular}[t]{@{}l@{}}
                        \TT{\textvisiblespace=\textvisiblespace}
			    {\em object-ids} \\
                        \TT{\textvisiblespace=\textvisiblespace}
			...
			\end{tabular} \\
		& $|$ & {\em attribute-label} \\
		& $|$ & \TT{no\textvisiblespace}{\em attribute-label}
\\[1ex]
{\em attribute-values} & ::= & {\em attribute-value} \\
		       & $|$ & \TT{\{*\textvisiblespace}{\em attribute-value}
		               \{ \TT{,\textvisiblespace}{\em attribute-value}
			       \}\PLUS{} \TT{\textvisiblespace *\}}
\\[1ex]
{\em object-ids} & ::= & {\em object-id} \\
		       & $|$ & \TT{\{*\textvisiblespace}{\em object-id}
		               \{ \TT{,\textvisiblespace}{\em object-id}
			       \}\PLUS{} \TT{\textvisiblespace *\}}
\\[1ex]
{\em attribute-label} & is & printed by \TT{min::print\_gen} \\
		      &    & using \TT{label\_format}
\\[1ex]
{\em reverse-attribute-label} & ::= & {\em attribute-label}
\\[1ex]
{\em attribute-flags} & is & printed using \TT{flags\_format} \\
		      &    & (see \pagref{ATTRIBUTE-FLAG-NAMES})
\\[1ex]
{\em attribute-value} & is & printed by \TT{min::print\_gen} \\
		      &    & using \TT{value\_format}
\\[1ex]
{\em object-id} & is & printed by \TT{min::print\_id} \\
\end{tabular}
\end{center}
...
\begin{enumerate}
\item
An {\em <attribute>} that is just `{\em attribute-label}'
is equivalent to\\
\hspace*{1in}`{\em attribute-label} \TT{= *TRUE*}'.
\item
An {\em <attribute>} that is just `\TT{no }{\em attribute-label}'
is equivalent to \\
\hspace*{1in}`{\em attribute-label} \TT{= *FALSE*}'.
\item
...
In normal format, if there are no {\em <element>'s}, then the
`\TT{|}'s and the trailing \TT{T} are omitted, regardless
of the setting of the \TT{NO\_\EOL TRAILING\_\EOL TYPE} flag.
Thus `\TT{\{T: level = 4\}}' is output instead of
`\TT{\{T: level = 4||T\}}'.

The \mkey{embedded line format}{of printed object} prints the object
in the same way as normal format but with {\em <attribute>}s after the
the {\em <element>}s and with each {\em <attribute>} on a line by
itself.  Also any last attribute and following trailing type is
separated by an \TT{obj\_\EOL attrbegin} (e.g., `\TT{:}').
Thus if
\begin{center}
\begin{tabular}{lcl}
\TT{obj\_bra}		& is	& \TT{"\{"} \\
...
list of elements is terminated by \TT{obj\_\EOL ketbegin} (\TT{|} here)
which introduces the attributes, one per line, with each attribute
but the last followed by \TT{obj\_\EOL attrsep} (`\TT{,}' here) on its line.
If attributes are followed by a type, the type is separated from
the last attribute by \TT{obj\_\EOL attrbegin} (\TT{:} here).
...
the embedded line format is identical to normal format except for
...
\subsubsubsection{The Object Text Type Table}
\label{OBJECT-TEXT-TYPE-TABLE}
...
\LABEL{MIN::UNDEFINED} \\
\verb|min::gen min::| & \MINKEY{UNUSED()}
\LABEL{MIN::UNUSED} \\
...
\begin{TABULAR}{3.7in}
\verb|typedef min::locatable_var<min::gen>| \\
\verb|min::| & \MINKEY{locatable\_gen}
\LABEL{MIN::LOCATABLE_GEN} \\
\verb|typedef min::locatable_var<min::stub_ptr>| \\
\verb|min::| & \MINKEY{locatable\_stub\_ptr}
\LABEL{MIN::LOCATABLE_STUB_PTR} \\
\end{TABULAR}
...
\verb|void * & MUP::| & \MUPKEY{ptr\_ref\_of}
	      \verb|( min::stub * s )| 
\LABEL{MUP::PTR_REF_OF_STUB} \\
\verb|void MUP::| & \MUPKEY{move\_body}\ARGBREAK
	                \verb|( min::stub * s1, min::stub * s2 )| 
\LABEL{MUP::MOVE_BODY} \\
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::str\_classifier} \\
\verb|    min::|
    & \MINKEY{quote\_value\_str\_classifier}
\LABEL{MIN::QUOTE_VALUE_STR_CLASSIFIER} \\
\multicolumn{2}{l}{\tt const min::str\_classifier} \\
...
\verb|min::gen map->| & \TTMKEY{ID\_assign}{of {\tt min::id\_map}} \\
		      & \verb|// Default:| MIN string \TT{":="}
\LABEL{MIN::ID_MAP_ID_ASSIGN} \\
\verb|min::gen_format * map->| & \TTMKEY{id\_gen\_format}%
					{of {\tt min::id\_map}} \\
			       & \verb|// Default:|
			         \TT{min::id\_map\_gen\_format}
\LABEL{MIN::ID_MAP_ID_GEN_FORMAT} \\
\end{TABULAR}
...
\begin{TABULAR}{0.0in}
\multicolumn{2}{l}{\tt const min::display\_control
    min::\MINLKEY{graphic\_and\_sp}{\_display\_control} =}\ARGBREAK[0.5in]
	\verb|{ min::IS_SP + min::IS_GRAPHIC, 0 };|
\LABEL{MIN::GRAPHIC_AND_SP_DISPLAY_CONTROL} \\
\multicolumn{2}{l}{\tt const min::display\_control
...
\verb|min::op min::| & \MINKEY{trailing}
\LABEL{MIN::TRAILING} \\
\verb|min::op min::| & \MINKEY{space\_if\_needed}
\LABEL{MIN::SPACE_IF_NEEDED} \\
\end{TABULAR}
...
\verb|min::op min::| & \MINKEY{trailing\_always}
\LABEL{MIN::TRAILING_ALWAYS} \\
\verb|min::op min::| & \MINKEY{space\_if\_needed\_always}
\LABEL{MIN::SPACE_IF_NEEDED_ALWAYS} \\
\end{TABULAR}
...
\INDEXHEADER{File Positions and Phrases}
...
\verb|min::uns32 min::|
    & \MINKEY{print\_\EOL line\RESIZE}\ARGBREAK
      ...
      \verb|  min::file file,|\ARGBREAK
      \verb|  min::uns32 line_number,|\ARGBREAK
      \verb|  const min::line_format * line_format =|\ARGBREAK
      \verb|                           NULL )|
\LABEL{MIN::PRINT_LINE} \\
\end{TABULAR}
...
\INDEXHEADER{Printing HTML Tags}
...
\LABEL{MIN::GEN_FORMAT}\ARGBREAK
    \verb|{|\ARGBREAK
    \verb|  min::pgen_function pgen;|\ARGBREAK
    \verb||\ARGBREAK
    \verb|  // The following are used by min::standard_pgen:|\ARGBREAK
    \verb|  //|\ARGBREAK
    \verb|  const min::num_format *  num_format;|\ARGBREAK
    \verb|  const min::str_format *  str_format;|\ARGBREAK
    \verb|  const min::lab_format *  lab_format;|\ARGBREAK
    \verb|  const min::special_format *  special_format;|\ARGBREAK
    \verb|  const min::obj_format *  obj_format;|\ARGBREAK
    \verb||\ARGBREAK
    \verb|  // The following is NOT used by min::standard_pgen:|\ARGBREAK
    \verb|  //|\ARGBREAK
    \verb|  const void *  non_standard;|\ARGBREAK
    \verb|};|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{compact\_gen\_format}:}
\LABEL{MIN::COMPACT_GEN_FORMAT}\ARGBREAK[0in]
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::line_obj_format                    // obj_format|
\end{TABULAR}
...
\begin{TABULAR}{1.0in}
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{text\_gen\_format}:}
\LABEL{MIN::TEXT_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{compact\_value\_gen\_format}:}
\LABEL{MIN::COMPACT_VALUE_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_value_str_format             // str_format|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{compact\_id\_gen\_format}:}
\LABEL{MIN::COMPACT_ID_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_value_id_str_format          // str_format|\ARGBREAK[0in]
\verb|min::compact_id_obj_format              // obj_format|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{id\_gen\_format}:}
\LABEL{MIN::ID_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_value_id_str_format          // str_format|\ARGBREAK[0in]
\verb|min::id_obj_format                      // obj_format|
\end{TABULAR}
...
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_value_str_format             // str_format|\ARGBREAK[0in]
\verb|min::isolated_line_id_obj_format        // obj_format|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{bracket\_quote\_gen\_format}:}
\LABEL{MIN::BRACKET_QUOTE_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
...
\multicolumn{2}{l}{\tt const min::gen\_format *
                   min::\MINKEY{quote\_gen\_format}:}
\LABEL{MIN::QUOTE_GEN_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_gen_format except for:|\ARGBREAK[0in]
\verb|min::quote_all_str_format               // str_format|
\end{TABULAR}
...
\INDEXHEADER{Printing Using Defined Formats}
...
\LABEL{MIN::OBJ_FORMAT}\ARGBREAK[0in]
    ...
    \verb|  const min::gen_format *              text_element_format;|\ARGBREAK[0in]
    ...
    \verb|  const min::stub *                    text_type_table;|\ARGBREAK[0in]
    ...
    \verb|  min::pstring                         obj_textsep;|\ARGBREAK[0in]
...
\begin{TABULAR}{0.3in} & \hspace*{0in}
\verb|min::compact_gen_format                  // element_format|\ARGBREAK[0in]
\verb|NULL                                     // top_element_format|\ARGBREAK[0in]
\verb|NULL                                     // text_element_format|\ARGBREAK[0in]
\verb|min::compact_value_gen_format            // value_format|\ARGBREAK[0in]
...
\begin{TABULAR}{0.3in} & \hspace*{0in}
\verb|min::NULL_STUB                           // text_type_table|\ARGBREAK[0in]
\end{TABULAR}
...
\verb|NULL                                     // obj_textsep|\ARGBREAK[0in]
\end{TABULAR}
...
\verb|min::space_equal_space_pstring           // obj_valreq|\ARGBREAK[0in]
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINLKEY{text}{\_obj\_format}:}%
\LABEL{MIN::TEXT_OBJ_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|  min::ENABLE_COMPACT                    // obj_op_flags|\ARGBREAK[0in]
\verb|+ min::ENABLE_LOGICAL_LINE|\ARGBREAK[0in]
\verb|+ min::ENABLE_INDENTED_PARAGRAPH|\ARGBREAK[0in]
\verb|+ min::ENABLE_TEXT|\ARGBREAK[0in]
\verb|min::text_gen_format                     // top_element_format|\ARGBREAK[0in]
\verb|min::text_gen_format                     // text_element_format|\ARGBREAK[0in]
\verb|min::space_if_needed_pstring             // obj_textsep|\ARGBREAK[0in]
\end{TABULAR}

\begin{TABULAR}{0.3in}
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINKEY{compact\_id\_obj\_format}:}%
\LABEL{MIN::COMPACT_ID_OBJ_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
\verb|  min::ENABLE_COMPACT                    // obj_op_flags|\ARGBREAK[0in]
\verb|+ min::DEFERRED_ID|\ARGBREAK[0in]
\verb|min::compact_id_gen_format               // element_format|\ARGBREAK[0in]
\verb|min::compact_id_gen_format               // value_format|\ARGBREAK[0in]
\verb|NULL                                     // obj_attr|\ldots\ARGBREAK[0in]
\verb|NULL                                     // flag_format|\ARGBREAK[0in]
\verb|0                                        // hide_flags|\ARGBREAK[0in]
\verb|NULL                                     // obj_val|\ldots\ARGBREAK[0in]
\end{TABULAR}

\begin{TABULAR}{0.3in}
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINKEY{id\_obj\_format}:}%
\LABEL{MIN::ID_OBJ_FORMAT}\ARGBREAK[0in]
\verb|  min::FORCE_ID                          // obj_op_flags|\ARGBREAK[0in]
\verb|// All other elements are NULL or 0|
\end{TABULAR}
...
\multicolumn{2}{l}{\tt const min::obj\_format
                   min::\MINKEY{isolated\_line\_id\_obj\_format}:}%
\LABEL{MIN::ISOLATED_LINE_ID_OBJ_FORMAT}\ARGBREAK[0in]
\verb|// Same as min::compact_obj_format except for:|\ARGBREAK[0in]
...
\verb|min::erase_all_space_colon_pstring       // obj_attrsep|\ARGBREAK[0in]
\verb|0                                        // hide_flags|
\end{TABULAR}
