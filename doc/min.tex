...
MIN is a language that describes things.
Consider the following MIN description of a room:
...
This room is a MIN object, and as such has a MIN object
name: {\tt \#\#523}.  As an object, the room has attributes,
such as {\tt type}, {\tt outline}, {\tt width}, etc.
Some of these attributes, e.g., {\tt outline}, do not have
simple numeric or symbolic values, but instead have values
that are computer programs written in a limited language
that describe things like video images or means of constructing
text naming the room.  The {\tt label} attribute of this room
object and the {\tt label} attribute of object {\tt \#\#598} (not shown)
might be used to construct the text `the green room under the main dungeon'.

The {\tt outline} and {\tt label} attribute values in this room description
are not immediately useful for producing output, but must be evaluated to
produce the picture or the name.
In order to show how evaluation works, we will explain in detail some
of the evaluation of \verb|outline|.
The \verb|outline| attribute value is a program written from the point
...
The {\tt type} attribute of the room indicates that the room object
inherits attributes from the {\tt standard room} object.  The room
has a hidden {\tt .ancestors} attribute that lists all the objects from
which the room inherits, and the {\tt type} attribute is used to compute the
{\tt .ancestors} attribute.
...
{\tt .display} attribute of the object to be
...
First the function creates an display object that contains
data to be used during evaluation and has {\tt .ancestors}
which control where names such as {\tt straight} are found during
evaluation.  One of the attributes of the display object is the
video output channel into which video commands are written by
functions such as `{\tt straight}'.
The first ancestor of the display object
is the room object, which is searched
for attribute names immediately after the display object is searched.
The next ancestor of the display object is a video
object that provides definitions for functions such as `{\tt straight}'
that define the video language.  The last ancestor of the
display object is the {\tt .GLOBAL} object that provides
definitions of global variables (such as `{\tt standard room}').
...
the display object also contains the current location and direction
of the robot.

After establishing a display object, the {\tt .display}
function evaluates the {\tt outline} attribute
value in the context of the display object to produce video output
in the display object video output channel.  The {\tt outline}
attribute value is a comma separated list of commands, each of
which is to be evaluated in sequence.

The first command is `{\tt straight width}'.   When `{\tt straight}' is 
read, a search is made for an attribute with a name
beginning with the word `{\tt straight}'.  One will be found in the
video object whose total
...
and as a function,
its evaluation context begins by creating an object called the function frame
which has an attribute named `{\tt length}' whose value is the corresponding
argument value, in this case \verb|3.2|, and will acquire an attribute
named `{\tt d}', which is set by the `\verb|d <-| \ldots' statement in the
function.

This concludes our (partial) explanation of the evaluation of the
`{\tt outline}' attribute ofr \verb|##523|.  We turn now to a brief
explanation of the `{\tt label}' and `{\tt over}' attribute values.

It is necessary to produce a name
...
Our last example of attribute evaluation is the `{\tt type}'
attribute of \verb|##523|, whose value is `{\tt standard room}'.
This attribute is evaluated to produce the list of objects
in the {\tt .ancestors} attribute.

TBD: flags and compilation.

\section{Remarks}

Min has several features that distinguish it from previous
programming languages.

First, the MIN attribute structure permits attributes to have
sets of values, and by using double arrows, permits direct
use of the entity-relation model of data description.
For examples, see the diagrams in \secref{RAW-REPRESENTATIONS}.

Second, MIN encourages descriptions by specialized programs, as
for the {\tt outline} and {\tt label} attributes of the room
object in the Overview (\secref{OVERVIEW}).  Some of these,
in particular the video descriptions, need to be compiled.
This is not easy for MIN supports inheritance, and any change
to inherited objects would, in a normal language, force
recompilation.  MIN deals with this by using flags to indicate
which attributes are to be treated as variables by compilation.
...
Expressions are evaluated in a context called an
`\key{evaluation context}'.  This is typically not a MIN datum,
...
The local variables of a function are stored in a `\key{frame}',
which is like an object, except that it is stored in its own
stack, the `\key{frame stack}'.

Each evaluation context has a frame or object called the
current evaluator.  This is the starting point for searching
for names of variables and functions. 
Every object or frame can have an `\ttdkey{ancestors}' attribute that is a
list of all the ancestors of the object or frame.
These are the objects or frames that are searched when a name is not
found in the given object or frame.
...
Ancestors are searched for a name depth first.  That is, the search
begins with an object, then searches the first ancestor of that object,
then the first ancestor of the first ancestor, and so forth.  This
is called the `\key{ancestor search order}'.\label{ANCESTOR-SEARCH-ORDER}.

\subsection{Evaluation Contexts}

An \key{evaluation context} has the following components:
...
\ttmkey{next}{of evaluation context}
    ...
    & The \skey{remaining argument}s
      ...
      If `{\tt expression}' is not a list, it is treated as if it were
...
\ttmkey{input values}{of evaluation context}
    & \begin{tabular}[t]{@{}l@{}}MIN list\\or MISSING\end{tabular}
    & The possibly empty list of input values, or the special MISSING
      value.  The MISSING value need not be a MIN value,
      may be the C++ NULL value, and is \underline{not} the same
      as an empty list.
      \\[0.5ex]
    &
    & Expressions of the form `$X$ \verb|<-| $Y$' cause values
      ...
      evaluation of $X$.
\\[0.5ex]
\ttmkey{output values}{of evaluation context}
    & \begin{tabular}[t]{@{}l@{}}MIN list\\or MISSING\end{tabular}
...
\begin{indpar}[0.1in]
\begin{tabular}{@{}p{1.2in}p{1.5in}p{3.1in}@{}}
\ttmkey{suboperation}{of evaluation context} & MIN symbol
    & Suboperation for a {\tt write} operation: either MISSING or one of
      \ttkey{set}, \ttkey{multi-set}, or {\tt delete}.
\end{tabular}
\end{indpar}
...
\item[\ttakey{throw}{evaluation flag}]~\\
    ...
    Forces termination of everything but a {\tt try} statement.

\item[\ttakey{skip catch}{evaluation flag}]~\\
    Just like {\tt skip else} but skips {\tt catch} and
    {\tt catch if} statements.

\item[\ttakey{return}{evaluation flag}]~\\
    Set when a {\tt return} statement is executed.

    Forces termination of everything but a call operation.
...
stack becomes empty.  Some builtin operations push an
...
An evaluation step may invoke one of three builtin operations:
the call operation, the write opration,
or the error operation.  These are described
...
\item
There are no remaining arguments in the current (topmost)
evaluation context, neither the {\tt throw} or {\tt return}
flags are set, and the input value list is \underline{not} missing.

\begin{indpar}[0.5em]
Execute an error operation.
\end{indpar}

\item
There are no remaining arguments in the current (topmost)
evaluation context, or the {\tt throw} or {\tt return}
flags are set.

\begin{indpar}[0.5em]
The current evaluation context is terminated.
Information in this context is
passed back to the operation that created the context.
\end{indpar}

\item
The output value list is not missing, contains just one element,
and that element is the name of an object.

\begin{indpar}[0.5em]
The current evaluator is set to the named object and the
output value list is reset to missing.
\end{indpar}

\item
The output value list is not missing, contains just one element,
and that element is the a function abstraction (a pair consisting
of a function body and a context object).

\begin{indpar}[0.5em]
The output value list is reset to missing and a
call operation using the function abstraction that was
in this list is performed.

\end{indpar}

\item
The output value list is not missing.

\begin{indpar}[0.5em]
An error operation is executed.
\end{indpar}
...
and the input value list is \underline{not} missing.
...
Perform a write operation on the current object, the name
of the attribute, and the input value list.
\end{indpar}

\item
The remaining arguments are a non-empty string of
atoms that name an attribute of an ancestor of the current
object, and the input value list is \underline{not} missing.

\begin{indpar}[0.5em]
The first ancestor in the ancestor search order
(\pagref{ANCESTOR-SEARCH-ORDER}) which has an
an attribute with the given name is selected.
The rest of the operation is the same as (\ref{LOCAL-WRITE})
but with the selected ancestor in place of the current object.
\end{indpar}
...
Set the output value list to list all the values of the
current object attribute, in any order.
The attribute may have zero or more values.

If any of the values is a function, this is converted into
a function abstraction before being placed in the output.
The function body in the abstraction is taken from the
attribute value, and the context object is set to the
current object (the object whose attribute provided the
function body).
\end{indpar}

\item
The remaining arguments begin with a non-empty string of
atoms that name an attribute of an ancestor of the
current object.

\begin{indpar}[0.5em]
The first ancestor in the ancestor search order
(\pagref{ANCESTOR-SEARCH-ORDER}) which has an
an attribute with a suitable name is selected.  If the ancestor
has several such attributes, the one with the longest name is selected.
The rest of the operation is the same as (\ref{LOCAL-READ})
but with the selected ancestor and attribute in place of the current object.
\end{indpar}
...
\subsection{The Call Operation}
\label{CALL-OPERATION}

The \key{call operation} invokes a function.  The operation takes
as input the current evaluation context and frame stacks,
and an abstraction, which consists of
a \key{function body}, which is a list beginning with the word
`{\tt function}', and a frame or object, which is called the
function \key{context object}.

The call operation does the following:

\begin{enumerate}

\item
The current context is called the \key{caller context}.

\item
A new evaluation context, called the \key{function context}, is pushed
into the evaluation stack.

\item
A new frame is pushed onto the frame stack.  This frame is given a
single ancestor, the context object named in the function abstraction.

\item
The caller context becomes the {\tt caller} of the function context.
The new frame becomes the {\tt initial evaluator} and also the
{\tt current evaluator} of the function context.  The function body
becomes the {\tt expression} of the function context.

\item
Evaluation steps are executed until the function context terminates.

\item
The caller context output value list and {\tt throw} flag are set
to equal the function context output value list and {\tt throw} flag.

Note that the {\tt return} flag in the function context is \underline{not}
copied to the caller context.

\item
The function context and its frame are popped from their stacks.
\end{enumerate}

\subsection{The Write Operation}
\label{WRITE-OPERATION}

The \key{write operation} takes as input an object, an attribute
name, the input value list from the current (topmost) evaluation context,
and the suboperation of that context.  The value set of the named
attribute of the object is changed according to the suboperation
as follows:
...
\item[MISSING]~\\
All the current values of the attribute are deleted, and the set
of attribute values is reset to the values in the input value list.
Some of these values may be duplicates (equal to each other).

\item[\ttakey{set}{evaluation suboperation}]~\\
    Those values in the input value list that are not already equal
    to values of the attribute are made values of the attribute.
    This is done recursively, so duplicate values in the input value
    list have the effect of a single unduplicated value.

\item[\ttakey{multi-set}{evaluation suboperation}]~\\
    All values in the input value list are made new values of the attribute.
    These may create new values that equal previous values, or equal
    each other.

\item[\ttakey{delete}{evaluation suboperation}]~\\
    For each value in the input value list, if an existing attribute value
    is equal to the input value, the existing attribute value is deleted.
    The input value list may contain duplicates to delete multiple equal
    attribute values.  It is not an error if an input value is not an
    existing value.
...
After the above is done, the input value list is set to MISSING and
the output value list is set to the values of the attribute, in any
order.

\subsection{The Error Operation}
\label{ERROR-OPERATION}

The \key{error operation} creates an expression to be evaluated,
and then evaluates it.  The expression calls the `{\tt error}' function
with arguments that describe the error.  The expression is evaluated
as if it were a subexpression of the currently evaluating expression.

Specifically, the error operation does the following:

\begin{enumerate}

\item
The current context is called the \key{parent context}.

\item
A new evaluation context, called the \key{error context}, is pushed
into the evaluation stack.

\item
The expression of the error context is set to a list whose first
element is `{\tt error}' and whose remaining elements, which are
called \skey{error argument}s, describe the error.
The error arguments should \underline{not} be evaluated.

\item
The {\tt caller} of the error context is copied from the parent context.
The {\tt initial evaluator} and {\tt current evaluator} of the error context
are copied from the {\tt initial evaluator} of the parent context.

\item
Evaluation steps are executed until the error context terminates.

\item
The {\tt throw} flag of the parent context is set.

\item
If the {\tt throw} flag of the error context is set, then the
parent context output value list is set
to equal the error context output value list.
Otherwise the parent context output value list is set equal to the
error context expression: so that `{\tt error}' becomes the first
output value, and the error arguments become the remaining output values.

\item
The error context is popped from its stack.


\end{enumerate}


\subsection{The Function Operation}
\label{FUNCTION-OPERATION}

Functions are executed by the call operation (\pagref{CALL-OPERATION})
which creates a function context in which the statements of the
function are executed.  The first statement of a function is the
`{\tt function}' statement that performs a \key{function operation}.
This operation reads and evaluates function arguments.

The function operation takes as input a list of argument names, each of which
names an attribute of the function execution's frame that is to be
set to an argument value.  The operation reads arguments from the {\tt caller}
of the function context, optionally evaluates the arguments, and assigns
the argument values to the argument names in order.

If an argument is to be evaluated, it is evaluated by the following
actions:

\begin{enumerate}

\item
A new context, the \key{argument evaluation context}, is created at the
top of the context stack.  The {\tt initial evaluator} and
{\tt caller} components of the new context
are copied from the {\tt caller} context of the
function context.  The {\tt expression} of the new
context is set to the next argument of the caller context expression,
and the {\tt current evaluator} of the new context
is set to the {\tt initial evaluator} of new context.

\item
Evaluation steps are executed until the argument evaluation context
terminates.

\item
If the {\tt throw} flag on the argument evaluation context is set,
the throw flag and output list are copied from the argument evaluation
context to the function context, and the function context is terminated.

\item
Otherwise the output values are assigned to argument names in order.
Specifically, a write operation is performed as if the input list had
a single element, the output value being assigned, the suboperation
were missing, and the argument name were the attribute name.

If the output value is missing, an error operation is executed.  If it
is empty, nothing is done.  If it has more elements than there are argument
names, the extra elements are discarded.


\end{enumerate}

If at the end of the function operation there are remaining unused
argument names, an error operation is executed.


\subsection{The Try Operation}
\label{TRY-OPERATION}
