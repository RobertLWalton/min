...
\item
An expression evaluation has as input an optional list of
values and has as output an optional list of values.
...
a missing input list means nothing is to be stored, and an empty
...
context in which they are read so they may be
evaluated later, in the same context as they would have been evaluated
in had they been evaluted arguments.
...
and modifies the input and output value lists and the current value.
The remaining argument list may also be modified, though usually the only
modification is to remove arguments as they are read.

\item
Current objects that return a single value when the input value
list is missing and the remaining argument list is emtpy, will
instead make the value they would have returned the new current value
