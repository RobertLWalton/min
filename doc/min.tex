...
The \TT{min::print\_line} function
(\pagref{MIN::PRINT_LINE}), and the other functions that use
\TT{line\_\EOL display}s,
print file lines with the \TT{line\_display} flags temporarily
merged into the printer \TT{op\_flags}.
...
\multicolumn{2}{l}{\tt const min::line\_format
    min::\MINKEY{standard\_line\_format} =}\ARGBREAK
...
\verb|  "MIN-LINE-NUMBER" };       // line_number_class|
\LABEL{MIN::STANDARD_LINE_FORMAT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{rl}
\multicolumn{2}{l}{\tt const min::line\_format
    min::\MINKEY{picture\_line\_format} =}\ARGBREAK
\verb|// Same as min::standard_line_format except for:|\ARGBREAK
\verb|  min::DISABLE_LINE_BREAKS   // op_flags|\ARGBREAK
\verb|      + min::EXPAND_HT|\ARGBREAK
\verb|      + min::DISPLAY_PICTURE|\ARGBREAK
\LABEL{MIN::PICTURE_LINE_FORMAT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{rl}
\multicolumn{2}{l}{\tt const min::line\_format
    min::\MINKEY{non\_graphic\_line\_format} =}\ARGBREAK
\verb|// Same as min::standard_line_format except for:|\ARGBREAK
\verb|  min::DISABLE_LINE_BREAKS   // op_flags|\ARGBREAK
\verb|      + min::EXPAND_HT|\ARGBREAK
\verb|      + min::DISPLAY_PICTURE|\ARGBREAK
...
\LABEL{MIN::NON_GRAPHIC_LINE_FORMAT} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{rl}
\multicolumn{2}{l}{\tt const min::line\_format
    min::\MINKEY{eol\_line\_format} =}\ARGBREAK
\verb|// Same as min::standard_line_format except for:|\ARGBREAK
\verb|  min::DISABLE_LINE_BREAKS   // op_flags|\ARGBREAK
\verb|      + min::EXPAND_HT|\ARGBREAK
\verb|      + min::DISPLAY_PICTURE|\ARGBREAK
\verb|      + min::DISPLAY_NON_GRAPHIC|\ARGBREAK
\verb|      + min::DISPLAY_EOL|\ARGBREAK
\verb|  "<BLANK-LINE>"             // blank_line|\ARGBREAK
\LABEL{MIN::EOL_LINE_FORMAT} \\
\end{tabular}\end{indpar}
...
\item[\ttmkey{OUTPUT\_HTML}%
               {in {\tt print\_\EOL format.op\_\EOL flags}}]~\\
If this flag is on, HTML special characters that are displayed as
...
are replaced in the
printer output buffer as follows:
\begin{center}
\begin{tabular}{ll@{~~~~~}l}
\bf HTML & & \\
\bf Special & \bf Output & \bf Column \\
\bf Character & & \bf Action
\\\hline
\TT{<}		& \TT{\&lt;}	& += 1 \\
\TT{>}		& \TT{\&gt;}	& += 1 \\
\TT{\&}		& \TT{\&amp;}	& += 1 \\
...
\TT{<FF>}	& \TT{<CR><CR>}	& = 0 \\
\TT{<VT>}	& \TT{<CR><CR>}	& = 0 \\
\TT{<HT>}	& \multicolumn{2}{c}{--- see \TT{EXPAND\_HT} ---} \\
\end{tabular}
\end{center}
...
is printed on a single \TT{printer} line with the
\TT{file->\EOL line\_\EOL display} flags
temporarily merged into
\TT{printer->\EOL print\_\EOL format.op\_\EOL flags} during the printing.
(see \pagref{FILE_LINE_DISPLAY}),
...
\TT{printer->\EOL print\_\EOL format\EOL .line\_\EOL for\-mat->\EOL
\{unavailable\_\EOL line, blank\_\EOL line, end\_\EOL of\_\EOL file\}}
...
character a \TT{mark} is printed.
...
with the \TT{line\_\EOL display} flags.
...
\subsubsection{Printing File Lines and Phrases to HTML}
\label{PRINTING-FILE-LINES-AND-PHRASES-TO-HTML}

UTF-8 encoded file lines and part lines may be printed in a form suitable
for including in HTML \TT{<pre>} tagged text.  The UTF-8 string
is printed as is except:
\begin{enumerate}
\item \TT{<} is printed as \TT{\&lt;}.
\item \TT{>} is printed as \TT{\&gt;}.
\item \TT{\&} is printed as \TT{\&amp;}.
\item A horizontal tab is expanded to single spaces.
\item A control character (in UNICODE class C) that
is not a horizontal tab, line feed, or carriage return
is printed as \TT{<0xxxx>} where \TT{xxxx} is the hexadecimal
represention of the character's code.
\end{enumerate}

\TT{printer->columns} is updated by adding one for characters
without the \TT{min::IS\_NON\_\EOL SPACING} character flag
(\pagref{MIN::IS_NON_SPACING}), except for
horizontal tabs which update the column assuming
tab stops every 8 columns.

The HTML printing functions are:

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::printer min::| & \MINKEY{html\_print\_unicode}\ARGBREAK
    \verb|( min::printer printer,|\ARGBREAK
    \verb|  min::unsptr n,|\ARGBREAK
    \verb|  min::ptr<const min::Uchar> p )|
\LABEL{MIN::HTML_PRINT_UNICODE} \\
\end{tabular}\end{indpar}

Note the vector of \TT{min::Uchar}'s may be in a relocatable body.

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|min::printer min::| & \MINKEY{html\_print\_cstring}\ARGBREAK
    \verb|( min::printer printer,|\ARGBREAK
    \verb|  min::unsptr n,|\ARGBREAK
    \verb|  const char * str )|
\LABEL{MIN::HTML_PRINT_CSTRING} \\
\end{tabular}\end{indpar}

Note the string length {\tt n} is explicitly given.

The various printer flags and formats are ignored by these
functions.
...
\begin{TABULAR}{0.0in}
\multicolumn{2}{l}{\tt const min::line\_format
    min::\MINKEY{standard\_line\_format} =}\ARGBREAK[1.0in]
\verb|{ "<BLANK-LINE>",            // blank_line|\ARGBREAK[1.0in]
\verb|  "<END-OF-FILE>",           // end_of_file|\ARGBREAK[1.0in]
\verb|  "<UNAVAILABLE-LINE>",      // unavailable_line|\ARGBREAK[1.0in]
\verb|  "MIN-LINE",                // line_class|\ARGBREAK[1.0in]
\verb|  "MIN-LINE-NUMBER" };       // line_number_class|
\LABEL{MIN::STANDARD_LINE_FORMAT} \\
\end{TABULAR}
