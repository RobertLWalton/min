...
CURRENT-OBJECT
CURRENT-STATEMENT
CURRENT-ITEM in CURRENT-STATEMENT
STATE, and integer containing the following flags:
    FAILURE-FLAG    (set when if condition is false)
    SKIP-FLAG       (set to skip to next statement)
    REPEAT-FLAG     (set to repeat this statement)
    RETURN-FLAG     (set to return from this block)
    FUNCTION-FLAGS  (by repeating function to communicate
                     with itself)
...
\begin{indpar}\begin{verbatim}
    ...
    evaluate ( EXPRESSION, OBJECT, CODE ):
        if EXPRESSION is a single number atom:
            return EXPRESSION
        else if EXPRESSION is `...' bracketed list:
            return quoted ( EXPRESSION, OBJECT, 1 )
        else if EXPRESSION is {...} bracketed list:
            return code ( EXPRESSION, OBJECT )
        else:
            return search ( EXPRESSION, OBJECT, CODE )
    ...
    quoted ( EXPRESSION, CONTEXT, DEPTH ):
        ...
        foreach ITEM in EXPRESSION:
            ...
            if ITEM is [...] bracketed:
                if DEPTH > 1:
                    append quote ( ITEM, OBJECT, DEPTH - 1 )
                           to end of RESULT
                else:
                    concatenate evaluate ( ITEM, OBJECT, 0 )
                                at end of RESULT
            else if ITEM is `...' quoted:
                append quote ( ITEM, OBJECT, DEPTH + 1 )
                       to end of RESULT
            else:
                append quote ( ITEM, OBJECT, DEPTH )
                       to end of RESULT
        return RESULT

    code ( EXPRESSION, OBJECT ):
        `EXPRESSION is {...} bracketed'
        STATE = 0
        foreach STATEMENT in EXPRESSION:
            STATE.REPEAT-FLAG = 1
            while STATE.REPEAT-FLAG:
                V = evaluate ( STATEMENT, OBJECT, STATE )
                if V is RETURN(OB): return OB
                if V is state: STATE = V
