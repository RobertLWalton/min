...
\\[1ex]
\emkey{flag-character} ::= {\em mark-character} except \verb|<| or \verb|>|
\\[1ex]
...
or alternatively if the end of
...
It is possible to place \skey{arrow flag}s on attributes (arrows)
...
In the picture the attribute flags have been added as subscripts on the
...
The difference between \ttnbkey{::} and \ttnbkey{>:} or \ttnbkey{>>:}
is that \verb|::| indicates that the object should not previously
exist, while \verb|>:| and \verb|>>:| indicate that the attributes are to
be added to any existing object, though if the object does not exist it will be
created.  For \verb|>:| it is an error to add to an existing object an
attribute with the same name as an existing attribute of that object.
For \verb|>>:| this is not an error, and the added attribute value becomes
an addition to the set of values of the attribute.
...
+1000 & \ttkey{boolean} &  \ttkey{AND} ~~~ \ttkey{OR} ~~~ \ttkey{NOT}
	& logical and, or, not \\
...
that is a list whose elements are atoms that are lexemes
and objects representing bracketted sublists of lexemes.  Quoted
atoms are specially represented.

\item[(2)] \ikey{Operator Scanning}{operator scanning}~~~~
The object representation is scanned for operators
and modified by inserting parsers.

\item[(3)] \ikey{Expansion}{expansion}~~~~
...
list elements which are attributes labelled {\tt 1}, {\tt 2}, \ldots.
...
and an {\tt .parser} attribute whose value is the atom `{\tt quoted}'.
...
by the lexical scanning algorithm and rearanges its contents according
...
\item If the object has a {\tt .parser} attribute value (such as
the value `{\tt quoted}' introduced by lexical scanning), the operator
scanning algorithm terminates without modifying the object.

\item The parsing mode argument to the algorthm is changed if the
object has an {\tt .initiator} attribute as follows:
...
to the current parsing mode (see \pagref{PARSING-MODE},
Figure \figref{SPECIAL-LEXEMES} and \secref{OPERATOR-DECLARAION}).

If there are opeators among the list elements, those with the lowest
...
(see Figure \figref{SPECIAL-LEXEMES} and \secref{OPERATOR-DECLARAION}).
