...
\begin{indpar}\begin{verbatim}
    ...
    evaluate context ( expression, context, frame ):
        search context ( expression, context, frame )

    search context ( expression, context, frame ):
        foreach object in context:
	    search object (expression, object, frame )
	search object ( expression, .global object, frame )

    search object ( expression, object, frame ):
        match names of object attributes to expression
	if several match:
	   pick the longest match
	   set r = expression with matched portion removed
	   set v = value of matched attribute
	   if v is function definition:
	       evaluate function ( r, v, frame )
	   else if v is object:
	       search ( r, v, frame )
	   else if r is empty:
	       terminate evaluation and return v as value
	   else:
	       error
	else if expression empty:
	   terminate evaluation and return object as value
	else return
