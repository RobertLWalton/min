...
The \verb|outline| attribute value is used to compute derived attributes
whose names begin with the word \verb|.video| and which describe how the
...
Evaluation of an \verb|outline| attribute can be done by a function
named {\tt .evaluate outline}, if such a function is defined by the
object with the {\tt outline} attribute (object \verb|##523|),
or one of its ancestors.
If this function does not exist, evaluation is done by the function
named {\tt .evalutate}, which is globally defined.  In any case,
evaluation can be tailored by defining an appropriately named function
in an object or one of its ancestors.

The evaluation function first establishes
...
just an attribute of the \verb|.global| object.  An evaluation function for
...
Then the evaluation function starts reading the value to be evaluated.  In this
case the it finds a comma separated list of commands, and evaluates
each command separately.  The output is a new list of new commands that is
stored in the \verb|.video| attribute of \verb|##523| and
is written in a video language
...
for the `{\tt doored wall width}' command.  This invokes the `{tt doored wall}'
...
function.  Evaluation of this function is similar, but as a function,
...
of \verb|##523| need to be explained.  It is necessary to produce a name
...
command invokes the `{\tt color}' global function with an argument that is
some object which is the value of the `{\tt green}' global variable to
