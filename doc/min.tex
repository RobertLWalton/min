...
First, the evaluator establishes
...
an object we call `\verb|global|'.  A global variable is by definition
just an attribute of the \verb|global| object.

Then the evaluator starts reading the value to be evaluated.  In this
case the evaluator expects a comma separated list of commands, and evaluates
each command separately.  The output is a list of new commands that is
stored in a \verb|.video| attribute and is written in a video language
that can be processed by the video engine to produce a picture.


A possible command is `{\tt straight 3.2}', meaning the robot is to go
straight \verb|3.2| units.  But in the example \verb|3.2| is replaced by
