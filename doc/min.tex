...
pointer to PREVIOUS-CONTEXT (contexts are like CONS cells)
\end{verbatim}\end{indpar}
...
OBJECT
pointer to PREVIOUS-FRAME (frames are like CONS cells)
\end{verbatim}\end{indpar}
...
evaluate ( EXPRESSION, CONTEXT ):
    if EXPRESSION is a single number atom:
        ...
        C.ITEM -> first item of EXPRESSION
        V = false
        while C.ITEM still points at an item
              and
              not C.STATE.RETURN-FLAG
              and
              not C.STATE.SKIP-FLAG:
              and
              not V == FAILURE
            V = search ( C )
    ...
    foreach ITEM in EXPRESSION:
        ...
        if ITEM is [...] bracketed:
            if DEPTH > 1:
                ...
                concatenate evaluate ( ITEM, CONTEXT )
                            at end of RESULT
...
code ( EXPRESSION, CONTEXT ):
    ...
    foreach STATEMENT in EXPRESSION:
        ...
        C.STATE = REPEAT-FLAG
        while C.STATE.REPEAT-FLAG:
            C.STATE.REPEAT-FLAG = 0
            V = evaluate ( CONTEXT )
...
search ( CONTEXT ):
    V = search ( CONTEXT, CONTEXT.FRAME )
    if V != FAILURE: return V
    return search ( CONTEXT, CONTEXT.FRAME.OBJECT )

search ( CONTEXT, OBJECT ):
   if CONTEXT.ITEM points at expression end and OBJECT is
      not a function with no arguments or an object with
      an attribute with the empty label:
        return OBJECT
   if OBJECT is function definition:
       return execute ( CONTEXT, OBJECT )
   ...
   if several match:
       ...
       return search ( CONTEXT, V )
   else:
       for each ANCESTOR of OBJECT:
           V = search ( CONTEXT, ANCESTOR )
           if V == FAILURE: break
       return V
   else:
       return FAILURE
