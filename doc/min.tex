...
\TT{PREALLOCATED} type (see \pagref{PREALLOCATED_ID} for an example).
...
The variable:
\begin{indpar}[0.1in]\begin{tabular}{r@{}l}
\verb|min::unsptr min::| & \MINKEY{max\_id\_str\_length} ~~~~[default \TT{63}]
\LABEL{MIN::MAX_ID_STR_LENGTH} \\
\end{tabular}\end{indpar}
...
\TT{min::max\_\EOL id\_\EOL str\_\EOL length} bytes
(see \pagref{MIN::MAX_ID_STR_LENGTH}),
...
An \key{identifier map} is a vector whose elements are \TT{min::gen} value.
This vector associates vector indices with \TT{min::gen} values, and
allows the input/ouput token `\TT{@}{\em index}' to be used
to denote the \TT{min::gen} value associated with the vector {\em index}.
...
so that \TT{map->length == 1} and \TT{map[0] == NONE()}.

When an identifier map is used for output, map elements are
assigned values by the functions:
...
These functions maintain an identifier map
...
map vector (exept for the value \TT{min::NONE()}.
...
function can be used to find the index of a \TT{min::gen} value
\TT{g} for use in output.  If \TT{g} is already
in the \TT{map}, its associated index is returned.  Otherwise \TT{g}
is pushed to the end of the \TT{map} vector and assigned an index which
is returned.
\TT{min::find\_and\_add(map,g)} also returns \TT{0} if \TT{g == min::NONE()}.

When objects are represented by
`\TT{@}{\em index}' tokens in a file, they may be defined \underline{after}
they are referenced.  When first referenced, a stub for them is allocated,
and then this is filled in later when the object is defined.

However if long strings are to be represented by 
`\TT{@}{\em index}' tokens in a file, they must be defined
before they are used.  This is because a stub for a long
string cannot be filled in later: when the long string is
defined, it will be given a new stub.
...
\smkey{numeric identifier}s{in identifier map}\index{identifier!numeric}
which are indices in the identifier map vector.
The external form of numeric identifiers is
`\TT{@}{\em index}' (where a different character may be substituted for
`\TT{@}').
...
output as themselves and not as numeric identifiers, although long
string values may be output as numeric identifiers.

Identifier maps can also be used on input to translate
`\TT{@}{\em index}' tokens to \TT{min::gen} values.
...
automatically.  The `\TT{@}{\em index}' tokens, on the
other hand, behave like unchangeable constants and not like variables.

A \TT{min::\EOL id\_\EOL map} is a packed vector with
\TT{min::gen} elements such that the index of a numeric identifier
is the vector index of the \TT{id\_map} element containing the
\TT{min::gen} value associated with the identifier.
This suffices for input that has been automatically generated.

For output a \TT{min::\EOL id\_\EOL map} uses
an associated \mkey{numeric hash table}{of identifier map}
that maps \TT{min::gen} values to numeric identifier indices
and maintains a 1-to-1 map between numeric identifier indices
and \TT{min::gen} values.  Thus for output numeric identifiers
act as pointers to objects and long strings.

For input a \TT{min::\EOL id\_\EOL map} has
an associated \mkey{symbolic hash table}{of identifier map}
that maps symbols to \TT{min::gen} values.  The symbols are
represented as MIN strings.
The \TT{min:gen} value a symbol is mapped to can be changed
and there is no 1-to-1 correspondence between \TT{min::gen} values
and symbols.  Thus the symbolic hash table makes symbols into
variables with \TT{min::gen} values.
Although a symbol cannot be deleted from this hash table, its
value can be set to \TT{min::NONE()}.

When an input identifier is looked up and its value is missing
or \TT{min::NONE()},
it is usually assumed that the identifier refers to
an object that will not be defined until later in the input.
In this case the identifier can be assigned
a preallocated stub (\pagref{MIN::PREALLOCATED}) value which
points at a place to put the object contents when
these contents are defined later.
...
\verb|min::Uchar map->| & \TTMKEY{id\_char}{of {\tt min::id\_map}}
			  ~~~~ (defaults to `\TT{@}')
\LABEL{MIN::ID_MAP_ID_CHAR} \\
\end{tabular}\end{indpar}
...
\verb|void min::| & \MINKEY{set\REL}\ARGBREAK
    ...
    \verb|  min::gen g )|
\LABEL{MIN::SET_OF_ID_MAP_ID} \\
\end{tabular}\end{indpar}

\begin{indpar}[1em]\begin{tabular}{r@{}l}
\verb|void min::| & \MINKEY{clear}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::uns32 id )|
\LABEL{MIN::CLEAR_OF_ID_MAP_ID} \\
\end{tabular}\end{indpar}
...
\verb|min::gen min::| & \MINKEY{get}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
\LABEL{MIN::GET_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}
...
\verb|void min::| & \MINKEY{set\REL}\ARGBREAK
    ...
    \verb|  min::gen g )|
\LABEL{MIN::SET_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}
...
\verb|void min::| & \MINKEY{clear}\ARGBREAK
    \verb|( min::id_map map,|\ARGBREAK
    \verb|  min::gen symbol )|
\LABEL{MIN::CLEAR_OF_ID_MAP_SYMBOL} \\
\end{tabular}\end{indpar}
...
If numeric identifiers are being input, \TT{min::get(map,id)} can be used
to return the \TT{min::gen} value associated with a numeric \TT{id}.
If there
is no such value, \TT{min::\EOL get(map,id)} will return \TT{min::\EOL NONE()},
and \TT{min::\EOL set(map,id,g)} can be used to associate the
\TT{min::gen} value {\tt g} with {\tt id}.  The \TT{min::set} function
can also be used to change the value associate with \TT{id}, but
it cannot be used to change the value to \TT{min::\EOL NONE()}.
To do this last, use the \TT{min::\EOL clear} function.

If symbolic identifiers are being input, \TT{min::get}, \TT{min::set},
and \TT{min::clear} can be used as for numeric identifiers, except that
in place of an \TT{min::uns32} numeric identifier one uses a
\TT{min::get} MIN string identifer.  The string is the {\em symbol},
with or without the bracketting \TT{@}'s.

If there is no value associated with an indentifier, the
\TT{min::\EOL new\_\EOL preallocate\_\EOL gen} function
(\pagref{MIN::NEW_PREALLOCATED_GEN}) can be used to create
a preallocated stub that becomes the value associated with \TT{id}
by \TT{min::\EOL set}.%
\label{PREALLOCATED_ID}
Later the contents of the preallocated stub can be filled in
even if other data already points at the
stub: see \itemref{FILLING-PREALLOCATED-STUBS}.

If instead the value to be associated with the identifier is not
an object, but, for example, it is a MIN string, the value can be
reset using \TT{min::\EOL id\_\EOL of\_\EOL preallocated} to find
the identifier and then using \TT{min::\EOL put} to change the
associated value.
However, in this case the preallocated value \underline{must not}
be pointed at by other data when 
\TT{min::\EOL put} is used.

The identifier map vector is lengthed
when it needs to become longer.
Identifier map hash tables are created by the first call that
requires them to exist.  If a hash table is too small, it is
recreated with a larger size.  For the numeric hash table
the \TT{map->\EOL occupied} member of the map, which records the number of
\TT{map} vector elements that are not \TT{min::NONE()},
is used to determine the proper size of the hash table.
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::trailing_always_colon_space_pstring // obj_attrbegin|\ARGBREAK
\verb|min::trailing_always_comma_pstring       // obj_attrsep|\ARGBREAK
\end{tabular}
...
\vspace{-4ex}\begin{tabular}{r@{}l}\hspace*{0.1in}\ARGBREAK
\verb|min::id_map_gen_format                   // element_format|\ARGBREAK
\end{tabular}
...
attribute labels, and attribute values, respectively, whenever the
object whose elements are being printed is \underline{not}
begin printed in
empty compact, quote compact, logical line compact,
and indented paragraph compact format.
...
\begin{TABULAR}{1.8in}
\verb|min::unsptr min::| & \MINKEY{max\_id\_str\_length}
\LABEL{MIN::MAX_ID_STR_LENGTH} \\
\end{TABULAR}
...
\verb|min::id_map_gen_format                   // element_format|\ARGBREAK[0in]
\verb|min::null_str_classifier                 // mark_classifier|\ARGBREAK[0in]
\verb|NULL                                     // obj_empty|\ARGBREAK[0in]
