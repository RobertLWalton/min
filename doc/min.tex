...
and 2-argument \TT{min::map\_clear} can be used as for numeric identifiers,
except that in place of an \TT{min::uns32} numeric identifier one uses a
\TT{min::gen} MIN string.  The string is the {\em symbol},
with or without the bracketing \TT{@}'s.  1-argument \TT{min::map\_clear}
erases the entire input hash table.
