...
Tab characters in a quoted string
represent one or more space characters, with tabs set
every 8 columns.  Columns are counted with respect to the beginning
of the line, and \underline{not} the beginning of the quoted string.
...
which parsing transforms into

\begin{indpar}\begin{verbatim}
##46:: straight 3.2, left, straight (.sum y `+' 9.4)
\end{verbatim}\end{indpar}
...
which are just the parts of the expression between lowest priority
operators.  The subexpressions in our example are \verb|x| and
...
(other possibilities are \verb|!| and \verb|?|).  A `de-capitalization
algorithm', \secref{DECAPITALIZATION}, is run that in this case
de-capitalizes `\verb|The|' to produce `\verb|the|' and the \verb|.initiator|
...
\OP{(} ~~~ \OP{)} \MAC{parentheses}{.ROUND} \\
\OP{[} ~~~ \OP{]} \MAC{square brackets}{.SQUARE} \\
\OP{\CurlyBra} ~~~ \OP{\CurlyKet} \MAC{curly brackets}{.CURLY} \\
\OP{`} ~~~ \OP{'} \MAC{quotes}{.QUOTE}
