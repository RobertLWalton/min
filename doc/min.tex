...
\TT{NULL},
\TT{printer->\EOL print\_\EOL format.line\_\EOL format} is used (at
least one of these must \underline{not} be \TT{NULL}).
...
if \TT{line\_\EOL format->\EOL unavailable\_\EOL line} is not \TT{NULL}.
...
all on one line, without an visible line feed, if
if \TT{line\_\EOL format->\EOL end\_\EOL of\_\EOL file} is not \TT{NULL}.
...
the characters in the line to be marked are wrapped in
\begin{center}
...
the characters are not wrapped and are not marked in any way.
...
and \TT{line\_EOL format->\EOL line\_\EOL number\_\EOL class}.s
...
which uses \TT{position.begin.line} + 1 and
\TT{position.end.line} + 1 as
...
The \TT{file->\EOL name}, if not \TT{min::\EOL MISSING()},
is printed before the line numbers.
...
When a printer \TT{min::OUTPUT\_HTML} flag is set,
reserved characters that are printed in the normal ways
are replaced as per Figure~\ref{HTML-RESERVED-CHARACTERS}.
...
which first flushes the printer buffer and then returns
\TT{printer->\EOL file->\EOL ostream}.  Elements printed
using this \TT{ostream} do not have their characters
replaced.  But it is important to flush the printer buffer
between any operations that put characters into the \TT{ostream}
retured by \TT{min::tag}.
...
Thus tags are invisible to print formatting.
