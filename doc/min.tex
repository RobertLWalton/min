...
with no values or flags (where values associated with
reverse attribute labels are counted as attribute values).

\item No attribute other than those with a \TT{hide\_\EOL flag}
has multiple values, flags, or reverse attribute (double arrows) values.
...
is printed just after the line.  Specifically, it is preceeded
