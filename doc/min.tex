...
of the form `\ttkey{\#\#}$I$' where $I$ is an integer that starts
with {\tt 1} for the first object created and continues in sequence as
objects are created.  E.g., the first three objects created have the
...
\key{raw-arrow-representation}
    ...
    \end{tabular}
\\[1ex]
\key{raw-single-arrow-representation} ::= {\em arrow-label}\ttkey{:}
					  {\em single-arrow-value} \verb|;|
\\[1ex]
\key{single-arrow-value} ::= {\em atom} $|$ {\em raw-object-name}
\\[1ex]
\key{raw-double-arrow-representation} ::= {\em arrow-label}\ttkey{:}
					  {\em double-arrow-value}
					  \ttkey{:}{\em arrow-label} \verb|;|
\\[1ex]
\key{double-arrow-value} ::= {\em raw-object-name}
\\[1ex]
\key{raw-object-name} ::= \ttkey{\#\#}{\em integer-above-zero}
\\[1ex]
...
one labeled {\tt type} whose target is the atom {\tt woman},
and one labeled {\tt name} whose target is the atom {\tt jill}.
...
of these atoms are to be part of an arrow label or value, they should
...
The semi-colons at the ends of arrow representations
in a raw representation may be omitted provided the next line is not
indented with respect to the first non-whitespace character of the
arrow representation with the omitted semi-colon, or if the end of
file immediately follows.  Thus the semi-colons in the example just
